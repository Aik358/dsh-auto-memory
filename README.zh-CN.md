...
| **Astra 式上下文管理（实验性）** | 上下文将满不再压成一段摘要——四段式交接笔记跨窗口续命，全量历史归档可搜，Agent 按需检索（默认关闭，设置 → 自动化开启） |
...
*交接能力为实验特性，本版本默认关闭——设置 → 自动化 → 交接白板 开启（见[「她怎么交接」](#她怎么交接实验性--默认关闭)）。*
...
> **实验特性**：本节能力随 2.1 上线但默认关闭——设置 → 自动化 → 交接白板 开启后生效。窗口大小自动取自当前模型（settings.yaml 的 contextWindow，如 1M），也可手动覆盖。
