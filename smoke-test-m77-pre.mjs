...
// 只写 semantic-pre/judgement-shadow.jsonl;每条带 sourceIds/contextVersion/
...
const mkRec = (i, key) => ({
  ...
  chunkId: 'chk_pre_' + hex32('c' + i), chunkOrdinal: 0, chunkCount: 1,
})
...
console.log('[R1] judgement shadow:分类/建议/字段完整性/零副作用')
{
  ...
  await c.request('context_push', pushOf('obs_pre_' + hex32('r1a'), miv, TEXTS.correction, 1))
  await c.request('context_push', pushOf('obs_pre_' + hex32('r1b'), miv, TEXTS.runbook, 2))
  await sleep(500)
  const rows = readFileSync(path.join(home, 'memory', 'semantic-pre', 'judgement-shadow.jsonl'), 'utf8').trim().split('\n').map((l) => JSON.parse(l))
  ...
  ok(rows.every((r) => r.policyVersion === 'judgement_shadow_pre_v1'), '全部 policyVersion=judgement_shadow_pre_v1')
  ...
  ok(rows.every((r) => r.observationId.startsWith('obs_pre_')), 'observationId 引用')
  // 零副作用:semantic-pre 外零写路径,MEMORY.md/evidence 不存在
  ...
  // 唯一写路径 = semantic-pre 目录(readdir 已列)
  ok(files.includes('judgement-shadow.jsonl'), 'judgement 落 semantic-pre')
