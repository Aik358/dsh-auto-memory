 ...
 * UTF-8 无 BOM。（唯一的模块依赖是 `l0-extract.js` 的 `isCurrentPre`，见下方 I5 一节；
 * 那也是个零 import 的纯模块，不引入 IO。）
 *
 * 为什么不在 `lib/tier0-catalog.js` 里做这段渲染：那个模块是"目录生成器"（C4，纯文本内核 +
...
import {
  ...
  estimateTokensPre,
  TIER0_DEFAULTS,
} from './tier0-catalog.js'
// 状态判定的**唯一权威**在检索侧（`lib/l0-extract.js:124 isCurrentPre`）。
// 这里刻意 **import 而不是复制一份**：复制会让「检索侧放行、注入侧挡下」这种漂移成为可能，
// 而本模块的 S9 检查只禁网络/LLM/子进程/await，不禁内部纯模块引用（该模块本身零 import）。
import { isCurrentPre } from './l0-extract.js'

export const TIER_INJECT_VERSION = 'tier_layer_inject_v1'

/** 契约预算（冻结；变更须同步 `THREE-LAYER-CONTRACT.md` §4.3）。 */
export const TIER_BUDGET_V1 = Object.freeze({
  B0: 800,
  L1: 140,
  K: 8,
  B2: 2400,
...
export const TIER_MARK_V1 = Object.freeze({
  degrade: '[降级]',
  gate: '[闸门]',
  account: '[层账]',
  quota: '[配额]',
...
export const TIER_LAYER_ORDER_V1 = Object.freeze(['project', 'whiteboard', 'user', 'reflection', 'log'])
...
/** 状态判定别名（导出给测试与调用方做跨模块一致性对照；实体是 `l0-extract.js:isCurrentPre`）。 */
...
export function filterExcludedSourcesPre(hits, patterns) {
  ...
  const layers = new Set(TIER_LAYER_ORDER_V1)
 ...
 */
export const TIER_HITS_REUSE_REASONS_V1 = Object.freeze({
  ok: '投影仍属当前快照',
...
export function describeReuseReasonPre(code) {
  const k = clean(code)
  return TIER_HITS_REUSE_REASONS_V1[k] || k || '未知原因'
}
...
export const SKIP_REASON_TEXT_V1 = Object.freeze({
  'missing-path': '路径未提供',
  'not-found': '文件不存在',
  'not-a-file': '路径不是文件',
  'too-large': '文件超出单文件上限',
...
export function describeReasonPre(reason) {
  ...
  if (!key) return '未知原因'
  return SKIP_REASON_TEXT_V1[key] || key
}
...
export function tierLayerAccountLinePre(perLayer, order) {
  const layers = Array.isArray(order) && order.length ? order : TIER_LAYER_ORDER_V1
  ...
  return TIER_MARK_V1.account + ' ' + segs.join(' · ') + (dropped ? ' · 合计裁剪 ' + dropped + ' 条' : '')
}
...
export function collectDegradationsPre(input = {}) {
  ...
  const out = []
  const mark = TIER_MARK_V1.degrade + ' '
...
export function buildTier1SectionPre(hits, opts = {}) {
  const K = Math.max(1, Number((opts && opts.maxItems) || TIER_BUDGET_V1.K))
  const L1 = Math.max(8, Number((opts && opts.itemChars) || TIER_BUDGET_V1.L1))
  const list = (Array.isArray(hits) ? hits.filter(Boolean) : [])
  ...
  const tail = overflow > 0 ? [TIER_MARK_V1.degrade + ' Tier-1 另 ' + overflow + ' 条命中未展开（K=' + K + ' 上限）；需要时用 memory_recall 取'] : []
  return {
...
export function buildTier2SectionPre(hits, opts = {}) {
  const B2 = Math.max(80, Number((opts && opts.maxChars) || TIER_BUDGET_V1.B2))
  const maxBlocks = Math.max(1, Number((opts && opts.maxBlocks) || TIER_BUDGET_V1.maxTier2Blocks))
  const list = (Array.isArray(hits) ? hits.filter(Boolean) : [])
    .map((h, i) => ({ h, i, s: Number(h.score) }))
    .sort((a, b) => (Number.isFinite(b.s) ? b.s : -1) - (Number.isFinite(a.s) ? a.s : -1) || a.i - b.i)
    .slice(0, maxBlocks)
...
export function composeTieredInjectionPre(input = {}) {
  const o = input && typeof input === 'object' ? input : {}
  const maxTokens = Math.max(60, Number(o.maxTokens) > 0 ? Math.min(Number(o.maxTokens), TIER_BUDGET_V1.B0) : TIER_BUDGET_V1.B0)
  ...
  const catalog = o.catalog || buildTier0CatalogFromTextPre(Array.isArray(o.sources) ? o.sources : [], {
    ...
    quota: { projectRatio: TIER_BUDGET_V1.projectRatio, floorRatio: TIER_BUDGET_V1.floorRatio },
  })
  ...
  if (exHits.droppedCount > 0) {
    degradations.push({
      ...
      text: TIER_MARK_V1.degrade + ' 已按用户排除项挡下 ' + exHits.droppedCount + ' 条命中（'
  ...
  if (gateHits.droppedCount > 0) {
    ...
    degradations.push({
      ...
      text: TIER_MARK_V1.degrade + ' 已按 I5 挡下 ' + gateHits.droppedCount + ' 条非 current 命中（'
  ...
  const head = '[Tier-0 常驻目录 · 指引层 · ≤B0=' + TIER_BUDGET_V1.B0 + ' token(实计 ' + catalog.tokens + ') · '
  ...
  const drillParts = []
  if (gate.level === 'tier0') {
    headParts.push(TIER_MARK_V1.gate + ' 本轮无语义命中（' + gate.reasons.join(',') + '）→ 仅目录层，未下探 Tier-1/Tier-2')
  } else {
    headParts.push(TIER_MARK_V1.gate + ' 本轮命中 ' + gate.hitCount + ' 条（' + gate.reasons.join(',') + '）→ 下探 Tier-1')
    if (tier1) drillParts.push(tier1.text)
    if (tier2) {
      drillParts.push(TIER_MARK_V1.gate + ' 需要证据语义 → 继续下探 Tier-2（单块 ≤' + TIER_BUDGET_V1.B2 + ' 字符）')
  ...
  if (maxTotal) {
    const noteFor = (n) => TIER_MARK_V1.degrade + ' 下探段超注入预算，已裁剪 ' + n + ' 行（目录层与降级标注不受影响）'
    ...
    if (claimedTotalChars > maxTotal) {
      ...
      const overNote = TIER_MARK_V1.degrade + ' 目录层与降级标注不可裁，本轮合计仍超注入预算 '
 ...
 * 薄封装 `tier0-catalog.js` 的估算器，保证全链路只有一个 token 口径（契约 §4.6）。
 */
