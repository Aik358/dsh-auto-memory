 ...
 * 与 `note-status-pre.js` 的分工：
 *   - `note-status-pre.js` = 状态行的**语法**（渲染/解析/剥离）
 ...
 */
import { NOTE_STATUS_OPEN_PRE_V1, renderStatusLinePre, stripStatusLinePre, statusOfBodyPre } from './note-status-pre.js'

/** 锚点：与 `l0-extract-pre.js` / `memory-anchor-pre.js` 同形态（此处独立声明，避免耦合）。 */
