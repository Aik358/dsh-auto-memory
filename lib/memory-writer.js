 ...
 * 同目录临时文件 + fsync + 有界 rename 重试;替换失败保留完整候选快照,不覆盖回放。
 */
export async function atomicReplace(target, data, fsApi = fsDefault, opts = {}) {
  const dir = path.dirname(target)
  const nonce = randomUUID()
  // .tmp suffix keeps pending/recovery snapshots out of *.md / *.json scans.
  ...
  let stage = 'open'
  try {
    // Never truncate or remove somebody else's temporary file on a collision.
    ...
    if (created && complete && opts.preserveOnFailure !== false) {
      // This is a candidate WHOLE-DOCUMENT snapshot, not an append command.
      // Never replay it automatically: another writer may have advanced target.
      ...
      try { await fsApi.rename(tmp, failed); recoveryPath = failed } catch (_) {
        // The recovery rename can itself be blocked. Keep the original temp.
      }
      try {
        ...
        // A scanner/other process may have removed the source. Do not claim
        // successful preservation merely because this function did not unlink.
    ...
    // Preserve the original fs error (and code/errno/syscall/path/dest) normally.
    // Frozen / non-Error throws must not mask the write failure with TypeError.
 ...
 */
// Share a queue across stores using the same fs backend; injected virtual
// filesystems remain isolated. This is NOT an inter-process lock.
...
/** Retain structured write state when the engine turns a store result into an error. */
