...
/** worker 侧 load_embedder 的档位名(m7_embedding_pre_v1.PROVIDER_REAL_INT8);发布构建会把 -pre-v1 折成 -v1。 */
...
export function createPythonSetupPre(opts = {}) {
  ...
  /** worker 读 <dsh-home>/memory/semantic-pre/embedding-config.json(见 worker_semantic_pre_v1.load_embedding_config_from_env);
   *  发布构建会把 `semantic-pre` 折成 `semantic`,故这里写带 -pre 的规范名。
   *  历史缺陷:向导曾写 <dsh-home>/memory/semantic/(缺 -pre),pre 线 worker 从不读该路径 → 装完即"找不到配置"。 */
  const embeddingConfigPath = () => path.join(dshHomeOf(), 'memory', 'semantic-pre', 'embedding-config.json')
