/**
 * 白板格式适配器（wb_contract_pre_v1）—— 白板线拥有「白板格式及其适配器」。
 *
 * 2026-09-14 建立（P0 最小适配边界）。**边界（总纲 §0.5 / ROUND3 §3.1 定案）**：
 *   - **3.0 主体**拥有「共同提交与保护入口」= `lib/memory-mutation-pre.js:validateMutationBoundaryPre`
...
// 账本四段判据复用既有解析器（`handoff-anchor-pre.js` 是账本段结构的既有权威，不另写一份）
import { parseHandoffLedgerPre, HANDOFF_LEDGER_SECTION_WEIGHTS_PRE_V1 } from './handoff-anchor-pre.js'

export const WB_CONTRACT_VERSION = 'wb_contract_pre_v1'

/** 锚点正则（与 `l0-extract-pre.js` 的 `MEM_ANCHOR_RE` 完全一致；契约 §2 要求"完全一致"）。 */
export const WB_ANCHOR_RE_PRE_V1 = /^<!--\s*memory:(mem_[0-9a-f]{32})\s*-->$/

/** 分区标记（契约 §5 逐字）。 */
export const WB_MARKERS_PRE_V1 = Object.freeze({
  modelOpen: '<!-- model -->',
  modelClose: '<!-- /model -->',
  userOpen: '<!-- user -->',
  userClose: '<!-- /user -->',
 ...
 *  ⇒ 保留 + 显式标注, 不做"沉默留着"。 */
export const WB_STATUSES_PRE_V1 = Object.freeze(['current', 'superseded', 'retracted'])

/** 解析/校验的原因码 → 可读中文。 */
export const WB_REASONS_PRE_V1 = Object.freeze({
  'not-string': '内容不是字符串',
  'empty': '内容为空',
 ...
 *  判定 = **对外契约保留**, 不删 —— 它是 `WB_REASONS_PRE_V1`(上文, **有消费者**) 的唯一
 *  取值入口; 删掉函数会让那份码表的"怎么读"离开公开面, 而码表本身是契约的一部分
 *  (发布版已有真实用户)。⇒ 保留 + 显式标注。 */
export function describeWbReasonPre(code) {
  const k = String(code == null ? '' : code)
  return WB_REASONS_PRE_V1[k] || k || '未知原因'
}

const isAnchor = (line) => WB_ANCHOR_RE_PRE_V1.test(String(line || '').replace(/\r$/, ''))
const anchorIdOf = (line) => {
  const m = WB_ANCHOR_RE_PRE_V1.exec(String(line || '').replace(/\r$/, ''))
  return m ? m[1] : ''
}
...
export function parseWhiteboardPre(text, opts = {}) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === WB_MARKERS_PRE_V1.userOpen) {
    ...
    if (trimmed === WB_MARKERS_PRE_V1.modelClose) {
...
export const WB_LINT_VERSION = 'wb_lint_pre_v1'

/** lint 问题码（§6 四类；⑤ 矛盾检测需 LLM，不在此列）。 */
export const WB_LINT_CODES_PRE_V1 = Object.freeze({
  ORPHAN: 'orphan',                   // ① 孤立条目
...
/** 默认口径。 */
export const WB_LINT_DEFAULTS_PRE_V1 = Object.freeze({
...
export function lintWhiteboardPre(entries, opts = {}) {
  ...
  const staleDays = Number.isFinite(Number(o.staleDays)) ? Number(o.staleDays) : WB_LINT_DEFAULTS_PRE_V1.staleDays
  const minSharedTags = Number.isFinite(Number(o.minSharedTags)) ? Number(o.minSharedTags) : WB_LINT_DEFAULTS_PRE_V1.minSharedTags
  ...
  for (const c of cards) {
    ...
    if (!referenced) push(WB_LINT_CODES_PRE_V1.ORPHAN, c, '无任何其他卡片或语料引用它的 id/标题')
  }
  ...
  for (const c of cards) {
    if (c.status !== 'current') {
      push(WB_LINT_CODES_PRE_V1.INACTIVE, c, 'status=' + c.status + '（已作废，仍在白板内）')
      continue
    }
    if (Number.isFinite(c.dateMs) && c.dateMs < cutoff) {
      const ageDays = Math.floor((nowMs - c.dateMs) / 86400000)
      push(WB_LINT_CODES_PRE_V1.EXPIRED, c, '仍标 current 但日期为 ' + c.dateRaw + '（' + ageDays + ' 天前，超阈值 ' + staleDays + ' 天）')
    }
  }
  ...
  for (const id of allIds) {
    if (!knownIds.has(id)) {
      findings.push({
        code: WB_LINT_CODES_PRE_V1.DANGLING_REF, id, title: '',
  ...
  for (let i = 0; i < cards.length; i++) {
    for (let j = i + 1; j < cards.length; j++) {
      ...
      if (!aRefB && !bRefA) {
        findings.push({
          code: WB_LINT_CODES_PRE_V1.NO_CROSS_REF, id: a.id, title: a.title,
...
// `memory-mutation-pre.js` 里，无条件生效**，任何开关都绕不过。
// ═══════════════════════════════════════════════════════════════════════════

export const CRITERIA_GATE_VERSION = 'wb_criteria_pre_v1'

/** 账本四段标题（逐字匹配；与 `handoff-anchor-pre.js` 的权重表同源，不另写一份）。 */
export const HANDOFF_REQUIRED_SECTIONS_PRE_V1 = Object.freeze(
  HANDOFF_LEDGER_SECTION_WEIGHTS_PRE_V1.map((x) => x.title),
)

/** 占位符黑名单（H3）：段 body 只含这些即视为未填写。 */
export const CRITERIA_PLACEHOLDERS_PRE_V1 = Object.freeze(['(待补充)', '（待补充）', 'todo', '同上', '略', 'n/a', '待补充'])

/** 账本/白板判据的阈值（与 `sanitizeForWrite` 上限同源，先于它执行以免双重截断语义混乱）。 */
export const CRITERIA_LIMITS_PRE_V1 = Object.freeze({
...
const PLACEHOLDER_SET = new Set(CRITERIA_PLACEHOLDERS_PRE_V1.map((x) => x.toLowerCase()))
...
export function checkHandoffCriteriaPre(text) {
  ...
  const missing = HANDOFF_REQUIRED_SECTIONS_PRE_V1.filter((t) => !gotTitles.includes(t))
  hard.push({
  ...
  if (parsed) {
    for (const s of parsed.sections) {
      ...
      if (!HANDOFF_REQUIRED_SECTIONS_PRE_V1.includes(name)) continue
      const real = bodyRealLines(s.body)
      const chars = real.join('').length
      if (!real.length || chars < CRITERIA_LIMITS_PRE_V1.sectionMinChars) thinSections.push(name + '(' + chars + ' 字符)')
    }
  } else {
  ...
  hard.push({
    ...
    detail: thinSections.length
      ? '这些段为空或少于 ' + CRITERIA_LIMITS_PRE_V1.sectionMinChars + ' 字符：' + thinSections.join('、')
      : '四段 body 均非空且达到最小长度',
  })
  ...
  hard.push({
    ...
    detail: placeholderHits.length
      ? '这些段的正文只有占位符（' + CRITERIA_PLACEHOLDERS_PRE_V1.slice(0, 4).join('/') + ' 等）：' + placeholderHits.join('、')
      : '无纯占位符段',
  })

  // H4 总长 ≤ 8000
  hard.push({
    id: 'H4', pass: src.length <= CRITERIA_LIMITS_PRE_V1.handoffMaxChars, missing: [],
    detail: '总长 ' + src.length + ' 字符（上限 ' + CRITERIA_LIMITS_PRE_V1.handoffMaxChars + '）',
  })
  ...
  if (parsed) {
    for (const s of parsed.sections) {
      ...
      if (n > CRITERIA_LIMITS_PRE_V1.sectionMaxLines) longSections.push(name + '(' + n + ' 行)')
    }
  }
  ...
  return {
    ...
    report: { chars: src.length, sections: gotTitles.length, requiredSections: HANDOFF_REQUIRED_SECTIONS_PRE_V1.length },
  }
}
...
export function checkPlanCriteriaPre(text, projection) {
  ...
  const nonEmpty = sections.filter((s) => {
    const body = s.split('\n').slice(1).join('\n').trim()
    return body.length >= CRITERIA_LIMITS_PRE_V1.sectionMinChars
  })
  hard.push({
    ...
    detail: nonEmpty.length
      ...
      : '没有任何非空 `## ` 顶层节（' + CRITERIA_LIMITS_PRE_V1.sectionMinChars + ' 字符以上）—— 这是"白板被整篇覆盖成骨架/空白板"的形态',
  })
  hard.push({
    id: 'P-H2', pass: src.length <= CRITERIA_LIMITS_PRE_V1.planMaxChars, missing: [],
    detail: '总长 ' + src.length + ' 字符（上限 ' + CRITERIA_LIMITS_PRE_V1.planMaxChars + '）',
  })
