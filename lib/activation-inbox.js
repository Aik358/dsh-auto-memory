 ...
 *
 * 组成:
 *   1) 策略/预算常量(REFERENCE_TAIL_BUDGET_V1 / 注入卫生 guard v2)
...
export function buildPacketId(activationId, contextVersion, memoryIndexVersion, exactDigest) {
  const parts = ['reference-tail-packet-v1', String(activationId || ''), contextVersion | 0, String(memoryIndexVersion || ''), String(exactDigest || '')]
...
export function makeFakeActivationRequestPre(opts = {}) {
  ...
  const activationId = ACTIVATION_ID_PREFIX + first32(sha256Str('fake-activation-v1\u0000' + seed))
  ...
  return {
    ...
    workerEpoch: opts.workerEpoch || 'fake-epoch-v1',
