 ...
 *
 * 组成:
 *   1) 策略/预算常量(REFERENCE_TAIL_BUDGET_PRE_V1 / 注入卫生 guard v2)
 ...
 * 独立于 REFERENCE_TAIL_BUDGET_PRE_V1(那个对象已冻结、变更须升 activationPolicyVersion),
 * 新增段只在此处开立自己的常量,不触碰既有键。
 */
export const SKILL_TAIL_BUDGET_PRE_V1 = Object.freeze({
  maxTextBytes: 1200,   // checklist 正文 UTF-8 上限(CJK 一汉字 3 字节,按字节裁非按字符)
  maxTitleChars: 120,
  version: 'skill_tail_pre_v1',
})
...
export const TAIL_FETCH_HINT_LINE_PRE_V1 = 'If a reference hints at what you need, use memory_recall or memory_read to fetch full details — do not guess from the excerpt.'
...
/**
 * 注入卫生 guard v2:控制符剔除/注释语法剥离/多空格折叠/换行折叠为分号空格/模板变量双花括号中和。
 * 变更必须升级 guard 版本。
 *
 * ★guard v2(2026-09-22,issue #132):新增双花括号 → 全角双花括号的中和。
 *   本尾注经 systemPrompt.context() 注入,与记忆主快照共用同一个**严格插值**装配器:宿主
 *   dsh-system-prompt 的 interpolate() 会把 context 文本当模板做变量替换 —— 组正则的
 *   字符类要求括号内不含花括号,命中后先校验变量名(须匹配小写名规则,含大写即判 malformed),
 *   再查变量表(未注册即判 unknown),两条都**抛错**并中止整轮装配。
 *   症状与记忆主快照 0.1.39 病例完全一致:该会话**每一轮的模型请求都在组装阶段失败**,
 *   而模型无从自救(它改不了自己的记忆正文)。
 *   中和口径与 lib/index.js 的 neutralizePromptTemplateVars(0.1.39)逐字相同:1:1 等长全角
 *   替换,对人仍可读,且**逐字节确定** ⇒ build 与重渲染两侧 exactDigest 自然一致。
 *   取「无差别双花括号」而非「成对形态」:宿主只在文本仍含开口双花括号时才可能抛错,不含则
 *   走不到校验分支 ⇒ 本口径对任意输入均**不可能残留可抛错形态**;成对替换会漏掉嵌套形态
 *   (双花括号内再套花括号),而那恰是宿主判 malformed 的真实输入。
 */
export function sanitizeTailText(text) {
  return neutralizeTailTemplateVars(String(text == null ? '' : text)
    .replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F]/g, '')
    .replace(/<!--|-->/g, '')
    .replace(/ {2,}/g, ' ')
    .split(/\r?\n/)
...
/**
 * guard v2 内核:双花括号 → 全角等长替换(与 lib/index.js 注入端 neutralizePromptTemplateVars 同口径)。
 * 纯函数、无 IO、逐字节确定;幂等(中和结果不再含 ASCII 双花括号)。
 */
function neutralizeTailTemplateVars(text) {
...
export function validateActivationSkillPre(s) {
  ...
  return {
    ...
    skill: {
      ...
      title: s.title.slice(0, SKILL_TAIL_BUDGET_PRE_V1.maxTitleChars),
 ...
 * 2026-09-14 P1-⑮:补 **Score 行** —— 0-1 相似度与批内排名此前在渲染时被丢弃,
 * 注入侧只看得见 lane/reasonCodes,看不到"到底多像"(用户报告"0-1 相似度排序没了")。
 * 分值与排名都取自 packet 内既有字段(item.score + 排序后的位置),确定性同 build/重渲染;
 * score 非法(非有限数)时整行省略,保持与旧版逐字节一致的降级行为。
 */
function renderItemBlock(item, reason, rank, total) {
  ...
  if (Number.isFinite(sc)) {
    const r = Number.isInteger(rank) && rank > 0 ? rank : null
    const t = Number.isInteger(total) && total > 0 ? total : null
    lines.push('Score: ' + sc.toFixed(2) + (r && t ? ' (rank ' + r + '/' + t + ')' : ''))
  }
...
function renderSkillBlock(skill, reason) {
  ...
  const lines = [
    ...
    'Checklist: ' + sanitizeTailText(clipBytes(skill.text, SKILL_TAIL_BUDGET_PRE_V1.maxTextBytes)),
  ]
  return lines.join('\n')
}
...
export function renderReferenceTail(items, opts = {}) {
  ...
  for (let rank = 0; rank < sorted.length; rank++) {
    ...
    const block = renderItemBlock(it, reason, rank + 1, sorted.length)
  ...
  // ★guard v2 唯一出口兜底(2026-09-23 二次修复,issue #132 残余)：**必须整段中和,不能只净 reference**。
  //   实测漏网(memoryId/scope/sourceVersion/recordDigest 直接拼进 Source: 身份行,
  //   skill.procedureId / skill.level 同理)：任一字段含双花括号即穿透,宿主照样抛
  //   malformed prompt variable reference "{{ }}"。故在此对**整段渲染文本**过一次中和——
  //   这里是 build 与重渲染的**共同出口**,两侧逐字节一致 ⇒ exactDigest 不受影响;
  //   且对所有既有/未来字段一次性兜住,不再依赖"记得净化每个字段"。
  const text = neutralizeTailTemplateVars(parts.join('\n') + '\n' + TAIL_FETCH_HINT_LINE_PRE_V1 + '\n' + TAIL_VERIFY_LINE_PRE_V1)
  return {
...
export function makeFakeActivationRequestPre(opts = {}) {
  ...
  const candidates = picked.map((rec, i) => ({
    ...
    // C8/I5（2026-09-14）：fixture 也要能构造"非 current"候选，否则注入侧过滤无从测起。
  ...
  return {
    ...
    // T0-2（2026-09-14）：请求侧可显式给 requestKey（同一次用户请求的稳定标识）。
    // 缺省为空串——下游对它 fail closed（"没有就不复用"），不会因此误判为同请求。
