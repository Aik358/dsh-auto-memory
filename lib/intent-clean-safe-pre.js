 ...
 * （`activation-inbox-pre.js:58` 的 `TAIL_MARKER_LINE_PRE_V1`）。
 *
 ...
 * （`activation-inbox-pre.js:223`）每块都会输出一行 `Reference: …` ⇒ 该行**必然幸存**并成为
 * `extractIntent`（`episodic-store-pre.js:167`）的输入 ⇒ 变成技能标题。
 ...
 *
 * `renderItemBlock`（`activation-inbox-pre.js:223`）写的是 `'Reference: ' + refText`，
