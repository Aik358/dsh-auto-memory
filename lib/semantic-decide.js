/**
 * JS 端激活判定核(js_activation_decide_pre_v1) —— 2026-08-27 补全 JS 默认闭环。
 *
 * 与 Python m7_activation_features_pre_v2.decide_activation_v2 逐字段对齐:
 *  - 意图头:char_wb 2-4gram + sublinear TF-IDF + L2 归一 + LR + Platt(读 recall_intent_lr_pre_v1.json)
...
export const JS_ACTIVATION_DECIDE_VERSION = 'js_activation_decide_pre_v1'
...
/** 意图头(读 recall_intent_lr_pre_v1.json,与 Python RecallIntentHead 对齐)。 */
...
function pack(features, policy, snapshot, decision, reasonCodes, lane) {
  return {
    featurePolicyVersion: (policy && policy.policyVersion) || 'activation_policy_pre_v2',
    activationPolicyVersion: (policy && policy.policyVersion) || 'activation_policy_pre_v2',
    decision,
    reasonCodes,
    advisoryOnly: null,
