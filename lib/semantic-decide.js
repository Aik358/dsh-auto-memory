 ...
 *   ② **`isalnum()` 语义**：Python 的 `str.isalnum()` 对**全角数字**（`１２３`）、带音标字母（`é`）为真，
...
function isAlnumPythonish(cp) {
  ...
  // 全角字母 ａ-ｚ / Ａ-Ｚ（★ 首轮遗漏：`toLowerCase()` 会把 Ａ(U+FF21) 折叠成 ａ(U+FF41)，
  // 若只覆盖数字，全角字母会在折叠后**整段消失**，与 Python `isalnum()` 仍为真的语义分叉）
  if (cp >= 0xff21 && cp <= 0xff3a) return true // Ａ-Ｚ（防御：未 lower 时）
  if (cp >= 0xff41 && cp <= 0xff5a) return true // ａ-ｚ（lower 后的实际落点）
...
/**
 * 双份策略工件的一致性对照（issue #109）。
 *
 * 冻结参数（D1–D11）目前**同时存在** `lib/policies/` 与 `python/policies/`：JS 侧优先读前者、
 * 回落到后者，Python worker 只读自己那份。`loadAndVerifyPolicy()` 只验**单份内部**自洽
 * （goldDigest/runId 互校），两边是否相等**无人校验** ⇒ 重新标定时只改一份，JS 档与 Python 档
 * 会对同一输入给出不同的激活决策，而运行时零告警。
 *
 * 本函数是纯对照（不抛、不改优先级），把漂移变成**可观测**的事实；是否拒绝加载留给调用方决策。
 *
 * @param {string[]} names 需要对照的文件名（如 ['activation_policy_v2.json', ...]）
 * @param {[string, string]} dirs 两个目录（顺序无意义）
 * @param {{existsSync?:Function,readFileSync?:Function}} [io] 测试注入点
 * @returns {{ok:boolean, drift:Array<{name:string,digestA:string,digestB:string}>, skipped:number}}
 *   任一侧缺文件 ⇒ 记 skipped（合法部署：纯 JS 安装无 python/，旧部署可能只有 python/policies）。
 */
export function checkPolicyPairConsistencyPre(names, dirs, io = {}) {
  const exists = typeof io.existsSync === 'function' ? io.existsSync : existsSync
  const reads = typeof io.readFileSync === 'function' ? io.readFileSync : readFileSync
  const drift = []
  let skipped = 0
  const digestsOf = (dir) => names.map((n) => {
    const p = dir + '/' + n
    if (!exists(p)) return null
    return createHash('sha256').update(reads(p)).digest('hex')
  })
  const A = digestsOf(dirs[0])
  const B = digestsOf(dirs[1])
  for (let i = 0; i < names.length; i++) {
    if (!A[i] || !B[i]) { skipped++; continue }
    if (A[i] !== B[i]) drift.push({ name: names[i], digestA: A[i].slice(0, 12), digestB: B[i].slice(0, 12) })
  }
  return { ok: drift.length === 0, drift, skipped }
}
