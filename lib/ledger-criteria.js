 ...
 *
 * 设计(方案 §2.1-2.5):
 *  - 纯函数、零 IO、fail closed(仿 handoff-anchor.js 契约)。
...
// 兼容性再导出(2026-09-16 勘误): P0 窗口的 wb-contract.js 已实现同名判据函数(H1-H4/S1-S4),
// 且 index.js 两咽喉(checkMutationPre)已接线它 — 本模块勿重复造轮,统一转发保持单一真源。
export { checkHandoffCriteriaPre as checkHandoffCriteriaContractPre, checkPlanCriteriaPre as checkPlanCriteriaContractPre } from './wb-contract.js'

import { parseHandoffLedgerPre } from './handoff-anchor.js'

/** 交接账本四段权威标题(与 handoff-anchor.js 权重表同源)。 */
export const HANDOFF_SECTIONS_V1 = Object.freeze(['任务状态', '目标', '已试方案与失败原因', '进度与下一步'])

/** 占位符黑名单(借 dsh-graph CRITERIA_PLACEHOLDERS 技巧, 方案 H3)。 */
export const CRITERIA_PLACEHOLDERS_V1 = Object.freeze(['(待补充)', '（待补充）', 'TODO', '同上', '略', 'N/A', 'n/a'])

/** 账本硬上限(H4, 与 sanitizeForWrite 8000 同源, 先于它执行)。 */
export const LEDGER_MAX_CHARS_V1 = 8000
/** 白板硬上限(P-H2, 200000)。 */
export const PLAN_MAX_CHARS_V1 = 200000
...
function isPlaceholderLine(line) {
  ...
  if (!t) return false
  return CRITERIA_PLACEHOLDERS_V1.some((p) => t === p || t === '-' + p || t === '*' + p)
}
...
export function checkHandoffCriteriaPre(text) {
  ...
  const missing = HANDOFF_SECTIONS_V1.filter((t) => !titles.includes(t))
  ...
  hard.push({ id: 'H4', pass: len <= LEDGER_MAX_CHARS_V1, missing: len > LEDGER_MAX_CHARS_V1 ? ['总长 ' + len + ' > ' + LEDGER_MAX_CHARS_V1] : [] })
...
export function checkPlanCriteriaPre(text) {
  ...
  // P-H2 长度
  hard.push({ id: 'P-H2', pass: src.length <= PLAN_MAX_CHARS_V1, missing: src.length > PLAN_MAX_CHARS_V1 ? ['总长 ' + src.length + ' > ' + PLAN_MAX_CHARS_V1] : [] })
  // P-S1 前瞻内容(软)
