...
export function createStorageManagerPre(opts = {}) {
  ...
  // getter 固化为 null,repair/delete 恒报 no-doc-store。保留向后兼容(直接传实例也认)。
  const docStoreOf = typeof opts.docStore === 'function' ? opts.docStore : () => opts.docStore || null
  const io = opts.io || {}
  ...
  function scanHealth(paths) {
    ...
    // 2026-09-09(no-doc-store 报障):锚定索引层(memoryAnchorEnabled → docStore)未就位时,
    // 缺 sidecar 属预期状态而非可修复的 stale——避免「报 stale 可修、修复恒 no-doc-store」死循环。
    // 顶层 indexEnabled=false 交 UI 提示;语料文件本身仍判 ok(健康),只是无索引可比对。
    const ds = docStoreOf()
    const indexEnabled = !!(ds && typeof ds.sidecarPath === 'function')
    if (!indexEnabled) {
      ...
      return {
        ok: true, indexEnabled: false, scannedAt: now(), version: STORAGE_MANAGE_VERSION_V1,
        sources, stale: [],
  ...
  function readSidecarPrev(file) {
    try {
      // issue#55:工厂作用域里只有 docStoreOf(活读),`const docStore` 仅存在于 repair()/deleteMemory()
      // 各自的函数作用域内。这里直接引用 docStore ⇒ ESM 运行时 ReferenceError,被下面的 catch 吞成
      // null ⇒ 本函数结构上不可能返回非 null ⇒ 每次 repair 都 mint 新 sourceEpoch、sourceVersion 回退 1。
