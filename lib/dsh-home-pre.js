/**
 * dsh-home-pre.js —— **DSH_HOME 的唯一解析口径**（上游 issue #86-3 修复）。
 *
 ...
 *   | semantic-js-pre.js:73/176 | `path.join(homedir(), '.dsh')` |
 *   | activation-host-pre.js:72 | 退 `homedir()` 再拼 `.dsh`，全失败退 **'.'** |
 *   | context-host-pre.js:40 | 退 **`USERPROFILE || HOME`** 再拼（**前缀不同**） |
 *   | shadow-host-pre.js:129 | 同 activation（但注释说漏拼过 `.dsh`） |
 *
