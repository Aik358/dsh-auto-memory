...
import { resolveDshHomeForEnginePre, resolveDshHomePre } from './dsh-home-pre.js'
import {
...
function diagCtx(msg) {
  try {
    ...
    //   即落成 `$DSH_HOME/.dsh/dsh-auto-memory-pre-diagnose.log`（**多拼一层 .dsh**）；
    //   未设置时才恰好正确。resolveDshHomePre() 返回的**已经是 dsh home 本身**，故不再拼 .dsh。
    const base = resolveDshHomePre()
    if (!base) return
    appendFileSync(path.join(base, 'dsh-auto-memory-pre-diagnose.log'),
...
export function createContextHost(opts = {}) {
  ...
  function storeFor() {
    if (!store) store = new EvidenceEventStore({
      root: path.join(dshHome(), 'memory', 'evidence-pre'),
      // ★P3-9（组2，2026-09-22）：注入统一节流诊断通道；不注入时 store 内回落 console.error。
      onDiag: (key, msg) => { try { diagCtx(msg) } catch (_) {} },
    })
    return store
  }
            ...
            void Promise.resolve()
              ...
              .then(async (dec) => {
                ...
                const act = {
                  ...
                  threshold: { policyVersion: dec.features ? 'activation_policy_pre_v2' : 'activation_policy_pre_v2', score: (dec.features && dec.features.intentProb) || 0, threshold: 0.45, reason: jsReason },
                  level: 'excerpt',
                  candidates: (function () {
                ...
                try {
                  ...
                  // 与晋升流程本身无关。改由接口契约模块 lib/procedure-switch-pre.js 统一解析：
  ...
  async function emitTextEvidence(ctxT) {
    ...
    if (seg.kind === 'user') {
      ...
      try {
        ...
        lexHits = CORRECTION_LEXICON_PRE_V1.filter((w) => norm.includes(w)).length
        if (lexHits > 0) {
          const st = storeFor()
  ...
  function disposeRuntime(runtime) {
    ...
    // ★P3-3(a)（2026-09-22）：pathsByKey 原先是「模块生命周期内永不释放」的集合——
    // 写入点只有 capturePaths（每会话一条），全文件零 delete/clear；且 disposeAll 走本函数，
    // 连全量退出路径也清不掉 ⇒ findProvenance 会在已释放的 runtime 上命中陈旧快照。
    // 键口径与 :316/:482/:683 三处读点一致（String(runtime.key || '')）。
