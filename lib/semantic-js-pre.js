...
import { existsSync, readFileSync, mkdirSync, renameSync, unlinkSync, writeFileSync, rmSync, statSync } from 'node:fs'
 ...
 * 返回命中的包目录,''=未找到。
 */
export function resolvePeerTransformersDir(pluginDir) {
  try {
  ...
  return defaultPeerDirCandidates(pluginDir).find((c) => existsSync(c)) || ''
}

/**
 * semanticAssetProbe 共享实现(index.js 的 status API / 档位解析 / 引导卡同源):
 * 模型资产=发行包 lib/models 优先,其次开发树;peer=resolvePeerTransformersDir。
 * 返回形状与 v0.1.36 的 semanticAssetProbe 完全一致(status API/引导卡契约不变)。
 */
export function probeJsSemanticAssets(pluginDir) {
  const modelCands = [
  ...
  const peerDir = resolvePeerTransformersDir(pluginDir)
  return {
    assetPresent,
