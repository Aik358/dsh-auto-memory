...
export function mutationRefusalTextPre(res) {
  ...
  return '写入被记忆保护门拦截（' + failed.map((h) => h.id).join('/') + '），原文件未改动：\n' + lines.join('\n')
    ...
    // ★v3.1.4 批 J 复原：批 J 把 archivedIds 接进了 memory_note（参数表 → writePlanSnapshot →
    //   本门），该出路**从本批起真的可达** ⇒ 文案重新给出，但只在"有意移除"的语义下给
    //   （t0-8:119 行为钉「显式归档 1 张 → 放行」，文案与它不能互相打脸）。
    + '\n请修正后重试（二选一）：①把上述卡片**原样带回**（标题 + 锚点行 + 正文，含其用户备注区，逐字节不变）；'
    + '②确属有意移除时，改传 archivedIds 声明归档（旧版整份已在 handoff/archive/ 留痕）。'
