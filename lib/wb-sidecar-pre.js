 ...
 *          kind?:string, mtime?:number, criteria?:string}} o
 */
...
function hintForExpand() {
  return '可用 memory_trace_pre(id) 回溯该条目的 cue/tag/邻居与归档版本'
}
function hintForTrace() {
  return '可用 memory_expand_pre(tag) 按 tag 展开同主题条目; neighbors 为同 tag/同 cue 的相邻条目'
}
  ...
  if (sectionCards) {
    ...
    for (const c of sectionCards) {
      ...
      const card = {
        ...
        full: body.slice(0, 6000),           // P12: 卡上可展开看全文
        tags: t.tags,
    ...
    for (const e of entries) {
      ...
      const card = {
        ...
        full: String(e.preview || '').slice(0, 6000),
        tags: t.tags,
        sectionTags: t.sectionTags,
