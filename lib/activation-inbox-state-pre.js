 ...
 * 纯内存状态机,零 IO、零依赖(activation-inbox-pre 纯核心);不接 Host、不碰 prompt。
...
import {
  ...
  REFERENCE_TAIL_BUDGET_PRE_V1, ACTIVATION_POLICY_VERSION, DELIVERY_STATES_PRE_V1,
} from './activation-inbox-pre.js'

/** cooldown 步数策略(冻结;投递后冷却)。 */
export const REFERENCE_TAIL_COOLDOWN_STEPS_PRE_V1 = 2
...
export function createActivationInboxPre(opts = {}) {
  ...
  function markDelivered(packetId, ctx = {}) {
    ...
    while (deliveredOrder.length > REFERENCE_TAIL_BUDGET_PRE_V1.deliveredIdsCapacity) {
      const oldest = deliveredOrder.shift()
      deliveredIds.delete(oldest)
    }
    cooldownUntilStep = nowStep + REFERENCE_TAIL_COOLDOWN_STEPS_PRE_V1
