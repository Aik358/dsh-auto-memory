 ...
 */
import { mkdirSync, appendFileSync, readFileSync, readdirSync, statSync, rmSync } from 'node:fs'
 ...
 */
const DIAG_THROTTLE_MS_V1 = 300000
const _diagAt = new Map()
function shouldReportDiag(key) {
  try {
    const now = Date.now()
    if (now - (Number(_diagAt.get(key)) || 0) < DIAG_THROTTLE_MS_V1) return false
    _diagAt.set(key, now)
  } catch (_) { /* 计时失败不阻断上报 */ }
  return true
}
