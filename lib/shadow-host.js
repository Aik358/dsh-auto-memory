...
import {
  ...
  SHADOW_GATE_POLICY_PRE_V1, LEXICAL_POLICY_VERSION, GATE_POLICY_VERSION, NAMESPACE,
...
/**
 * ★P3-9（组2，2026-09-22）：诊断**节流**表。
 * 原先 `no-paths` 每个 Segment 都直接 console.error ⇒ 单次历史回放能刷出上百行。
 * 表放**模块级**而非闭包级：同进程多实例共享 ⇒ 不会各自持一份而双写。
 * 语义同 index.js 的 diagThrottled：首条立即透出，窗口内重复压掉。
 */
const DIAG_THROTTLE_MS_PRE_V1 = 300000
const _diagAt = new Map()
function shouldReportDiag(key) {
  try {
    const now = Date.now()
    if (now - (Number(_diagAt.get(key)) || 0) < DIAG_THROTTLE_MS_PRE_V1) return false
    _diagAt.set(key, now)
  } catch (_) { /* 计时失败不阻断上报 */ }
  return true
}
...
/**
 * @param {{engine: object, onDiag?: Function|null, recallStats?: object|null}} opts
 *   `recallStats` = 召回统计实例（可选）。**依赖注入而非模块级单例**：本模块保持零耦合，
 *   测试可传假对象（不碰真磁盘）；不传时埋点整段跳过，行为与从前逐字节相同。
 *   ★命名：不得叫 `stats` —— 本函数内部已有同名 const（shadow 自身计数器），会作用域冲突。
 */
export function createShadowHost({ engine, onDiag = null, recallStats = null }) {
  ...
  /** ★P3-9（组2）：节流上报。未注入 onDiag 时回落 console.error（保持既有行为，只去掉刷屏）。 */
  function reportDiag(key, msg) {
    if (!shouldReportDiag(key)) return
    if (typeof onDiag === 'function') { try { onDiag(key, msg); return } catch (_) { /* 回落 */ } }
  ...
  function auditDir() { return path.join(memoryDir('retrieval', dshHome), 'audit') }
  function dshHome() {
  ...
  return {
    ...
    onSegmentAccepted(runtime, seg, envelope) {
      try {
        ...
        if (!paths) { reportDiag('shadow:no-paths', '[shadow-diag] no-paths'); stats.suppressed++; pushVolatile({ reason: 'no-paths-captured', contextVersion: seg.contextVersion }); return }
        ...
        st.cooldownUntilSegment = dec.action === 'retrieve' ? (st.processedSegmentCount + SHADOW_GATE_POLICY_PRE_V1.cooldownSegments) : st.cooldownUntilSegment
        stats.evaluated++
  ...
  async function runRetrieval(ctxR) {
    ...
    // ★③ 召回统计 · shadow 通道（2026-09-22 用户拍板「分开计算」）：
    //   这条通路 = 系统**主动判断该唤起**，与「模型主动检索」是两回事，必须分账。
    //   语义：命中率（kept 空 ⇒ 唤起了但没东西）+ 哪些内容被系统认为该唤起。
    //   ★只读：不改 ls / st / 决策，不影响任何 shadow 行为（守卫断言）。
    if (recallStats && typeof recallStats.observe === 'function') {
      try {
        recallStats.observe(ls.kept.map((k) => ({ id: k.memoryId, layer: k.sourceClass || k.scope || 'shadow' })), { channel: 'shadow' })
      } catch (_) {}
    }
