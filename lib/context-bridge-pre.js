 ...
 *
 * 组成:
 *   1) 策略/预算常量(CONTEXT_BRIDGE_BUDGET_PRE_V1 / 版本化词典)
...
export { NAMESPACE }
export const CONTEXT_BRIDGE_POLICY_VERSION = 'context_bridge_pre_v1'
export const EVIDENCE_POLICY_VERSION = 'evidence_pre_v1'
export const OBSERVATION_PREFIX = 'obs_pre_'
export const EVIDENCE_PREFIX = 'ev_pre_'

/** §4 budget(冻结;变更必须升级 contextPolicyVersion)。 */
export const CONTEXT_BRIDGE_BUDGET_PRE_V1 = Object.freeze({
  maxSegments: 8,
  maxInputBytes: 4096,
  maxMemoryRefs: 8,
  maxEvidenceItems: 16,
...
/** §6 AccessKindPre 枚举。 */
export const ACCESS_KINDS_PRE_V1 = Object.freeze(['seen', 'read', 'cite', 'reuse', 'success', 'correction'])

/** §5 ContextAckPre reason 枚举(版本化)。 */
export const ACK_REASONS_PRE_V1 = Object.freeze(['ok', 'disabled', 'busy', 'unsupported', 'oversize', 'stale'])

/** §7 cite 引用分类器 v1:只认完整 memoryId token(保守;变更必须升级版本)。 */
export const CITATION_MEMORY_ID_PATTERN_PRE_V1 = /mem_[0-9a-f]{32}/g

/** §7 correction 分类器 v1 冻结词典(用户纠正/拒绝/反例;precision-first)。 */
export const CORRECTION_LEXICON_PRE_V1 = Object.freeze([
...
export function validateAuthorizedMemoryRefPre(ref) {
  ...
  if (ref.excerpt !== undefined) {
    ...
    else if (Buffer.byteLength(ref.excerpt, 'utf8') > CONTEXT_BRIDGE_BUDGET_PRE_V1.excerptBytes) p.push('excerpt-budget')
  }
  if (p.length) return { ok: false, reason: 'invalid:' + p.join(',') }
  return { ok: true, ref }
}
...
export function validateAccessEvidencePre(ev) {
  ...
  if (!ACCESS_KINDS_PRE_V1.includes(ev.kind)) p.push('kind')
...
export function buildEvidenceId(input) {
  const parts = ['access-evidence-pre-v1', input.kind, input.memoryId, String(input.sessionId || ''), input.eventSeq | 0,
...
export function createCiteEvidencesFromText(input) {
  const ids = [...new Set(String(input.text == null ? '' : input.text).match(CITATION_MEMORY_ID_PATTERN_PRE_V1) || [])].sort()
  const out = []
...
export function createCorrectionEvidencesFromText(input) {
  ...
  if (!CORRECTION_LEXICON_PRE_V1.some((w) => norm.includes(w))) return []
...
export function validateContextAckPre(ack) {
  ...
  if (ack.reason !== undefined && !ACK_REASONS_PRE_V1.includes(ack.reason)) return { ok: false, reason: 'reason' }
  return { ok: true, ack }
}
...
export function buildContextPushEnvelopePre(input) {
  const B = CONTEXT_BRIDGE_BUDGET_PRE_V1
  const drop = []
  ...
  const frame = {
    ...
    policy: {
      ...
      gatePolicyVersion: String(input.policyVersionGate || 'gate_pre_v1'),
      lexicalPolicyVersion: String(input.policyVersionLexical || 'lexical_pre_v2'),
...
export function createContextPushBridge(opts = {}) {
  ...
  const sent = new BoundedIdSet(CONTEXT_BRIDGE_BUDGET_PRE_V1.sentObservationIds)
