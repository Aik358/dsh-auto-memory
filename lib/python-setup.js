...
export function createPythonSetupPre(opts = {}) {
  ...
  // issue #105 测试接缝：模型清单 / tokenizer 清单 / fetch 均可注入，默认与生产完全一致。
  // 之所以需要注入：这条链路历史上**没有任何测试引用过**（`.on()` 崩溃因此潜伏六个版本），
  // 而本地 fixture 服务器可以完整覆盖「下载→校验→落位」而不碰公网。
  const modelSpec = opts.modelSpec && typeof opts.modelSpec === 'object' ? opts.modelSpec : MODEL_SPEC
  const tokenizerFiles = Array.isArray(opts.tokenizerFiles) ? opts.tokenizerFiles : TOKENIZER_FILES
  const fetchImpl = typeof opts.fetchImpl === 'function' ? opts.fetchImpl : (u, o) => fetch(u, o)
  ...
  // 落位名跟随清单（issue #105）：以前写死 'model_int8.onnx'，注入的 modelSpec.file 不被尊重，
  // 于是「换档位/换文件名」只能改代码，也让本地 fixture 无法自证。生产口径不变（basename 同名）。
  const modelPath = () => path.join(modelsDir(), path.basename(modelSpec.file))
  ...
  const st = {
    ...
    depsOk: false,
    configOk: false,
    dl: { bytesDone: 0, bytesTotal: modelSpec.bytes, mirror: '', startedAt: 0, etaSec: 0 },
    cancelled: false,
    // issue #105：完整性口径必须**如实**暴露。历史上清单里 sha256 留空、只比 size 下限，
    // 而模块头与 UI 都按「SHA256 校验」叙述 ⇒ 用户以为验过，实际从未验。
    integrity: '',         // ''|'sha256'|'size-only'|'sha256-mismatch'
    integrityDetail: {},   // { '<rel>': 'sha256' | 'size-only' | 'size+json' }
  }
  let dlAbort = null
...
function configReadyForModels() {
  ...
  async function detect() {
    ...
    st.modelReady = existsSync(modelPath()) && tokenizerFiles.every((f) => existsSync(path.join(modelsDir(), f)))
    st.pythons = out
    st.phase = 'idle'
    return snapshot()
  }
  ...
  function snapshot() {
    return {
      ...
      modelBytes: st.modelReady ? statSync(modelPath()).size : 0, modelExpectedBytes: modelSpec.bytes,
      integrity: st.integrity, integrityDetail: { ...st.integrityDetail },
      dl: { ...st.dl },
    }
  }
  ...
  async function downloadModel() {
    ...
    st.cancelled = false
    for (const mirror of modelSpec.mirrors) {
      if (st.cancelled) break
      const url = mirror.url(modelSpec.repo + '/resolve/main/' + modelSpec.file)
      st.phase = 'downloading'; st.dl.mirror = mirror.id; st.dl.startedAt = Date.now()
      try {
        await downloadWithResume(url, modelPath(), (done, total) => {
          st.dl.bytesDone = done; st.dl.bytesTotal = total || modelSpec.bytes
          const el = (Date.now() - st.dl.startedAt) / 1000
          st.dl.etaSec = done > 0 ? Math.max(0, Math.round(el / done * (st.dl.bytesTotal - done))) : 0
        }, () => st.cancelled)
        st.phase = 'verifying'
        const size = statSync(modelPath()).size
        // issue #105：校验从「只看 size 下限」升级为「按清单声明逐项校验」，且**回读落盘文件**。
        // 清单里 sha256 仍为空（远端 LFS 校验不可靠的历史口径），故本档目前实测是 size 精确匹配；
        // 一旦发布侧冻结出摘要，填进 MODEL_SPEC.sha256 即刻生效，无需再改这段代码。
        st.integrityDetail = {}
        st.integrityDetail[modelSpec.file] = await verifyArtifact(modelPath(), { bytes: modelSpec.bytes, sha256: modelSpec.sha256 })
        // issue #28:补下 tokenizer 套件到同目录(缺任一文件即失败,走 catch → 下一镜像)
        for (const tf of tokenizerFiles) {
          const tu = mirror.url(modelSpec.repo + '/resolve/main/' + tf)
          await downloadWithResume(tu, path.join(modelsDir(), tf), () => {}, () => st.cancelled)
          st.integrityDetail[tf] = await verifyArtifact(path.join(modelsDir(), tf), { kind: tf.endsWith('.json') ? 'json' : '' })
        }
        const modes = Object.values(st.integrityDetail)
        st.integrity = modes.includes('size-only') ? 'size-only' : 'sha256'
        if (st.integrity === 'size-only') {
          // 不谎称"已做 SHA256 校验"（模块头与 UI 历史上都是这么叙述的）。
          diagOf('[降级] python-setup: 清单未声明 sha256 ⇒ 本次仅校验大小/JSON 可解析，未做内容校验(model=' + size + ' bytes)')
        }
        st.error = ''
        ...
        try {
          await patchEmbeddingConfig({
            ...
            onnxFile: path.basename(modelSpec.file),
        ...
        if (e && e.integrityFailure) {
          // 校验不过的文件不能留在正式名上：`detect()` 判 modelReady 只看「模型 + tokenizer 是否都在」，
          // 留下坏正本会让下次启动把它当已装好的资产直接喂给 worker（比下载失败更糟）。半截体只该留在 .part。
          try { rmSync(modelPath(), { force: true }) } catch (_) {}
          for (const tf of tokenizerFiles) { try { rmSync(path.join(modelsDir(), tf), { force: true }) } catch (_) {} }
          diagOf('[降级] python-setup: 校验失败的产物已清除，不留作正式文件(镜像 ' + mirror.id + ')')
        }
  ...
  async function downloadWithResume(url, target, onProgress, isCancelled) {
    ...
    try {
      let resp = await fetchImpl(url, { headers: done > 0 ? { Range: 'bytes=' + done + '-' } : {}, signal: ac.signal })
      if (done > 0 && resp.status !== 206) {
        ...
        resp = await fetchImpl(url, { headers: {}, signal: ac.signal })
      }
