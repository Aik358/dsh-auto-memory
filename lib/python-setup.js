 ...
 *              cn(hf-mirror)/intl(hf 官方)双通道+大小校验(声明 SHA256 时追加哈希校验),复用 JS 档下载器的状态机形态。
...
const MODEL_SPEC = {
  ...
  bytes: 568456694,
  sha256: '', // 尚未冻结可信哈希：严格检查声明的 bytes，但不能宣称已验证模型身份。
  mirrors: [
...
function configReadyForModels() {
  ...
  async function downloadModel() {
    ...
    st.cancelled = false
    st.error = ''; st.modelReady = false; st.configOk = false
      ...
      try {
        ...
        for (const tf of TOKENIZER_FILES) {
          ...
          await downloadWithResume(tu, path.join(modelsDir(), tf), () => {}, () => st.cancelled, {
            minBytes: 1, kind: tf.endsWith('.json') ? 'json' : 'binary',
          })
        }
        st.modelReady = true; st.phase = 'ready'; st.error = ''
   ...
   */
  async function downloadWithResume(url, target, onProgress, isCancelled, spec = {}) {
    ...
    try {
      ...
      if (resp.status === 206) {
        ...
        if (!range || Number(range[1]) !== done || Number(range[2]) < done ||
            (range[3] !== '*' && Number(range[3]) <= Number(range[2]))) {
          if (resp.body) await resp.body.cancel()
          const err = new Error('Content-Range 与断点不匹配')
      ...
      // Verify the actual temporary file before publishing; failed downloads must
      // never replace a previously usable artifact.
      const integrity = await verifyArtifact(part, spec)
      if (integrity === 'size-only') diagOf('[降级] python-setup: size-only verification (no SHA256): ' + path.basename(target))
      ...
      if (e && e.integrityFailure) dropPart()
      else saveMeta(bytesOnDisk()) // 网络/写盘中断才保留断点；坏产物必须重新下载。
