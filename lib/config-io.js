 ...
 * `{ok:false, error:'ENOENT … rename … .tmp -> …'}`（见 `tests/smoke/smoke-test-p3-batchb-guards-pre.mjs` P3B-2d）。
 *
...
export function writeTextAtomicPreSync(file, text) {
  const tmp = file + ATOMIC_TMP_SUFFIX_V1 + tmpUniqueSuffixPre()
  try {
 ...
 * 被 `tests/smoke/smoke-test-t8-configio-pre.mjs` 的 T8c-6 钉死 —— 配置 I/O 层不得因为
 * 任何兄弟模块的加载期故障而失效。故此处**内联**同一套退避刻度（只用 node: 内置），
 * 语义与 `fs-retry.js` 的 `retryRename` 保持一致。
 */
const RENAME_RETRY_DELAYS_V1 = Object.freeze([0, 50, 150, 400, 1000])
const TRANSIENT_RENAME_CODES_V1 = new Set(['EPERM', 'EACCES', 'EBUSY'])
async function renameBoundedRetryPre(from, to) {
  const last = RENAME_RETRY_DELAYS_V1.length - 1
  for (let attempt = 0; attempt <= last; attempt++) {
    const wait = RENAME_RETRY_DELAYS_V1[attempt]
    if (wait) await sleepDelayPre(wait)
    try {
      await rename(from, to)
      return
    } catch (e) {
      const transient = TRANSIENT_RENAME_CODES_V1.has(e && e.code)
