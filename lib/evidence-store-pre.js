...
/** §10 持久化策略(冻结)。 */
export const EVIDENCE_STORE_POLICY_PRE_V1 = Object.freeze({
  schemaVersion: 1,
  storePolicyVersion: 'evidence_store_pre_v1',
  keepDays: 30,
...
export function projectEvidenceForDurable(ev, opts = {}) {
  ...
  const maxBytes = Number(opts.eventMaxBytes) || EVIDENCE_STORE_POLICY_PRE_V1.eventMaxBytes
  const projected = {
    schemaVersion: 1,
    namespace: NAMESPACE,
    storePolicyVersion: EVIDENCE_STORE_POLICY_PRE_V1.storePolicyVersion,
 ...
 * 原先 `_writeLine` 每次写失败都直接 console.error ⇒ 持续失败（磁盘满/权限错）时刷屏。
 * 表放**模块级**而非实例级：同进程多实例共享 ⇒ 不会各自持一份而双写。
...
export class EvidenceEventStore {
  constructor(opts = {}) {
    ...
    this._appended = new BoundedIdSet(EVIDENCE_STORE_POLICY_PRE_V1.appendedIdCache)
    this._swept = false
    this.stats = { appended: 0, duplicates: 0, oversize: 0, invalid: 0, writeFailed: 0, sweptFiles: 0, lastWriteError: null }
    // ★P3-9（组2）：可注入诊断通道（宿主不注入时 reportDiag 回落 console.error，行为向后兼容）。
    this._onDiag = typeof opts.onDiag === 'function' ? opts.onDiag : null
  }

  /** ★P3-9（组2）：节流上报。首条立即透出，窗口内重复压掉。 */
  reportDiag(key, msg) {
    if (!shouldReportDiag(key)) return
    if (this._onDiag) { try { this._onDiag(key, msg); return } catch (_) { /* 回落 */ } }
  ...
  async _writeLine(line) {
    try {
      ...
      try { this.reportDiag('evidence:write-failed', '[evidence-store] write-failed: ' + this.stats.lastWriteError) } catch (_) {}
      return false
    }
  }
  ...
  sweepRetention(force = false) {
    ...
    try {
      ...
      for (const f of files) {
        ...
        if (ageDays > EVIDENCE_STORE_POLICY_PRE_V1.keepDays || total > EVIDENCE_STORE_POLICY_PRE_V1.maxTotalBytes) {
...
/** aggregate 快照持久化(evidence-pre/aggregates/index.json;可重建派生物,仅调试/导出用)。 */
export function persistAggregatesSnapshot(root, aggregates) {
  const dir = path.join(root, 'aggregates')
  mkdirSync(dir, { recursive: true })
  const payload = { schemaVersion: 1, namespace: NAMESPACE, storePolicyVersion: EVIDENCE_STORE_POLICY_PRE_V1.storePolicyVersion, rebuiltAt: Date.now(), aggregates }
