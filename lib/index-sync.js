 ...
 *
 * 约束(冻结):单页 ≤64 records 且 ≤252KiB(JSON 字节;页预算必须为线帧信封留余量,见
 * INDEX_SYNC_PAGE_BUDGET_V1 注释);pageDigest=sha256(canonical(records));
...
// maxPageBytes 必须小于 m7-wire 的 MAX_LINE_BYTES(256KiB):页 payload 出站时还要包一层
// 帧 envelope(requestId/workerEpoch/sentAt/frameId/pageDigest 等,实测约 230B)+ 换行符。
// 若预算贴满上限,"builder 接受"的临界页在线上必超 MAX_LINE_BYTES,Python worker 对超行帧
// fail-closed 直接退出(line-oversize → break),而熔断计数被 begin 成功帧清零、永远达不到
// 阈值 → 每 Segment 重生 worker 重建同一坏页,索引永不 ready。给 4KiB 余量(信封的 ~18 倍)。
export const INDEX_SYNC_PAGE_BUDGET_V1 = Object.freeze({ maxRecordsPerPage: 64, maxPageBytes: 252 * 1024 })
