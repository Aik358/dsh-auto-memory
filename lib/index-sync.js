...
import {
  ...
  M7_INDEX_POLICY_VERSION_PRE_V1,
} from './m7-wire.js'

/**
 * ★ 页字节预算（2026-09-19 上游 PR #78 / issue #64 同步落地）。
 *
 * **为什么是 252KiB 而不是 256KiB**：`maxPageBytes` 只约束**页 payload**，
 * 而 m7-wire 的 `MAX_LINE_BYTES` 是 **256KiB**，约束的是**整条线帧**（payload + 帧信封 + 换行）。
 * 两者同为 256KiB ⇒ payload 落入约 231B 临界窗口的页 **builder 接受、线上必超限**
 * ⇒ worker 判 `line-oversize` fail-closed 退出。
 * 更糟的是叠加了死锁：① 熔断阈值 3，但每段 `index_sync_begin` 的成功帧把计数清零 ⇒ 峰值 2 永不熔断；
 * ② `syncId` 由 recordCount **确定性派生** ⇒ 重试恒重建同一坏页 ⇒ 每 Segment 重生 worker、索引永不 ready。
 *
 * 故留 **4KiB 余量**（≈ 实测信封 202–230B 的 18 倍）。
 */
export const INDEX_SYNC_PAGE_BUDGET_PRE_V1 = Object.freeze({ maxRecordsPerPage: 64, maxPageBytes: 252 * 1024 })
...
export function buildIndexSyncPlansPre(input) {
  const B = INDEX_SYNC_PAGE_BUDGET_PRE_V1
  ...
  for (const scope of SCOPE_ORDER_PRE) {
    ...
    const begin = {
      ...
      sourceTuples, recordCount, pageCount, indexPolicyVersion: M7_INDEX_POLICY_VERSION_PRE_V1,
    }
