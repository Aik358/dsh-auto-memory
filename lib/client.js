    ...
    function PlanTab() {
      ...
      async function oneClickContinue() {
        ...
        try {
          ...
          var newId = created && (created.sessionId || created.id)
          ...
          await rf.session.prompt({ requestId: reqId, sessionId: newId, mode: 'queue', content: [{ type: 'text', text: d.carryText }] })
      ...
      if (open) {
        ...
        if (data.planVersions && data.planVersions.length) {
          rows.push(h(Card, { title: t('planVersions') }, h('div', { 'data-dam-row': '' }, data.planVersions.map(function (v) {
            return h('button', { 'data-dam-btn': '', key: v.name, onClick: function () { setOpen(v.name) } }, v.name)
          }))))
        }
        ...
        apiPost(API.handoffContinue, {}).then(function (d) {
          ...
          return p.then(function (created) {
            ...
            return chain.then(function () {
              ...
              return remoteFace.session.prompt({ requestId: reqId, sessionId: newId, mode: 'queue', content: [{ type: 'text', text: d.carryText }] })
            })
          })
        }).then(function () {
          try { localStorage.setItem('dsh-auto-memory-pre.autoCont.last', String(Date.now())) } catch (eL) {}
