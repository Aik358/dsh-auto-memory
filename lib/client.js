    ...
    function PlanTab() {
      ...
      if (wl && (wl.window > 0 || wl.at || Object.keys(wl).length)) {
        ...
        rows.unshift(h(Card, { title: t('waterCardTitle') }, h('div', { 'data-dam-content': '' }, [
          ...
          // 过期不是无数据:保留最后实测值并如实标出它是几点测的(2026-09-23 用户实证:闲置 10 分钟后整卡谎报「尚未测量」)
          wl.stale ? h('div', { 'data-dam-hint': '', style: { marginBottom: '4px' } },
            t('waterStaleMeasured').replace('{time}', new Date(Number(wl.at) || 0).toLocaleTimeString())) : null,
