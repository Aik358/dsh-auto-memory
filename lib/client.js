    ...
    function PlanTab() {
      ...
      async function oneClickContinue() {
        ...
        try {
          ...
          // M-CM6-B v2:同 cwd 创建 → 工作区绑定/权限状态与旧会话一致;失败回退无参创建
          var created
          try { created = await rf.session.create(d.ws ? { cwd: d.ws } : {}) } catch (eCwd) { created = await rf.session.create({}) }
          ...
          // 沿用旧会话模型(provider/model 由 host 从旧会话 request/context 折叠而来;不可用则静默回退路由默认)
          if (d.provider && d.model && typeof rf.session.selectModel === 'function') {
            setContMsg(locale === 'zh' ? '正在沿用旧模型 ' + d.model + '…' : 'Restoring model ' + d.model + '…')
            try { await rf.session.selectModel({ sessionId: newId, provider: d.provider, model: d.model }) } catch (eMdl) {}
          }
    ...
    function AutoContinueHost() {
      ...
      var runAutoContinue = function () {
        ...
        apiPost(API.handoffContinue, {}).then(function (d) {
          if (!d || !d.ok) throw new Error((d && d.error) || 'no handoff material')
          var p
          try { p = remoteFace.session.create(d.ws ? { cwd: d.ws } : {}) } catch (eC) { p = remoteFace.session.create({}) }
          return p.then(function (created) {
            var newId = extractSessionId(created)
            if (!newId) throw new Error('create: no sessionId')
            var chain = Promise.resolve()
            if (d.provider && d.model && typeof remoteFace.session.selectModel === 'function') {
              chain = chain.then(function () { try { return remoteFace.session.selectModel({ sessionId: newId, provider: d.provider, model: d.model }) } catch (eM) { return null } })
            }
