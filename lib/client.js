      ...
      apiGet(API.updateCheck).then(function (d) {
        if (d && d.current) {
          try {
            ...
            var isExisting = !!seen || !!localStorage.getItem('dsh-auto-memory-pre.firstRunDone') || !!localStorage.getItem('dsh-auto-memory-pre.semWizardDone')
            var seenBeforeMajor = isExisting && (!seen || cmpVersion(seen, '0.1.30') < 0)
            if (seenBeforeMajor && !localStorage.getItem('dsh-auto-memory-pre.semWizardDone')) {
              openDialog({ kind: 'welcomeTour' })
