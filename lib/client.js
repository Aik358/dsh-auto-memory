 ...
 */
console.log('[dsh-auto-memory] client v3.1.1 fingerprint: migrate-pack-quiet-logs')
window.__ModuleLoader__.load({
  ...
  recallStats: ROUTE_PREFIX + '/recall-stats',
    ...
    function KanbanCard(props) {
      ...
      var body = expanded ? (c.full || c.preview || '') : (c.preview || '')
      var canExpand = (c.full || '').length > (c.preview || '').length
      return h('div', {
        'data-dam-kcard': '',
        ...
        (canExpand || (c.full || '').length > 300) ? h('button', {
      ...
      h('div', { 'data-dam-kx-scroll': '', style: { height: '100%', overflow: 'auto', padding: '12px 16px', boxSizing: 'border-box' } }, [
        ...
        drawer ? kxPortal(h('div', {
          ...
          (drawer.sectionTags && drawer.sectionTags.length)
            ...
            : null,
          h('pre', { key: 'body', 'data-dam-kx-full': drawer.id, style: { whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'inherit', margin: 0, fontSize: '12px' } }, drawer.full || drawer.preview || ''),
        ].map(function (node, i) {
    ...
    function StorageTab(props) {
      ...
      function migExport(opts) {
        ...
        apiPost(API.migrateExport, { outPath: migOut || undefined }).then(function (j) {
          setMigBusy('')
      ...
      function migPreview() {
        ...
        apiPost(API.migrateInspect, { packPath: migPack }).then(function (j) {
          setMigBusy('')
      ...
      function migApply() {
        ...
        apiPost(API.migrateImport, { packPath: migPack, onConflict: migConflict }).then(function (j) {
          setMigBusy('')
        ...
        var TOUR_STEPS = [
          ...
          { art: 'radar', core: '', kicker: locale === 'zh' ? '唤起与固化' : 'ACTIVATION',
            ...
            toggles: [
              ...
              { key: 'procedureInjectEnabled', name: locale === 'zh' ? '技能固化与注入' : 'Skill crystallization', sub: locale === 'zh' ? '重复流程固化为 checklist 自动附上；跨会话验证后晋升（「记忆中枢」页审批）' : 'Turn repeated flows into auto-attached checklists; promote after validation (Memory Hub tab)', where: locale === 'zh' ? '记忆中枢' : 'Memory Hub' },
