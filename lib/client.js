...
window.__ModuleLoader__.load({
  ...
  kanbanCard: ROUTE_PREFIX + '/kanban-card',
    ...
    function KanbanView(props) {
      ...
      // Hooks must run on every render, including when the drawer is closed.
      var drawerFull = useCardFull(drawer)
      ...
      h('div', { 'data-dam-kx-scroll': '', style: { height: '100%', overflow: 'auto', padding: '12px 16px', boxSizing: 'border-box' } }, [
        ...
        drawer ? kxPortal(h('div', {
          ...
          (drawer.sectionTags && drawer.sectionTags.length)
            ...
            : null,
          h('pre', { key: 'body', 'data-dam-kx-full': drawer.id, style: { whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'inherit', margin: 0, fontSize: '12px' } }, (drawerFull || drawer.full || drawer.preview || '')),
        ].map(function (node, i) {
          ...
          pipeline.map(function (p) {
            ...
            return h('div', { 'data-dam-content': '', key: p.procedureId, style: { padding: '5px 0', borderBottom: '1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgba(255,255,255,.16)) 40%, transparent)' } },
              ...
              h('div', { style: { display: 'flex', gap: '6px', marginTop: '5px' } },
                ...
                //   现改为**直接依赖只读投影** `p.promotion`(宿主 memory-hub 用 evaluatePromotion 生成,
                ...
                //   判据来自宿主只读投影 p.promotion.overridable（memory-hub 用 promotionOverrideView 生成，
...
function WorkspaceTab() {
  ...
  if (tab === 'stats') return h(StatsTab)
        ...
        var TOUR_STEPS = [
          ...
          { art: 'radar', core: '', kicker: locale === 'zh' ? '唤起与固化' : 'ACTIVATION',
            ...
            toggles: [
              ...
              //   默认关：procedural 线只留模型直写（memory_procedure）与人工两条正经通路。
    ...
    //   `promptSections` 为准（宿主常量 PROMPT_SECTION_KEYS_V1 是唯一真源），本表只提供文案；
    //   键集合必须与宿主常量相等 —— 由守卫 smoke-test-t1-prompt-sections-pre.mjs 断言。
    //   offZh/offEn 只给 must 段（宿主 PROMPT_SECTION_MUST_V1），其余用通用提示。
  ...
  field(t('fConsolidateMin'), h('input', { 'data-dam-input': '', type: 'number', min: 80, value: cfg.autoConsolidateMinChars === undefined ? 240 : cfg.autoConsolidateMinChars, onChange: function (e) { set('autoConsolidateMinChars', Number(e.target.value) || 240) } }), t('fConsolidateMinHint')),
        ...
        section('skills', sectionLabels.skills, [
          ...
          // ★v3.1.3 补控件（引导审计 K-3）：本键功能是通的（index.js 有 getter → memory-hub 消费），
  ...
  field(t('fUserDir'), h('input', { 'data-dam-input': '', value: cfg.userMemoryDir, onChange: function (e) { set('userMemoryDir', e.target.value) } }), t('fUserDirHint')),
        ...
        section('about', sectionLabels.about, [
          // WB-GRAPH 白板线一键切换(2026-09-16, board_mode_v1): 旧版白板(默认) / 新版看板(dsh-graph vendor
  ...
  field(t('fVersion'), h('div', { 'data-dam-row': '' },
    ...
    function apply(ctx) {
      ...
      // 语言切换时通知所有订阅者重渲染
      var disposeLocale = onLocale(function () { emit() })
      ctx.effect(function () { return disposeLocale }, 'dsh-auto-memory: locale')
      ...
      var semanticDetectTimer = setTimeout(function () {
        apiGet(API.semanticDeepDetect).then(function (d) {
          try {
            if (!d || d.ready) return
      ...
      ctx.effect(function () {
        return function () {
          ...
          clearTimeout(semanticDetectTimer)
        }
      }, 'dsh-auto-memory: background-polling')
      ...
      function registerSurfaces() {
        try {
          ...
          surfaceDisposers.push(slots.inject('shell.overlay', function () {
            return slots.register({ name: 'shell.overlay', id: 'auto-memory-autocont', order: 7 }, function () { return h(AutoContinueHost) })
          }))
          ...
          if (controller.panelPos() === 'page' || controller.panelPos() === 'both') {
            surfaceDisposers.push(slots.inject('conversation.view', function () {
              return slots.register(
                { name: 'conversation.view', id: 'auto-memory-panel', order: 78, label: function () { return locale === 'zh' ? '记忆' : 'Memory' } },
          ...
          surfaceDisposers.push(slots.inject('conversation.view', function () {
            return slots.register(
              { name: 'conversation.view', id: 'auto-memory-kanban', order: 80, label: function () { return locale === 'zh' ? '白板看板' : 'Whiteboard' } },
          ...
          if (WBG_ENABLED) {
            surfaceDisposers.push(slots.inject('conversation.view', function () {
              return slots.register(
                { name: 'conversation.view', id: 'auto-memory-graph', order: 81, label: function () { return locale === 'zh' ? '白板画布' : 'Graph' } },
      ...
      ctx.effect(function () {
        return function () {
          if (surfacesRefreshHook === refreshSurfaces) setSurfacesRefreshHook(null)
          for (var i = 0; i < surfaceDisposers.length; i++) { try { surfaceDisposers[i]() } catch (e) {} }
          surfaceDisposers = []
        }
