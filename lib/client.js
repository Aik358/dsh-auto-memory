          ...
          h('div', { style: box },
            ...
            h('button', { 'data-dam-btn': '', style: close, onClick: function () {
              try {
                ...
                // 规范 F2(RELEASE-SEMANTIC-OPTION.md):首启关闭后,若 C2 缺失则追加一次
                // 「安装向导」弹窗(立即安装/稍后 双选;不下载继续 C1,设置保留入口)。
                if (semStatus && semStatus.status === 'missing' && !localStorage.getItem('dsh-auto-memory-pre.semWizardDone')) {
                  openDialog({ kind: 'modelDownload' })
                }
              } catch (e3) {}
              closeDialog()
            } }, t('gotIt'))))
      }
      if (dialogState.kind === 'modelDownload') {
        // C2 首启安装向导(规范 F2/F3):双选确认;下载/校验/自检进度内联;稍后继续 C1。
        ...
        var dlActive2 = dl.phase === 'downloading' || dl.phase === 'verifying'
        var wizReady = wizSt.ready
        var totalJs2 = wizSt.manifestBytes || Math.round(130 * 1024 * 1024)
        var prog2 = dlActive2 || dl.phase === 'done'
          ? Math.min(100, Math.round(((dl.bytesDone || 0) / Math.max(1, dl.bytesTotal || totalJs2)) * 100))
          : (wizReady ? 100 : 0)
        var fmt2 = function (b) { return b ? (b / (1024 * 1024)).toFixed(1) + ' MB' : '—' }
        var wizPhase = dl.phase === 'verifying' ? t('dlVerifying')
          : dl.phase === 'downloading' ? (t('dlDownloading') + ' · ' + fmt2(dl.bytesDone || 0) + ' / ' + fmt2(dl.bytesTotal || totalJs2))
          : dl.phase === 'error' ? (t('dlError') + ': ' + String(dl.error || '').slice(0, 100))
          : dl.phase === 'done' ? t('dlDone')
          : (locale === 'zh' ? '下载完成后将自动校验(SHA256)并做最小推理自检,通过即自动启用。' : 'After download: SHA256 verify + minimal inference self-test, then auto-enable.')
        var wizDone = function () {
          try { localStorage.setItem('dsh-auto-memory-pre.semWizardDone', '1') } catch (e4) {}
          closeDialog()
        }
        var startWiz = function () {
          apiPost('/api/dsh-auto-memory-pre/semantic-download', { action: 'start', mirror: 'auto' }).then(function () {
            fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r2) { return r2.json() }).then(function (j2) { Object.assign(wizSt, j2); try { emit() } catch (e8) {} }).catch(function () {})
          }).catch(function () {})
        }
        return h('div', { style: overlay },
          h('div', { style: box },
            h('div', { style: head }, locale === 'zh' ? '启用内置语义引擎？' : 'Enable the built-in semantic engine?'),
            h('div', { style: sub }, locale === 'zh' ? '下载约 130MB 本地量化模型（multilingual-e5-small q8）。完全本地运行，记忆不出电脑；缓存位置在插件目录 models/。可随时在设置中卸载或切换回词法检索。' : 'Downloads a ~130MB local quantized model (multilingual-e5-small q8). Fully local — memories never leave this machine; cached under the plugin models/ dir. Uninstall or switch back to lexical anytime in settings.'),
            wizReady ? h('div', { style: { fontSize: 'calc(11.5px * var(--dam-scale))', color: '#7fdcb0', fontWeight: 700 } }, '✓ ' + (locale === 'zh' ? '语义引擎已就绪' : 'Semantic engine ready')) : null,
            !wizReady && wizSt.loaded ? h('div', { style: { marginTop: '2px' } },
              h('div', { style: { display: 'flex', justifyContent: 'space-between', fontSize: 'calc(10px * var(--dam-scale))', opacity: .7, marginBottom: '3px', gap: '8px' } },
                h('span', null, wizPhase),
                h('span', null, prog2 + '%')),
              h('div', { style: { height: '7px', borderRadius: '99px', background: 'rgba(128,128,128,.14)', overflow: 'hidden' } },
                h('div', { style: { height: '100%', width: prog2 + '%', borderRadius: '99px', background: dl.phase === 'error' ? 'linear-gradient(90deg,#c44a4a,#e08a8a)' : 'linear-gradient(90deg, var(--dam-accent, #2456c4), #6f9bff)', transition: 'width .4s ease' } })),
              h('div', { style: { fontSize: 'calc(10px * var(--dam-scale))', opacity: .6, marginTop: '3px' } },
                (locale === 'zh' ? '体积: 约130MB（5 个文件）· 下载源: ' : 'Size: ~130MB (5 files) · Source: ') + (dl.mirrorUsed === 'cn' ? t('mCn') : dl.mirrorUsed === 'intl' ? t('mIntl') : t('mAuto')) + (locale === 'zh' ? ' · 失败自动切备用源' : ' · auto-failover'))) : null,
            h('div', { style: { display: 'flex', justifyContent: 'flex-end', gap: '8px', marginTop: '6px' } },
              wizReady ? h('button', { 'data-dam-btn': '', style: close, onClick: wizDone }, t('gotIt'))
                : dlActive2 ? h('button', { 'data-dam-btn': '', style: close, onClick: function () { apiPost('/api/dsh-auto-memory-pre/semantic-download', { action: 'cancel' }).catch(function () {}) } }, t('semDlCancel'))
                : h('button', { 'data-dam-btn': '', style: close, onClick: function () { startWiz() } }, (dl.phase === 'error' || dl.phase === 'cancelled') ? t('semDlRetry') : t('semInstall')),
              !wizReady && !dlActive2 ? h('button', { 'data-dam-btn': '', onClick: wizDone }, locale === 'zh' ? '稍后再说(继续词法)' : 'Later (keep lexical)') : null)))
      }
