        ...
        var poll = function () {
          ...
          apiGet(API.autoContState, sidQ ? { sessionId: sidQ } : {}).then(function (s) {
            ...
            if (arm && arm.ratio >= thr) {
              ...
              setAcConfirm({ ratio: Number(arm.ratio) || 0, tokens: Number(arm.tokens) || 0, window: Number(arm.window) || 0, ring: Number(arm.ring) || 0, wall: Number(arm.wall) || 0, edgeAt: Number(arm.edgeAt) || 0, deferCount: Number(arm.deferCount) || 0, awaitIdle: !!arm.awaitIdle })
      ...
      var runAuto = function () {
        ...
        apiPost(API.autoContDecide, { action: 'agree', edgeAt: edgeAt }).then(function (r) {
          if (r && r.ok && r.deferred) {
            // 2026-09-14 (issue#35 Defect A):回合仍在运行 → 宿主顺延 20s 再评估,armed 保留、
            // 倒计时顺延,轮询会带回新 expiresAt。此时**尚未开始接续**:不收卡、不显示
            // 「正在接续」,如实提示;用户可随时拒绝。
            setAcSt(locale === 'zh' ? '回合仍在进行中,确认卡保留,将在回合空闲后自动接续(可拒绝)…' : 'Turn still active — card kept, will auto-continue once idle…')
            return
          }
          if (r && r.ok) {
            setAcConfirm(null)
      ...
      if (acConfirm || acCd > 0 || acSt) {
        return h('div', { 'data-dam-autocont': '', style: { position: 'fixed', right: '16px', bottom: '16px', zIndex: 9000, maxWidth: 'min(440px, calc(100vw - 32px))', boxSizing: 'border-box', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(128,128,128,.35)', background: 'rgba(18,22,30,.93)', color: '#e8eaed', fontSize: '12px', lineHeight: '1.55', fontFamily: 'system-ui', boxShadow: '0 10px 30px rgba(0,0,0,.35)', backdropFilter: 'blur(10px)' } },
          acConfirm ? h('div', { 'data-dam-autocont-confirm': '' }, [
            ...
            // 2026-09-14 (issue#35):awaitIdle 倒计时因回合活跃被顺延时,如实告知用户当前状态
            acConfirm.deferCount > 0 ? h('div', { 'data-dam-hint': '', style: { marginTop: '2px', color: 'var(--dsw-alias-warn, #e8c584)' } }, locale === 'zh' ? '回合仍在进行中,已自动顺延 ' + acConfirm.deferCount + ' 次,将在回合空闲后接续' : 'Turn active — deferred ×' + acConfirm.deferCount + ', will continue when idle') : null,
