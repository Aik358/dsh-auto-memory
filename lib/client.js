  ...
  factory: (require) => {
    ...
    var CHANGELOG = {
      '0.1.30': { zh: [
        ...
        '补丁 0.1.31:发布物运行时 section 名 dsh:m6-reference-tail-pre → 裸名(发布身份统一,不再带预览标识)。',
        ...
        '补丁 0.1.39:修复动态记忆内容含 {{...}} 模板变量时整轮运行失败(malformed prompt variable reference)——记忆/日志等注入内容已在全部出口中和为全角括号,模板引擎零命中,展示效果不变。',
        '提示:这是 0.1.x 稳定线的最终稳定版本,此后仅针对 issue 做稳定性更新;全新版本将在未来两周内发布,工作重心为对标 GPT-6 Astra「上下文管理」的新能力——结构化任务笔记跨窗口交接、历史归档可搜索、按需主动检索。',
      ], en: [
        ...
        'Patch 0.1.31: release runtime section name dsh:m6-reference-tail-pre → bare name (unified release identity, no preview marker).',
        ...
        'Patch 0.1.39: fixed whole-turn failures (malformed prompt variable reference) when dynamic memory content contained {{...}} template variables — injected memories/logs/reflections are now neutralized to full-width braces at every exit; the template engine never sees them and display is unchanged.',
        'Note: this is the final stable release of the 0.1.x line; only issue-driven stability updates will follow. The next version ships within two weeks, focused on GPT-6 Astra-style context management — structured task notes handed off across windows, searchable history archive, and on-demand retrieval.',
      ] },
      '0.1.29': { zh: [
