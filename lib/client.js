 ...
 */
console.log('[dsh-auto-memory] client v3.0.0 fingerprint: index-self-heal')
  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        fNoteCap: '项目笔记容量上限(字符)', fNoteCapHint: '项目笔记 MEMORY.md 的容量上限,默认 12000。超出时自动整理:先把较早内容交给模型折叠成要点,失败则退回整条归档(原文进 archive,不丢)——新记忆不会被堵在外面。注意与上面的「注入预算」不是一回事:这个管文件本体大小,注入预算管每轮往上下文塞多少摘要。',
        fUserCap: '用户级记忆容量上限(字符)', fUserCapHint: '用户级 MEMORY.md(跨项目规则)的容量上限,默认 12000。整理方式与项目笔记相同(先折叠成要点,必要时整条归档)。',
    ...
    function KanbanView(props) {
      ...
      useEffect(function () {
        ...
        apiGet(API.kanbanBoard, { sessionId: currentSessionIdClient() })
          .then(function (k) { if (alive) setData(k && k.enabled ? k : null) })
        ...
        rows.push(h(Card, { title: (locale === 'zh' ? '技能审批队列' : 'Skill approval queue') + ' (' + pipeline.length + ')' },
          pipeline.map(function (p) {
            ...
            return h('div', { 'data-dam-content': '', key: p.procedureId, style: { padding: '5px 0', borderBottom: '1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgba(255,255,255,.16)) 40%, transparent)' } },
              h('div', null, h('b', null, p.title), h('span', { 'data-dam-hint': '', style: { marginLeft: '6px' } }, '[' + p.stage + ']' + (p.observationOnly ? (locale === 'zh' ? ' · 观察（不参与自动晋升）' : ' · observation-only') : '') + (p.pinned ? ' 📌' : '') + (p.riskLevel === 'high' ? ' ⚠' : '')),
    ...
    function PlanTab() {
      ...
      if (!data.enabled) return h('div', null, [switchCard, h(Card, { title: t('planTitle') }, h('div', { 'data-dam-hint': '' }, t('planDisabled')))])
      ...
      return h('div', { 'data-dam-settings': '' },
        ...
        section('semantic', sectionLabels.semantic, [
          ...
          guide === 'js' || guide === 'python' ? (function () {
            ...
            var startDl = function () {
              apiPost(API.semanticDownload, { action: 'start', mirror: mirror }).then(refreshSem).catch(function () {})
            }
            var cancelDl = function () {
              apiPost(API.semanticDownload, { action: 'cancel', mirror: mirror }).then(refreshSem).catch(function () {})
            }
            return h('div', { style: { border: '1px solid color-mix(in srgb, var(--dam-accent, #2456c4) 40%, transparent)', borderRadius: '10px', padding: '10px 12px', marginBottom: '8px', fontSize: 'calc(11.5px * var(--dam-scale))', lineHeight: 1.6 } },
        ...
        field(t('fNoteCap'), h('input', { 'data-dam-input': '', type: 'number', min: 500, value: cfg.noteCapacityChars === undefined ? 12000 : cfg.noteCapacityChars, onChange: function (e) { set('noteCapacityChars', Number(e.target.value) || 12000) } }), t('fNoteCapHint')),
        field(t('fUserCap'), h('input', { 'data-dam-input': '', type: 'number', min: 500, value: cfg.userCapacityChars === undefined ? 12000 : cfg.userCapacityChars, onChange: function (e) { set('userCapacityChars', Number(e.target.value) || 12000) } }), t('fUserCapHint')),
