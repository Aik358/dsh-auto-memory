...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CHANGELOG = {
      '0.1.30': { zh: [
        '修复:DSH Desktop 增强模式(透明/Mica 窗口材质)下记忆面板半透明、文字几乎不可读——面板打开时实测主题令牌 alpha,过低时自动提升到 0.96(保留色相),普通模式玻璃观感零变化。感谢 @ProperSAMA 的贡献(PR #12)。',
        '修复:记忆面板默认位置不再遮挡侧边栏左下角「记忆」入口按钮——改为锚定按钮正上方,打开时若重叠自动让出;另支持点击面板外部或按 Esc 关闭(入口按钮 toggle 语义保留)。感谢 @ProperSAMA(PR #12)。',
        '改进:首启欢迎向导上线——分步介绍全部功能,每项当场开关(含引擎检测/下载/外部来源扫描);感谢社区反馈驱动(#10 @nkh0472)。',
        '说明:无人值守/批处理场景已有完整支持——设置→自动化「无人值守模式」+「夜间/批量自动托管」(22:00-08:00),托管期间不注入欢迎回来/寒暄/行为指令,日历静默(#10 @nkh0472)。',
      ], en: [
        'Fix: In DSH Desktop enhanced mode (transparent/Mica window material) the memory panel turned semi-transparent and text was barely readable — the theme token alpha is now measured on open and the panel background raised to 0.96 when too low (hue preserved); normal modes keep their glass look. Thanks to @ProperSAMA (PR #12).',
        'Fix: The memory panel no longer covers the sidebar\'s bottom-left "Memory" entry button — the default position anchors above the button and overlapping geometries auto-yield on open; you can also close the panel by clicking outside it or pressing Escape (button toggle semantics kept). Thanks to @ProperSAMA (PR #12).',
        'New: First-launch welcome tour — step-by-step tour of every feature with per-feature switches right in the tour (engine detection/download and external-source scanning included); driven by community feedback (#10, @nkh0472).',
        'Note: Unattended/batch workflows are fully supported — Settings → Automation offers "Unattended mode" + "Auto-unattended at night" (22:00-08:00); while engaged, welcome-back/niceties/behavioural directives are stripped and calendar stays silent (#10, @nkh0472).',
      ] },
      '0.1.29': { zh: [
    ...
    var CSS = [
      ...
      // ── 每步 Office 式立体图形(纯 CSS,替换 emoji)——玻璃小物件悬浮在堆叠体上方 ──
      '[data-dam-tour-art="bubble"] .orb { left: 7px; top: 7px; width: 32px; height: 32px; border-radius: 50%; background: radial-gradient(circle at 35% 28%, rgba(255,255,255,.85), rgba(255,255,255,.18) 55%, rgba(170,200,255,.30)); }',
      '[data-dam-tour-art] { position: absolute; left: 50%; top: 50%; width: 46px; height: 46px; margin: -58px 0 0 -23px;',
      '  transform: translateZ(46px); transform-style: preserve-3d;',
      '  animation: dam-core-pop .40s cubic-bezier(.2,.9,.3,1.25) both; }',
      '[data-dam-tour-art] span { position: absolute; box-sizing: border-box; }',
      // 共用玻璃面材质:白磨砂+白边光(matte,严禁霓虹)
      '[data-dam-tour-art] .gf { background: linear-gradient(135deg, rgba(255,255,255,.30), rgba(255,255,255,.10)); border: 1.2px solid rgba(255,255,255,.55); box-shadow: inset 0 1px 0 rgba(255,255,255,.6), 0 5px 12px rgba(10,20,60,.25); backdrop-filter: blur(3px); -webkit-backdrop-filter: blur(3px); }',
      // art=store 记忆库:三层微缩玻璃板(呼应主 Logo)
      '[data-dam-tour-art="store"] .p { left: 5px; width: 36px; height: 11px; border-radius: 4px; }',
      '[data-dam-tour-art="store"] .p1 { top: 4px; } [data-dam-tour-art="store"] .p2 { top: 17px; } [data-dam-tour-art="store"] .p3 { top: 30px; }',
      // art=inject 注入:光针(竖立圆角针+底部光晕点)
      '[data-dam-tour-art="inject"] .needle { left: 19px; top: 2px; width: 8px; height: 34px; border-radius: 4px 4px 8px 8px; }',
      '[data-dam-tour-art="inject"] .dot { left: 15px; top: 36px; width: 16px; height: 6px; border-radius: 50%; background: radial-gradient(ellipse at center, rgba(160,190,255,.85), rgba(160,190,255,0) 70%); border: none; box-shadow: none; backdrop-filter: none; -webkit-backdrop-filter: none; }',
      // art=bell 问候:玻璃罩铃(圆顶+横杆底座)
      '[data-dam-tour-art="bell"] .dome { left: 8px; top: 6px; width: 30px; height: 24px; border-radius: 15px 15px 6px 6px; }',
      '[data-dam-tour-art="bell"] .bar { left: 4px; top: 32px; width: 38px; height: 7px; border-radius: 4px; }',
      // art=calendar 日历:玻璃方块+两条"页缝"
      '[data-dam-tour-art="calendar"] .cube { left: 5px; top: 8px; width: 36px; height: 32px; border-radius: 8px; }',
      '[data-dam-tour-art="calendar"] .ring { top: 2px; width: 4px; height: 12px; border-radius: 2px; background: linear-gradient(180deg, rgba(255,255,255,.75), rgba(255,255,255,.35)); border: none; box-shadow: inset 0 1px 0 rgba(255,255,255,.7); }',
      '[data-dam-tour-art="calendar"] .r1 { left: 12px; } [data-dam-tour-art="calendar"] .r2 { left: 30px; }',
      // art=link 外部连接:两个交叠玻璃环
      '[data-dam-tour-art="link"] .ring { width: 22px; height: 22px; border-radius: 50%; background: transparent; border-width: 5px; box-shadow: inset 0 1px 0 rgba(255,255,255,.5); }',
      '[data-dam-tour-art="link"] .ra { left: 1px; top: 12px; } [data-dam-tour-art="link"] .rb { left: 23px; top: 12px; opacity: .92; }',
      // art=engine 引擎:玻璃六边棱块(中心多面体)
      '[data-dam-tour-art="engine"] .hex { left: 7px; top: 7px; width: 32px; height: 32px; border-radius: 10px; transform: rotate(45deg); }',
      '[data-dam-tour-art="engine"] .core-dot { left: 17px; top: 17px; width: 12px; height: 12px; border-radius: 50%; background: radial-gradient(circle at 35% 30%, rgba(255,255,255,.9), rgba(190,210,255,.5)); border: none; box-shadow: none; backdrop-filter: none; -webkit-backdrop-filter: none; }',
      // art=radar 唤起:同心双环+扫描点
      '[data-dam-tour-art="radar"] .ring { border-radius: 50%; background: transparent; }',
      '[data-dam-tour-art="radar"] .outer { left: 2px; top: 2px; width: 42px; height: 42px; border-width: 3px; opacity: .8; }',
      '[data-dam-tour-art="radar"] .inner { left: 12px; top: 12px; width: 22px; height: 22px; border-width: 4px; }',
      '[data-dam-tour-art="radar"] .ping { left: 19px; top: 19px; width: 8px; height: 8px; border-radius: 50%; background: radial-gradient(circle at 40% 35%, rgba(255,255,255,.95), rgba(170,200,255,.55)); border: none; box-shadow: 0 0 10px rgba(160,190,255,.5); backdrop-filter: none; -webkit-backdrop-filter: none; }',
      // art=rocket 完成:上升箭头塔(三层阶梯+顶端光点)
      '[data-dam-tour-art="rocket"] .s { border-radius: 4px; }',
      '[data-dam-tour-art="rocket"] .s1 { left: 14px; top: 26px; width: 18px; height: 8px; }',
      '[data-dam-tour-art="rocket"] .s2 { left: 9px; top: 16px; width: 28px; height: 8px; }',
      '[data-dam-tour-art="rocket"] .s3 { left: 4px; top: 6px; width: 38px; height: 8px; }',
      '[data-dam-tour-art="rocket"] .tip { left: 19px; top: -4px; width: 8px; height: 8px; border-radius: 50%; background: radial-gradient(circle at 40% 35%, rgba(255,255,255,.95), rgba(170,200,255,.5)); border: none; box-shadow: 0 0 12px rgba(160,190,255,.6); backdrop-filter: none; -webkit-backdrop-filter: none; }',
    ...
    function DialogHost() {
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        ...
        return h('div', { 'data-dam-tour-backdrop': '',
          ...
          h('div', { 'data-dam-tour': '' },
            ...
            h('div', { 'data-dam-tour-orb-wrap': '', key: 'orb' + tourStep, 'data-step': String(tourStep) },
              h('div', { 'data-dam-tour-bokeh': 'a' }),
              h('div', { 'data-dam-tour-bokeh': 'b' }),
              h('div', { 'data-dam-tour-bokeh': 'c' }),
              h('div', { 'data-dam-tour-stage': '', key: 'stage' + tourStep },
                h('div', { 'data-dam-tour-slab': 'bot' }),
                h('div', { 'data-dam-tour-slab': 'mid' }),
                h('div', { 'data-dam-tour-slab': 'top' })),
              // 每步 Office 式玻璃图形(纯 CSS;art 类型见 CSS [data-dam-tour-art=...];step.core 兜底)
              step.art ? h('div', { 'data-dam-tour-art': step.art, key: 'art' + tourStep },
                h('span', { className: 'gf p p1' }), h('span', { className: 'gf p p2' }), h('span', { className: 'gf p p3' }),
                h('span', { className: 'gf needle' }), h('span', { className: 'dot' }),
                h('span', { className: 'gf dome' }), h('span', { className: 'gf bar' }),
                h('span', { className: 'gf cube' }), h('span', { className: 'ring r1' }), h('span', { className: 'ring r2' }),
                h('span', { className: 'gf ring ra' }), h('span', { className: 'gf ring rb' }),
                h('span', { className: 'gf hex' }), h('span', { className: 'core-dot' }),
                h('span', { className: 'gf ring outer' }), h('span', { className: 'gf ring inner' }), h('span', { className: 'ping' }),
                h('span', { className: 'gf s s1' }), h('span', { className: 'gf s s2' }), h('span', { className: 'gf s s3' }), h('span', { className: 'tip' }),
                h('span', { className: 'orb' })) : h('div', { 'data-dam-tour-orb-core': '', key: 'core' + tourStep }, step.core)),
      ...
      // 通用开场舞台(HY4 的 Logo 组装→展开→消散序列):first / 非 urgent notice / update 三种弹窗复用。
      // children=弹窗真实内容(始终在 DOM,视觉延迟显示);点击任意处跳过;urgent 通知不套(紧急直达)。
      ...
      return h('div', { style: overlay },
        h('div', { style: box },
          ...
          h('button', { 'data-dam-btn': '', title: t('close'), onClick: function () {
            try { if (lastV) localStorage.setItem('dsh-auto-memory-pre.seenVersion', lastV) } catch (eX) {}
            closeDialog()
