  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        fWaterThreshold: '水位建议阈值', fWaterThresholdHint: '水位比例超过该值(0.1-1.5)即触发交接建议/骨架账本;默认 0.8。',
        ...
        waterCardTitle: '上下文水位', waterAutoSrc: '自动检测', waterManualSrc: '手动设定', waterOfficialSrc: '官方路由容量', waterFallbackSrc: '回退默认值', waterMeterOfficial: '官方计量', waterMeterHeuristic: '启发式降级', waterThresholdHint: '水位达到 {t} 时注入交接建议并自动补写账本;计量优先用官方 token-meter(与聊天框 context ring 同源),不可用时降级启发式估算;窗口优先取官方路由容量(request/context 的 contextWindow),其次按当前模型查 settings.yaml contextWindow,也可在下方手动覆盖。',
      ...
      sumTitle: 'Period summary',
        ...
        fWaterThreshold: 'Water-level threshold', fWaterThresholdHint: 'Inject the handoff advisory / skeleton ledger once the ratio passes this value (0.1-1.5); default 0.8.',
    ...
    function PlanTab() {
      ...
      useEffect(function () {
        apiGet(API.config).then(function (c) {
          setAutoCfg({ enabled: !(c && c.autoContinueEnabled === false), threshold: (c && Number(c.autoContinueThreshold)) || 0.8 })
      ...
      useEffect(function () {
        var alive = true
        apiGet(API.handoffState).then(function (d) { if (alive) setData(d) }).catch(function () {})
        return function () { alive = false }
      }, [])
      useEffect(function () {
        if (!open) return
      ...
      if (wl.window > 0) {
        ...
        rows.unshift(h(Card, { title: t('waterCardTitle') }, h('div', { 'data-dam-content': '' }, [
          ...
          h('div', { 'data-dam-hint': '', style: { marginTop: '4px' } }, t('waterThresholdHint').replace('{t}', String(wl.threshold || 0.8))),
        ])))
      }
    ...
    function AutoContinueHost() {
      ...
      useEffect(function () {
        ...
        if (!on) return
        var thr = (acCfg && Number(acCfg.autoContinueThreshold)) || 0.8
        ...
        var maybePrompt = function (edgeAt) {
          ...
          apiGet(API.handoffState).then(function (d) {
      ...
      }, [acCfg && acCfg.autoContinueEnabled === false, acCfg && (Number(acCfg.autoContinueThreshold) || 0.8), acCfg && acCfg.autoContinueConfirmSeconds, acCfg && acCfg.autoContinueCooldownMinutes, acBusy])
    ...
    function SettingsPage() {
      ...
      return h('div', { 'data-dam-settings': '' },
        ...
        field(t('fWaterThreshold'), h('input', { 'data-dam-input': '', type: 'number', min: 0.1, max: 1.5, step: 0.05, value: cfg.waterLevelThreshold === undefined ? 0.8 : cfg.waterLevelThreshold, onChange: function (e) { set('waterLevelThreshold', Number(e.target.value) || 0.8) } }), t('fWaterThresholdHint')),
