...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    async function runContinueFlow(opts) {
      ...
      var sid = String(currentSessionIdClient() || '')
      if (!sid) throw new Error('source-session-unavailable')
      continueFlowBusy = true
      try {
        onMsg(locale === 'zh' ? '正在核对旧会话状态并准备接续…' : 'Checking the source before continuation…')
        var r = await apiPost(API.autoContDecide, { action: 'manual', fromSessionId: sid })
        if (r && (r.deferred || r.uncertain)) {
          onMsg((locale === 'zh' ? '接续尚未完成；宿主已保留请求，不会强制并行或重复投递：' : 'Continuation pending; no forced parallel execution or duplicate submission: ') + (r.reason || r.error || 'waiting'))
          return r
        }
        if (!r || !r.ok || !r.sessionId) throw new Error(r && r.error || 'continuation-failed')
        try { if (sessions && typeof sessions.open === 'function') sessions.open(r.sessionId) } catch (_) {}
        return r
    ...
    function PlanTab() {
      ...
      async function oneClickContinue() {
        ...
        try {
          ...
          if (r && (r.deferred || r.uncertain)) return
    ...
    function AutoContinueHost() {
      ...
      useEffect(function () {
        ...
        var load = function () {
          ...
          apiGet(API.config).then(function (c) { if (alive) setAcCfg((c && (c.config || c)) || {}) }).catch(function () {})
      ...
      useEffect(function () {
        ...
        // Keep polling explicitly manual requests even with automatic arming disabled.
        ...
        var poll = function () {
          ...
          apiGet(API.autoContState, sidQ ? { sessionId: sidQ } : {}).then(function (s) {
            ...
            var arm = s.armed
            if (arm && (arm.manual || arm.ratio >= thr)) {
              // ring/wall 必须一起带进来:卡片读的是 acConfirm.ring / acConfirm.wall,
              // 宿主透出了但这里不拷 → 双口径那行永远不渲染(2026-09-10 自查发现)
              setAcConfirm({ ratio: Number(arm.ratio) || 0, tokens: Number(arm.tokens) || 0, window: Number(arm.window) || 0, ring: Number(arm.ring) || 0, wall: Number(arm.wall) || 0, edgeAt: Number(arm.edgeAt) || 0, manual: !!arm.manual, sessionId: arm.sessionId, waitReason: arm.waitReason || '' })
      ...
      var runAuto = function () {
        ...
        apiPost(API.autoContDecide, { action: 'agree', edgeAt: edgeAt, fromSessionId: acConfirm && acConfirm.sessionId }).then(function (r) {
          if (r && (r.deferred || r.uncertain)) {
            setAcSt((locale === 'zh' ? '接续待确认：' : 'Continuation pending: ') + (r.reason || r.error || 'waiting')); return
          }
          if (r && r.ok) {
            setAcConfirm(null)
            setAcCd(0)
      ...
      var onReject = function () {
        var edgeAt = (acConfirm && acConfirm.edgeAt) || 0
        apiPost(API.autoContDecide, { action: 'reject', edgeAt: edgeAt, fromSessionId: acConfirm && acConfirm.sessionId }).then(function (r) {
          if (!r || !r.ok || !r.rejected) { setAcSt('✗ ' + String(r && r.error || 'reject failed')); return }
          cancelAc(); setAcConfirm(null); setAcSt(t('autoContRejected'))
        }).catch(function (e) { setAcSt('✗ ' + String(e && e.message || e)) })
      }
      if (acConfirm || acCd > 0 || acSt) {
        return h('div', { 'data-dam-autocont': '', style: { position: 'fixed', right: '16px', bottom: '16px', zIndex: 9000, maxWidth: 'min(440px, calc(100vw - 32px))', boxSizing: 'border-box', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(128,128,128,.35)', background: 'rgba(18,22,30,.93)', color: '#e8eaed', fontSize: '12px', lineHeight: '1.55', fontFamily: 'system-ui', boxShadow: '0 10px 30px rgba(0,0,0,.35)', backdropFilter: 'blur(10px)' } },
          acConfirm ? h('div', { 'data-dam-autocont-confirm': '' }, [
            h('div', { style: { marginBottom: '4px' } }, acConfirm.manual ? (locale === 'zh' ? '手动接续请求待执行' : 'Manual continuation pending') : t('autoContConfirm').replace('{p}', String(Math.round((acConfirm.ratio || 0) * 100)))),
            ...
            acConfirm.waitReason ? h('div', { 'data-dam-hint': '' }, acConfirm.waitReason) : null,
