  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        waterCardTitle: '上下文水位', waterAutoSrc: '自动检测', waterManualSrc: '手动设定', waterMeterOfficial: '官方计量', waterMeterHeuristic: '启发式降级', waterThresholdHint: '水位达到 {t} 时注入交接建议并自动补写账本;计量优先用官方 token-meter(与聊天框 context ring 同源),不可用时降级启发式估算;窗口大小自动取自当前模型(settings.yaml contextWindow),也可在下方手动覆盖。',
    ...
    function PlanTab() {
      ...
      if (wl.window > 0) {
        var pct = Math.min(Math.round((wl.ratio || 0) * 100), 150)
        var barColor = pct >= 80 ? '#d4744f' : pct >= 50 ? '#d4a94f' : 'var(--dam-accent, #4f7cff)'
        rows.unshift(h(Card, { title: t('waterCardTitle') }, h('div', { 'data-dam-content': '' }, [
          h('div', { style: { fontSize: 'calc(12px * var(--dam-scale))', marginBottom: '6px' } },
            ...
            ' ', h('span', { 'data-dam-hint': '' }, wl.source ? (wl.source.indexOf('auto') === 0 ? t('waterAutoSrc') + ': ' + wl.source.slice(5) : t('waterManualSrc')) : ''),
