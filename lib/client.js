...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function DialogHost() {
      ...
      useEffect(function () {
        // 每次向导变为可见时回到第一步(重开向导从欢迎页开始)
        if (tourShowing) setTourStep(0)
      }, [tourShowing])
      useEffect(function () {
          ...
          h('div', { style: box },
            ...
            h('button', { 'data-dam-btn': '', style: close, onClick: function () {
              try {
                ...
                // 规范 F2(RELEASE-SEMANTIC-OPTION.md):首启关闭后,若 C2 缺失则进入
                // 「欢迎向导」(分步介绍+内联检测/下载;不下载继续 C1,设置保留入口)。
                if (!localStorage.getItem('dsh-auto-memory-pre.semWizardDone')) {
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        // 首启引导向导(2026-08-31 美术大改):屏幕中央悬浮窗 + 分步功能介绍 +
        // 2.5D 液态玻璃动态图标(形态渐变/浮沉/高光扫过/鼠标跟随) + 上一步/下一步 +
        // 语义引擎检测/下载/自检内联进向导流(规范 F2/F3/G)。
        var TOUR_STEPS = [
          { core: '🫧', kicker: 'WELCOME',
            title: locale === 'zh' ? '欢迎使用 dsh-auto-memory' : 'Welcome to dsh-auto-memory',
            text: locale === 'zh' ? '这是 DeepSeek Harness 的个人联想记忆插件。三分钟带你过一遍核心能力——最后一步可一键下载语义增强引擎（可选，约 130MB，完全本地运行）。'
              : 'A personal associative-memory plugin for DeepSeek Harness. A 3-minute tour of what it does — at the end you can optionally download the semantic engine (~130MB, fully local).' },
          { core: '🧠', kicker: locale === 'zh' ? '记忆注入' : 'MEMORY',
            title: locale === 'zh' ? '自动想起，而不是去找' : 'Recall that finds you',
            text: locale === 'zh' ? '插件持续观察对话与工具事件，把相关记忆锚定成条目；当模型需要回忆时，相关内容经固定边界（Reference Tail）注入下一轮上下文——不破坏前缀缓存，也不需要模型主动调用任何工具。'
              : 'The plugin observes conversations and tool events, anchors memories, and injects relevant ones into the next turn via a fixed boundary (Reference Tail) — prefix-cache friendly, zero tool calls needed.' },
          ...
          { core: '🔔', kicker: locale === 'zh' ? '唤起与技能' : 'ACTIVATION',
            ...
            text: locale === 'zh' ? '每次"要不要打断你"都由双车道策略决定：显式回忆可注入（canary 档），生活闲聊静默。高频流程会被固化成技能清单（checklist），在匹配的对话里自动附上。所有决策可在「唤起回顾」里复核并打分。'
              : 'A two-lane policy decides every interruption: explicit recall can inject (canary mode), small talk stays silent. Frequent workflows crystallize into skill checklists that attach automatically. Review and grade every decision in the Recall review panel.' },
          { core: '🗂️', kicker: locale === 'zh' ? '记忆中枢' : 'MEMORY HUB',
            title: locale === 'zh' ? '会沉淀，也会整理' : 'It remembers and tidies up',
            text: locale === 'zh' ? '三层记忆店（情节/事实/技能）自动沉淀与老化：技能跨会话验证后晋升，90 天未用自动归档（可置顶），事实经置信治理写回长期笔记。存储管理页可扫描/修复/删除。'
              : 'Three stores (episodic/fact/procedure) consolidate and age automatically: skills promote after cross-session validation, archive after 90 idle days (pinnable), facts write back to long-term notes under confidence governance. Storage tools can scan/repair/delete.' },
          { core: '🚀', kicker: locale === 'zh' ? '完成' : 'READY', final: true,
            title: locale === 'zh' ? '一切就绪' : 'All set',
            text: locale === 'zh' ? '所有能力默认温和运行，随时可在左下角「记忆」面板查看唤起记录、调整检索档位、审批技能晋升。祝使用愉快！'
              : 'Everything runs gently by default. Open the Memory panel (bottom-left) anytime to review recalls, switch retrieval tiers, or approve skill promotions. Enjoy!' },
        ]
        ...
        return h('div', { 'data-dam-tour-backdrop': '',
          ...
          h('div', { 'data-dam-tour': '' },
            ...
            h('button', { 'data-dam-tour-close': '', title: t('close'), onClick: finishTour }, '✕'),
            ...
            h('div', { 'data-dam-tour-body': '', key: 'body' + tourStep, 'data-dam-tour-swap': '' },
              ...
              step.final && wizReady ? h('div', { style: { marginTop: '6px' } },
                h('span', { 'data-dam-tour-badge': '', style: { background: 'rgba(47,164,106,.22)', color: '#7fdcb0' } }, '⚡ ' + (locale === 'zh' ? '语义引擎已就绪' : 'Semantic engine ready'))) : null),
            ...
            h('div', { 'data-dam-tour-foot': '' },
              h('button', { 'data-dam-tour-skip': '', onClick: finishTour }, locale === 'zh' ? '跳过向导' : 'Skip tour'),
