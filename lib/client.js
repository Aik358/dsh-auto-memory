...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function StorageTab(props) {
      ...
      // ★issue #91:三联动删除的两步武装确认键(与 ConnectTab.removeImported 同款模式)
      var delArmPair = useState('')
      var delArm = delArmPair[0]
      var setDelArm = delArmPair[1]
      var delKey = delFile + '|' + delId
      useEffect(function () {
        var alive = true
          ...
          h('div', { style: { display: 'flex', gap: '6px', marginTop: '6px' } },
            h('button', { 'data-dam-btn': '', style: { fontSize: 'calc(10.5px * var(--dam-scale))', padding: '3px 10px', opacity: (delFile && delId) ? 1 : .5 },
              onClick: function () {
                ...
                // ★2026-09-19 修复（issue #91）：三联动删除不可逆（正文原子删除 + 在途激活包清理 + 派生事实撤销），
                // 接入 removeImported 同款两步武装确认 —— 第一次点进入武装态（键含当前输入，改选后自动失效），再点才执行。
                if (delArm !== delKey) { setDelArm(delKey); setMsg(''); return }
                setDelArm('')
                act('delete', { filePath: delFile, memoryId: delId }, function () { setDelId('') })
              } }, locale === 'zh' ? (delArm === delKey ? '确认删除?' : '删除') : (delArm === delKey ? 'Confirm delete?' : 'delete'))))))
    ...
    function ConnectTab() {
      ...
      function toggleView(source) {
        ...
        // ★2026-09-19 修复（issue #91）：改函数式 setState —— 旧实现 .then 闭包持点击帧的 viewCache
        // 快照整体替换,快速展开两个来源时后完成者把先完成者的条目整体抹掉;残留 {loading:true} 墓碑
        // 又被缓存门当「已缓存」⇒ 永久卡「正在读取来源…」,收起重展也不恢复。函数式更新只动本条目
        // key;墓碑/失败条目视为未缓存可重发,在途条目仍防重复请求。
        var cur = viewCache[source.id]
        if (cur && cur.loading) return
        if (cur && !cur.error) return
        setViewCache(function (prev) { var n = Object.assign({}, prev); n[source.id] = { source: source, loading: true }; return n })
        apiGet(API.externalView, { source: source.id }).then(function (d) {
          setViewCache(function (prev) { var m = Object.assign({}, prev); m[source.id] = Object.assign({ source: source }, d, { loading: false }); return m })
        }).catch(function (e) {
          setViewCache(function (prev) { var m = Object.assign({}, prev); m[source.id] = { source: source, loading: false, error: e.message }; return m })
        })
      }
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        ...
        var TOUR_STEPS = [
          ...
          { art: 'engine', core: '', kicker: locale === 'zh' ? '检索引擎' : 'RETRIEVAL', dl: true,
            ...
            toggles: [
              { key: 'reasoningObserverEnabled', name: locale === 'zh' ? '思维链监听' : 'Reasoning observer', sub: locale === 'zh' ? '监听模型思维链分段作为检索信号（默认开——闭源模型的概括式思维链同样纳入；可在设置页关闭）' : 'Watch model CoT segments as a retrieval signal (on by default — closed-model summaries are included; turn off in settings)', def: true, where: locale === 'zh' ? '自动记忆引擎' : 'Semantic engine' },
            ] },
        ...
        var finishTour = function () {
          ...
          try {
            apiGet(API.updateCheck).then(function (d) {
              ...
              // ★2026-09-19 修复（issue #91）：更新说明卡取**当前版本**的条目（与启动分发 dispatchStartupDialog
              // 同口径）。旧实现写死 '2.1.0'：新用户走完向导看到的是十三个版本前的旧说明，而 seenVersion
              // 被钉到当前版 ⇒ 真正的当前版更新卡被 dispatchStartupDialog 的 `seen !== current` 永久抑制。
              var latestKey = (CHANGELOG[cur] ? cur : '2.1.0')
        ...
        var poll = function () {
          ...
          apiGet(API.autoContState, sidQ ? { sessionId: sidQ } : {}).then(function (s) {
            ...
            if (arm && arm.ratio >= thr) {
              ...
              // ★2026-09-19 修复（issue #94⑦）：旧写法 Math.ceil(msDiff)/1000 —— ceil 作用于毫秒再除
              // 1000,倒计时显示小数秒(「29.437 秒后自动接续」)。意图是 ceil(msDiff/1000)。
      ...
      return h('div', { 'data-dam-settings': '' },
        ...
        section('semantic', sectionLabels.semantic, [
          ...
          guide === 'js' || guide === 'python' ? (function () {
            ...
            // ★2026-09-19 修复（issue #89）：局部 var refreshSem 遮蔽模块级同名函数 refreshSem(apply, onError)
            // → 自递归爆栈被 .catch 吞 ⇒ 点「开始下载/取消」后 sem 永不刷新、1.5s 轮询闸也永不启动。改名引用模块级函数。
