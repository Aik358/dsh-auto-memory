...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    hubWhyHighRisk: '它是高风险操作，需要你确认后才会固化。',
    ...
    hubWhyHighRisk: 'High risk — needs your confirmation before crystallizing.',
    ...
    function LogsTab() {
      ...
      if (open) {
        ...
        // ★R7：硬性约束编辑面板（用户要求「能自行增删改」）
        // ★2026-09-20 崩溃修复：本处原写 `{ nonce: nonce }`，但 **LogsTab 不接收任何 props**
        //   （`function LogsTab()`，调用点 `h(LogsTab)` 未传参），而 `nonce` 只在 MemoryPanel
        //   作用域内定义（L3969-3970）⇒ 打开日志页签时求值 nonce 抛
        //   ReferenceError: nonce is not defined，整个窗格崩溃。
        //   RulesEditPanel 本就把它当可选（`props && props.nonce ? … : 0`），故传空对象即可。
        rows.push(h(RulesEditPanel, {}))
      }
          ...
          pipeline.map(function (p) {
            ...
            return h('div', { 'data-dam-content': '', key: p.procedureId, style: { padding: '5px 0', borderBottom: '1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgba(255,255,255,.16)) 40%, transparent)' } },
              ...
              h('div', { style: { display: 'flex', gap: '6px', marginTop: '5px' } },
                ...
                !p.observationOnly && h('button', { 'data-dam-btn': '', style: { fontSize: 'calc(10.5px * var(--dam-scale))', padding: '3px 10px' }, onClick: function () { hubAct('promote', p.procedureId) } }, locale === 'zh' ? '晋升' : 'promote'),
    ...
    function PlanTab() {
      ...
      return h('div', null, [switchCard].concat(rows))
    }
    ...
    function MemoryPanel() {
      ...
      else if (tab === 'plan') body = h(PlanTab)
      ...
      var tabs = [['overview', t('overview')], ['logs', t('logs')], ['refine', t('refineTab')], ['hub', t('hubTab')], ['storage', t('storageTab')], ['notes', t('notes')], ['plan', t('planTab')], ['reflections', t('reflections')], ['connect', t('connect')], ['calendar', t('calendar')], ['search', t('search')], ['workspaces', t('workspaces')]]
      var style = {
        left: g.left + 'px',
        top: g.top + 'px',
        width: g.width + 'px',
    ...
    function setMany(patch) { setCfg(function (prev) { return Object.assign({}, prev, patch) }); setDirty(true) }
      ...
      var sectionLabels = {
        ...
        context: locale === 'zh' ? '上下文管理' : 'Context management',
      ...
      return h('div', { 'data-dam-settings': '' },
          ...
          guide === 'js' || guide === 'python' ? (function () {
            ...
            return h('div', { style: { border: '1px solid color-mix(in srgb, var(--dam-accent, #2456c4) 40%, transparent)', borderRadius: '10px', padding: '10px 12px', marginBottom: '8px', fontSize: 'calc(11.5px * var(--dam-scale))', lineHeight: 1.6 } },
              ...
              h('div', { style: { display: 'flex', justifyContent: 'flex-end', gap: '6px', alignItems: 'center', marginTop: '8px' } },
                ...
                ready ? h('button', { 'data-dam-btn': '', onClick: function () { setGuide(''); var n2 = Object.assign({}, cfg); n2.semanticEngineMode = guide; if (guide === 'js') { n2.activationSource = 'js'; n2.contextSinkMode = 'null' } else if (guide === 'python') { n2.activationSource = 'python'; n2.contextSinkMode = 'python'; try { var gp = localStorage.getItem('dsh-auto-memory-pre.pyGpu'); if (gp === '1') n2.pythonGpu = true } catch (eL) {} } setCfg(n2); setDirty(true) } }, locale === 'zh' ? '启用并继续' : 'Enable & continue') : null,
        ...
        section('context', sectionLabels.context, [field(t('fHandoff'), h('input', { type: 'checkbox', 'data-dam-key': 'handoffEnabled', checked: cfg.handoffEnabled !== false, onChange: function (e) { set('handoffEnabled', e.target.checked) } }), t('fHandoffHint')),
