...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // #40: host configuration controls auto-play; localStorage records user actions only.
      ...
      return h('div', { 'data-dam-settings': '' },
        ...
        section('appearance', sectionLabels.appearance, [
          ...
          field(t('fWelcomeTour'), h('div', { 'data-dam-row': '' },
            ...
            h('button', { 'data-dam-btn': '', onClick: function () { try { openManualWelcomeTourPre() } catch (eTour) {} } }, t('tourReplay')),
   ...
   try {
    ...
    if (seen && seen !== d.current) {
     ...
     if (CHANGELOG[d.current]) openDialog({ kind: 'update', versions: [{ version: d.current, items: CHANGELOG[d.current] }], currentVersion: d.current })
      ...
      // #40: do not race update-check against host config; unknown config never auto-plays.
      var startupEpoch = tourNavigationEpoch
      Promise.all([
        apiGet(API.updateCheck).catch(function () { return null }),
        loadWelcomeConfigPre(5000),
      ]).then(function (pair) {
        // Config-disabled cleanup may increment the epoch. Still deliver version notices.
        var cfg = pair[1]
