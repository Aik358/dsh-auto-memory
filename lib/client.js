  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        autoContTitle: '自动接续', autoContHint: '开启后:水位达到阈值且会话空闲(连续两轮水位不增长)时,倒计时 30 秒自动执行一键接续(总结→写账本→同工作区开新会话→注入接续材料);触发后 30 分钟内不重复;随时可取消。', autoContOn: '自动接续:开', autoContOff: '自动接续:关', autoContThreshold: '阈值', autoContCountdown: '⏳ {s} 秒后自动接续(总结并开新会话)…', autoContCancel: '取消',
      ...
      sumTitle: 'Period summary',
        ...
        autoContTitle: 'Auto continue', autoContHint: 'When on: once the water level passes the threshold and the session is idle (level unchanged across two polls), a 30s countdown auto-runs One-click continue (summarize → write ledger → open a new session in the same workspace → inject the handoff prompt); suppressed for 30 minutes after each run; cancel anytime.', autoContOn: 'Auto continue: on', autoContOff: 'Auto continue: off', autoContThreshold: 'Threshold', autoContCountdown: '⏳ Auto-continue in {s}s (summarize & open new session)…', autoContCancel: 'Cancel',
    ...
    var autoState = { cdIv: null, lastTokens: null }
    ...
    function PlanTab() {
      ...
      // M-CM6-C 自动接续(水位阈值触发,默认关;轮询 handoff-state,连续两轮水位不增长视为会话空闲)
      var autoPair = useState(null)
      var autoCfg = autoPair[0]
      var setAutoCfg = autoPair[1]
      var cdPair = useState(0)
      var cd = cdPair[0]
      var setCd = cdPair[1]
      useEffect(function () {
      ...
      var autoStartCountdown = function () {
        if (autoState.cdIv) return
        var left = 30
        setCd(left)
        autoState.cdIv = setInterval(function () {
          left -= 1
          if (left <= 0) {
            clearInterval(autoState.cdIv); autoState.cdIv = null; setCd(0)
      ...
      async function oneClickContinue() {
        if (contBusy) return
        setContBusy(true)
        setContMsg(locale === 'zh' ? '正在构造交接材料(含旧会话转写)…' : 'Preparing handoff material (incl. prev-session transcript)…')
        try {
          ...
          setContMsg(locale === 'zh' ? '正在创建新会话(沿用旧工作区与权限)…' : 'Creating session (same workspace & permissions)…')
          // M-CM6-B v2:同 cwd 创建 → 工作区绑定/权限状态与旧会话一致;带 agentPreset 沿用预设;失败回退
          var created
          try { created = await rf.session.create(d.ws ? { cwd: d.ws, agentPreset: d.agentPreset || undefined } : (d.agentPreset ? { agentPreset: d.agentPreset } : {})) } catch (eCwd) { created = await rf.session.create(d.agentPreset ? { agentPreset: d.agentPreset } : {}) }
          ...
          // 沿用旧会话模型(provider/model/reasoningEffort 由 host 从旧会话 request/context 折叠而来;不可用则静默回退路由默认)
          if (d.provider && d.model && typeof rf.session.selectModel === 'function') {
            setContMsg(locale === 'zh' ? '正在沿用旧模型 ' + d.model + '…' : 'Restoring model ' + d.model + '…')
          ...
          var reqId = (window.crypto && typeof crypto.randomUUID === 'function') ? crypto.randomUUID() : 'amc-' + Date.now() + '-' + Math.random().toString(36).slice(2)
          var amCtz = (typeof Intl !== 'undefined' && typeof Intl.DateTimeFormat === 'function') ? (Intl.DateTimeFormat().resolvedOptions().timeZone || undefined) : undefined
          await rf.session.prompt({ requestId: reqId, sessionId: newId, mode: 'queue', content: [{ type: 'text', text: d.carryText }], clientTimeZone: amCtz })
          setContMsg(locale === 'zh' ? '✓ 已创建新会话(模型/工作区已沿用)并预载交接材料' : '✓ Session created (model/workspace restored) with ledger injected')
        } catch (e) {
      ...
      useEffect(function () {
        ...
        var tick = function () {
          apiGet(API.handoffState).then(function (d) {
            ...
            var wl = d.waterLevel || {}
            if (!wl.window || !(wl.ratio >= (autoCfg.threshold || 0.7))) { autoState.lastTokens = null; return }
            var idle = autoState.lastTokens !== null && wl.tokens === autoState.lastTokens
            autoState.lastTokens = wl.tokens
            if (!idle) return
            var last = 0
        ...
        var iv = setInterval(tick, 20000)
        tick()
        return function () { alive = false; clearInterval(iv) }
      }, [autoCfg && autoCfg.enabled, autoCfg && autoCfg.threshold])
      useEffect(function () {
      ...
      if (autoCfg && wl.window > 0) {
        rows.unshift(h(Card, { title: t('autoContTitle') }, h('div', { 'data-dam-content': '' }, [
          ...
          h('div', { 'data-dam-row': '' },
            h('button', { 'data-dam-btn': '', onClick: function () { autoCancel(); autoSave({ autoContinueEnabled: !autoCfg.enabled }) } }, autoCfg.enabled ? t('autoContOn') : t('autoContOff')),
          ...
          cd > 0 ? h('div', { 'data-dam-hint': '', style: { marginTop: '6px', color: '#d4744f' } }, t('autoContCountdown').replace('{s}', String(cd)) + ' ', h('button', { 'data-dam-btn': '', onClick: autoCancel }, t('autoContCancel'))) : null,
        ])))
      }
      if (open) {
    ...
    // M-CM6-C v2·常驻自动接续 watcher(2026-09-08):全局生效,与面板/页签是否打开无关。
    // 条件:autoContinueEnabled !== false(缺省=开)&& 水位≥阈值(缺省0.8)&& 连续两轮 tokens 不增长(会话空闲)
    // && 距上次触发>30min → 30s 可取消倒计时 → 自动执行接续(写账本→建会话→切换→注入材料)。
    function AutoContinueHost() {
      ...
      useEffect(function () {
        ...
        var tick = function () {
          apiGet(API.handoffState).then(function (d) {
            if (!alive || !d || !d.enabled) return
            var wl = d.waterLevel || {}
            if (!wl.window || !(wl.ratio >= thr)) { autoState.lastTokens = null; return }
            var idle = autoState.lastTokens !== null && wl.tokens === autoState.lastTokens
            autoState.lastTokens = wl.tokens
            if (!idle || autoState.cdIv || acBusy) return
            var last = 0
            try { last = Number(localStorage.getItem('dsh-auto-memory-pre.autoCont.last') || 0) } catch (eL) {}
            if (Date.now() - last < 30 * 60 * 1000) return
            setAcSt(null)
        ...
        tick()
        return function () { alive = false; clearInterval(iv) }
      }, [acCfg && acCfg.autoContinueEnabled === false, acCfg && (Number(acCfg.autoContinueThreshold) || 0.8), acBusy])
      var cancelAc = function () {
        if (autoState.cdIv) { clearInterval(autoState.cdIv); autoState.cdIv = null }
        setAcCd(0)
      }
      var runAutoContinue = function () {
        if (acBusy) return
        ...
        setAcBusy(true)
        setAcSt(locale === 'zh' ? '正在自动接续:写账本→建新会话→注入材料…' : 'Auto-continuing: ledger → new session → inject…')
        apiPost(API.handoffContinue, {}).then(function (d) {
          if (!d || !d.ok) throw new Error((d && d.error) || 'no handoff material')
          var p
          try { p = remoteFace.session.create(d.ws ? { cwd: d.ws, agentPreset: d.agentPreset || undefined } : (d.agentPreset ? { agentPreset: d.agentPreset } : {})) } catch (eC) { p = remoteFace.session.create(d.agentPreset ? { agentPreset: d.agentPreset } : {}) }
          return p.then(function (created) {
            var newId = extractSessionId(created)
            if (!newId) throw new Error('create: no sessionId')
            var chain = Promise.resolve()
            if (d.contSeq && remoteFace.session.rename && typeof remoteFace.session.rename === 'function') {
              var contTitle = (locale === 'zh' ? '接续 #' : 'Cont.#') + String(d.contSeq) + (d.wsBase ? ' · ' + d.wsBase : '')
              chain = chain.then(function () { try { return remoteFace.session.rename({ sessionId: newId, title: contTitle }) } catch (eR) { return null } })
            }
            if (d.provider && d.model && typeof remoteFace.session.selectModel === 'function') {
              chain = chain.then(function () { try { return remoteFace.session.selectModel({ sessionId: newId, provider: d.provider, model: d.model, reasoningEffort: d.reasoningEffort || undefined }) } catch (eM) { return null } })
            }
            return chain.then(function () {
              try { if (sessions && typeof sessions.open === 'function') sessions.open(newId) } catch (eO) {}
              var reqId = (window.crypto && typeof crypto.randomUUID === 'function') ? crypto.randomUUID() : 'ac-' + Date.now() + '-' + Math.random().toString(36).slice(2)
              var amCtz = (typeof Intl !== 'undefined' && typeof Intl.DateTimeFormat === 'function') ? (Intl.DateTimeFormat().resolvedOptions().timeZone || undefined) : undefined
              return remoteFace.session.prompt({ requestId: reqId, sessionId: newId, mode: 'queue', content: [{ type: 'text', text: d.carryText }], clientTimeZone: amCtz })
            })
          })
        }).then(function () {
          try { localStorage.setItem('dsh-auto-memory-pre.autoCont.last', String(Date.now())) } catch (eL) {}
          setAcSt(locale === 'zh' ? '✓ 已自动接续到新会话,可直接上手' : '✓ Auto-continued into a new session')
          setAcBusy(false)
      ...
      if (acCd > 0 || acSt) {
        return h('div', { 'data-dam-autocont': '', style: { position: 'fixed', right: '16px', bottom: '16px', zIndex: 9000, maxWidth: 'min(440px, calc(100vw - 32px))', boxSizing: 'border-box', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(128,128,128,.35)', background: 'rgba(18,22,30,.93)', color: '#e8eaed', fontSize: '12px', lineHeight: '1.55', fontFamily: 'system-ui', boxShadow: '0 10px 30px rgba(0,0,0,.35)', backdropFilter: 'blur(10px)' } },
          acCd > 0 ? h('div', null, t('autoContCountdown').replace('{s}', String(acCd)), ' ', h('button', { style: { marginLeft: '6px', border: '1px solid rgba(232,234,237,.45)', borderRadius: '8px', background: 'transparent', color: '#e8eaed', cursor: 'pointer', padding: '2px 10px' }, onClick: cancelAc }, t('autoContCancel'))) : null,
          acSt ? h('div', { style: acCd > 0 ? { marginTop: '4px' } : {} }, acSt) : null)
      }
      return null
    }
