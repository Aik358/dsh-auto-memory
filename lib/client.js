  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        fWaterThreshold: '水位建议阈值', fWaterThresholdHint: '水位比例超过该值(0.1-1.5)即触发交接建议/骨架账本;默认 0.75——官方自动压缩阈值是 80%,必须留出余量才来得及走完交接。',
      ...
      sumTitle: 'Period summary',
        ...
        fWaterThreshold: 'Water-level threshold', fWaterThresholdHint: 'Inject the handoff advisory / skeleton ledger once the ratio passes this value (0.1-1.5); default 0.75 — the official auto-compaction threshold is 80%, so a 5% margin is needed to finish the handoff first.',
        ...
        waterCardTitle: 'Context water level', waterAutoSrc: 'auto-detected', waterManualSrc: 'manual', waterOfficialSrc: 'route capacity', waterFallbackSrc: 'fallback default', waterNotMeasured: 'Not measured for this session yet (updates after the next turn)', waterMeterOfficial: 'official meter', waterMeterHeuristic: 'heuristic fallback', waterThresholdHint: 'Past {t} of the window, the handoff advisory is injected and the skeleton ledger is backfilled; metering prefers the official token-meter (same source as the chat context ring), falling back to the heuristic when unavailable; the window prefers the official route capacity (request/context contextWindow), then the active model in settings.yaml, or your manual override below.',
        autoContTitle: 'Auto continue', autoContHint: 'When on: once the water level passes the threshold AND the old session is idle (the authoritative harness running bit flipped to false at a turn boundary, replacing the old "level unchanged across two polls"), a confirm card appears — Agree = continue now, Decline = skip this boundary (no re-prompt for it), no answer for 30-40s = unattended, continue automatically. Before continuing, the old agent refreshes PLAN + ledger; the new session keeps workspace/model/reasoning effort and receives layered handoff material; suppressed for 30 minutes after each run.', autoContOn: 'Auto continue: on', autoContOff: 'Auto continue: off', autoContThreshold: 'Threshold', autoContCountdown: '⏳ Auto-continue in {s}s…', autoContCancel: 'Cancel', autoContConfirm: 'Water level at {p}% and the old session is idle (turn boundary) — continue into a new session now?', autoContAgree: 'Continue', autoContReject: 'Decline', autoContTimeout: 'auto-continue in {s}s (unattended)', autoContRejected: 'Skipped this handoff (will ask again at the next turn boundary)',
    ...
    try {
      window['dsh-auto-memory-pre.openWelcomeTour'] = function () { openDialog({ kind: 'welcomeTour' }) }
    } catch (eOpenTour) {}
    ...
    var API = {
      ...
      reflectAuto: '/api/dsh-auto-memory-pre/reflect-auto',
      note: '/api/dsh-auto-memory-pre/note',
      external: '/api/dsh-auto-memory-pre/external',
      externalView: '/api/dsh-auto-memory-pre/external-view',
      externalRemove: '/api/dsh-auto-memory-pre/external-remove',
      externalImport: '/api/dsh-auto-memory-pre/external-import',
      ...
      semanticDeepDetect: '/api/dsh-auto-memory-pre/semantic-deep-detect',
      pyDetect: '/api/dsh-auto-memory-pre/python-setup/detect',
      pyVenv: '/api/dsh-auto-memory-pre/python-setup/venv',
      pyDeps: '/api/dsh-auto-memory-pre/python-setup/deps',
      pyModel: '/api/dsh-auto-memory-pre/python-setup/model',
      pyCancel: '/api/dsh-auto-memory-pre/python-setup/cancel',
      pyStatus: '/api/dsh-auto-memory-pre/python-setup/status',
      handoffState: '/api/dsh-auto-memory-pre/handoff-state',
      handoffContinue: '/api/dsh-auto-memory-pre/handoff-continue',
      handoffPermission: '/api/dsh-auto-memory-pre/handoff-permission',
      autoContState: '/api/dsh-auto-memory-pre/auto-continue-state',
      autoContDecide: '/api/dsh-auto-memory-pre/auto-continue-decide',
      semanticDownload: '/api/dsh-auto-memory-pre/semantic-download',
    }
    ...
    function RefineTab() {
      ...
      useEffect(function () {
        var alive = true
        fetch('/api/dsh-auto-memory-pre/shadow-recent').then(function (r) { return r.json() }).then(function (j) {
          if (alive) setData((j && j.rows) || [])
        }).catch(function (e) { if (alive) setErr(String(e && e.message)) })
        fetch('/api/dsh-auto-memory-pre/review-feedback').then(function (r) { return r.json() }).then(function (j) {
          if (alive) setFb(j || {})
        }).catch(function () {})
        return function () { alive = false }
      }, [])
      function send(obsId, choice) {
        fetch('/api/dsh-auto-memory-pre/review-feedback', { method: 'POST',
    ...
    function MemoryHubTab(props) {
      ...
      useEffect(function () {
        var alive = true
        fetch('/api/dsh-auto-memory-pre/memory-hub').then(function (r) { return r.json() }).then(function (j) {
      ...
      function hubAct(action, procedureId, v) {
        fetch('/api/dsh-auto-memory-pre/memory-hub', { method: 'POST',
          headers: { 'Content-Type': 'application/json' },
    ...
    function StorageTab(props) {
      ...
      useEffect(function () {
        var alive = true
        fetch('/api/dsh-auto-memory-pre/storage-manage').then(function (r) { return r.json() }).then(function (j) {
          if (alive) setData(j || null)
        }).catch(function (e) { if (alive) setErr(String(e && e.message)) })
        return function () { alive = false }
      }, [nonce])
      function act(action, payload, onDone) {
        ...
        fetch('/api/dsh-auto-memory-pre/storage-manage', { method: 'POST',
          ...
          .then(function (j) {
            ...
            if (onDone) onDone(j)
            var again = fetch('/api/dsh-auto-memory-pre/storage-manage').then(function (r2) { return r2.json() })
    ...
    async function executeContinue(d, onMsg) {
      ...
      if (d.contSeq && rf.session.rename && typeof rf.session.rename === 'function') {
        ...
        try { await rf.session.rename({ sessionId: newId, title: contTitle }) } catch (eRen) {}
      }
    ...
    async function runContinueFlow(opts) {
      ...
      try {
        var cfg = null
        try { cfg = await apiGet(API.config) } catch (eCfg) {}
    ...
    function PlanTab() {
      ...
      useEffect(function () {
        apiGet(API.config).then(function (c) {
          setAutoCfg({ enabled: !(c && c.autoContinueEnabled === false), threshold: (c && Number(c.autoContinueThreshold)) || 0.75 })
        }).catch(function () { setAutoCfg({ enabled: false, threshold: 0.7 }) })
      }, [])
      var autoSave = function (patch) {
        setAutoCfg(function (p) { return Object.assign({}, p, patch) })
        apiPost(API.config, patch).catch(function () {})
      }
      ...
      if (!data) return h(Loading)
      if (!data.enabled) return h(Card, { title: t('planTitle') }, h('div', { 'data-dam-hint': '' }, t('planDisabled')))
      var wl = data.waterLevel || {}
      var rows = [h(Card, { title: t('planTitle') }, h('div', { 'data-dam-content': '' }, data.plan || t('planEmpty')))]
      if (wl.window > 0) {
      ...
      if (autoCfg && wl.window > 0) {
        rows.unshift(h(Card, { title: t('autoContTitle') }, h('div', { 'data-dam-content': '' }, [
          h('div', { 'data-dam-hint': '', style: { marginBottom: '6px' } }, t('autoContHint')),
          h('div', { 'data-dam-row': '' },
            h('button', { 'data-dam-btn': '', onClick: function () { autoSave({ autoContinueEnabled: !autoCfg.enabled }) } }, autoCfg.enabled ? t('autoContOn') : t('autoContOff')),
            h('label', { 'data-dam-hint': '', style: { marginLeft: '8px' } }, t('autoContThreshold') + ' ',
              h('input', { 'data-dam-input': '', type: 'number', min: 0.5, max: 0.95, step: 0.05, value: String(autoCfg.threshold), style: { width: '64px' }, onChange: function (e) { var v = Number(e.target.value); if (v >= 0.5 && v <= 0.95) autoSave({ autoContinueThreshold: v }) } }))),
        ])))
      }
      if (open) {
    ...
    function DialogHost() {
      ...
      if (!wizSt) {
        ...
        fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r) { return r.json() }).then(function (j) {
      ...
      useEffect(function () {
        ...
        if (tourShowing) {
          ...
          fetch('/api/dsh-auto-memory-pre/config').then(function (r) { return r.json() }).then(function (j) {
            ...
            try {
              fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r2) { return r2.json() }).then(function (j2) {
          ...
          fetch('/api/dsh-auto-memory-pre/external').then(function (r) { return r.json() }).then(function (j) {
            wizSt.extScan = j
      ...
      useEffect(function () {
        ...
        apiPost('/api/dsh-auto-memory-pre/semantic-download', { action: 'start', mirror: 'auto' }).catch(function () {})
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        ...
        var tourToggleClick = function (tg) {
          ...
          if (tg.mode) {
            apiPost('/api/dsh-auto-memory-pre/semantic-emit', { mode: patch[tg.key] }).catch(function () {})
          } else {
            apiPost('/api/dsh-auto-memory-pre/config', patch).catch(function () {})
          }
        }
        ...
        var tourExtToggle = function (id) {
          ...
          dlgTick()
          apiPost('/api/dsh-auto-memory-pre/config', { externalSources: cur }).catch(function () {})
        }
    ...
    function AutoContinueHost() {
      ...
      useEffect(function () {
        var alive = true
        var load = function () {
          apiGet(API.config).then(function (c) { if (alive) setAcCfg(c || {}) }).catch(function () {})
          apiGet(API.autoContState).then(function (s) { if (alive && s) setAcState(s) }).catch(function () {})
        }
        load()
      ...
      useEffect(function () {
        ...
        var poll = function () {
          if (!alive) return
          apiGet(API.autoContState).then(function (s) {
            if (!alive || !s) return
            setAcState(s)
            if (s.executing) {
            ...
            if (arm && arm.ratio >= thr) {
              setAcConfirm({ ratio: Number(arm.ratio) || 0, tokens: Number(arm.tokens) || 0, window: Number(arm.window) || 0, edgeAt: Number(arm.edgeAt) || 0 })
    ...
    function SettingsPage() {
      ...
      function buildModelDrawer() {
        ...
        if (!mdlLoading && !mdlErr && mdlData) {
          kids.push(h('button', { key: '__default__', 'data-dam-btn': '', style: { display: 'block', width: '100%', textAlign: 'left', padding: '4px 6px', opacity: cfg.subagentModel ? 1 : 0.75 }, onClick: function () { set('subagentModel', ''); set('subagentProvider', ''); setMdlOpen(false) } },
          ...
          ;(mdlData.providers || []).forEach(function (p) {
            var modelBtns = (p.models || []).length
              ? p.models.map(function (m) {
                  return h('button', { key: p.id + '/' + m.id, 'data-dam-btn': '', style: { display: 'block', width: '100%', textAlign: 'left', padding: '3px 6px', fontWeight: cfg.subagentModel === m.id ? 700 : 400 }, onClick: function () { set('subagentModel', m.id); set('subagentProvider', p.id); setMdlOpen(false) } },
          ...
          kids.push(h('input', { key: '__manual__', 'data-dam-input': '', style: { marginTop: '6px', width: '100%' }, value: cfg.subagentModel || '', placeholder: locale === 'zh' ? '手动输入(可选,provider 跟随路由默认)' : 'manual entry (optional, provider follows routing default)', onChange: function (e) { set('subagentModel', String(e.target.value || '').trim()); set('subagentProvider', '') } }))
        }
      ...
      function runDetect() {
        ...
        fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r2) { return r2.json() }).then(function (j2) { setSem(Object.assign({ loaded: true }, j2)) }).catch(function () {})
      }
      function onEngineModeChange(e) {
        ...
        try {
          fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r2) { return r2.json() }).then(function (j2) {
            var s2 = Object.assign({ loaded: true }, j2)
            setSem(s2)
      ...
      return h('div', { 'data-dam-settings': '' },
        ...
        section('semantic', sectionLabels.semantic, [
          ...
          field(t('fEmitMode'), h('select', { 'data-dam-select': '', value: (sem && sem.activationEmitMode) || 'shadow', onChange: function (e) { var m = e.target.value; apiPost('/api/dsh-auto-memory-pre/semantic-emit', { mode: m }).then(function () { try { fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r2) { return r2.json() }).then(function (j2) { setSem(Object.assign({ loaded: true }, j2)) }).catch(function () {}) } catch (_) {} }).catch(function () {}) } },
          ...
          guide === 'js' || guide === 'python' ? (function () {
            ...
            var startDl = function () {
              apiPost('/api/dsh-auto-memory-pre/semantic-download', { action: 'start', mirror: mirror }).then(refreshSem).catch(function () {})
            }
            var cancelDl = function () {
              apiPost('/api/dsh-auto-memory-pre/semantic-download', { action: 'cancel', mirror: mirror }).then(refreshSem).catch(function () {})
            }
            return h('div', { style: { border: '1px solid color-mix(in srgb, var(--dam-accent, #2456c4) 40%, transparent)', borderRadius: '10px', padding: '10px 12px', marginBottom: '8px', fontSize: 'calc(11.5px * var(--dam-scale))', lineHeight: 1.6 } },
        ...
        section('context', sectionLabels.context, [field(t('fHandoff'), h('input', { type: 'checkbox', checked: cfg.handoffEnabled !== false, onChange: function (e) { set('handoffEnabled', e.target.checked) } }), t('fHandoffHint')),
        ...
        if (d && d.current) {
          try {
            ...
            if (seen && seen !== d.current) {
              // 只弹 2.1.0 大更新卡(历次小更新已总结并入),不逐个弹历史版本——大更新内容才是要传达的
              var bigKey = '2.1.0'
              if (CHANGELOG[bigKey]) {
