  ...
  factory: (require) => {
    ...
    var CHANGELOG = {
      '2.2.5': { zh: [
        ...
        '★ 新增悬浮钉:记忆面板标题栏加了 📌 按钮,点一下钉住——之后再点桌面或其他地方,面板不会自动收起(再点一下取消)。钉住状态会记住。',
        ...
        '★ New pin button: the panel header now has a 📌 toggle — pin it and clicking elsewhere on the desktop no longer auto-hides the panel (click again to unpin). The pinned state is remembered.',
    ...
    function PySetupWizard() {
      ...
      var allReady = data.venvOk && data.depsOk && data.modelReady
    ...
    async function refreshOldSession(onMsg) {
      ...
      // 任何失败/超时都 fail-soft(用现有材料继续),绝不阻塞接续。
      try {
        var st = await apiGet(API.handoffState)
        var rf = st && st.refresh
        if (!rf || !rf.sessionId || !rf.prompt) return { ok: false, skipped: true }
        if (!remoteFace || !remoteFace.session || typeof remoteFace.session.prompt !== 'function') return { ok: false, skipped: true }
      ...
      } catch (eRf) { return { ok: false, error: String(eRf && eRf.message ? eRf.message : eRf) } }
    }
    ...
    async function runContinueFlow(opts) {
      ...
      try {
        ...
        var d = await apiPost(API.handoffContinue, {})
    ...
    function AutoContinueHost() {
      ...
      useEffect(function () {
        ...
        var confirmSec = Number(acCfg && acCfg.autoContinueConfirmSeconds)
        if (!(confirmSec >= 10 && confirmSec <= 120)) confirmSec = 35
        var cooldownMin = Number(acCfg && acCfg.autoContinueCooldownMinutes)
        if (!(cooldownMin > 0)) cooldownMin = 30
        var alive = true
        var maybePrompt = function (edgeAt) {
          if (!alive || acBusy || autoState.cdIv || autoState.promptVisible) return
          if (!edgeAt || Date.now() - edgeAt > 10 * 60 * 1000) return   // 边界太旧不再提示
          if (Date.now() < autoState.suppressUntil) return
          if (autoState.rejectedEdgeAt === edgeAt) return                // ②拒绝过同一边界
          apiGet(API.handoffState, { sessionId: currentSessionIdClient() }).then(function (d) {
            if (!alive || !d || !d.enabled) return
            var wl = d.waterLevel || {}
            if (!wl.window || !(wl.ratio >= thr)) return
            var last = 0
            try { last = Number(localStorage.getItem('dsh-auto-memory-pre.autoCont.last') || 0) } catch (eL) {}
            if (Date.now() - last < cooldownMin * 60 * 1000) return
            if (autoState.promptVisible || autoState.cdIv) return
            setAcConfirm({ ratio: Number(wl.ratio) || 0, tokens: Number(wl.tokens) || 0, window: Number(wl.window) || 0, edgeAt: edgeAt })
            setAcSt('')
            autoState.promptVisible = true
            var left = confirmSec
            setAcCd(left)
            autoState.cdIv = setInterval(function () {
              left -= 1
              if (left <= 0) {
                clearInterval(autoState.cdIv); autoState.cdIv = null; setAcCd(0)
                autoState.promptVisible = false; setAcConfirm(null)
                runAuto()   // ③超时 = 挂机 → 自动接续兜底
              } else setAcCd(left)
            }, 1000)
          }).catch(function () {})
        }
        // 轮次边界:running true→false 边沿 + 3s 短确认
        var verifyEdge = function (edgeAt) {
          if (!alive || autoState.edgeAt !== edgeAt) return
          var info = currentRunningInfo()
          if (info && info.running) return   // 短确认失败:新一轮已开始,不算边界
          maybePrompt(edgeAt)
        }
        var armEdge = function () {
          autoState.edgeAt = Date.now()
          var info = currentRunningInfo()
          autoState.edgeSession = info ? info.sessionId : ''
          var edgeAt = autoState.edgeAt
          setTimeout(function () { verifyEdge(edgeAt) }, 3000)
        }
        var observe = function () {
          if (!alive) return
          var info = currentRunningInfo()
          if (!info) return
          if (autoState.lastRunning === null) {
            autoState.lastRunning = info.running
            if (!info.running) armEdge()   // 首次观察即空闲(挂机/刚进会话):按边界处理
            return
          }
          if (autoState.lastRunning === true && info.running === false) armEdge()
          autoState.lastRunning = info.running
        }
        var unsub = null
        try { if (sessions && sessions.list && typeof sessions.list.subscribe === 'function') unsub = sessions.list.subscribe(observe) } catch (eSub) {}
        observe()
        var iv = setInterval(function () { observe(); maybePrompt(autoState.edgeAt) }, 20000)  // 兜底:水位后到 / 订阅不可用
        return function () {
          alive = false
          clearInterval(iv)
          if (unsub) { try { unsub() } catch (eU) {} }
        }
      }, [acCfg && acCfg.autoContinueEnabled === false, acCfg && (Number(acCfg.autoContinueThreshold) || 0.75), acCfg && acCfg.autoContinueConfirmSeconds, acCfg && acCfg.autoContinueCooldownMinutes, acBusy])
      var cancelAc = function () {
        if (autoState.cdIv) { clearInterval(autoState.cdIv); autoState.cdIv = null }
        autoState.promptVisible = false
        setAcCd(0)
      }
      var runAuto = function () {
        ...
        cancelAc()
        setAcConfirm(null)
        if (!remoteFace || !remoteFace.session || typeof remoteFace.session.create !== 'function') {
          setAcSt(locale === 'zh' ? '✗ remote.session 不可用——请重启 dsh web 后重试' : '✗ remote.session unavailable — restart dsh web')
          return
        }
        setAcBusy(true)
        setAcSt(locale === 'zh' ? '正在自动接续:刷新白板/账本 → 建新会话 → 注入材料…' : 'Auto-continuing: refresh PLAN/ledger → new session → inject…')
        runContinueFlow({ onProgress: function (m) { setAcSt(m) } }).then(function (r) {
          setAcSt((locale === 'zh' ? '✓ 已自动接续到新会话,可直接上手' : '✓ Auto-continued into a new session') + (r && r.model ? ' · ' + r.model + (r.reasoningEffort ? '/' + r.reasoningEffort : '') : ''))
          setAcBusy(false)
        }).catch(function (e) { setAcSt('✗ ' + String(e && e.message ? e.message : e)); setAcBusy(false) })
      }
      var onAgree = function () { runAuto() }
      var onReject = function () {
        var edgeAt = (acConfirm && acConfirm.edgeAt) || autoState.edgeAt || 0
        cancelAc()
        setAcConfirm(null)
        autoState.rejectedEdgeAt = edgeAt
      ...
      if (acConfirm || acCd > 0 || acSt) {
        return h('div', { 'data-dam-autocont': '', style: { position: 'fixed', right: '16px', bottom: '16px', zIndex: 9000, maxWidth: 'min(440px, calc(100vw - 32px))', boxSizing: 'border-box', padding: '10px 12px', borderRadius: '12px', border: '1px solid rgba(128,128,128,.35)', background: 'rgba(18,22,30,.93)', color: '#e8eaed', fontSize: '12px', lineHeight: '1.55', fontFamily: 'system-ui', boxShadow: '0 10px 30px rgba(0,0,0,.35)', backdropFilter: 'blur(10px)' } },
          ...
          !acConfirm && acCd > 0 ? h('div', null, t('autoContCountdown').replace('{s}', String(acCd)), ' ', h('button', { style: { marginLeft: '6px', border: '1px solid rgba(232,234,237,.45)', borderRadius: '8px', background: 'transparent', color: '#e8eaed', cursor: 'pointer', padding: '2px 10px' }, onClick: cancelAc }, t('autoContCancel'))) : null,
