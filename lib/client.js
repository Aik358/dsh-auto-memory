  ...
  factory: (require) => {
    ...
    function SettingsPage() {
      ...
      function runDetect() {
        ...
        setDetOpen(true)
        setDetBusy(true)
        apiGet(API.semanticDeepDetect).then(function (d) { setDet(d || {}); setDetBusy(false) }).catch(function () { setDet({ failed: true }); setDetBusy(false) })
        ...
        section('semantic', sectionLabels.secSemantic, [
          ...
          detOpen ? (function () {
            ...
            if (detBusy && !det) {
              ...
              kids.push(row('Python 模型(int8)', sem.pythonInt8Present ? '✓ 就绪' : '✗ 未就绪'))
              ...
              if (det.recommendation && det.recommendation !== 'none') {
                ...
                if (det.recommendation === 'setup-both') btns.push(h('button', { key: 'py', 'data-dam-btn': '', onClick: function () { setGuide('python'); setDetOpen(false) } }, locale === 'zh' ? '打开 Python 向导' : 'Open Python wizard'))
