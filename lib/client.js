 ...
 */
console.log('[dsh-auto-memory] client v2.5.3 fingerprint: per-session-workspace')
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CHANGELOG = {
      '2.5.3': { zh: [
        '修复:provider 前缀式模型 id(如 deepseek/deepseek-v4.1-flash、z-ai/glm-5.3-flash、meituan/LongCat-2.0:free)的上下文窗口此前被静默丢弃——解析窗口的字符类不含斜杠,整条 id 行匹配不上,该路由的窗口退回兜底 131072;真实窗口 100 万的会话水位被放大 7.63 倍(12.8% 显示成 98%),未到阈值就误弹「接续到新会话」确认卡。现字符类含斜杠,前缀式 id 能取到真实窗口。',
        '修复:id 行存在但格式不被识别时(带引号/含空格等),紧随其后的 contextWindow 不再被错记到上一条模型头上——错配会给出偏小的窗口、让水位漏报,比读不到更危险。',
        '内部:群日报与群反馈跟踪的 CI 脚本同步(对使用无影响)。',
      ], en: [
        'Fix: context windows for provider-prefixed model ids (deepseek/deepseek-v4.1-flash, z-ai/glm-5.3-flash, meituan/LongCat-2.0:free) were silently dropped — the id character class had no "/", so the whole id line failed to match and that route fell back to 131072; on a session whose real window is 1,000,000 the water level was inflated 7.63x (12.8% shown as 98%), firing the "continue in a new session" prompt below the threshold. The character class now includes "/".',
        'Fix: when an id line exists but still cannot be parsed (quoted, spaces, etc.), the following contextWindow is no longer attributed to the previous model — that misattribution yields a too-small window and under-reports the water level, which is worse than not reading it at all.',
        'Internal: CI scripts for the daily group digest and group feedback tracking synced (no user-facing effect).',
      ] },
      '2.5.2': { zh: [
