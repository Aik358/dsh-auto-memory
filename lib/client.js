 ...
 * Data flows over /api/dsh-auto-memory-dev/* (loopback-only host routes).
 */
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var GEOM_KEY = 'dsh-auto-memory-dev.panel.geom'
    var DEFAULT_W = 440
    var DEFAULT_H = 560
    var DEFAULT_GAP = 16
    ...
    var controller = {
      ...
      close: function () {
        ...
        closeTimer = setTimeout(function () {
          ...
          try { localStorage.setItem('dsh-auto-memory-dev.lastActive', String(Date.now())) } catch (e) {}
        }, 170)
      },
    ...
    var I18N = {
      zh: {
        ...
        notesHint: '建议直接用对话让 agent 调 memory_note_dev;此处为手动追加。',
        ...
        sessionSource: '会话源:共 ', sessionSourceSuffix: ' 个会话文件,用 memory_recall_dev 按需检索',
        ...
        saved: '已保存',
        settingsHeader: '记忆存储与行为设置(保存到 DSH 主目录 dsh-auto-memory-dev.json):',
      ...
      updateTitle: 'dsh-auto-memory Updated', updateSub: 'What\'s new in this update:', gotIt: 'Got it', noticeOpen: 'Learn more',
        ...
        notesHint: 'Tip: ask the agent to call memory_note_dev in chat; this is a manual fallback.',
        ...
        saved: 'Saved',
        settingsHeader: 'Memory storage & behavior (saved to ~/.dsh/dsh-auto-memory-dev.json):',
    ...
    try {
      var savedAccent = localStorage.getItem('dsh-auto-memory-dev.accentTheme.v1')
      var savedDensity = localStorage.getItem('dsh-auto-memory-dev.graphDensity.v1')
    ...
    function isAway() {
      ...
      var lastSeen = 0
      try { lastSeen = Number(localStorage.getItem('dsh-auto-memory-dev.lastActive') || 0) } catch (e) {}
    ...
    var API = {
      ...
      reflectAuto: '/api/dsh-auto-memory-dev/reflect-auto',
      note: '/api/dsh-auto-memory-dev/note',
      external: '/api/dsh-auto-memory-dev/external',
      externalView: '/api/dsh-auto-memory-dev/external-view',
      externalRemove: '/api/dsh-auto-memory-dev/external-remove',
      externalImport: '/api/dsh-auto-memory-dev/external-import',
    ...
    function DialogHost() {
        ...
        return h('div', { style: overlay },
          h('div', { style: box },
            ...
            h('button', { 'data-dam-btn': '', style: close, onClick: function () {
              try {
                localStorage.setItem('dsh-auto-memory-dev.firstRunDone', '1')
                if (dialogState && dialogState.currentVersion) localStorage.setItem('dsh-auto-memory-dev.seenVersion', dialogState.currentVersion)
              } catch (e3) {}
              closeDialog()
            } }, t('gotIt'))))
      }
          ...
          h('div', { style: box },
            ...
            h('div', { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end', marginTop: '6px' } },
              ...
              h('button', { 'data-dam-btn': '', style: close, onClick: function () {
                try {
                  var arr = []
                  try { arr = JSON.parse(localStorage.getItem('dsh-auto-memory-dev.seenNotices') || '[]') } catch (e3) {}
                  if (n.id && arr.indexOf(n.id) < 0) arr.push(n.id)
                  localStorage.setItem('dsh-auto-memory-dev.seenNotices', JSON.stringify(arr))
                } catch (e3) {}
                closeDialog()
              } }, t('gotIt')))))
      }
      ...
      return h('div', { style: overlay },
        h('div', { style: box },
          ...
          h('button', { 'data-dam-btn': '', style: close, onClick: function () {
            try { if (lastV) localStorage.setItem('dsh-auto-memory-dev.seenVersion', lastV) } catch (e3) {}
            closeDialog()
          } }, t('gotIt'))))
    }
    ...
    function SettingsPage() {
      ...
      function setAccent(value) {
        ...
        try { localStorage.setItem('dsh-auto-memory-dev.accentTheme.v1', accentTheme) } catch (e) {}
        emit()
      }
      function setDensity(value) {
        graphDensity = value === 'compact' ? 'compact' : 'relaxed'
        try { localStorage.setItem('dsh-auto-memory-dev.graphDensity.v1', graphDensity) } catch (e) {}
        emit()
      }
      var sectionLabels = {
    ...
    function apply(ctx) {
      ...
      try {
        var savedScale = localStorage.getItem('dsh-auto-memory-dev.fontScale.v2')
      ...
      apiGet(API.updateCheck).then(function (d) {
        if (d && d.current) {
          try {
            var seen = localStorage.getItem('dsh-auto-memory-dev.seenVersion')
            if (!seen && !localStorage.getItem('dsh-auto-memory-dev.firstRunDone')) {
              ...
              else try { localStorage.setItem('dsh-auto-memory-dev.seenVersion', d.current) } catch (e3) {}
            } else if (!seen && localStorage.getItem('dsh-auto-memory-dev.firstRunDone')) {
              try { localStorage.setItem('dsh-auto-memory-dev.seenVersion', d.current) } catch (e3) {}
            }
          } catch (e) {}
        }
      }).catch(function () {})
      ...
      function checkNotices() {
        apiGet(API.notices).then(function (d) {
          try {
            ...
            var seen = []
            try { seen = JSON.parse(localStorage.getItem('dsh-auto-memory-dev.seenNotices') || '[]') } catch (e2) {}
        ...
        apiGet(API.state).then(function (d) {
          try {
            ...
            if (d && d.pendingSummary && d.pendingSummary.summary) {
              var seenKey = 'dsh-auto-memory-dev.seenSummary.' + (d.pendingSummary.date || '') + '.' + (d.pendingSummary.time || '')
              try {
      ...
      function registerSurfaces() {
        try {
          surfaceDisposers.push(slots.inject('sidebar.footer.action', function () {
            return slots.register({ name: 'sidebar.footer.action', id: 'auto-memory-dev', order: 5, label: t('memory') + ' (dev)' }, function () { return h(SidebarButton) })
          }))
          surfaceDisposers.push(slots.inject('shell.overlay', function () {
            return slots.register({ name: 'shell.overlay', id: 'auto-memory-dev', order: 5 }, function () { return h(MemoryPanel) })
          }))
          surfaceDisposers.push(slots.inject('shell.overlay', function () {
            return slots.register({ name: 'shell.overlay', id: 'auto-memory-dev-dialogs', order: 6 }, function () { return h(DialogHost) })
          }))
          surfaceDisposers.push(slots.inject('settings.section', function () {
            return slots.register({ name: 'settings.section', id: 'auto-memory-dev', order: 25, label: t('autoMemory') + ' (dev)' }, function (props) { return h(SettingsPage, { close: props && props.close }) })
          }))
        } catch (e) {
