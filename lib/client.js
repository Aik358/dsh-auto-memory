 ...
 */
console.log('[dsh-auto-memory] client v3.1.0 fingerprint: promotion-override')
  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        memory: '记忆',
        autoMemory: '自动记忆',
        overview: '概览', logs: '日志', refineTab: '唤起回顾', notes: '笔记', reflections: '反思', connect: '接续', calendar: '日历', search: '检索', workspaces: '工作区', hubTab: '记忆中枢', storageTab: '存储管理', planTab: '白板', planTitle: '白板 · PLAN.md(项目全貌)', planEmpty: '(白板还是空的——模型理解项目全貌后会自动写;也可在对话中让它用 memory_note_pre(kind=plan) 写)', planVersions: '白板历史版本', planLedgers: '交接账本时间线', planDisabled: '交接白板未启用(设置 → 自动化)。', planWsUnbound: '当前会话未绑定工作区——白板与交接账本暂不可用(会话的工作区身份解析不出来;把会话开进项目文件夹或绑定工作区后自动恢复)。', planFileTitle: '查看',
    ...
    var DEFAULT_PROMPT_LAYERS_CLIENT = {
      ...
      snapshotInscription: '[铭文 · 每轮提醒 {date}]\n'
        ...
        + '③ 长期记忆判断（三个去处）—— 跨会话有用的 → memory_note_pre / 跨项目规则 → memory_user_pre / **跑通且可复用的多步流程 → memory_procedure_pre**（技能库唯一模型入口，不写就没有；建议填 successCriteria）；\n'
    ...
    function MEMORY_TABS() {
      return [['overview', t('overview')], ['logs', t('logs')], ['refine', t('refineTab')], ['hub', t('hubTab')], ['storage', t('storageTab')], ['notes', t('notes')], ['plan', t('planTab')], ['reflections', t('reflections')], ['connect', t('connect')], ['calendar', t('calendar')], ['search', t('search')], ['workspaces', t('workspaces')]]
    }
