 ...
 * Data flows over /api/dsh-auto-memory-pre/* (loopback-only host routes).
 */
console.log('[dsh-auto-memory] client v3.1.3 fingerprint: session-name-guide-onboarding')
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var POS_KEY = 'dsh-auto-memory-pre.panel.pos'
    ...
    var controller = {
      ...
      close: function () {
        ...
        closeTimer = setTimeout(function () {
          ...
          try { localStorage.setItem('dsh-auto-memory-pre.lastActive', String(Date.now())) } catch (e) {}
        }, 170)
      },
  ...
  statsSources: '按通路分布',
  statsTop: '召回最多的条目',
  statsEmpty: '(还没有统计数据——用 memory_recall_pre 检索几次后再回来看)',
  statsReset: '清零统计',
  statsResetDone: '已清零',
  ...
  statsNever: '从未', logs: '日志', refineTab: '唤起回顾', notes: '笔记', reflections: '反思', connect: '接续', calendar: '日历', search: '检索', workspaces: '工作区', hubTab: '记忆中枢', storageTab: '存储管理', planTab: '白板', planTitle: '白板 · PLAN.md(项目全貌)', planEmpty: '(白板尚未建立——宿主会在该工作区第一次会话结束时自动落一版骨架,模型理解项目全貌后把它重写成真内容;也可在对话中让它用 memory_note_pre(kind=plan) 写)', planVersions: '白板历史版本', planLedgers: '交接账本时间线', planDisabled: '交接白板未启用(设置 → 长会话接续)。', planWsUnbound: '当前会话未绑定工作区——白板与交接账本暂不可用(会话的工作区身份解析不出来;把会话开进项目文件夹或绑定工作区后自动恢复)。', planFileTitle: '查看',
        ...
        fJsExcerpt: '唤起注入内容长度(字符)', fJsExcerptHint: 'Reference Tail 的 Reference 行内容上限。默认 40=几个字/关键词级(省 token,需要细节时模型用 memory_read_pre 取全文);调大可注入更多记忆正文。范围 20-480。',
    ...
    rulesPreviewHint: '预览：下一轮注入时会变成这样',
        ...
        notesHint: '建议直接用对话让 agent 调 memory_note_pre;此处为手动追加。',
        ...
        sessionSource: '会话源:共 ', sessionSourceSuffix: ' 个会话文件,用 memory_recall_pre 按需检索',
        ...
        saved: '已保存',
        settingsHeader: '记忆存储与行为设置(保存到 DSH 主目录 dsh-auto-memory-pre.json):',
        ...
        fHandoffPlan: '白板注入预算(字符)', fHandoffPlanHint: 'PLAN.md 全貌注入动态快照的硬截断预算;全文可经 memory_read_pre 或面板白板页查看。默认 1200。',
        ...
        fConsSchedule: '定时做梦式固化', fConsScheduleHint: '每天到点自动读最近日志发散提炼长期要点,写入项目笔记/用户级记忆(等价 memory_consolidate_pre)。',
  ...
  statsEmpty: '(no data yet — run memory_recall_pre a few times and come back)',
  ...
  statsNever: 'never', logs: 'Logs', refineTab: 'Recall review', notes: 'Notes', reflections: 'Reflections', connect: 'Connect', calendar: 'Calendar', search: 'Search', workspaces: 'Workspaces', hubTab: 'Memory Hub', storageTab: 'Storage', planTab: 'Whiteboard', planTitle: 'Whiteboard · PLAN.md (project plan)', planEmpty: '(not created yet — the host seeds a skeleton board when a turn first ends in this workspace; the model rewrites it with real content once it grasps the picture, or ask it to write memory_note_pre kind=plan)', planVersions: 'Plan versions', planLedgers: 'Handoff ledger timeline', planDisabled: 'Handoff whiteboard is disabled (Settings → Handoff).', planWsUnbound: 'This session has no bound workspace — whiteboard and handoff ledgers are unavailable (the session workspace could not be resolved; open the session inside a project folder or bind a workspace to restore).', planFileTitle: 'View',
        ...
        fJsExcerpt: 'Activation excerpt length (chars)', fJsExcerptHint: 'Max length of the Reference line in the Reference Tail. Default 40 = a few words/keyword level (saves tokens; model uses memory_read_pre for details). Raise to inject more memory body. Range 20-480.',
    ...
    rulesPreviewHint: 'Preview: how it will be injected next turn',
        ...
        notesHint: 'Tip: ask the agent to call memory_note_pre in chat; this is a manual fallback.',
        ...
        saved: 'Saved',
        settingsHeader: 'Memory storage & behavior (saved to ~/.dsh/dsh-auto-memory-pre.json):',
      ...
      sumTitle: 'Period summary',
        ...
        fHandoffPlan: 'Plan injection budget (chars)', fHandoffPlanHint: 'Hard truncation budget for injecting the PLAN.md snapshot into the dynamic memory; full text via memory_read_pre or the Whiteboard tab. Default 1200.',
        ...
        fConsSchedule: 'Scheduled deep consolidation', fConsScheduleHint: 'Once a day at the set time, read recent logs and distill long-term value into project notes / user memory (same flow as memory_consolidate_pre).',
    ...
    var DEFAULT_PROMPT_LAYERS_CLIENT = {
      ...
      snapshotInscription: '[铭文 · 每轮提醒 {date}]\n'
        ...
        + '① 写记忆文件 memory_log_pre —— kind 按性质选，**不要一律 fact**：rule=用户约束/约定（会被规则层当硬约束注入）、preference=偏好、fact=事实记录、todo=待办；\n'
        + '② 更新白板与账本 —— ★**硬映射(满足即必须做,别自行判断"算不算"变化)**:\n   · 本轮**改过 lib/ 下任何文件**(含测试/工具) ⇒ 必写 **memory_note_pre(kind=handoff)** 四段式账本;\n   · 白板"当前进度"与本轮结束时的**事实不符**(回归数字/已完成项/下一步) ⇒ 必做 **memory_note_pre(kind=plan)** 重写白板;\n   · 仅补充一条可复用结论 ⇒ 才用 **memory_note_pre(kind=note, action=append)**。\n   ⚠️ **kind=note 不等于白板/账本**——只写 note 却宣称"已更新白板与账本"是**失职**;write 直接写 docs/ 的 md 也**不算**插件记忆。\n'
        + '③ 长期记忆判断（三个去处）—— 跨会话有用的 → memory_note_pre / 跨项目规则 → memory_user_pre / **规则条目增删改（用户级硬约束，含过时条目真删）→ memory_rules_pre** / **跑通且可复用的多步流程 → memory_procedure_pre**（技能库唯一模型入口，不写就没有；建议填 successCriteria）；\n'
    ...
    try {
      var savedAccent = localStorage.getItem('dsh-auto-memory-pre.accentTheme.v1')
      var savedDensity = localStorage.getItem('dsh-auto-memory-pre.graphDensity.v1')
    ...
    function isAway() {
      ...
      var lastSeen = 0
      try { lastSeen = Number(localStorage.getItem('dsh-auto-memory-pre.lastActive') || 0) } catch (e) {}
    ...
    var CHANGELOG = {
      ...
      '3.0.0': { zh: [
        ...
        '★ 白板默认「新版看板」：结构化索引（条目/标签/线索/版本链）+ 列式泳道视图 + 两个遍历工具（按标签展开 memory_expand_pre / 按 id 回溯 memory_trace_pre）。旧的文字白板完整保留，设置页一键切回（「旧版白板」）。',
    ...
    function welcomeAutoAllowedPre(cfg) {
      ...
      try { if (localStorage.getItem('dsh-auto-memory-pre.tourDismissed') === '1') return false } catch (_) { return false }
      return true
    }
    function dismissWelcomeTourPre(reason) {
      tourNavigationEpoch++
      if (reason === 'user') {
        try { localStorage.setItem('dsh-auto-memory-pre.tourDismissed', '1') } catch (_) {}
      }
    ...
    // 调试/重看入口:控制台 window['dsh-auto-memory-pre.openWelcomeTour']() 随时重开首启向导
    try {
      window['dsh-auto-memory-pre.openWelcomeTour'] = function () { openManualWelcomeTourPre() }
    } catch (eOpenTour) {}
    ...
    var ROUTE_PREFIX = '/api/dsh-auto-memory-pre'
  ...
  kanbanCard: ROUTE_PREFIX + '/kanban-card',
    ...
    //
    // 为什么需要: 卡片数据来自 Markdown 的 `## ` 小节直通(wb-sidecar-pre.js:252→:666),
    //   全链路无摘要/清洗/标题正文分离 ⇒ 实测 868 张卡只有 195 个不同标题
    //   (「目标」重复 109 次), 且正文首行常是未剥离的 `<!-- type:goal -->`。
    //   title 与 body 因此互为镜像 —— 两个字段的信息量只有 1 个。
    //
    // 立场: 纯前端派生, 不改后端契约(wb-contract-pre.js 的写入门与 sidecar 结构零改动)。
    ...
    function KanbanBoard(props) {
      ...
      if (!lanes.length || !total) {
        return h('div', { 'data-dam-hint': '' }, locale === 'zh'
          ? '新版看板已启用,但索引里还没有条目——写一次白板或账本(memory_note_pre)后这里会出现泳道卡片。'
          : 'Graph board is on, but the index has no entries yet — write a plan or ledger (memory_note_pre) and cards will appear.')
      }
      var zh = locale === 'zh'
      ...
      h('div', { 'data-dam-kx-scroll': '', style: { height: '100%', overflow: 'auto', padding: '12px 16px', boxSizing: 'border-box' } }, [
        ...
        drawer ? kxPortal(h('div', {
          ...
          (drawer.sectionTags && drawer.sectionTags.length)
            ...
            : null,
          h('pre', { key: 'body', 'data-dam-kx-full': drawer.id, style: { whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'inherit', margin: 0, fontSize: '12px' } }, (useCardFull(drawer) || drawer.full || drawer.preview || '')),
        ].map(function (node, i) {
      ...
      if (!data || !cards.length) {
        ...
        var msg = reason === 'legacy-mode'
          ...
          : reason === 'error'
            ...
            : (zh ? '画布暂无内容 —— 写一次白板或账本(memory_note_pre)后这里会出现节点。'
              : 'No nodes yet — write a plan or ledger (memory_note_pre) and nodes will appear.')
    ...
    function PySetupWizard() {
      ...
      var gpuPair = useState(function () { try { return localStorage.getItem('dsh-auto-memory-pre.pyGpu') === '1' } catch (eG) { return false } })
      var gpuPref = gpuPair[0]
      var setGpuPref = function (v) {
        gpuPair[1](v)
        try { localStorage.setItem('dsh-auto-memory-pre.pyGpu', v ? '1' : '0') } catch (eG) {}
      }
      var actGpu = function () {
        if (busy) return
        setBusy(true)
    ...
    //   AI（memory_user_pre）也可能写错 ⇒ 必须让用户能自己改。
    ...
    function MemoryHubTab(props) {
      ...
      //   (memory-hub-pre.js:245 → stores.facts.getStats()),前端此前零渲染
    ...
    async function runContinueFlow(opts) {
      ...
      try {
        ...
        try { localStorage.setItem('dsh-auto-memory-pre.autoCont.last', String(Date.now())) } catch (eL) {}
      ...
      if (data.structured && !kbData) {
        ...
        if ((st.tags || []).length) {
          ...
          tagKids.push(h('div', { 'data-dam-hint': '', style: { marginTop: '4px' } },
            locale === 'zh' ? '标签来自账本/白板行内标记(tag:xxx / type:xxx / topic:xxx)。模型侧可用 memory_expand_pre(tag) 按标签展开条目、memory_trace_pre(id) 回溯来源与版本链。' : 'Tags come from inline markers (tag:xxx / type:xxx / topic:xxx). The model can use memory_expand_pre(tag) to expand by tag and memory_trace_pre(id) to trace origin and version chain.'))
        } else {
      ...
      if (wl.window > 0) {
        ...
        rows.unshift(h(Card, { title: t('waterCardTitle') }, h('div', { 'data-dam-content': '' }, [
          h('div', { style: { fontSize: 'calc(12px * var(--dam-scale))', marginBottom: '6px' } },
            (wl.tokens || 0).toLocaleString() + ' / ' + wl.window.toLocaleString() + ' token · ' + pct + '%',
          ...
          h('div', { style: { height: '6px', borderRadius: '3px', background: 'color-mix(in srgb, currentColor 12%, transparent)', overflow: 'hidden' } },
            h('div', { style: { height: '100%', width: Math.min(pct, 100) + '%', background: barColor, borderRadius: '3px' } })),
          h('div', { 'data-dam-hint': '', style: { marginTop: '4px' } }, t('waterThresholdHint').replace('{t}', String(wl.threshold || 0.75))),
        ])))
      }
      ...
      return h('div', { 'data-dam-flow': '' },
        ...
        smart ? h(Card, { title: t('smartAnswer') },
          ...
          Array.isArray(smart.hits) && smart.hits.length ? h('div', { 'data-dam-hint': '', style: { marginTop: '6px' } },
            ...
            //   (index.js:6469 → l0-extract-pre.js:295+,文案形如「⚠已撤回」),前端此前零渲染
...
function StatsTab() {
  ...
  useEffect(function () {
    ...
    fetch(API.recallStats)
      ...
      .then(function (j) { if (alive) setData(j) })
...
function WorkspaceTab() {
  ...
  if (tab === 'stats') return h(StatsTab)
    ...
    function DebugCenter() {
      ...
      //   getLastPrune()/retentionLimit()（fact-store-pre.js:830/834）此前在宿主与前端**双侧**零调用方，
    ...
    function DialogHost() {
      ...
      var wizSt = window['dsh-auto-memory-pre.wizStatus']
      if (!wizSt) {
        wizSt = { loaded: false }
        window['dsh-auto-memory-pre.wizStatus'] = wizSt
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        ...
        var TOUR_STEPS = [
          ...
          { art: 'store', core: '', kicker: locale === 'zh' ? '核心能力' : 'CORE',
            ...
            text: locale === 'zh' ? '先说清楚两条互不依赖的路：①自动联想（下面的开关）——插件持续观察对话与工具事件，锚定记忆；需要回忆时经固定边界注入下一轮，不破坏前缀缓存。②就算关掉它，AI 仍会在每轮结尾默认写项目 memory（memory_log），也能用 memory_recall_pre 主动读取；日历、问候等面板功能也独立运行。'
              : 'Two independent paths: (1) automatic association (switch below) — the plugin anchors memories from conversations and injects relevant ones into the next turn via a fixed boundary. (2) Even with it off, the AI still writes project memory each turn (memory_log) and can read via memory_recall_pre; calendar, greeting and other panel features run independently.' },
          ...
          { art: 'radar', core: '', kicker: locale === 'zh' ? '唤起与固化' : 'ACTIVATION',
            ...
            toggles: [
              ...
              //   背景：`crossFeed()` 会把 episode 意图**机械截断**成技能名与步骤（无模型介入），
              //   产出的观察行「既不像技能名也不像步骤」——用户报「审批里看不懂」的来源。
              //   默认关：procedural 线只留模型直写（memory_procedure_pre）与人工两条正经通路。
              //   ⚠️ 解耦：本项**只**影响 procedure 切片，fact 分支/episode 巩固/judgement 消费不动。
              { key: 'hubMechanicalProcedureFeedEnabled', name: locale === 'zh' ? '机械流程切片（不推荐）' : 'Mechanical flow slicing (not recommended)', sub: locale === 'zh' ? '把情节意图截断成"技能"，无模型介入、可读性差；关=只保留模型自己写的技能（推荐关）' : 'Truncates episode intent into "skills" with no model involvement; off = only model-authored skills (recommended)', where: locale === 'zh' ? '记忆中枢' : 'Memory Hub' },
            ] },
        ...
        var finishTour = function () {
          try { localStorage.setItem('dsh-auto-memory-pre.semWizardDone', '1') } catch (e4) {}
          closeDialog()
          ...
          try {
            apiGet(API.updateCheck).then(function (d) {
              ...
              try { localStorage.setItem('dsh-auto-memory-pre.seenVersion', cur) } catch (e5) {}
            }).catch(function () {
      ...
      if (dialogState.kind === 'notice') {
        ...
        var noticeButton = h('button', { 'data-dam-btn': '', style: close, onClick: function () {
          try {
            var arr = []
            try { arr = JSON.parse(localStorage.getItem('dsh-auto-memory-pre.seenNotices') || '[]') } catch (e3) {}
            if (n.id && arr.indexOf(n.id) < 0) arr.push(n.id)
            localStorage.setItem('dsh-auto-memory-pre.seenNotices', JSON.stringify(arr))
          } catch (e3) {}
          closeDialog()
        } }, t('gotIt'))
        var noticeChildren = [
      ...
      if (dialogState.kind === 'semSetup') {
        ...
        var sdSnooze = function () {
          try { localStorage.setItem('dsh-auto-memory-pre.semDetectSnoozeUntil', String(Date.now() + 24 * 3600 * 1000)) } catch (eSn) {}
          closeDialog()
        }
        if (sd.deep && sd.deep.integrated) {
          ...
          if (sd.recommendation === 'download-model' || sd.recommendation === 'setup-both') {
            ...
            sdActions.push(h('button', { 'data-dam-btn': '', 'data-primary': 'true', style: close, onClick: function () {
              apiPost(API.semanticDownload, { action: 'start', mirror: 'auto' }).catch(function () {})
              try { localStorage.setItem('dsh-auto-memory-pre.semDetectSnoozeUntil', String(Date.now() + 24 * 3600 * 1000)) } catch (eSn2) {}
              closeDialog()
      ...
      return h('div', { style: overlay },
        h('div', { style: box },
          ...
          h('button', { 'data-dam-btn': '', title: t('close'), onClick: function () {
            ...
            try { var seenV = (dialogState && dialogState.currentVersion) || lastV; if (seenV) localStorage.setItem('dsh-auto-memory-pre.seenVersion', seenV) } catch (eX) {}
            closeDialog()
          ...
          DamIntroBox([
            ...
            h('button', { 'data-dam-btn': '', style: close, onClick: function () {
              ...
              try { var seenV2 = (dialogState && dialogState.currentVersion) || lastV; if (seenV2) localStorage.setItem('dsh-auto-memory-pre.seenVersion', seenV2) } catch (e3) {}
              closeDialog()
            } }, t('gotIt'))])))
    }
    ...
    function SettingsPage() {
      ...
      function buildModelDrawer() {
        ...
        kids.push(h('div', { key: '__effort__', style: { marginTop: '10px', paddingTop: '8px', borderTop: '1px solid color-mix(in srgb, var(--dsw-alias-border-l1, rgba(128,128,128,.25)) 45%, transparent)' } },
          h('b', { style: { display: 'block', fontSize: 'calc(12px * var(--dam-scale))' } }, locale === 'zh' ? '思考强度（子代理）' : 'Reasoning effort (subagents)'),
          ...
          h('div', { 'data-dam-hint': '' }, locale === 'zh'
            ? 'off=关闭思维链，low/high/max 递增思考深度（DeepSeek 默认 high）。只作用于本插件的子代理（时段总结/问候/自动沉淀/蒸馏），不影响主对话；留空或模型不支持时跟随模型默认。'
            : 'off disables thinking; low/high/max increase depth (DeepSeek default: high). Applies only to this plugin’s subagents, never the main conversation; empty follows the model default.')))
    ...
    function setMany(patch) { setCfg(function (prev) { return Object.assign({}, prev, patch) }); setDirty(true) }
      ...
      function setAccent(value) {
        ...
        try { localStorage.setItem('dsh-auto-memory-pre.accentTheme.v1', accentTheme) } catch (e) {}
        emit()
      }
      function setDensity(value) {
        graphDensity = value === 'compact' ? 'compact' : 'relaxed'
        try { localStorage.setItem('dsh-auto-memory-pre.graphDensity.v1', graphDensity) } catch (e) {}
        emit()
      }
  ...
  field(t('fInject'), h('input', { type: 'checkbox', checked: !!cfg.injectEnabled, onChange: function (e) { set('injectEnabled', e.target.checked) } }), t('fInjectHint')),
        ...
        section('capacity', sectionLabels.capacity, [
          ...
          // 2026-09-22 补接线(P0-4):facts 淘汰上限(fact-store-pre.js pruneIfNeeded)。
  ...
  field(t('fConsolidateMin'), h('input', { 'data-dam-input': '', type: 'number', min: 80, value: cfg.autoConsolidateMinChars === undefined ? 240 : cfg.autoConsolidateMinChars, onChange: function (e) { set('autoConsolidateMinChars', Number(e.target.value) || 240) } }), t('fConsolidateMinHint')),
        ...
        section('skills', sectionLabels.skills, [
          ...
          //   (procedure-switch-pre.js 先判它);此前设置页只有旧键 ⇒ 用户改了也无效(灰控件)。
          ...
          // ★v3.1.3 补控件（引导审计 K-3）：本键功能是通的（index.js 有 getter → memory-hub-pre 消费），
          //   但此前**只有向导里有控件** ⇒ 用户在向导关掉后无处回改。与 procedureInjectEnabled 同节。
          field(locale === 'zh' ? '机械流程切片' : 'Mechanical flow slicing', h('input', { type: 'checkbox', 'data-dam-key': 'hubMechanicalProcedureFeedEnabled', checked: cfg.hubMechanicalProcedureFeedEnabled === true, onChange: function (e) { set('hubMechanicalProcedureFeedEnabled', e.target.checked) } }), locale === 'zh' ? '把情节意图机械截断成"技能"（无模型介入、可读性差）。关=只保留模型自己写的技能（推荐关）。' : 'Truncates episode intent into "skills" with no model involvement (poor readability). Off = only model-authored skills (recommended).'),
  ...
  field(t('fUserDir'), h('input', { 'data-dam-input': '', value: cfg.userMemoryDir, onChange: function (e) { set('userMemoryDir', e.target.value) } }), t('fUserDirHint')),
        ...
        section('about', sectionLabels.about, [
          ...
          field(locale === 'zh' ? '白板模式（旧版白板 / 新版看板）' : 'Board mode (legacy / graph)', (function () {
            ...
            locale === 'zh' ? '一键切换白板形态。默认「旧版白板」=一切行为不变；切到「新版看板」后启用 dsh-graph 看板、白板结构化索引（handoff/index.json）与两个遍历工具（memory_expand_pre / memory_trace_pre，工具数 14→16）。切换后需重启 dsh web 生效；再点回旧版即完全回滚。' : 'One-click switch for the board form. Default "Legacy board" keeps everything unchanged; "Graph board" enables the dsh-graph kanban, the structured handoff index (handoff/index.json) and two traversal tools (memory_expand_pre / memory_trace_pre, tool count 14→16). Restart dsh web after switching; switch back to fully roll back.'),
  field(t('fVersion'), h('div', { 'data-dam-row': '' },
    ...
    function apply(ctx) {
      ...
      try {
        var savedScale = localStorage.getItem('dsh-auto-memory-pre.fontScale.v2')
      ...
      function dispatchStartupDialog(d, cfg) {
        ...
        if (d && d.current) {
          try {
            var seen = localStorage.getItem('dsh-auto-memory-pre.seenVersion')
            var majorPending = !localStorage.getItem(MAJOR_TOUR_KEY)
            var wizDone = localStorage.getItem('dsh-auto-memory-pre.semWizardDone')
            var firstDone = localStorage.getItem('dsh-auto-memory-pre.firstRunDone')
            ...
            if (seen && seen !== d.current) {
              ...
              if (CHANGELOG[bigKey]) {
                ...
                try { localStorage.setItem('dsh-auto-memory-pre.seenVersion', d.current) } catch (e3b) {}
              } else {
                ...
                else try { localStorage.setItem('dsh-auto-memory-pre.seenVersion', d.current) } catch (e3) {}
              }
            } else if (!seen && firstDone) {
              // 老用户但 seen 缺失(旧版升级/缓存清理):只看当前版本一条,不补历史链,避免把旧 log 塞给用户
              try { localStorage.setItem('dsh-auto-memory-pre.seenVersion', d.current) } catch (e3) {}
      ...
      if (typeof window['dsh-auto-memory-pre.disposeTourNavigation'] === 'function') window['dsh-auto-memory-pre.disposeTourNavigation']()
      ...
      window['dsh-auto-memory-pre.disposeTourNavigation'] = function () {
      ...
      if (typeof ctx !== 'undefined' && ctx && typeof ctx.effect === 'function') ctx.effect(function () { return window['dsh-auto-memory-pre.disposeTourNavigation'] }, 'auto-memory:tour-navigation')
      ...
      function checkNotices() {
        apiGet(API.notices).then(function (d) {
          try {
            ...
            var seen = []
            try { seen = JSON.parse(localStorage.getItem('dsh-auto-memory-pre.seenNotices') || '[]') } catch (e2) {}
        ...
        apiGet(API.semanticDeepDetect).then(function (d) {
          try {
            ...
            if (!(d.deep && d.deep.integrated)) {
              var sdUntil = 0
              try { sdUntil = Number(localStorage.getItem('dsh-auto-memory-pre.semDetectSnoozeUntil') || 0) } catch (eSn) {}
        ...
        apiGet(API.state).then(function (d) {
          try {
            ...
            if (d && d.pendingSummary && d.pendingSummary.summary) {
              var seenKey = 'dsh-auto-memory-pre.seenSummary.' + (d.pendingSummary.date || '') + '.' + (d.pendingSummary.time || '')
              try {
