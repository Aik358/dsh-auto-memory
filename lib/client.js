  ...
  factory: (require) => {
    ...
    var I18N = {
      zh: {
        ...
        guideTitle: '欢迎使用 dsh-auto-memory(预览版)', guideSub: '这个插件能干什么:',
    ...
    var CSS = [
      ...
      '[data-dam-tour-orb-wrap] { position: relative; width: 150px; height: 150px; margin: 34px auto 4px; perspective: 620px; z-index: 2; }',
      '[data-dam-tour-orb] { position: absolute; inset: 7px; border-radius: 44% 56% 52% 48% / 46% 42% 58% 54%;',
      '  background: radial-gradient(circle at 32% 26%, rgba(255,255,255,.95), rgba(255,255,255,.30) 26%, rgba(150,180,255,.16) 52%, rgba(96,128,240,.30) 100%);',
      '  backdrop-filter: blur(3px) saturate(1.7); -webkit-backdrop-filter: blur(3px) saturate(1.7);',
      '  box-shadow: 0 20px 44px rgba(64,96,224,.30), inset 0 2px 8px rgba(255,255,255,.9), inset 0 -12px 24px rgba(255,255,255,.30), inset 10px -8px 18px rgba(126,156,255,.32);',
      '  animation: dam-orb-morph 8s ease-in-out infinite alternate, dam-orb-float 5.5s ease-in-out infinite; }',
      '[data-dam-tour-orb]::after { content: ""; position: absolute; inset: 5px; border-radius: inherit;',
      '  background: conic-gradient(from var(--dam-orb-a, 0deg), transparent 0deg, rgba(255,255,255,.55) 26deg, transparent 74deg, transparent 246deg, rgba(255,255,255,.30) 288deg, transparent 336deg);',
      ...
      '@keyframes dam-orb-morph { 0% { border-radius: 44% 56% 52% 48% / 46% 42% 58% 54%; } 50% { border-radius: 52% 48% 44% 56% / 54% 48% 52% 46%; } 100% { border-radius: 48% 52% 56% 44% / 42% 56% 44% 58%; } }',
      '@keyframes dam-orb-float { 0%, 100% { transform: translateY(0) rotate(-2deg); } 50% { transform: translateY(-9px) rotate(2.5deg); } }',
      '[data-dam-tour-orb-core] { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-size: 58px;',
      '  filter: drop-shadow(0 7px 14px rgba(18,38,120,.35));',
      '  transform: rotateX(calc((var(--dam-my) - 42) * -0.16deg)) rotateY(calc((var(--dam-mx) - 50) * 0.22deg)); transition: transform .14s ease-out; }',
      '[data-dam-tour-body] { position: relative; padding: 0 46px; text-align: center; z-index: 2; }',
      '[data-dam-tour-kicker] { font-size: 10.5px; letter-spacing: .18em; text-transform: uppercase; opacity: .55; font-weight: 700; }',
      '[data-dam-tour-title] { font-size: 21px; font-weight: 700; margin: 5px 0 8px; letter-spacing: -.01em; }',
      '[data-dam-tour-text] { font-size: 13px; opacity: .84; line-height: 1.75; min-height: 66px; }',
      '[data-dam-tour-swap] { animation: dam-tour-swap .32s cubic-bezier(.2,.8,.3,1) both; }',
      '@keyframes dam-tour-swap { from { opacity: 0; transform: translateX(26px); } to { opacity: 1; transform: none; } }',
      ...
      '[data-dam-tour-dots] { display: flex; gap: 7px; justify-content: center; margin: 16px 0 2px; z-index: 2; position: relative; }',
      ...
      '[data-dam-tour-foot] { display: flex; align-items: center; gap: 10px; padding: 8px 24px 20px; z-index: 2; position: relative; }',
      '[data-dam-tour-skip] { border: none; background: transparent; color: inherit; opacity: .45; cursor: pointer; font-size: 12px; margin-right: auto; padding: 6px 10px; border-radius: 8px; }',
      '[data-dam-tour-skip]:hover { opacity: .85; background: rgba(128,128,128,.12); }',
      '[data-dam-tour-btn] { min-width: 96px; padding: 9px 22px; border-radius: 99px; font-size: 13px; font-weight: 600; cursor: pointer; transition: all .22s ease; border: 1px solid rgba(255,255,255,.4); color: inherit; }',
      ...
      // 向导内功能开关(即时写配置)
      '[data-dam-tour-toggles] { display: flex; flex-direction: column; gap: 8px; margin: 10px auto 0; max-width: 440px; text-align: left; }',
      '[data-dam-tour-tg] { display: flex; align-items: center; gap: 12px; padding: 9px 14px; border-radius: 13px; background: rgba(255,255,255,.07); border: 1px solid rgba(255,255,255,.14); cursor: pointer; transition: background .2s ease, border-color .2s ease; text-align: left; color: inherit; font: inherit; }',
      '[data-dam-tour-tg]:hover { background: rgba(255,255,255,.14); }',
      '[data-dam-tour-tg][data-on="true"] { border-color: color-mix(in srgb, var(--dam-accent, #3a6df0) 55%, transparent); background: color-mix(in srgb, var(--dam-accent, #3a6df0) 12%, rgba(255,255,255,.06)); }',
      '[data-dam-tour-tg-txt] { flex: 1; min-width: 0; }',
      '[data-dam-tour-tg-name] { font-size: 12.5px; font-weight: 700; }',
      '[data-dam-tour-tg-sub] { font-size: 10.5px; opacity: .62; margin-top: 1px; }',
      '[data-dam-tour-sw] { flex: none; width: 38px; height: 22px; border-radius: 99px; position: relative; background: rgba(128,128,128,.35); transition: background .25s ease; }',
      '[data-dam-tour-sw]::after { content: ""; position: absolute; top: 2px; left: 2px; width: 18px; height: 18px; border-radius: 50%; background: #fff; transition: left .25s cubic-bezier(.4,0,.2,1); box-shadow: 0 1px 4px rgba(0,0,0,.3); }',
      '[data-dam-tour-sw][data-on="true"] { background: var(--dam-accent, #3a6df0); }',
      '[data-dam-tour-sw][data-on="true"]::after { left: 18px; }',
      '[data-dam-tour-chips] { display: flex; flex-wrap: wrap; gap: 6px; justify-content: center; margin-top: 8px; }',
      '[data-dam-tour-where] { font-size: 11px; opacity: .7; line-height: 1.7; margin-top: 8px; text-align: left; max-width: 460px; margin-left: auto; margin-right: auto; }',
      '[data-dam-tour-where] b { opacity: .95; }',
    ].join('\n')
    ...
    function DialogHost() {
      ...
      useEffect(function () {
        ...
        var iv = setInterval(function () {
          fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r) { return r.json() }).then(function (j) {
            Object.assign(wizSt, j); try { emit() } catch (e10) {}
          }).catch(function () {})
        }, 1500)
      ...
      if (dialogState.kind === 'first') {
        ...
        var semStatus = window['dsh-auto-memory-pre.semStatus']
        if (!semStatus) {
          semStatus = { status: 'checking' }
          window['dsh-auto-memory-pre.semStatus'] = semStatus
          fetch('/api/dsh-auto-memory-pre/semantic-status').then(function (r) { return r.json() }).then(function (j) {
        ...
        return h('div', { style: overlay },
          h('div', { style: box },
            ...
            h('button', { 'data-dam-btn': '', style: close, onClick: function () {
              try {
                localStorage.setItem('dsh-auto-memory-pre.firstRunDone', '1')
                if (dialogState && dialogState.currentVersion) localStorage.setItem('dsh-auto-memory-pre.seenVersion', dialogState.currentVersion)
                // 规范 F2(RELEASE-SEMANTIC-OPTION.md):首启关闭后进入「欢迎向导」
                // (分步介绍+内联检测/下载;受设置 welcomeTourEnabled 门控,默认开)。
                fetch('/api/dsh-auto-memory-pre/config').then(function (r) { return r.json() }).then(function (cj) {
                  var cc = (cj && cj.config) || cj || {}
                  if (cc.welcomeTourEnabled !== false && !localStorage.getItem('dsh-auto-memory-pre.semWizardDone')) {
                    openDialog({ kind: 'welcomeTour' })
                  } else {
                    try { localStorage.setItem('dsh-auto-memory-pre.semWizardDone', '1') } catch (e5) {}
                  }
                }).catch(function () {
                  if (!localStorage.getItem('dsh-auto-memory-pre.semWizardDone')) openDialog({ kind: 'welcomeTour' })
                })
              } catch (e3) {}
              closeDialog()
            } }, t('gotIt'))))
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        ...
        var TOUR_STEPS = [
          { core: '🫧', kicker: 'WELCOME',
            ...
            text: locale === 'zh' ? '这是 DeepSeek Harness 的个人联想记忆插件。接下来一分钟把所有功能过一遍——每个功能都有开关，当场决定开不开；最后会告诉你在哪里随时改。'
              : 'A personal associative-memory plugin for DeepSeek Harness. A quick tour of every feature — each has a switch you flip right here; the last page tells you where to change them later.' },
          { core: '🧠', kicker: locale === 'zh' ? '核心能力' : 'CORE',
            title: locale === 'zh' ? '自动想起，而不是去找' : 'Recall that finds you',
            text: locale === 'zh' ? '插件持续观察对话与工具事件，把相关记忆锚定成条目；模型需要回忆时，相关内容经固定边界（Reference Tail）注入下一轮上下文——不破坏前缀缓存，不需要模型调用工具。这是核心能力，始终运行；总开关在 设置→自动记忆引擎。'
              : 'The plugin observes conversations and tool events, anchors memories, and injects relevant ones into the next turn via a fixed boundary (Reference Tail) — prefix-cache friendly, zero tool calls. This core always runs; the master switch lives in Settings → Semantic engine.' },
          { core: '🎛️', kicker: locale === 'zh' ? '日常体验' : 'EXPERIENCE',
            title: locale === 'zh' ? '两个贴心的默认' : 'Two handy defaults',
            text: locale === 'zh' ? '按你的习惯开或关（点击立即生效）：'
              : 'Flip these to your taste (applied instantly):',
            toggles: [
              { key: 'autoConsolidate', name: locale === 'zh' ? '自动沉淀' : 'Auto-consolidation', sub: locale === 'zh' ? '每轮对话结束自动把结论沉淀进每日日志' : 'Consolidate conclusions into the daily log after each turn', where: locale === 'zh' ? '自动化' : 'Automation' },
              { key: 'autoPopupEnabled', name: locale === 'zh' ? '暂离问候' : 'Welcome-back greeting', sub: locale === 'zh' ? '离开超过 1 小时回来，自动弹出记忆面板并送上问候' : 'Auto-open the memory panel with a greeting after >1h away', where: locale === 'zh' ? '自动化' : 'Automation' },
            ] },
          { core: '📅', kicker: locale === 'zh' ? '每日助理' : 'DAILY ASSISTANT',
            title: locale === 'zh' ? '反思与总结' : 'Reflections & summaries',
            text: locale === 'zh' ? '让记忆按天组织、按时汇报：'
              : 'Keep memories organized by day:',
            toggles: [
              { key: 'reflectEnabled', name: locale === 'zh' ? '每日反思' : 'Daily reflection', sub: locale === 'zh' ? '每天第一次会话时，主动呈现前一天的工作反思' : 'Present yesterday\'s reflection at the first session of each day', where: locale === 'zh' ? '自动化' : 'Automation' },
          ...
          { core: '🔗', kicker: locale === 'zh' ? '外部与隐私' : 'EXTERNAL & PRIVACY',
            title: locale === 'zh' ? '接入别的 AI，或保持独处' : 'Other AIs in, or keep it private',
            text: locale === 'zh' ? '两项与数据边界有关的开关（都涉及读取范围，按需开）：'
              : 'Two data-boundary switches (both affect what gets read):',
            toggles: [
              { key: 'externalSources', name: locale === 'zh' ? '外部记忆接入' : 'External memory sources', sub: locale === 'zh' ? '读取 WorkBuddy / Claude / Codex 等其他 AI 工具的会话与记忆，只存路径指针' : 'Read sessions/memories from WorkBuddy / Claude / Codex etc. (path pointers only)', groupAll: true, where: locale === 'zh' ? '记忆窗口' : 'Memory window' },
              { key: 'reasoningObserverEnabled', name: locale === 'zh' ? '思维链监听' : 'Reasoning observer', sub: locale === 'zh' ? '监听模型的思维链分段做检索信号（默认关；内容更敏感）' : 'Watch model CoT segments as retrieval signal (default off; more sensitive)', where: locale === 'zh' ? '自动记忆引擎' : 'Semantic engine' },
            ] },
          { core: '⚡', kicker: locale === 'zh' ? '检索引擎' : 'RETRIEVAL', dl: true,
            title: locale === 'zh' ? '三级语义引擎' : 'Three-tier semantic engine',
            text: locale === 'zh' ? '词法检索（0GB）永远可用作保底；内置语义引擎（约 130MB 量化模型）显著提升召回；进阶 Python 引擎（BGE-M3，约 563MB）面向深度用户。以下为自动检测结果，可在此直接安装。'
              : 'Lexical search (0GB) is the always-on floor; the built-in engine (~130MB quantized model) boosts recall; the advanced Python engine (BGE-M3, ~563MB) is optional. Live detection below — install right here if you like.' },
          { core: '🔔', kicker: locale === 'zh' ? '唤起与技能' : 'ACTIVATION',
            title: locale === 'zh' ? '该出手时才出手' : 'Interrupt only when it matters',
            text: locale === 'zh' ? '每次"要不要打断你"由双车道策略决定：显式回忆可注入（canary 档），闲聊静默；每次决策都能在「唤起回顾」页复核打分。高频流程还能固化成技能清单：'
              : 'A two-lane policy decides every interruption: explicit recall can inject (canary mode), small talk stays silent; grade every decision in the Recall review tab. Frequent workflows can crystallize into skill checklists:',
            toggles: [
              { key: 'procedurePromotionEnabled', name: locale === 'zh' ? '技能固化与晋升' : 'Skill crystallization', sub: locale === 'zh' ? '重复流程固化为 checklist，匹配对话自动附上；跨会话验证后晋升（「记忆中枢」页审批）' : 'Turn repeated flows into auto-attached checklists; promote after cross-session validation (Memory Hub tab)', where: locale === 'zh' ? '记忆中枢' : 'Memory Hub' },
            ] },
          { core: '🚀', kicker: locale === 'zh' ? '完成' : 'READY', final: true,
        ...
        return h('div', { 'data-dam-tour-backdrop': '',
          ...
          h('div', { 'data-dam-tour': '' },
            ...
            h('div', { 'data-dam-tour-orb-wrap': '' },
              h('div', { 'data-dam-tour-orb': '' }),
              h('div', { 'data-dam-tour-orb-core': '', key: 'core' + tourStep }, step.core)),
            h('div', { 'data-dam-tour-body': '', key: 'body' + tourStep, 'data-dam-tour-swap': '' },
                ...
                step.toggles.map(function (tg, ti) {
                  ...
                  return h('button', { key: ti, 'data-dam-tour-tg': '', 'data-on': String(on), onClick: function () { tourToggleClick(tg) } },
                    h('div', { 'data-dam-tour-tg-txt': '' },
                      h('div', { 'data-dam-tour-tg-name': '' }, tg.name),
              ...
              step.final ? h('div', null,
                ...
                h('div', { 'data-dam-tour-where': '' },
                  h('div', null, '· ', h('b', null, locale === 'zh' ? '自动化' : 'Automation'), locale === 'zh' ? ' —— 自动沉淀 / 暂离问候 / 每日反思 / 定时总结' : ' — consolidation / greeting / reflection / summaries'),
                  h('div', null, '· ', h('b', null, locale === 'zh' ? '记忆窗口' : 'Memory window'), locale === 'zh' ? ' —— 记忆注入与外部记忆接入' : ' — injection & external sources'),
                  h('div', null, '· ', h('b', null, locale === 'zh' ? '自动记忆引擎' : 'Semantic engine'), locale === 'zh' ? ' —— 检索档位 / 思维链监听' : ' — retrieval tier / reasoning observer'),
                  h('div', null, '· ', h('b', null, locale === 'zh' ? '记忆中枢' : 'Memory Hub'), locale === 'zh' ? ' —— 技能固化与晋升审批' : ' — skill promotion & approvals'),
      ...
      return h('div', { style: overlay },
        h('div', { style: box },
          ...
          h('button', { 'data-dam-btn': '', style: close, onClick: function () {
            try { if (lastV) localStorage.setItem('dsh-auto-memory-pre.seenVersion', lastV) } catch (e3) {}
            closeDialog()
          } }, t('gotIt'))))
    }
    ...
    function SettingsPage() {
      ...
      return h('div', { 'data-dam-settings': '' },
        ...
        section('appearance', sectionLabels.appearance, [
          ...
          // 欢迎向导:开关(首启自动播放)+ 立即重看按钮(点击即弹,用户随时预览)
          field(t('fWelcomeTour'), h('div', { 'data-dam-row': '' },
            ...
            h('button', { 'data-dam-btn': '', onClick: function () { try { window['dsh-auto-memory-pre.openWelcomeTour']() } catch (eTour) {} } }, t('tourReplay'))), t('fWelcomeTourHint')),
      ...
      apiGet(API.updateCheck).then(function (d) {
        if (d && d.current) {
          try {
            ...
            if (!seen && !localStorage.getItem('dsh-auto-memory-pre.firstRunDone')) {
            ...
            } else if (!seen && localStorage.getItem('dsh-auto-memory-pre.firstRunDone')) {
