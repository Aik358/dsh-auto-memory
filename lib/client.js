...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CHANGELOG = {
      '0.1.30': { zh: [
        '修复:DSH Desktop 增强模式(透明/Mica 窗口材质)下记忆面板半透明、文字几乎不可读——面板打开时实测背景 alpha,低于 0.9 时自动提升到 0.96(保留色相),普通模式视觉零变化。感谢 @ProperSAMA 的贡献(PR #12)。',
        '修复:记忆面板默认位置不再遮挡侧边栏左下角「记忆」入口按钮——改为锚定按钮正上方,打开时若重叠自动让出;另支持点击面板外部或按 Esc 关闭(入口按钮 toggle 语义保留)。感谢 @ProperSAMA(PR #12)。',
      ], en: [
        'Fix: In DSH Desktop enhanced mode (transparent/Mica window material) the memory panel turned semi-transparent and text was barely readable — panel background alpha is now measured on open and raised to 0.96 when below 0.9 (hue preserved); normal modes are visually unchanged. Thanks to @ProperSAMA (PR #12).',
      ...
      if (dialogState.kind === 'welcomeTour' || dialogState.kind === 'modelDownload') {
        ...
        var TOUR_STEPS = [
          ...
          { core: '🔔', kicker: locale === 'zh' ? '日常体验' : 'EXPERIENCE',
            title: locale === 'zh' ? '暂离问候与面板' : 'Greeting & panel',
            text: locale === 'zh' ? '离开超过一小时回来，自动打开记忆面板并送上问候与近期工作小结。日历提醒、检索、笔记、工作区关系图都在面板里，始终可用。'
              : 'After >1h away the memory panel auto-opens with a greeting and a recent-work digest. Calendar, search, notes and the workspace graph live in the panel and are always available.',
            toggles: [
              { key: 'autoPopupEnabled', name: locale === 'zh' ? '暂离问候' : 'Welcome-back greeting', sub: locale === 'zh' ? '暂离超 1 小时回归时自动弹出（推荐开）' : 'Auto-open after >1h away (recommended)', rec: true, where: locale === 'zh' ? '自动化' : 'Automation' },
            ] },
