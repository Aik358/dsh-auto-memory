/**
 * wb-sidecar-pre —— WB-GRAPH P2 结构化 sidecar(2026-09-16, wb_sidecar_pre_v1)。
 *
...
export const WB_SIDECAR_VERSION = 'wb_sidecar_pre_v1'
...
export function splitSectionsPre(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const am = line.trim().match(WB_ANCHOR_LINE_RE_PRE_V1)
...
/**
 * ★2026-09-24 修复「白板只剩一行」：**多节文档自动分节**的条目构建入口。
 *
 * 背景（实测根因）：`buildSidecarEntryPre` 是**单卡**函数（契约返回单个对象、无 `body` 字段），
 * 但 `writePlanSnapshot` 把**整篇 PLAN.md**（22 个 `##` 小节）交给它 ⇒ 整篇被压成 1 张卡、
 * `body` 缺失、真内容塞进 `preview`（单行化截断）。症状即用户报的「白板只剩一行」。
 * 重建路径 `rebuildSidecarIndexPre` 当时也调同一个单卡函数 ⇒ 索引里永远只有那 1 张，
 * 其 id 与看板用的 22 张分节卡（id=各节锚点）**完全不相交**。
 *
 * 本函数 = 两条路径的**唯一分节出口**：复用 `buildSectionCardsPre` 的既有切分口径，
 * 保证「写入产出」与「重建产出」算出的 id 一致（20/22 与正文锚点相同，其余 2 个是历史锚点漂移）。
 *
 * ★向后兼容铁律：**只对多节文档分节**。无 `##`（或只有一个 `##`）的文档
 * 走原单卡分支 ⇒ 账本类文档（四段式，恰好单节）行为**逐字节不变**，既有守卫不受影响。
 *
 * @param {{workspaceKey:string, relPath:string, text:string, title:string, ts?:string,
 *          kind?:string, mtime?:number, criteria?:string, status?:string}} o
 * @returns {Array<object>} 至少 1 个条目（原单卡契约的数组形态）
 */
export function buildSidecarEntriesPre(o = {}) {
  const base = {
  ...
  try {
    const { sections } = splitSectionsPre(String(o.text || ''))
    // 只在**真正多节**时分节；单节/无节保持原单卡行为（向后兼容）
    if (!Array.isArray(sections) || sections.length <= 1) {
      return [buildSidecarEntryPre(base)]
    }
    const cards = buildSectionCardsPre(o.workspaceKey, [{
      relPath: o.relPath, text: o.text, title: o.title, kind: o.kind,
      mtime: o.mtime, criteria: o.criteria, ts: o.ts,
    }])
    if (!cards.length) return [buildSidecarEntryPre(base)]
    // 分节卡 → §4.1 条目契约（补齐 sidecarEntryToCatalogItemPre 需要的字段）
    return cards.map((c) => Object.assign({}, c, {
      layer: 'whiteboard',
      status: String(o.status || 'current'),
      cue: String(c.preview || '').slice(0, 120),
    }))
  } catch (_) {
    // fail-soft：分节失败退回原单卡，绝不因修复把写入搞挂
    try { return [buildSidecarEntryPre(base)] } catch (_2) { return [] }
  }
}
...
export function rebuildSidecarIndexPre(workspaceKey, docs) {
  ...
  for (const d of Array.isArray(docs) ? docs : []) {
    try {
      // ★2026-09-24：改走**分节出口** —— 此前直调单卡函数，把多节 PLAN.md 压成 1 张卡
      //   （用户报的「白板只剩一行」）。分节后重建产出的 id 与看板分节卡对齐。
      //   单节/无 ## 文档由 buildSidecarEntriesPre 内部退回原单卡行为，语义不变。
      for (const e of buildSidecarEntriesPre({
        workspaceKey,
        relPath: d.relPath,
        text: d.text,
        title: d.title,
...
export const WB_ANCHOR_LINE_RE_PRE_V1 = /^<!--\s*memory:(mem_[0-9a-f]{32})\s*-->$/
...
export function applyAnchorsPre(workspaceKey, relPath, text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const have = next.trim().match(WB_ANCHOR_LINE_RE_PRE_V1)
    if (have) {
...
export function collectAnchorIdsPre(text) {
  ...
  for (const line of String(text || '').split('\n')) {
    const m = line.trim().match(WB_ANCHOR_LINE_RE_PRE_V1)
    if (m) ids.push(m[1])
  }
  return ids
}
...
export const WB_KANBAN_LANES_PRE_V1 = [
...
export function laneOfEntryPre(entry) {
  ...
  for (const lane of WB_KANBAN_LANES_PRE_V1) {
...
export function buildKanbanPre(index, opts = {}) {
  ...
  const lanes = WB_KANBAN_LANES_PRE_V1.map((l) => ({ key: l.key, label: l.label, cards: [] }))
...
export function buildKanbanMatrixPre(cards, opts = {}) {
  ...
  const colKeys = Array.isArray(opts.colKeys) && opts.colKeys.length
    ? opts.colKeys
    : WB_KANBAN_LANES_PRE_V1.map((l) => l.key).concat(['misc'])
  const columns = colKeys.map((k) => {
    const lane = WB_KANBAN_LANES_PRE_V1.find((l) => l.key === k)
