/**
 * acceptance-pre —— P5 分档运行验收清单(2026-09-16, acceptance_pre_v1)。
 *
...
export const ACCEPTANCE_VERSION = 'acceptance_pre_v1'

/** V2-P5 验收清单(必需项与依据)。 */
export const ACCEPTANCE_ITEMS_PRE_V1 = Object.freeze([
...
export function createAcceptanceLedgerPre() {
  ...
  for (const item of ACCEPTANCE_ITEMS_PRE_V1) entries.set(item.id, { status: 'not-run', evidence: '', note: '', at: 0 })
  ...
  function recordAcceptancePre(id, status, { evidence = '', note = '' } = {}) {
    const item = ACCEPTANCE_ITEMS_PRE_V1.find((x) => x.id === id)
  ...
  function acceptanceSummaryPre() {
    const items = ACCEPTANCE_ITEMS_PRE_V1.map((item) => ({ ...item, ...entries.get(item.id) }))
