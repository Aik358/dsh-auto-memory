/**
 * 记忆注入信封 · 分项账本（memory_envelope_v1）—— 契约 T0-3 / 总纲 §0.5 第 6 条。
 *
 ...
 *
 * **本模块的三条不变式**（每条都有能红的断言，见 `tests/smoke/smoke-test-t0-3-budget-ledger-pre.mjs`）：
...
export const MEMORY_ENVELOPE_VERSION = 'memory_envelope_v1'

/** 分项取值域（固定顺序仅用于**账本展示**；序列化顺序仍由调用方决定）。 */
export const ENVELOPE_BUCKETS_V1 = Object.freeze(['rules', 'memoryReferences', 'otherDynamic'])
 ...
 */
export const ENVELOPE_PRIORITIES_V1 = Object.freeze(['low', 'normal', 'must'])
...
export const ENVELOPE_BUCKET_TEXT_V1 = Object.freeze({
  rules: '规则类',
...
/** 账本相关的原因码 → 可读中文。 */
export const ENVELOPE_REASONS_V1 = Object.freeze({
...
export function composeMemoryEnvelopePre(input = {}) {
  ...
  const segs = rawSegs.map((s, i) => {
    ...
    if (!ENVELOPE_BUCKETS_V1.includes(bucket)) {
      degradations.push({
    ...
    for (;;) {
      ...
      if (!droppable.length) {
        ...
        degradations.push({
          ...
          text: '[降级] ' + ENVELOPE_BUCKET_TEXT_V1[bucket] + '超出分项上限 ' + lim + ' 字符，但剩余 '
      ...
      if (droppable.length >= 2) {
        ...
        degradations.push({
          ...
          text: '[降级] ' + ENVELOPE_BUCKET_TEXT_V1[bucket] + '超出分项上限 ' + lim + ' 字符 → 丢弃优先级最低的一段'
      ...
      // ★2026-09-19 修复（issue #94③）：预算必须先扣掉同分项内 **must 段**的占用 —— 旧实现
      // room = lim − mark 不扣 must ⇒ 超限纯由 must 段造成时，对**本已放得下**的段也追加
      // 「已截断」标记（越截越长）并注入失实降级说明，lim ≤ mark+1 时甚至整段清空放得下的段。
      ...
      if (available <= mark.length + 1) {
        ...
        degradations.push({
          ...
          text: '[降级] ' + ENVELOPE_BUCKET_TEXT_V1[bucket] + 'must 段占用 ' + mustChars + ' 字符后余量 ' + available + ' 字符过小（放不下截断标记）→ 本段本轮为空',
        })
      } else if (only.text.length <= available - mark.length) {
        // 本已放得下 ⇒ 原样保留，不伪造「已截断」；如实报告超限源自 must 段（与全 must 分项同口径）
        degradations.push({
          code: 'over-limit-must', bucket, inject: true,
          text: '[降级] ' + ENVELOPE_BUCKET_TEXT_V1[bucket] + '超出分项上限 ' + lim + ' 字符，超限源自 must 级段占用 '
            + mustChars + ' 字符；唯一可丢段已完整保留（' + only.text.length + ' 字符，未截断）',
        })
      } else {
        ...
        degradations.push({
          code: 'over-limit-truncated', bucket, inject: true,
          text: '[降级] ' + ENVELOPE_BUCKET_TEXT_V1[bucket] + '扣除 must 段占用后余量不足 → 已截断（' + before + ' → ' + only.text.length + '，上限 ' + lim + '）',
        })
      }
      ...
      if (bucketTotal(bucket) > lim) {
        degradations.push({
          code: 'over-limit-must', bucket, inject: true,
          text: '[降级] ' + ENVELOPE_BUCKET_TEXT_V1[bucket] + '截断后仍超上限 ' + lim + ' 字符（剩余为 must 级不可丢）⇒ 如实报告超额 '
  ...
  if (budgetChars !== null && chars.total > budgetChars) {
    const parts = ENVELOPE_BUCKETS_V1.map((b) => ENVELOPE_BUCKET_TEXT_V1[b] + ' ' + chars[b]).join(' · ')
    degradations.push({
...
export function describeEnvelopeCharsPre(envelope) {
  ...
  return ENVELOPE_BUCKETS_V1.map((b) => ENVELOPE_BUCKET_TEXT_V1[b] + ' ' + (c[b] || 0)).join(' · ')
    + ' · 合计 ' + (c.total || 0)
}
