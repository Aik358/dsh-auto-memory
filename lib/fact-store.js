...
export function createFactStorePre(opts = {}) {
  ...
  function defaultFactId(subject, predicate, object, scope) {
    const h = createHash('sha256').update(
      ['fact-v1', String(scope), String(subject), String(predicate), String(object == null ? '' : object)].join('\u0000')
    ).digest('hex')
  ...
  function upsert(cand) {
    ...
    if (existing && isFactConflict(existing, c)) {
      ...
      conflicts.push({
        ...
        // 左侧取检测时快照:existing 是 store 内活对象,后续 merge 会原地改写
        // confidence 并对 provenance 数组原地 push,活引用(含共享数组)会让
        // 已展示/已落盘(facts.json)的冲突左侧 ≠ 检测时值,审计面失真。
        left: { ...existing, provenance: [...(existing.provenance || [])] }, right: c, detectedAt: now, resolved: false,
      })
