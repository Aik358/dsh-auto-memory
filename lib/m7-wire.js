...
export function makeRequestFramePre(input) {
  ...
  const frame = {
    ...
    frameId: 'frm_' + first32(sha256Str(JSON.stringify(['m7-frame-v1', type, requestId, sentAt]))),
    requestId,
...
export function buildChunkIdPre(memoryId, recordDigest) {
  return 'chk_' + first32(sha256Str(JSON.stringify(['semantic-chunk-v1', String(memoryId || ''), String(recordDigest || '')])))
}
...
export function buildSyncIdPre(workspaceRef, scope, memoryIndexVersion, recordCount) {
  return 'syn_' + first32(sha256Str(JSON.stringify(['index-sync-v1', String(workspaceRef || ''), String(scope || ''), String(memoryIndexVersion || ''), Number(recordCount) | 0])))
}
