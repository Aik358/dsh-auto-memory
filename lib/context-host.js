...
const DROP_LOG_WINDOW_MS = 30000
const dropLoggedAt = new Map()
const dropLoggedSkip = new Map()
...
export function createContextHost(opts = {}) {
  ...
  // C5/I7:最近一次"索引未就绪"降级(可读、可观测;含原因与时间,不含任何语料文本)
  // ★多工作区适配(2026-09-17):改为**按会话分片**。旧实现是单变量,任何会话的 index-not-ready
  // 都写进去,而读取方(`index.js` 的 buildTierLayerInjection)只判 10 分钟时间窗、**不判会话**
  // ⇒ B 会话的"未就绪"会把 A 会话污染十分钟,表现为**跨会话假降级**(A 明明正常却被标注降级)。
  const indexDegradeBySession = new Map()
  let lastIndexDegrade = null // 兼容投影:最近一次(供 debugView 等旧读取方)
  ...
  function pushDrop(reason, contextVersion, runtimeKey) {
    volatileDrops.push({ at: Date.now(), reason, contextVersion })
    if (volatileDrops.length > MAX_DROPS_RING) volatileDrops.shift()
    // 回放风暴限流(2026-09-10):DSH 载入/观测一个历史会话时会对每一段历史都调一次 accept,
    // 而该 runtime 从未 capturePaths → 同一原因一次性刷出上百行(实测单次回放 288 行,占诊断日志近半)。
    // 同因日志按时间窗限流:窗口内只写首条,首条附 runtime key(定位是哪个会话),其余仅计数。
    const _k = String(reason)
    const _now = Date.now()
    if (_now - (dropLoggedAt.get(_k) || 0) < DROP_LOG_WINDOW_MS) {
      dropLoggedSkip.set(_k, (dropLoggedSkip.get(_k) || 0) + 1)
      return
    }
    dropLoggedAt.set(_k, _now)
    const _skip = dropLoggedSkip.get(_k) || 0
    dropLoggedSkip.set(_k, 0)
    diagCtx('ctx-host drop: ' + reason + ' cv=' + contextVersion
      + (runtimeKey ? ' key=' + String(runtimeKey).slice(0, 48) : '')
      + (_skip ? ' (+' + _skip + ' suppressed in ' + (DROP_LOG_WINDOW_MS / 1000) + 's)' : ''))
  }
  ...
  function onSegmentAccepted(runtime, seg, envelope) {
    try {
      ...
      if (!paths) { pushDrop('no-paths-captured', seg.contextVersion, runtime.key); return }
      ...
      void Promise.all([readyPromise, c2RankPromise]).then(([readyRes, rank]) => {
        // C5/I7(2026-09-14):索引未就绪**不再静默丢弃注入**。旧行为是 pushDrop('index-not-ready:…')
        // 后直接 return —— 语义索引一卡死,注入面就整段消失且无人知(07:43–08:35 的违规现场)。
        // 新行为 = 显式降级 + fail-open:记一条可读降级(reason 进注入层 `[降级]` 行),本轮按**词法命中**
        // 继续组装并推送 envelope(Tier-0 目录层照常常驻),稠密臂本轮不参与融合(不装懂)。
        let indexNotReady = ''
        if (!(readyRes && readyRes.ready)) {
          indexNotReady = String((readyRes && readyRes.reason) || 'unknown')
          stats.indexDegraded = (stats.indexDegraded || 0) + 1
          ...
          // ★issue #94⑤:先 delete 再 set 刷新插入序,淘汰按「最久未用」而非「最老创建」。
          ...
          if (indexDegradeBySession.size > 32) {
            ...
            indexDegradeBySession.delete(oldest)
          }
          lastIndexDegrade = degradeRec // 兼容投影(最近一次)
          try { engine._lastIndexDegrade = degradeRec } catch (_) {}
          try { engine._indexDegradeBySession = indexDegradeBySession } catch (_) {}
          pushDrop('index-not-ready:degraded:' + indexNotReady, runtime.contextVersion, runtime.key)
          diagCtx('ctx-host degrade: index-not-ready(' + indexNotReady + ') → 词法命中 + 常驻目录层继续注入')
        }
        if (st.disposed || runtime.disposed) return
        let keptList = ls.kept
        if (!indexNotReady && rank && rank.scores && rank.scores.size) {
        ...
        if ((seg.kind === 'user' || seg.kind === 'tool' || seg.kind === 'reasoning' || seg.kind === 'assistant') && rank && rank.scores && rank.scores.size && typeof engine._jsDecide === 'function' && typeof engine._activationHost !== 'undefined' && engine._activationHost) {
          try {
            ...
            // ★2026-09-19 修复（issue #94④）：尊重用户显式配置 —— 旧实现 `|| 5` 把 0（不冷却）反转成
            // 5 分钟（UI 侧 client.js 同类 `|| 1` 吃 0 已修，#19）；仅缺失/非有限数才回落默认 1 分钟。
            const cdRaw = Number(engine.config.jsDecideCooldownRounds)
            const cd = Number.isFinite(cdRaw) && cdRaw >= 0 ? cdRaw : 1
            ...
            void Promise.resolve()
              ...
              .then(async (dec) => {
                ...
                // 冷却落点已移到 offerExternalActivation 结果之后（issue #94④）：
                // 旧实现在 offer 结果未知前就落定,被 duplicate-activation/stale-index/suppressed
                // 拒绝的激活也烧掉冷却窗 ⇒ 主动联想「该弹不弹」。shadow 模式在上方 return,本就不烧。
                ...
                // 2026-09-14 P1-⑮:注入侧 reason 串补 0-1 分值(intent/dense/margin)。
                // 数值段置于 reasonCodes **之前**,160 字符截断只砍代码列表、不砍分值。
                const num2 = (x) => (Number.isFinite(Number(x)) ? Number(x).toFixed(2) : '0.00')
                const jsReason = ('js-decide lane=' + dec.lane + ' ' + (dec.decision || 'emit') +
                  ' intent=' + num2(dec.features && dec.features.intentProb) +
                  ' dense=' + num2(dec._denseTop) + ' margin=' + num2(dec._margin) +
                  ' ' + (dec.reasonCodes || []).join(',')).slice(0, 160)
                const act = {
                  ...
                  observationId: obs, workerEpoch: 'js-decide-v1',
                  ...
                  threshold: { policyVersion: dec.features ? 'activation_policy_v2' : 'activation_policy_v2', score: (dec.features && dec.features.intentProb) || 0, threshold: 0.45, reason: jsReason },
                  level: 'excerpt',
                  candidates: (function () {
                ...
                // C5 闸门:记下本轮 query(激活请求本身不带 query 文本),供注入层判"是问为什么还是只要索引"
                try {
                  const qSid = String((act && act.sessionId) || '')
                  const qRec = {
                    at: Date.now(),
                    sessionId: qSid,
                    text: String(jsDecideQueryText || '').slice(0, 1200),
                  }
                  // 多工作区适配(2026-09-17):按会话分片。旧实现单槽无条件覆盖 ⇒ A 的 query 被 B 顶掉,
                  // 读取侧因会话不匹配而退回 triggerText(语义降级)。分片后各会话各记一份。
                  if (!engine._lastTierQueryBySession || typeof engine._lastTierQueryBySession.set !== 'function') {
                    engine._lastTierQueryBySession = new Map()
                  }
                  // ★issue #94⑤:先 delete 再 set 刷新插入序(淘汰最久未用,不挤掉最活跃会话)
                  engine._lastTierQueryBySession.delete(qSid)
                  engine._lastTierQueryBySession.set(qSid, qRec)
                  if (engine._lastTierQueryBySession.size > 32) {
                    const oldest = engine._lastTierQueryBySession.keys().next().value
                    engine._lastTierQueryBySession.delete(oldest)
                  }
                  engine._lastTierQuery = qRec // 兼容投影(最近一次)
                } catch (_) {}
                // ★issue #94④:冷却只跟**成功投递**绑定 —— offer 被拒(duplicate/stale/suppressed)不烧窗
                const offerRes = (function () { try { return engine._activationHost.offerExternalActivation(act) } catch (eOffer) { return { ok: false, reason: String((eOffer && eOffer.message) || eOffer) } } })()
                if (cd > 0 && offerRes && offerRes.ok) stNow._jsDecideNextAt = Date.now() + cd * 60000
                // 命中投影补上 query(记录发生在 offer 内部;此处补字段,顺序确定)
                // 多工作区适配(2026-09-17):分片条目与兼容投影**都要补**,否则按会话取到的投影缺 question。
                try {
                  const actSid = String((act && act.sessionId) || '')
                  const qText = String(jsDecideQueryText || '').slice(0, 1200)
                  const rec = engine._tierGateHitsBySession && typeof engine._tierGateHitsBySession.get === 'function'
                    ? engine._tierGateHitsBySession.get(actSid) : null
                  if (rec && String(rec.sessionId) === actSid) rec.question = qText
                  if (engine._tierGateHits && String(engine._tierGateHits.sessionId) === actSid) {
                    engine._tierGateHits.question = qText
                  }
                } catch (_) {}
              }).catch(() => {})
          } catch (_) {}
  ...
  function onToolResult(runtime, envelope) {
    try {
      ...
      // ★2026-09-19 修复（issue #93）：observed digest 按**逐记录自身 sourceRef** 解析。旧实现只取
      // candidates[0] 的文件 digest,多文件工具结果(memory_recall L0 跨文件/跨工作区是常态)中其余
      // 文件的记录全被误判 stale-source,read 证据系统性漏记;且 observed 与 rec.fileDigest 同源,
      // stale 门只产假阳性不产真阳性。按来源分组后各组用本组文件的当前 digest 比对,
      // computeReadCoverage 纯函数签名与 m51 锁定行为不变。
      const byRef = new Map()
      for (const c of candidates) {
        const k = String(c.sourceRef || '')
        if (!byRef.has(k)) byRef.set(k, [])
        byRef.get(k).push(c)
      }
      const cov = { covered: [], stale: [] }
      for (const [ref, group] of byRef) {
        const observedDigest = pickObservedFileDigest(corpusSnap, ref)
        const g = computeReadCoverage(group, { text: preview, observedFileDigest: observedDigest })
        cov.covered.push(...g.covered)
        cov.stale.push(...g.stale)
      }
  ...
  function debugView() {
    ...
    return {
      ...
      // C5/I7:索引未就绪降级(latest) —— 供面板/注入层解释"为什么这轮只有词法命中"
      indexDegrade: lastIndexDegrade,
