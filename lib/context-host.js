...
export function createContextHost(opts = {}) {
  ...
  const registry = new CorpusRegistry({ sidecarDir: path.join(memoryDir('index', dshHome), 'files') })
  ...
  function storeFor() {
    if (!store) store = new EvidenceEventStore({
      root: memoryDir('evidence', dshHome),
  ...
  function evidenceRootExists() {
    const p = memoryDir('evidence', dshHome)
  ...
  function jsDecideShadowLog(entry) {
    try {
      const dir = memoryDir('semantic', dshHome)
  ...
  function debugView() {
    ...
    return {
      ...
      evidenceDirPath: memoryDir('evidence', dshHome),
