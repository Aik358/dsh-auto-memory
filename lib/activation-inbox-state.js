...
export class ActivationInboxRegistry {
  ...
  /**
   * 按 sessionId 摘掉它在**所有工作区**下的 inbox(会话释放用;返回清理条数)。
   * disposeSession 需要 workspaceKey,而 runtime 释放时只有 sessionId —— 没有它,已释放会话的
   * inbox(含烘焙好的 references)会一直挂在注册表里(issue#58)。
   */
  disposeBySession(sessionId) {
    const sid = String(sessionId || '')
    if (!sid) return 0
    const prefix = 'session:' + sid + '|ws:'
    let n = 0
    for (const [key, box] of this._byKey) {
      if (!key.startsWith(prefix)) continue
      box.dispose('runtime-disposed')
