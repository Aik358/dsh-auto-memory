/**
 * rules-edit-pre.js —— 「用户级硬性约束」的**条目级**解析与增删改（R7 需求）。
 *
 * ## 背景（用户 2026-09-20 明确要求「这个功能必须要落在前端」）
 *
 * `[规则 — 用户级硬性约束 · 必须遵守]` 这一段是**每轮无条件注入**的硬约束，
 * 真源 = `~/.dsh/memory/MEMORY.md`（见 `rules-layer-pre.js:12` 的真源纪律）。
 * 它**不走语义层** —— 即没有检索/打分/淘汰环节 ⇒ **过时条目不会被自动清理**；
 * 而 AI（`memory_user_pre`）也会往里写错东西。
 * ⇒ 用户必须能**自己增/删/改**，否则错一条就每轮都被误导。
 *
 * ## 与既有渲染层的关系（重要：本模块不改变注入语义）
 *
 * `rules-layer-pre.js` 的 `extractRulesLayerPre()` 负责**读**（把 MEMORY.md 解析成规则段），
 ...
 * `rules-layer-pre.js` 的渲染器只取 `- ` 开头的行（且跳过日期段）。
 * ⇒ **一条规则 = 一个 `- ` 行**。本模块据此提供：
...
/** 日期段标题（与 rules-layer-pre.js 的 DATE_SECTION_RE 同口径）。 */
export const RULE_DATE_SECTION_RE_PRE_V1 = /^##\s*\d{4}-\d{2}-\d{2}\s*$/

/** 条目行：允许前导缩进 + `- `（渲染器同样只看 `- ` 行）。 */
export const RULE_ITEM_RE_PRE_V1 = /^(\s*)-\s+(.*)$/

/** 单条规则长度上限（与写闸门同量级，防止一次塞进整篇文档）。 */
export const RULE_ITEM_MAX_CHARS_PRE_V1 = 2000
...
export function listRuleItemsPre(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const l = lines[i]
    if (RULE_DATE_SECTION_RE_PRE_V1.test(l)) { curDate = l.replace(/^##\s*/, '').trim(); continue }
    const m = RULE_ITEM_RE_PRE_V1.exec(l)
    if (!m) continue
    const body = m[2].trim()
    if (!body) continue
    out.push({
      ...
      indent: m[1] || '',
      text: body,
      // 落在日期段内 ⇒ 多为 AI（memory_user_pre 会带日期标题）写入；否则视为用户手写
...
export function updateRuleItemPre(text, idx, nextBody) {
  ...
  if (body.length > RULE_ITEM_MAX_CHARS_PRE_V1) return { ok: false, error: 'too-long' }
...
export function appendRuleItemPre(text, body, opts = {}) {
  ...
  if (b.length > RULE_ITEM_MAX_CHARS_PRE_V1) return { ok: false, error: 'too-long' }
  ...
  let secAt = -1
  for (let i = 0; i < lines.length; i++) {
    if (RULE_DATE_SECTION_RE_PRE_V1.test(lines[i]) && lines[i].replace(/^##\s*/, '').trim() === want) { secAt = i; break }
  }
  if (secAt < 0) return { ok: false, error: 'date-section-not-found' }
  let end = secAt + 1
  while (end < lines.length && !RULE_DATE_SECTION_RE_PRE_V1.test(lines[end])) end++
  let ins = end
 ...
 *
 * 只做**只读**格式化，不引入 rules-layer-pre.js 的其它依赖（避免循环 import 风险）。
 */
