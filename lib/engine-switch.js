/**
 * 引擎切换状态机（engine_switch_v1）—— P2 / T2-9 切档隔离与进度条（V2-P2 卡 points 4/5）。
 *
...
export const ENGINE_SWITCH_VERSION = 'engine_switch_v1'
...
export function createEngineSwitchPre(opts = {}) {
  ...
  function beginEngineSwitchPre(input = {}) {
    ...
    if (!/^engid_[0-9a-f]{32}$/.test(toIdentity)) return { ok: false, switchId, generation, reason: 'bad-identity' }
    // 幂等：同 id 已完成 → 不重跑
