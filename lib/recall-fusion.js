 ...
 * ★M2.5b：层次优先级序（与注入侧一致）。
 *
 * 出处：`tier-layer-inject.js:58 TIER_LAYER_ORDER_V1` —— **必须与之逐字相同**。
 *
 ...
 *   任何一侧改动而另一侧未同步，测试立刻红。
 */
export const FUSION_LAYER_ORDER_V1 = Object.freeze(['project', 'whiteboard', 'user', 'reflection', 'log'])
...
export function rankFusionRRFPre(pairs, opts = {}) {
   ...
   * 出处：`tier-layer-inject.js:58 TIER_LAYER_ORDER_V1`
  ...
  const layerRankOf = (name) => {
    const i = FUSION_LAYER_ORDER_V1.indexOf(name)
    return i >= 0 ? i : FUSION_LAYER_ORDER_V1.length
  }
  const cmpLayer = (a, b) => {
  ...
  //   （tier-layer-inject.js:58 TIER_LAYER_ORDER_V1 = ['project','whiteboard','user','reflection','log']
