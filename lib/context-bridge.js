...
export function buildObservationId(sessionId, contextVersion, triggerDigest, policyVersion = CONTEXT_BRIDGE_POLICY_VERSION) {
  const parts = ['context-push-v1', String(sessionId || ''), Number(contextVersion) || 0, String(triggerDigest || ''), policyVersion]
...
export function createFakeContextSinkPre(opts = {}) {
  ...
  return {
    ...
    async push(frame) {
      ...
      stats.accepted++
      return { observationId: id, accepted: true, workerEpoch: 'fake-epoch-v1', reason: 'ok' }
    },
