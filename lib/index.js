import { planBudgetedWritePre, transactMemoryFilePre, legacyRecordStartsPre, legacyAppendPre, anchoredRecordStartsPre } from './memory-capacity-safe.js'
/**
...
import { MemoryDocumentStore, appendAnchoredRecord, renderReplace } from './memory-writer.js'
...
const DEFAULT_PROMPT_LAYERS = Object.freeze({
  ...
  snapshotPlanTitle: '[白板 PLAN.md — 项目全貌快照(模型维护,用户在面板可见);内容过时/理解有实质变化时,用 memory_note(kind=plan) 重写]',
...
class MemoryEngine {
  ...
  async buildPrevSessionPack(preferSid) {
    try {
      ...
      for (const m of msgs) {
        ...
        if (total + chunk.length > 60000) { body.push('(更早内容已截断——完整历史可用 memory_recall(scope=\'sessions\') 关键词检索)'); break }
        body.push(chunk)
        total += chunk.length
      }
      try {
    ...
    try {
      ...
      if (pack) {
        ...
        if (pack.transcriptPath) {
          ...
          guide.push('- 完整转写已归档,**不必在接续前通读**:仅在第0-2层不足以推进时再 read;优先用 memory_recall(scope=\'sessions\', query=\'关键词\') 定位片段,避免整篇读入。')
        } else {
          guide.push('- 旧会话 ID: ' + pack.sessionId + '(转写文件生成失败,可用 memory_recall 检索)')
        }
        guide.push('- 历史会话全文检索: memory_recall(scope=\'sessions\', query=\'关键词\') 可跨全部历史会话查证细节。')
   ...
   */
  /** #38: plan, archive and final write share the writer's existing per-file transaction. */
  async writeBudgetedMemory(agent, layer, text, options = {}) {
    const p = options.paths || await this.resolvePaths(agent)
    const file = options.file || (layer === 'user' ? p.userFile : p.notesPath)
    const anchoredStore = options.store || this.docStore
    const store = anchoredStore || (this._legacyMemoryStore || (this._legacyMemoryStore = new MemoryDocumentStore()))
    const date = this.memToday()
    const limits = (this._lastCompactAt && typeof this._lastCompactAt === 'object') ? this._lastCompactAt : (this._lastCompactAt = {})
    const allowFold = options.allowFold !== undefined ? options.allowFold : !(limits[layer] && Date.now() - limits[layer] < COMPACT_THROTTLE_MS)
    let foldAttempted = false
    const archiveFile = layer === 'user' ? path.join(dshHome(), 'memory', 'archived-user.md') : path.join(p.projectDir, 'archive', 'notes-archived.md')
    const r = await transactMemoryFilePre(store, file, async (current, state) => {
      const parsed = anchoredStore ? parseAnchors(Buffer.from(current, 'utf8')) : null
      if (parsed && parsed.status !== 'clean') return { ok: false, reason: 'dirty-file' }
      let limit = options.maxChars === undefined ? this.capacityLimit(layer) : Math.max(0, Math.floor(options.maxChars))
      if (options.deficit !== undefined) limit = Math.max(0, current.length - Math.max(1, Number(options.deficit) || 1))
      const pendingId = anchoredStore ? store.idFactory() : ''
      const summaryId = anchoredStore ? store.idFactory() : ''
      const asText = rr => { if (!rr || !rr.ok) throw new Error(rr && rr.reason || 'render-failed'); return Buffer.isBuffer(rr.text) ? rr.text.toString('utf8') : rr.text }
      const render = (kept, summary) => {
        if (options.replace) return anchoredStore ? renderReplace(current, kept, { idFactory: store.idFactory }) : kept
        let base = kept
        if (summary) {
          const block = '\n## 整理要点 ' + date + '\n' + summary + '\n\n'
          base = (anchoredStore ? asText(appendAnchoredRecord('', { memoryId: summaryId, text: block })) : block) + kept
        }
        if (options.compactOnly) return base
        const record = '\n## ' + date + '\n' + String(text == null ? '' : text)
        return anchoredStore ? appendAnchoredRecord(base, { memoryId: pendingId, text: record }) : legacyAppendPre(base, record)
      }
      return planBudgetedWritePre({ current, limit, replace: !!options.replace, replacement: String(text == null ? '' : text), render,
        starts: anchoredStore ? anchoredRecordStartsPre(current, parsed) : legacyRecordStartsPre(current),
        fold: allowFold ? async (raw, budget) => {
          if (budget <= 0) return ''
          foldAttempted = true
          return this.foldTextToSummaryPre(raw, layer, agent, budget)
        } : null,
      })
    }, { archiveFile })
    if (foldAttempted) limits[layer] = Date.now()
    if (r.ok && typeof r.body === 'string') {
      if (layer === 'user') this.state.userText = r.body
      else this.state.notesText = r.body
      this.state.loadedAt = Date.now()
    }
    return r
  }
  ...
  /** Compatibility-only preflight; all production writers use writeBudgetedMemory. No mutation. */
  async ensureBudget(agent, layer, text, options = {}) {
    const p = await this.resolvePaths(agent), file = layer === 'user' ? p.userFile : p.notesPath
    const current = await this.readTextSafe(file), store = this.docStore
    const raw = String(text == null ? '' : text)
    let serialized
    if (store) {
      const rr = options.replace ? renderReplace(current, raw, { idFactory: store.idFactory })
        : appendAnchoredRecord(current, { memoryId: store.idFactory(), text: '\n## ' + this.memToday() + '\n' + raw })
      if (!rr.ok) return { ok: false, reason: rr.reason, written: false }
      serialized = rr.text.toString('utf8')
    } else serialized = options.replace ? raw : legacyAppendPre(current, '\n## ' + this.memToday() + '\n' + raw)
    const limit = this.capacityLimit(layer)
    const acct = { limit, used: current.length, need: serialized.length, add: serialized.length - current.length, ok: serialized.length <= limit }
    return { ok: acct.ok, acct, reason: acct.ok ? undefined : 'budget-preflight-only', written: false }
  }
  budgetRefusalTextPre(res, layer) {
    const name = layer === 'user' ? '用户级记忆' : '项目笔记'
    const r = res || {}, a = r.acct || {}, reason = String(r.reason || 'write-failed')
    const details = {
      'replacement-over-capacity': '整体替换后的实际序列化内容超过容量上限；没有把旧文件计入新文件额度',
      'write-over-capacity': '仅本次新增内容及写入标记就已超过容量上限',
      'no-removable': '在保留最新完整记录的前提下无法安全腾位；请缩小本次内容、提高容量或整体替换',
      'dirty-file': '锚点校验失败，未进行切片或覆盖',
      'conflict-external-edit': '检测到文件在整理期间被其他写入者修改，已拒绝覆盖',
      'budget-preflight-only': '预检超过容量；实际写入须经 writeBudgetedMemory 原子计划与归档',
    }
    const size = Number.isFinite(a.need) ? '（写入后需 ' + a.need + '/' + a.limit + ' 字符）' : ''
    return name + '写入未完成' + size + '：' + (details[reason] || reason)
      + (r.written ? '。正文可能已经提交，但后置校验失败；请读取实际文件核对，不要直接重复追加。' : '。本次未覆盖正文。')
  }
  async compactLayer(agent, layer, opts = {}) {
    const p = await this.resolvePaths(agent)
    const file = layer === 'user' ? p.userFile : p.notesPath
    const cur = await this.readTextSafe(file)
    const pending = Math.max(0, (Number(opts.needChars) || cur.length + 1) - cur.length)
    return this.writeBudgetedMemory(agent, layer, '', { paths: p, compactOnly: true, maxChars: Math.max(0, this.capacityLimit(layer) - pending) })
  }
  async compactLegacyLayer(agent, layer, p, keepBudget, allowFold) {
    return this.writeBudgetedMemory(agent, layer, '', { paths: p, compactOnly: true, maxChars: keepBudget, allowFold })
  }
  async foldTextToSummaryPre(text, layer, agent, maxChars = COMPACT_FOLD_MAX_CHARS) {
    try {
      const cap = Math.min(COMPACT_FOLD_MAX_CHARS, Math.max(0, Math.floor(Number(maxChars) || 0)))
      if (!cap || !this._subagents || !agent || !String(text || '').trim()) return ''
      const label = layer === 'user' ? '用户级记忆' : '项目笔记'
      const prompt = [
        '将以下' + label + '压缩成仍然有效的要点。原文会完整归档。',
        '保留强制规则、偏好、关键决策、路径与已确认结论；不编造信息。',
        '只输出完整的 Markdown 要点，每条尽量一句话；总长度不超过 ' + cap + ' 字符。',
        '无法在预算内完整概括时返回(无)，不要输出半句或截断代码。',
        '', truncateTail(String(text), 6000),
      ].join('\n')
      const summary = String(await this.runSubagent(prompt, 'auto-memory-fold', agent) || '').trim()
      if (!summary || summary.includes('(无)') || summary.length > cap) return ''
      return summary
    } catch (e) {
  ...
  async compactAnchoredLayer(store, filePath, layer, today, paths, deficit, agent, allowFold) {
    return this.writeBudgetedMemory(agent, layer, '', { paths, file: filePath, store, compactOnly: true, deficit, allowFold })
  }
  // ---------- 注入渲染(同步,基于缓存) ----------
  ...
  renderMemoryStatic() {
    ...
    lines.push('- 记忆容量(设置项 noteCapacityChars/userCapacityChars,默认各 12000 字符):项目笔记与用户级记忆各有容量上限;超出时框架自动整理——先把较早内容交给 AI 折叠成要点、失败则退回整条归档(原文进 archive,信息不丢),整理后仍超才拒绝写入,不足以安全腾位时会如实拒绝；原文不会被切片删除。整理同一层 10 分钟内只做一次;每日日志无容量限制。注意这与「注入预算」不同:注入预算管每轮往上下文塞多少摘要,容量上限管文件本体大小。')
  ...
  async appendText(p, text) {
    ...
    if (store) {
      const r = await store.append(p, String(text == null ? '' : text))
      if (!r.ok) throw new Error('memory-anchor-append-failed:' + r.reason)
      return await this.readTextSafe(p)
    }
    const legacy = this._legacyMemoryStore || (this._legacyMemoryStore = new MemoryDocumentStore())
    const r = await transactMemoryFilePre(legacy, p, current => ({ ok: true, text: legacyAppendPre(current, String(text == null ? '' : text)) }))
    if (!r.ok) throw new Error('memory-append-failed:' + r.reason)
    return r.body
  }
  ...
  async writeFull(p, text) {
    const store = this.docStore
    if (store) {
      const r = await store.replace(p, String(text == null ? '' : text))
      if (!r.ok) throw new Error('memory-anchor-replace-failed:' + r.reason)
      return
    }
    const legacy = this._legacyMemoryStore || (this._legacyMemoryStore = new MemoryDocumentStore())
    const r = await transactMemoryFilePre(legacy, p, () => ({ ok: true, text: String(text == null ? '' : text) }))
    if (!r.ok) throw new Error('memory-replace-failed:' + r.reason)
  }
  // ---------- 检索 ----------
  ...
  async consolidateTurn(turn, agent) {
    ...
    runtime.consolidating = (async () => {
      try {
        ...
        if (notePts.length) {
          const r = await this.writeBudgetedMemory(agent, 'note', notePts.map((x) => '- ' + x).join('\n'))
          if (r.ok) {
            const body = r.body
        ...
        if (userPts.length) {
          const r = await this.writeBudgetedMemory(agent, 'user', userPts.map((x) => '- ' + x).join('\n'))
          if (r.ok) {
            const body = r.body
  ...
  async consolidateMemory(agent, days = 7) {
    ...
    if (projectPts.length) {
      const r = await this.writeBudgetedMemory(agent, 'note', projectPts.map((x) => '- ' + x).join('\n'))
      if (r.ok) {
        const body = r.body
        this.state.notesText = body
  ...
  const tools = [
    ...
    defineTool('memory_note', '更新当前项目长期笔记 .dsh-memory/MEMORY.md(本项目专属的约定、决策、架构要点),或写交接白板。kind=note(默认):action=append 追加一段(自动带日期标题)/action=replace 整体替换(需先基于注入内容或 memory_recall 结果给出完整新内容),本项目笔记有**容量上限**(字符,设置项 noteCapacityChars,默认 12000);超出时自动整理——先把较早内容交给 AI 折叠成要点、失败则退回整条归档(原文都进 archive,不丢),不足以安全腾位时拒绝写入并保留原文。kind=handoff:写一篇四段式交接账本(任务状态/目标/已试方案与失败原因/进度与下一步)到 handoff/,阶段产出或方向变化时用,给下一个上下文窗口续命。kind=plan:整体重写白板 PLAN.md(人能读的项目全貌规划图),对项目全貌的理解发生实质变化时用,旧版自动归档。**调用后必须在本轮回复正文中向用户转述:更新了什么**。', {
      ...
      if (args.kind === 'handoff' || args.kind === 'plan') {
        ...
        if (!r || !r.ok) return 'memory_note: 白板写入失败(' + ((r && r.error) || '未知') + ')。'
      ...
      const acct = await engine.writeBudgetedMemory(exec.agent, 'note', write, { replace })
      if (!acct.ok) return 'memory_note: ' + engine.budgetRefusalTextPre(acct, 'note')
    ...
    defineTool('memory_user', '更新用户级记忆 ~/.dsh/memory/MEMORY.md(跨所有项目的长期规则/偏好,用户明确要求记住时用)。action=append 追加;action=replace 整体替换。有**容量上限**(字符,设置项 userCapacityChars,默认 12000);超出时自动整理——先把较早内容交给 AI 折叠成要点、失败则退回整条归档(原文都进 archive,不丢),不足以安全腾位时拒绝写入并保留原文。**调用后必须在本轮回复正文中向用户转述:已记住该规则/偏好**。', {
      ...
      const acct = await engine.writeBudgetedMemory(exec.agent, 'user', write, { replace })
      if (!acct.ok) return 'memory_user: ' + engine.budgetRefusalTextPre(acct, 'user')
      const body = acct.body
