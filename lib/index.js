...
class MemoryEngine {
  ...
  async recycleSubagentSession(agent) {
    try {
      ...
      try {
        await retryRename(dir, dest, { fs: { rename }, delays: [0, 300, 1000] })
      } catch (e) {
  ...
  return {
    ...
    async execute(args, exec) {
      try {
        ...
        // #48: preserve failed writes as rejected executions; DSH owns isError.
        // Keep the canonical success schema/string and unrelated tools unchanged.
        if (name === 'memory_log' || name === 'memory_note') throw e
