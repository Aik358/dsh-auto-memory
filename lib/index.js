...
class SessionRuntimeStore {
  ...
  dispose(agent) {
    ...
    // issue#58:激活 inbox 的 per-runtime 态必须随会话释放一起清理(runtimeState / stepsByRuntime /
    // pathsByKey + 注册表里那条已释放会话的 claimed 包)。此前 disposeRuntime 全仓零调用方,
    // 只有插件整体卸载时的 disposeAll 会清 ⇒ 频繁开关会话时每个 runtime 遗留一条,进程存活期内不释放。
    try { if (this._activationHost) this._activationHost.disposeRuntime(runtime) } catch (e) {}
...
class MemoryEngine {
  ...
  async writePlanSnapshot(projectDir, content, opts) {
    ...
    try {
      ...
      if (existing && existing.trim() && existing.trim() !== incoming.trim()) {
        ...
        // ★2026-09-19 修复（issue #94②）：同秒两次重写会算出同一归档名并**静默覆盖**上一份 ——
        // 补上与姊妹路径（PLAN-history :1982 / 账本 :2038）同款的 -b/-c 防撞后缀循环。
  ...
  async listHandoffLedgers(dir, limit = 12) {
    try {
      // ★issue #94②:PLAN 归档补 -b/-c 防撞后缀后,扫描正则同步接受带后缀名
      return (await readdir(dir)).filter((n) => /^(?:handoff-\d{8}-\d{6}(-[a-z])?|PLAN-\d{8}-\d{6}(-[a-z])?)\.md$/.test(n)).sort().reverse().slice(0, limit)
    } catch (e) { return [] }
  }
  ...
  async checkWaterLevel(agent) {
    try {
      ...
      // ★2026-09-19 修复（issue #88）：state 写入落点改为 rtOwn.state。旧写法 `this.state` 在裸调路径
      // （turn-stopping / pre-step / 心跳接续均不在 withAgent ALS 内）落 default runtime，而注入回调在
      // withAgent 内读 per-runtime state ⇒ 两份对象，水位 advisory 与催更注入块从未触发。无 agent 裸调时
      // runtimeFor(undefined)=default，与旧口径同对象，测试行为不变。
      rtOwn.state.waterLevelRatio = armRatio
      rtOwn.state.waterLevelTokens = estTokens
      rtOwn.state.waterLevelWindow = triggerWin
      rtOwn.state.waterLevelSource = winSource
      rtOwn.state.waterLevelMeter = meterLayer
      rtOwn.state.waterLevelModel = sessModel.model || winInfo.model || ''
      rtOwn.state.waterLevelAt = Date.now()
      rtOwn.state.waterLevelMeasuredTokens = estTokens
      rtOwn.state.waterLevelReserve = reserve
      ...
      rtOwn.state.waterLevelRing = (triggerWin > 0 && Number.isFinite(estTokens)) ? (estTokens / triggerWin) : 0
      // provider 自报的硬上限只有撞过墙(400 文本)才拿得到;拿得到就把「真实可写上限」也算出来,
      // 让界面能显示「距硬墙还剩多少 token」(本机 = 1,048,576 − 384,000 = 664,576)。
      rtOwn.state.waterLevelWall = hardWin > 0 ? Math.max(0, hardWin - reserve) : 0
      rtOwn.state.waterLevelHardTrigger = hard ? (compacted ? 'compaction' : overflowed ? 'overflow' : 'wall') : ''
      ...
      this.rememberWaterRecord(agent && agent.session && agent.session.id ? String(agent.session.id) : '', {
        ...
        windowRaw: win, reserve, source: winSource, meter: meterLayer, model: rtOwn.state.waterLevelModel || '',
        threshold, hardTrigger: rtOwn.state.waterLevelHardTrigger, at: Date.now(), live: true,
      })
      ...
      if (this.handoffChainEnabledPre()) {
        rtOwn.state.planUpdatePending = {
          reason: compacted ? 'compact' : (overflowed ? 'overflow' : 'water'),
          at: Date.now(),
        }
      }
      diag('water level advisory: ratio=' + ratio.toFixed(2) + (hard ? '→' + armRatio.toFixed(2) + '(hard:' + rtOwn.state.waterLevelHardTrigger + ')' : '') +
      ...
      if (this.handoffChainEnabledPre() && this.config.waterLevelAutoHandoff !== false && (!rt.waterLevelAutoHandoffDone || compacted || overflowed)) {
        ...
        // ★2026-09-19 修复（issue #88）：latestReflection 由 refresh()（withAgent 内）写进 per-runtime
        // state，裸调时读 default 恒为空 —— 与上方水位写入同口径改走 rtOwn.state。
        const refDigest = String(reflectionDigest(rtOwn.state.latestReflection || '') || '').split(NL).filter((l) => l.trim()).slice(0, 4)
        ...
        if (r && r.ok) {
          rtOwn.state.latestHandoffText = await this.readTextSafe(r.path)
          rtOwn.state.loadedAt = Date.now()
          ...
          if (r2 && r2.ok) {
            rtOwn.state.latestHandoffText = await this.readTextSafe(r2.path)
            rtOwn.state.loadedAt = Date.now()
  ...
  markContinuedSession(sid, toSid) {
    ...
    if (this.handoffChainEnabledPre()) {
      // ★2026-09-19 修复（issue #88）：本方法由心跳接续/HTTP 回调**裸调**，旧写法 `this.state` 落
      // default runtime，而催更注入块在 withAgent 内读 per-runtime state ⇒ 标记从未被注入侧看到。
      // 按新窗口 sid 定位 runtime 写入（接续材料是写给新窗口的）；注册表取不到 agent（或沙箱测试
      // 未提供 agentForSessionId）时回落 default 置位，与旧口径兼容。
      try {
        const targetAg = this.agentForSessionId(toSid || sid)
        const targetRt = targetAg ? this.runtimeFor(targetAg) : null
        if (targetRt && !targetRt.disposed) {
          targetRt.state.planUpdatePending = { reason: 'continue', at: Date.now() }
        } else {
  ...
  async buildPrevSessionPack(preferSid) {
    try {
      ...
      // ★2026-09-19 修复（issue #90）：转写包落盘目录与序号记账按**包内会话**解析 —— 旧写法
      // resolvePaths(undefined) 裸调落「GUI 最后轮询的工作区」，多工作区下包会写进错误工作区的
      // handoff/。活会话走 resolvePaths(agent)（含 C 类回退与无人值守按会话锁），冷会话走
      // registry/持久化头反查；空 sid 时 resolvePathsForSession 内部回落全局口径（与旧行为一致）。
      const NL = String.fromCharCode(10)
  ...
  async buildContinueCarry(preferSid) {
    try {
      ...
      // ★2026-09-19 修复（issue #90）：白板/账本按**被接续会话**（preferSid=旧会话）解析，不再取
      // 全局 default state.ws —— 多窗口并行时旧实现把「GUI 最后轮询工作区」的 PLAN/账本当接续材料
      // 交给新窗口（张冠李戴）。preferSid 为空时 resolvePathsForSession('') 内部回落全局口径（兼容）。
  ...
  async handoffPanelData(fileQ, sessionId) {
    ...
    if (fileQ) {
      ...
      // ★issue #94②:PLAN 归档补 -b/-c 防撞后缀后,白名单同步接受带后缀名。
      const m = String(fileQ).match(/^(?:PLAN\.md|handoff-\d{8}-\d{6}(?:-[a-z])?\.md|archive\/PLAN-\d{8}-\d{6}(?:-[a-z])?\.md|index\.json|events\.jsonl)$/)
    ...
    for (const n of archNames) {
      // ★issue #94②:接受 -b/-c 防撞后缀(同秒多次归档不再互相覆盖)
      if (!/^PLAN-\d{8}-\d{6}(-[a-z])?\.md$/.test(n)) continue
  ...
  async checkUpdate(force) {
    ...
    // ★2026-09-19 修复（issue #94①）：失败结果不写 12h 缓存 —— 旧实现无 error 判断一律落盘,
    // 启动时一次断网 ⇒ 12 小时内设置页恒「版本检查失败」且 latest='' 压住更新提醒,仅手动 force
    // 可绕。缓存只锁成功结果;失败不写,下次自动检查自然重试(单次 8s 超时,无请求风暴风险)。
    if (!result.error) {
      try {
   ...
   *   索引状态 = `this._indexDegradeBySession.get(agentSessionId)`(context-host 在 index-not-ready 时
   *   按会话记录 → I7 降级标注;2026-09-19 起不再回落全局兼容槽,防跨会话假降级污染,issue #92)。
  ...
  buildTierLayerInjection(agent) {
    ...
    try {
      ...
      // 多工作区适配(2026-09-17):按**当前会话**取降级记录。旧实现只读全局单槽 + 10 分钟窗
      // ⇒ B 会话的"未就绪"会把 A 会话污染十分钟(跨会话假降级)。
      // ★2026-09-19 修复（issue #92）：删去 `|| this._lastIndexDegrade` 全局兜底 —— 兼容槽存的是
      // 「最近任一会话」的记录,无自有记录的会话(或 agentSessionId 为空的渲染路径)仍会被渲染别人的
      // 「[降级] 索引未就绪」行,击穿分片语义。会话身份缺失按无降级处理(fail-open 正常渲染),
      // 与 _tierGateHits 兜底下游有 session 门 fail-closed 的口径对齐。
  ...
  renderMemoryDynamic(context) {
    ...
    // ★2026-09-19 修复（issue #88）：pushPart 只收 4 参（第 4 参才是 priority）—— 旧调用传了
    // 5 个实参，'must' 被 JS 静默丢弃 ⇒ 该段 priority 落成 normal，预算超限时接续催更段可被整段丢弃。
...
export function apply(ctx, config) {
  ...
  // issue#58:把激活 host 回填给 runtime 释放路径(SessionRuntimeStore.dispose 里按 this._activationHost
  // 清理 per-runtime 激活态)。runtimes 在引擎构造期创建,早于各 host ⇒ 只能在此处回填。
