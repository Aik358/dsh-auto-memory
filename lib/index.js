...
import { scanPluginSubagentSessions, recycleSessions, PLUGIN_LABEL_PREFIX } from './subagent-gc-pre.js'
import { parseModelWindowsPre, pickWindowPre, findOfficialContextWindowPre } from './water-window-pre.js'
  ...
  async resolveWaterWindow() {
    try {
      ...
      if (text) {
        ...
        if (model) value = pickWindowPre(parseModelWindowsPre(text), provider, model)
      }
  ...
  async checkWaterLevel(agent) {
    try {
      ...
      if (win > 0 && !/^manual/.test(winSource)) {
        // 官方容量:request/context 事件的 contextWindow(路由权威值,last-wins)。
        // 2.2.4 修复:该事件只在请求头部变化时追加(实测 2999 条事件里仅 2 条,且位于会话开头),
        // 旧实现只扫最近 256 条 → 长会话永远扫不到,窗口退化为 fallback 131072(水位虚高到 150%+)。
        // 现在全量向前扫 + 按会话 id 缓存 5 分钟,避免每轮重复遍历。
        try {
          const sid = agent && agent.session && agent.session.id ? String(agent.session.id) : ''
          const cached = this._officialWindowCache
          let cw = 0
          if (cached && sid && cached.sid === sid && Date.now() - cached.at < 300000) {
            cw = cached.window
          } else {
            cw = findOfficialContextWindowPre(sessionEventsOf(agent && agent.session))
            this._officialWindowCache = { sid, at: Date.now(), window: cw }
          }
          if (cw > 0) { win = cw; winSource = 'official-context' }
        } catch (eWin) {}
      }
      if (win <= 0) return
...
export function apply(ctx, config) {
  ...
  const heartbeatTimer = setInterval(() => { void writeHeartbeat(); try { engine.tickTime() } catch (e) {}; try { void engine.subagentGcSweep() } catch (e) {} }, 15000)
