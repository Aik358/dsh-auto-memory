...
export function apply(ctx, config) {
  ...
  {
    // issue#103:游标改以**行内容指纹**为准，不再用行数。`judgement-shadow.jsonl` 是**保尾丢弃的环形
    // 文件**（`python/worker_semantic_v1.py:49` SHADOW_LOG_MAX=256，:503/:726/:834 每次 `lines[-256:]`
    // 重写），旧实现 `start = lines.length >= prev.count ? prev.count : lines.length` 在文件写满后
    // 恒等于 lines.length ⇒ 增量区间永远为空 ⇒ 学习面静默永久停摆（无日志、无降级留痕，事后不可发现）。
    const hubFeedState = { checked: false, size: -1, mtimeMs: -1, emptyTicks: 0 }
    // 游标本体在纯模块里（可脱离引擎单测，见 lib/jsonl-tail-cursor.js 顶部说明）。
    ...
    const hubFeedTick = () => {
      try {
        ...
        const prev = hubFeedState
        if (prev.checked && prev.size === st.size && prev.mtimeMs === st.mtimeMs) return
        const lines = readFileSync(f, 'utf8').split('\n').filter((l) => l.trim())
        // 冷启动只登记现状、不追喂（与旧口径一致：旧实现 prev 为 null 时 start = lines.length，
        // 一行都不喂）。之后每轮只拿「指纹没见过」的行，与文件是否被环形截断**无关**。
        const cur = hubFeedCursor.take(lines)
        prev.checked = true
        prev.size = st.size
        prev.mtimeMs = st.mtimeMs
        if (!cur.seeded) {
          if (lines.length) diag('hub feed: 冷启动登记 ' + lines.length + ' 行现状（不追喂）')
          return
        }
        const newRows = []
        for (const l of cur.fresh) {
          try { const d = JSON.parse(l); if (d && d.kindCandidate) newRows.push(d) } catch (_) {}
        }
        if (!newRows.length) {
          // 文件确实变了却一行没喂 ⇒ 旧口径在这里静默 return，那正是 #103「停摆长期无人察觉」的形状。
          // 按 I7「任何一层缺数据都必须可读标注，绝不静默丢弃」补一条节流降级留痕。
          prev.emptyTicks++
          diagThrottled('hub-feed-empty', '[降级] hub feed: judgement-shadow 已变化但零新行(连续 '
            + prev.emptyTicks + ' 次; 环内 ' + lines.length + ' 行全部命中指纹)', 600000)
          return
        }
        prev.emptyTicks = 0
