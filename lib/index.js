 ...
 *     生活化/专业性/由内容决定),agent 生成后调 memory_reflect_dev 落盘
 *   - 配置:~/DSH_HOME/dsh-auto-memory.json(存储位置、注入预算、反思风格等),UI 经
 *     /api/dsh-auto-memory-dev/config 读写
 *   - 工具:memory_log_dev / memory_note_dev / memory_user_dev / memory_recall_dev / memory_maintain_dev /
 ...
 *   - 路由:/api/dsh-auto-memory-dev/{state,list,file,recall,config,reflect}(loopback-only)
 */
...
export const name = 'auto-memory-dev'
...
export const GUIDANCE = '本机已安装 dsh-auto-memory-dev 插件（开发版,集中式自动记忆 + 外部记忆继承）：三层本地记忆（用户级 ~/.dsh/memory/MEMORY.md、项目笔记与每日日志 .dsh-memory/）+ 会话自动注入 + 每日反思 + 其他 AI 工具记忆接入。能力：memory_log_dev 追加今日日志（append-only，完成实质性工作后必须调用）；memory_note_dev 更新项目笔记；memory_user_dev 更新用户级规则；memory_recall_dev 检索本地记忆 + 外部记忆（WorkBuddy/CodeBuddy/Claude Code/Codex 历史会话与画像）+ 历史 DSH 会话；memory_external_dev 查看/接入外部记忆源；memory_maintain_dev 归档 30 天前日志；memory_reflect_dev 保存每日反思；memory_status_dev 查看状态；memory_consolidate_dev 让 AI 读日志发散提炼长期要点固化进笔记。自动沉淀：每轮对话结束插件自动评估本轮内容并写今日日志/升格长期记忆（寒暄轮跳过，间隔与每日额度可在设置页「自动化」分组调整），无需你手动调 memory_log_dev。主动性纪律：任务开始遇到不熟悉的代码/领域/历史决策时，先 memory_recall_dev 检索本机全部 AI 工具历史，不凭空猜测；新工作区主动探索历史。限制：记忆文件为明文 Markdown；不存密钥除非用户明确要求；外部会话检索为关键词级（非语义）；GUI 侧边栏「记忆」面板（含「接续」页签，可查看来源内容、从记忆 prompt 移除已导入段落）与设置页可查看/配置/接入。用户提到「记忆 / 昨天做了什么 / 之前怎么做的 / 每日反思 / 接续 / 其他 AI 的记忆」时即指本插件，请据此协作。'

/** Route family. */
export const API = {
  state: '/api/dsh-auto-memory-dev/state',
  list: '/api/dsh-auto-memory-dev/list',
  file: '/api/dsh-auto-memory-dev/file',
  recall: '/api/dsh-auto-memory-dev/recall',
  smartRecall: '/api/dsh-auto-memory-dev/smart-recall',
  workspaces: '/api/dsh-auto-memory-dev/workspaces',
  debug: '/api/dsh-auto-memory-dev/debug',
  scanDirty: '/api/dsh-auto-memory-dev/scan-dirty',
  browseDir: '/api/dsh-auto-memory-dev/browse-dir',
  pickDir: '/api/dsh-auto-memory-dev/pick-dir',
  updateCheck: '/api/dsh-auto-memory-dev/update-check',
  update: '/api/dsh-auto-memory-dev/update',
  config: '/api/dsh-auto-memory-dev/config',
  reflect: '/api/dsh-auto-memory-dev/reflect',
  'reflect-auto': '/api/dsh-auto-memory-dev/reflect-auto',
  note: '/api/dsh-auto-memory-dev/note',
  external: '/api/dsh-auto-memory-dev/external',
  'external-view': '/api/dsh-auto-memory-dev/external-view',
  'external-import': '/api/dsh-auto-memory-dev/external-import',
  'external-remove': '/api/dsh-auto-memory-dev/external-remove',
  calendar: '/api/dsh-auto-memory-dev/calendar',
  summarize: '/api/dsh-auto-memory-dev/summarize',
  greet: '/api/dsh-auto-memory-dev/greet',
  notices: '/api/dsh-auto-memory-dev/notices',
}
...
const truncateHead = (s, n) => (s && s.length > n) ? s.slice(0, n) + '\n…(截断,完整内容用 memory_recall_dev 或 GUI 面板)' : (s || '')
const truncateTail = (s, n) => (s && s.length > n) ? '…(截断,完整内容用 memory_recall_dev 或 GUI 面板)\n' + s.slice(-n) : (s || '')
...
/** 诊断输出:写 ~/.dsh/dsh-auto-memory-dev-diagnose.log(append)+console.log 双保险。验证完移除。 */
let _diagChain = Promise.resolve()
function diag(msg) {
  try {
    const line = new Date().toISOString() + ' ' + msg + '\n'
    _diagChain = _diagChain.then(() => appendFile(path.join(dshHome(), 'dsh-auto-memory-dev-diagnose.log'), line, 'utf8')).catch(() => {})
    console.log('[dsh-auto-memory-dev] ' + msg)
  } catch (e) {}
}
...
class MemoryEngine {
  constructor() {
    ...
    this._configPath = path.join(dshHome(), 'dsh-auto-memory-dev.json')
  ...
  async saveConfig(patch) {
    ...
    } catch (e) { console.error('[dsh-auto-memory-dev] config migrate failed', e) }
  ...
  async migrateLegacy(ws, projectDir) {
    try {
      ...
      console.log('[dsh-auto-memory-dev] migrated memory: ' + legacy + ' -> ' + projectDir)
    } catch (e) { console.error('[dsh-auto-memory-dev] migrate failed', e) }
  }
  ...
  async checkUpdate(force) {
    const cacheFile = path.join(dshHome(), 'memory', 'update-check-dev.json')
  ...
  async fetchNotices(force) {
    const cacheFile = path.join(dshHome(), 'memory', 'notices-cache-dev.json')
    if (!force) {
      try {
  ...
  async _doRefresh(agent) {
    try {
      if (!this.configLoaded) await this.loadConfig()
        ...
        console.error('[dsh-auto-memory-dev] refresh failed', e)
      }
  }
  ...
  async compactLayer(agent, layer) {
    ...
    if (this._subagents && agent) {
      try {
        ...
        console.error('[dsh-auto-memory-dev] compactLayer AI failed', e && e.message ? e.message : e)
      }
    }
    if (!newOld) {
    ...
    this.state.loadedAt = Date.now()
    console.log('[dsh-auto-memory-dev] compacted ' + layer + ' memory: ' + cur.length + ' -> ' + body.length + ' chars')
    return true
  }
  ...
  renderMemoryDynamic(context) {
    ...
    // 记忆地图:告诉模型其他工作区记忆存在,需要时用 memory_recall_dev 跨区检索
    if (s.workspaceMap && s.workspaceMap.length) {
      lines.push('其他工作区记忆(开发/排查时可调用 memory_recall_dev 检索其日志/笔记): ' + s.workspaceMap.join('、'))
    }
    let used = 0
    ...
    if (this.external.cache && this.external.cache.length) {
      ...
      const ext = this.external.cache
        ...
        .slice(0, 3)
      if (ext.length) lines.push('\n[外部记忆 — 其他 AI 工具遗产,可继承(内容按需读取,不整段注入)]\n' + ext.join('\n') + '\n需要这些记忆时:直接读取上述绝对路径文件(你有文件读取能力),或用 memory_recall_dev 按需检索;不要凭空猜测其内容。')
      const sess = this.external.cache.filter((x) => x.kind === 'sessions')
      if (sess.length) {
        lines.push('· 历史会话索引: ' + sess.map((x) => x.name + ' ' + x.files.length + ' 个').join(', ') + ' —— 需要时用 memory_recall_dev 检索。')
      }
    }
    ...
    if (this.state.calendarText && this.state.calendarText.trim()) {
      ...
      if (calEntries.length) {
        ...
        lines.push('\n[日历与日程(未完成)]\n' + calLines + '\n主动关注这些安排:对话中若提及相关时间点,主动用 calendar_add_dev 补充新事项、calendar_done_dev 标记完成、calendar_remove_dev 删除过期事项;回复正文中向用户转述日历变更。')
      }
    }
  ...
  renderMemoryStatic() {
    ...
    lines.push('\n[记忆写入纪律 — 必须遵守]')
    lines.push('- 会话开始:若任务与历史工作/历史决策相关,先回顾以上记忆;**遇到不熟悉的代码、领域或项目时,主动调用 memory_recall_dev 检索本机所有 AI 工具的历史记忆(WorkBuddy/CodeBuddy/Claude Code/Codex 会话),或直接读取外部记忆标注的绝对路径文件,不要凭空猜测**。')
    lines.push('- 新工作区(无历史日志/笔记):主动用 memory_recall_dev 探索本机历史,判断该项目是否曾在其他 AI 工具中工作过;也可调用 memory_external_dev 查看并接入外部记忆;检索时在正文中说明"我先查一下之前的记录"。')
    lines.push('- 完成实质性工作后立即调用 memory_log_dev 追加今日日志(append-only,绝不覆盖):建/改应用、修 bug、写文档、重构、技术选型、用户约定或偏好。')
    lines.push('- progress 与 memory 一起写:写日志的同时,把有跨会话长期价值的内容一并写入记忆——跨项目规则 → memory_user_dev,仅本项目 → memory_note_dev;两者在同一轮完成,互不冲突、不遗漏。')
    ...
    lines.push('- **记忆操作必须在正文可见(摘要链)**:调用 memory_log_dev/note/user/reflect 更新记忆后,必须把结果写进本轮回复的正文文本(用户直接看到的那段文字,不是工具调用区),并在**回复末尾**用加粗或换行使其醒目(如"**已更新今日日志**\n新增:修复了XXX");调用 memory_recall_dev/memory_external_dev 检索时,在正文开头写明"我查了记忆,发现..."。工具返回值只是辅助,正文转述是强制要求。')
    lines.push('- 用户明确要求长期记住:跨项目规则 → memory_user_dev;仅本项目 → memory_note_dev。')
    lines.push('- 定期调用 memory_maintain_dev 做 30 天蒸馏:AI 提炼旧日志要点进项目笔记,原文保底归档;不存密钥,除非用户明确要求。')
    lines.push('- 自动沉淀:每轮对话结束,插件会自动评估本轮内容,把有记录价值的写进今日日志([自动沉淀] 标记),有长期价值的升格到项目笔记/用户级记忆,寒暄轮自动跳过。你仍须按上方纪律转述自己的显式记忆操作;也可调用 memory_consolidate_dev 让 AI 读日志发散提炼长期要点。')
    lines.push('- 记忆仅作补充,不替代正常回复与交付物。')
    lines.push('- 注入上下文只含精简记忆(最近1天日志/反思精华/路径索引);**需要某天完整日志、反思全文或记忆文件全文时,调用 memory_read_dev 按需读取(kind=log/reflection/user/notes/calendar),不要要求用户粘贴**。')
  ...
  renderReflectionRequest() {
    ...
    return [
      ...
      '要求:' + styleText,
      '生成后调用 memory_reflect_dev(date="' + pending.date + '", text=完整反思内容)保存,之后该提示不再出现。',
    ].join('\n')
  }
  ...
  async recall(query, limit = 8, agent) {
    ...
    if (!q) return 'memory_recall_dev: query 为空。'
  ...
  async saveReflection(date, text, agent) {
    if (!DATE_RE.test(date)) return 'memory_reflect_dev: date 必须是 YYYY-MM-DD。'
    const content = String(text || '').trim()
    if (!content) return 'memory_reflect_dev: text 为空,未保存。'
    const p = await this.resolvePaths(agent)
  ...
  async triggerAutoSummary(timePoint) {
    try {
      ...
      if (out && out.summary) {
        ...
        console.log('[dsh-auto-memory-dev] auto summary triggered at ' + timePoint + ' (period ' + period + ')')
      }
    } catch (e) {}
  }
  ...
  async runSubagent(text, label, agent, timeoutMs) {
    ...
    try {
      ...
      console.error('[dsh-auto-memory-dev] ' + label + ' failed', em)
      ...
      try {
        ...
        if (notePts.length) {
          ...
          if (r.ok) {
            ...
            console.log('[dsh-auto-memory-dev] auto-consolidate: NOTE 超预算且压缩不可用,已跳过')
          }
        }
        if (userPts.length) {
          ...
          if (r.ok) {
            ...
            console.log('[dsh-auto-memory-dev] auto-consolidate: USER 超预算且压缩不可用,已跳过')
          }
        }
        if (written) {
          // 自动沉淀统计(跨天重置)
          ...
          this.state.loadedAt = Date.now()
          console.log('[dsh-auto-memory-dev] auto-consolidated turn ' + turn + ': +' + written + ' points (log ' + logPts.length + ', note ' + notePts.length + ', user ' + userPts.length + ')')
        }
      } catch (e) {
        console.error('[dsh-auto-memory-dev] consolidateTurn failed', e && e.message ? e.message : e)
      } finally {
  ...
  async consolidateMemory(agent, days = 7) {
    const subagents = this._subagents
    if (!subagents) return 'memory_consolidate_dev: subagents 服务不可用,无法调用 AI 提炼。'
    const p = await this.resolvePaths(agent)
    ...
    if (!recent.length) return 'memory_consolidate_dev: 最近 ' + days + ' 天没有日志,没有可提炼的内容。'
    const prompt = [
    ...
    if (!text) return 'memory_consolidate_dev: AI 提炼失败(超时或 subagent 不可用),未写入任何内容。'
    if (text.includes('(无)')) return 'memory_consolidate_dev: AI 判断最近日志没有值得长期记录的新内容。'
    const projectPts = []
    const userPts = []
    let section = ''
    for (const raw of text.split('\n')) {
    ...
    this.state.loadedAt = Date.now()
    if (!written.length) {
      if (skipped.length) return 'memory_consolidate_dev: AI 已提炼,但今日写入预算已用尽,以下层被跳过(明天再固化): ' + skipped.join('、') + '。'
      return 'memory_consolidate_dev: AI 未提炼出值得长期记录的内容。'
    }
    const detail = []
    ...
    return 'memory_consolidate_dev 完成,已固化 ' + written.join('、') + '(带日期标题)。\n' + detail.join('\n\n')
  }
  ...
  async maintain(days = 30, agent) {
    ...
    if (this._subagents && agent) {
      try {
        ...
        console.error('[dsh-auto-memory-dev] maintain distill failed', e && e.message ? e.message : e)
      }
    }
...
class ExternalMemory {
  ...
  async injectionText(budget = 1400) {
    try {
      ...
      if (sess.length) {
        ...
        parts.push('· 历史会话可用: ' + sess.map((s) => s.name + ' ' + s.files.length + ' 个').join(', ') + '(需要时用 memory_recall_dev 检索)')
      }
   ...
   *  内容留在原文件, 模型需要时用 memory_read_dev / 直接读取路径按需获取。
  ...
  async importInto(sourceId, target, engine, agent) {
    ...
    if (src.kind === 'sessions') return '会话类源不支持整体接入,请用 memory_recall_dev 按需检索(' + src.files.length + ' 个会话文件)。'
    ...
    const linkBlock = '## 来自 ' + src.tool + '(' + src.name + ') — 接入于 ' + engine.memToday() + ' [链接模式]\n'
      + fileRefs + '\n- 用法: 需要时用 memory_read_dev 或直接读取上述路径按需获取, 不整段写入。'
    if (target === 'user') {
...
export function apply(ctx, config) {
  ...
  try {
    process.on('uncaughtException', (err) => { console.error('[dsh-auto-memory-dev] uncaughtException:', err && (err.stack || err.message) || err) })
    process.on('exit', (code) => { console.log('[dsh-auto-memory-dev] process exit code=' + code) })
  } catch (e) {}
  ...
  try {
    if (!process._dshAutoMemoryRejectionGuard) {
      ...
      process.on('unhandledRejection', (reason) => { console.error('[dsh-auto-memory-dev] unhandledRejection guard:', reason && (reason.stack || reason.message) || reason) })
    }
  } catch (e) {}
  ...
  ctx.on('agent/turn-stopping', (payload) => {
    ...
    try {
      ...
      if (hasAgent) {
        ...
        setTimeout(() => {
          void engine.consolidateTurn(payload.turn, agent).catch((e) => console.error('[dsh-auto-memory-dev] consolidateTurn unhandled', e && (e.stack || e.message) || e))
        }, 600)
      }
  ...
  const disposeSection = ctx.systemPrompt.section({
    name: 'dsh:auto-memory-dev-rules',
    order: SECTION_ORDER,
    text: () => {
  ...
  // ---------- 工具 ----------
  const tools = [
    defineTool('memory_log_dev', '向当前工作区的 .dsh-memory/ 今日日志追加一条工作记录(append-only,自动建目录/文件)。完成实质性工作(改代码/修 bug/写文档/重构/技术选型/用户偏好约定)后必须调用;有跨会话长期价值的内容在同一轮内一并写入记忆(memory_note_dev 项目/ memory_user_dev 跨项目),progress 与 memory 一起写;不要记录临时信息。**调用后必须在本轮回复正文(摘要可见的正文,不是工具调用区)中向用户转述一句:如"已把 X 记入今日日志"**。', {
      ...
      if (!gate.ok) return 'memory_log_dev: 写入被记忆卫生闸门拦截(' + WRITE_GATE_REASON[gate.reason] + '),未写入。请改写为客观陈述后重试。'
      const existing = engine.state.logText || (await engine.readTextSafe(logPath)) || ''
      if (tailHas(existing, note)) return 'memory_log_dev: 该条目与日志尾部已有内容重复,拒绝写入(复读防护)。'
    ...
    defineTool('memory_note_dev', '更新当前项目长期笔记 .dsh-memory/MEMORY.md(本项目专属的约定、决策、架构要点)。action=append 追加一段(自动带日期标题);action=replace 整体替换(需先基于注入内容或 memory_recall_dev 结果给出完整新内容)。每日预算 3000 字/天,超限自动压缩旧内容腾空间(不拒绝写入)。**调用后必须在本轮回复正文中向用户转述:更新了项目笔记、加入什么要点**。', {
      ...
      if (!content) return 'memory_note_dev: content 为空,未写入。'
      ...
      if (!gate.ok) return 'memory_note_dev: 写入被记忆卫生闸门拦截(' + WRITE_GATE_REASON[gate.reason] + '),未写入。请改写为客观陈述后重试。'
      const write = gate.clean || content
      if (!replace) {
        const existing = engine.state.notesText || (await engine.readTextSafe(p.notesPath)) || ''
        if (tailHas(existing, write)) return 'memory_note_dev: 与笔记尾部已有内容重复,拒绝写入(复读防护)。'
      }
      const acct = await engine.ensureBudget(exec.agent, 'note', write)
      if (!acct.ok) return 'memory_note_dev: 项目笔记今日预算已用尽(上限 ' + acct.acct.limit + ' 字/天)且自动压缩不可用(刚压缩过或 AI 不可用),本次未写入。可稍后再试或调用 memory_maintain_dev 整理。'
      let body
      if (replace) {
        body = write
    ...
    defineTool('memory_user_dev', '更新用户级记忆 ~/.dsh/memory/MEMORY.md(跨所有项目的长期规则/偏好,用户明确要求记住时用)。action=append 追加;action=replace 整体替换。每日预算 4000 字/天,超限自动压缩旧内容腾空间(不拒绝写入)。**调用后必须在本轮回复正文中向用户转述:已记住该规则/偏好**。', {
      content: { type: 'string', required: true, description: '要记住的规则或偏好内容。' },
      action: { type: 'string', enum: ['append', 'replace'], required: true, description: 'append=追加, replace=整体替换。' },
    }, async (args, exec) => {
      const p = await engine.resolvePaths(exec.agent)
      const content = String(args.content || '').trim()
      if (!content) return 'memory_user_dev: content 为空,未写入。'
      const replace = args.action === 'replace'
      // 写闸门: append 单条上限 8000 字; replace(整篇重写)放行到 20 万字, 但同样经受乱码/复读/重复块质量闸门
      const gate = sanitizeForWrite(content, { maxEntryChars: replace ? 200000 : 8000 })
      if (!gate.ok) return 'memory_user_dev: 写入被记忆卫生闸门拦截(' + WRITE_GATE_REASON[gate.reason] + '),未写入。请改写为客观陈述后重试。'
      const write = gate.clean || content
      if (!replace) {
        const existing = engine.state.userText || (await engine.readTextSafe(p.userFile)) || ''
        if (tailHas(existing, write)) return 'memory_user_dev: 与用户级记忆尾部已有内容重复,拒绝写入(复读防护)。'
      }
      const acct = await engine.ensureBudget(exec.agent, 'user', write)
      if (!acct.ok) return 'memory_user_dev: 用户级记忆今日预算已用尽(上限 ' + acct.acct.limit + ' 字/天)且自动压缩不可用(刚压缩过或 AI 不可用),本次未写入。可稍后再试或调用 memory_maintain_dev 整理。'
      let body
      if (replace) {
        body = write
    ...
    defineTool('memory_read_dev', '按需读取记忆文件完整内容(某日日志/反思全文、用户级记忆、项目笔记、日历),注入上下文只含精简摘要,需要细节时用本工具,不要要求用户粘贴。', {
      ...
      else return 'memory_read_dev: kind 无效(可选 log/reflection/user/notes/calendar)。'
      const text = await engine.readTextSafe(file)
      if (!text) return '未找到' + label + '文件: ' + file
      const cap = 8000
      return text.length > cap
        ? label + '(' + file + ') 内容过长,显示前 ' + cap + ' 字符:\n' + text.slice(0, cap) + '\n...(如需更多,用 memory_recall_dev 检索关键词)'
        : label + '(' + file + '):\n' + text
    }),

    defineTool('memory_recall_dev', '检索记忆:本地记忆文件(当前工作区 + 其他所有工作区的每日日志、项目笔记、用户级记忆、反思)关键词匹配 + 历史 DSH 会话全文检索(如部署启用)。开发/排查中不懂的、用户提到过去的做法/讨论/决定而当前上下文没有时调用——跨工作区的记忆也能检索到(结果标注来源工作区)。查询必须自包含。**检索后必须在本轮回复正文中向用户转述:检索了什么、找到什么(或没找到)**。', {
    ...
    defineTool('memory_maintain_dev', '维护记忆(30 天蒸馏):把 days(缺省30)天前的 .dsh-memory/ 每日日志交给 AI 蒸馏提炼出有长期价值的要点写入项目 MEMORY.md,原文保底归档到 .dsh-memory/archive/ 后从活跃日志移除。AI 不可用时降级为原样归档,不丢信息。', {
      days: { type: 'integer', description: '归档阈值天数,缺省 30。' },
    }, async (args, exec) => engine.maintain(args.days, exec.agent)),

    defineTool('memory_status_dev', '查看自动记忆的当前状态:存储位置、各记忆文件大小、今日日志条数、待反思、上次刷新时间。用于确认记忆系统工作正常。', {}, async (_args, exec) => {
    ...
    defineTool('memory_reflect_dev', '保存每日反思(在收到「昨日反思待生成」提示、并已在回复中呈现反思后调用)。将反思全文落盘到 .dsh-memory/reflections/YYYY-MM-DD.md,并标记该日反思完成。', {
    ...
    defineTool('memory_external_dev', '查看/接入其他 AI 工具(AI 助手/CodeBuddy/Claude Code/Codex/项目约定文件)的记忆。action=list 列出全部检测到的外部记忆源(路径/大小/预览/会话数);action=import 以纯链接模式接入(source 为源 id,target=project 接进项目笔记 / user 接进用户级记忆,只记录源文件路径指针、不写入内容,需要时按需读取;防止外部脏内容混入本地记忆)。首次在新工作区工作、或用户提到其他软件里做过的事时调用。', {
      ...
      if (args.action === 'list') {
        ...
        lines.push('接入: memory_external_dev(action="import", source="<id>", target="project"|"user")')
        return lines.join('\n')
      }
      return engine.external.importInto(String(args.source || ''), args.target === 'user' ? 'user' : 'project', engine, exec.agent)
    }),

    defineTool('calendar_add_dev', '向用户级日历(~/.dsh/memory/CALENDAR.md)添加日程/事项。主动从对话中提取 deadline、约定时间、任务节点等信息写入日历(跨对话有效、重装不丢)。调用后必须在本轮回复正文中向用户转述:已把 X 记入日历。', {
    ...
    defineTool('calendar_list_dev', '列出日历条目(可按日期过滤、含完成状态)。用于查看已有安排、回答"我最近有什么安排"等问题。', {
    ...
    defineTool('calendar_done_dev', '标记日历条目完成。参数需与 calendar_list_dev 结果一致(date/time/title)。', {
    ...
    defineTool('calendar_remove_dev', '删除日历条目。', {
      date: { type: 'string', required: true, description: '日期 YYYY-MM-DD。' },
      time: { type: 'string', required: true, description: '时间 HH:MM 或 --:--。' },
      title: { type: 'string', required: true, description: '事项标题。' },
    }, async (args, exec) => engine.calendarRemove(args.date, args.time, args.title, exec.agent)),

    defineTool('memory_consolidate_dev', 'AI 主动维护长期记忆(做梦式固化):读最近 days 天的工作日志,由 AI 发散提炼出有跨会话长期价值的决策/架构/用户偏好,自动写入项目笔记 MEMORY.md(带日期标题)与用户级 MEMORY.md(跨项目规则),并在正文向用户转述固化结果。适合隔一段时间主动调用一次;每轮对话结束的自动沉淀也基于同一套提炼逻辑。', {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const content = src.kind === 'sessions'
            ? '这是会话类来源，含 ' + src.files.length + ' 个可检索会话文件。请在“检索”页输入关键词，或让 AI 调用 memory_recall_dev 按需取回。'
  ...
  console.log('[dsh-auto-memory-dev] ready: engine + ' + tools.length + ' tools + injection + ' + routes.length + ' routes (external memory: ' + Object.keys(DEFAULT_CONFIG.externalSources).length + ' sources)')
}
