...
class MemoryEngine {
  ...
  async writePlanSnapshot(projectDir, content) {
    ...
    try {
      ...
      await this.writeFullRaw(planPath, String(content || ''))
      return { ok: true, path: planPath, archived }
    } catch (e) {
  ...
  const tools = [
    ...
    defineTool('memory_note_pre', '更新当前项目长期笔记 .dsh-memory/MEMORY.md(本项目专属的约定、决策、架构要点),或写交接白板。kind=note(默认):action=append 追加一段(自动带日期标题)/action=replace 整体替换(需先基于注入内容或 memory_recall_pre 结果给出完整新内容),每日预算 3000 字/天,超限自动压缩旧内容腾空间(不拒绝写入)。kind=handoff:写一篇四段式交接账本(任务状态/目标/已试方案与失败原因/进度与下一步)到 handoff/,阶段产出或方向变化时用,给下一个上下文窗口续命。kind=plan:整体重写白板 PLAN.md(人能读的项目全貌规划图),对项目全貌的理解发生实质变化时用,旧版自动归档。**调用后必须在本轮回复正文中向用户转述:更新了什么**。', {
      ...
      if (args.kind === 'handoff' || args.kind === 'plan') {
        ...
        if (args.kind === 'plan') { engine.state.planText = writeH; engine.state.loadedAt = Date.now() }
        else { engine.state.latestHandoffText = '# 交接账本 · ' + engine.memToday() + ' ' + nowHm() + '\n\n' + writeH; engine.state.loadedAt = Date.now() }
