...
class MemoryEngine {
  ...
  async _writeSidecarIndexPre(projectDir, index) {
    ...
    await mkdir(sideDir, { recursive: true })
    await writeFile(path.join(sideDir, 'index.json'), JSON.stringify(fresh, null, 2), 'utf8')
    return fresh
  }
  ...
  async rebuildSidecarIndexPre(projectDir) {
    ...
    try { await mkdir(sideDir, { recursive: true }); await writeFile(path.join(sideDir, 'index.json'), JSON.stringify(index, null, 2), 'utf8') } catch (_) {}
    return index
  }
