...
const DEFAULT_CONFIG = {
  ...
  /** M-CM4 水位感知:会话消息总字符估计窗口填充;0=关闭。默认 24 万字符(约 8-10 万 token 混合文本,保守值)。 */
  waterLevelWindowChars: 240000,
...
const DEFAULT_PROMPT_LAYERS = Object.freeze({
  ...
  snapshotWaterBody: '上下文水位约 {pct}%(启发式估计)。请立即:①调用 memory_note_pre(kind=handoff) 写四段式交接账本(任务状态/目标/已试方案与失败原因/进度与下一步);②用 memory_note_pre(kind=plan) 刷新白板全貌;③在回复中建议用户开新窗口续接——交接材料已就绪。任务中可分解的部分优先派子代理隔离上下文。',
...
class MemoryEngine {
  ...
  /** M-CM4 水位感知(启发式):会话消息总字符 / waterLevelWindowChars。越阈值→每会话一次 advisory 标志+自动写骨架账本(可关)。 */
  async checkWaterLevel(agent) {
    try {
      const win = Number(this.config.waterLevelWindowChars) || 0
      if (win <= 0 || this.config.handoffEnabled === false) return
      const messages = extractSessionMessages(agent)
      let chars = 0
      for (const m of messages) chars += String((m && (m.text || m.content)) || '').length
      const ratio = Math.min(chars / win, 1.5)
      const rt = this.runtimeFor(agent)
      rt.waterLevel = ratio
      rt.waterLevelChars = chars
      ...
      if (!rt.waterLevelAdvised) {
        rt.waterLevelAdvised = true
        diag('water level advisory: ratio=' + ratio.toFixed(2) + ' chars=' + chars + '/' + win)
      }
      if (this.config.waterLevelAutoHandoff !== false && !rt.waterLevelAutoHandoffDone) {
        rt.waterLevelAutoHandoffDone = true
        const p = await this.resolvePaths(agent)
        const tail = (await this.readTextSafe(p.logPath)).split(String.fromCharCode(10)).filter(Boolean).slice(-14).join(String.fromCharCode(10))
        const skeleton = [
          '## 任务状态', '(系统水位自动快照 · 上下文水位约 ' + Math.round(ratio * 100) + '%;模型应尽快用 memory_note_pre(kind=plan) 重写白板全貌)', '',
          '## 目标', '(以最近日志与白板为准)', '',
          '## 已试方案与失败原因', '(见最近日志尾部)', '',
          '## 进度与下一步', tail || '(今日日志为空)',
  ...
  renderMemoryDynamic(context) {
    ...
    // M-CM1 交接白板:PLAN.md 全貌+最新交接账本,动态快照首位(动态层内容,不碰静态字节;敏感段清洗+硬预算截断)
    if (cfg.handoffEnabled !== false && (s.planText || s.latestHandoffText)) {
      const planPart = truncateHead(stripSensitiveSections(sanitizeForInjection(s.planText || '')), Math.max(Number(cfg.handoffPlanChars) || 1200, 200))
      if (planPart) lines.push('\n' + L('snapshotPlanTitle') + '\n' + planPart)
      const ledgerPart = truncateHead(stripSensitiveSections(sanitizeForInjection(s.latestHandoffText || '')), Math.max(Number(cfg.handoffLedgerChars) || 800, 200))
  ...
  renderMemoryStatic() {
    ...
    lines.push('- **交接与白板(长任务续命)**:阶段产出或方向变化时,调用 memory_note_pre(kind=handoff) 写四段式交接账本——任务状态/目标/已试方案与失败原因/进度与下一步,给下一个上下文窗口续命;对项目全貌的理解发生实质变化时,用 memory_note_pre(kind=plan) 重写白板 PLAN.md(人能读的项目规划图,旧版自动归档,用户在面板实时可见)。两者会注入到你的动态快照首位,是跨窗口交接的凭据。')
