...
class MemoryEngine {
  ...
  /** 最新一篇四段式交接账本(按 mtime 取最新;mtime 并列时按 <ts> + 防撞后缀序号判定)。 */
  async readLatestHandoff(handoffDir) {
    try {
      const names = (await readdir(handoffDir)).filter((n) => /^handoff-\d{8}-\d{6}(-[a-z])?\.md$/.test(n))
      if (!names.length) return ''
      // 同一秒连续写入在 Linux/Windows 上都可能落到完全相同的 mtimeMs；readdir 顺序又无契约。
      // 文件名的 -b/-c… 本来就是同秒写入序号，因此只在 mtime 并列时用它做确定性 tie-break。
      const orderKey = (n) => {
        const m = /^handoff-(\d{8}-\d{6})(?:-([a-z]))?\.md$/.exec(String(n || ''))
        if (!m) return ''
        const seq = m[2] ? Math.max(1, m[2].charCodeAt(0) - 96) : 0
        return m[1] + ':' + String(seq).padStart(2, '0')
      }
      let best = '', bestMt = -1, bestOrder = ''
      for (const n of names) {
        const st = await stat(path.join(handoffDir, n)).catch(() => null)
        const mt = st ? Number(st.mtimeMs) : 0
        const ord = orderKey(n)
        if (mt > bestMt || (mt === bestMt && ord > bestOrder)) {
          bestMt = mt
          bestOrder = ord
          best = n
        }
  ...
  /** 列出交接账本/白板归档文件(按时间戳 + 同秒防撞后缀倒序=新→旧)。 */
  async listHandoffLedgers(dir, limit = 12) {
    try {
      const names = (await readdir(dir)).filter((n) => /^(?:handoff|PLAN)-\d{8}-\d{6}(?:-[a-z])?\.md$/.test(n))
      const orderKey = (n) => {
        const m = /^(?:handoff|PLAN)-(\d{8}-\d{6})(?:-([a-z]))?\.md$/.exec(String(n || ''))
        if (!m) return ''
        const seq = m[2] ? Math.max(1, m[2].charCodeAt(0) - 96) : 0
        return m[1] + ':' + String(seq).padStart(2, '0')
      }
      return names.sort((a, b) => orderKey(b).localeCompare(orderKey(a)) || String(b).localeCompare(String(a))).slice(0, limit)
    } catch (e) { return [] }
  }
