...
class MemoryEngine {
  ...
  async buildContinueCarry() {
    try {
      ...
      if (ledger) parts.push('【第1层 · 交接账本 ' + (ledgerName || ledgerFrom) + '】写于 ' + fmtMt(ledgerMt) + ' (' + ledgerFrom + ')' + NL + ledger.slice(0, 8000))
      if (pack) {
  ...
  async recall(query, limit = 8, agent, scope = 'all', opts = null) {
    ...
    if (l0Mode) {
      ...
      const l0Hits = l0Corpus
        .filter((c) => c.lex > 0 || (typeof c.sem === 'number' && c.sem >= 0.5))
