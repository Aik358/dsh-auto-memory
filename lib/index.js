...
import { MemoryDocumentStore, memoryWriteError } from './memory-writer-pre.js'
import { retryRename } from './fs-retry-pre.js'
import { parseAnchors, stripAnchorLines } from './memory-anchor-pre.js'
import { createShadowHost } from './shadow-host-pre.js'
import { createContextHost } from './context-host-pre.js'
// R3（2026-09-18）：降级留痕层。fail-soft 本身是对的，缺陷在「降级不可见」——
// 检索链四条臂各自静默失效，使用者只感到「检索不太对」。本层让「哪条臂没在工作」可查询。
import { createDegradeSinkPre, deriveArmsHealthPre, persistDegradeLedgerPre, createQuotaProbePre, deriveQuotaVerdictPre } from './degrade-pre.js'
import { createSuccessEvidencePre } from './context-bridge-pre.js'
import { createIndexSyncHostPre } from './m7-index-sync-host-pre.js'
import { createActivationHost } from './activation-host-pre.js'
import { createJsSemanticEnginePre, createSemanticDownloaderPre, fuseD6Pre, probeJsSemanticAssets, deepScanPeerTransformers, E5_SMALL_Q8_MANIFEST_PRE_V1 } from './semantic-js-pre.js'
import { loadAndVerifyPolicy, decideActivationV2, lexicalContainment } from './semantic-decide-pre.js'
import { createL0IndexSyncPre } from './l0-index-sync-pre.js'
// ★P2（2026-09-15 引擎隔离 T2-9）：宽引擎身份（模型/权重指纹/tokenizer/精度/维度/池化/归一化/输入版本）。
import { computeEngineIdentityPre, JS_E5_IDENTITY_DESC_PRE_V1 } from './engine-identity-pre.js'
import { createPythonSidecarClientPre, defaultWorkerScriptPathPre } from './python-sidecar-client-pre.js'
import { createPythonSetupPre } from './python-setup-pre.js'
import { createEpisodicStorePre } from './episodic-store-pre.js'
import { createFactStorePre } from './fact-store-pre.js'
import { createProcedureStorePre } from './procedure-store-pre.js'
import { exportSkillForPre, resolveSkillsRootPre } from './skill-export-host-pre.js'
import { createMemoryHubPre } from './memory-hub-pre.js'
// ★#110（2026-09-22）：hub 持久化 IO 适配器与其健康度投影（原本内联在本文件里、失败被静默吞掉）。
import { createHubIoPre, createHubIoHealthPre, hubIoHealthSnapshotPre } from './hub-io-pre.js'
// ★issue#103：环形 JSONL 增量游标（内容指纹）。见 lib/jsonl-tail-cursor-pre.js 文件头。
import { createJsonlTailCursorPre } from './jsonl-tail-cursor-pre.js'
import { createRecallStatsPre } from './recall-stats-pre.js'
import { pickConsolidationTextPre } from './intent-clean-pre.js'
// T1-3（2026-09-19）：hubFlushTick 写入前的**内容卫生门**复用 ⑨ 同源清洗器。
// 必须独占一行（本仓契约守卫以字面量断言既有 import 行，禁止为排版合并）。
import { stripRuntimeIntentPre, looksRuntimeResiduePre } from './intent-clean-safe-pre.js'
import { createStorageManagerPre } from './storage-manage-pre.js'
import { workspaceSlugPre as migrateSlugPre, buildPackPre, validatePackPre, planImportPre, mergeSummaryRecordPre, calendarMergePre, MIGRATE_PACK_FORMAT_PRE } from './migrate-pack-pre.js'
import { rankWorkspacesByMemoryRecencyPre } from './ws-overview-rank.js'
import { scanPluginSubagentSessions, recycleSessions, PLUGIN_LABEL_PREFIX, decodeZstdFrames, decodeZstdFramesHead } from './subagent-gc-pre.js'
import { parseModelWindowsPre, pickWindowPre, findOfficialContextWindowPre, findSessionModelPre, scanPressureSignalsPre, reusableWindowCachePre, shouldArmAutoContinuePre } from './water-window-pre.js'
import { buildSourceCatalog, CorpusRegistry, canonicalize, sourceFingerprint } from './m4-corpus-pre.js'
// C5 三层注入装配（Tier-0 常驻目录 + 闸门下探 + I7 降级标注）：纯函数模块，注入路径零 LLM 调用（S9）
import { composeTieredInjectionPre, TIER_BUDGET_PRE_V1, TIER_MARK_PRE_V1, selectReusableTierHitsPre, describeReuseReasonPre } from './tier-layer-inject-pre.js'
// T0-3 分项账本（chars/limits 单一口径）：纯函数模块，注入路径零 LLM 调用（S9）
import { composeMemoryEnvelopePre } from './memory-envelope-pre.js'
...
import { validateMutationBoundaryPre, mutationRefusalTextPre } from './memory-mutation-pre.js'
import { parseWhiteboardPre, toMutationProjectionPre, extractProtectedRegionsPre, checkHandoffCriteriaPre, checkPlanCriteriaPre, criteriaRefusalTextPre, WB_MARKERS_PRE_V1, WB_CONTRACT_VERSION } from './wb-contract-pre.js'
import { resolveBoardModePre } from './board-mode-pre.js'
// ★#86-3（2026-09-20）：DSH_HOME 解析统一到单一口径（此前全仓 7 处、4 种回落）。
import { resolveDshHomePre } from './dsh-home-pre.js'
// ★R7（2026-09-20 用户要求「用户级硬性约束必须可以让用户自己增删改」）：
//   条目级解析/增删改的**纯逻辑**（不含 IO；IO 走既有 writeFull 事务）。
import { listRuleItemsPre, updateRuleItemPre, removeRuleItemPre, appendRuleItemPre } from './rules-edit-pre.js'
// ★#82（2026-09-20）：配置读写切到原子写 + 损坏隔离模块。
//   原实现：写侧裸 writeFileSync（写到一半被杀 ⇒ 半截 JSON），读侧 catch 静默回落出厂默认。
import { writeTextAtomicPreSync, writeTextAtomicPre, readJsonQuarantinePreSync } from './config-io-pre.js'
import { WB_SIDECAR_VERSION, buildSidecarEntryPre, rebuildSidecarIndexPre, expandByTagPre, traceByIdPre, applyAnchorsPre, collectAnchorIdsPre, wbRefPre, normalizeRelPathPre, normalizeTitlePre, buildKanbanPre, buildSectionCardsPre, splitSectionsPre, buildKanbanMatrixPre, ledgerDateOfPre, WB_KANBAN_LANES_PRE_V1 } from './wb-sidecar-pre.js'
// P6A 规则层（规则类与参考类分开措辞；规则真源 = 既有用户级记忆，不新增 RULES.md）
import { extractRulesLayerPre, renderRulesSectionPre, RULES_SECTION_TITLE_PRE_V1, RULES_SECTION_GUIDE_PRE_V1, REFERENCE_SECTION_GUIDE_PRE_V1, LOG_KINDS_PRE_V1 } from './rules-layer-pre.js'

/**
 * ★P9（2026-09-22）：用户级硬性约束的**条目级增删改** —— 前端路由与模型工具共用的**唯一写盘口**。
 *
 * 为什么必须存在：[规则 — 用户级硬性约束 · 必须遵守] 段**每轮无条件注入且不参与裁剪**
 * （memory-envelope-pre.js 的 rules 分项不受限），而这一层在 P9 之前**认不出状态标记**。
...
import { buildIndex as buildMemoryIndexFile, verifyRecord as verifyMemoryRecord, coverage as memoryCoverage, INDEX_MAX_FILE_BYTES } from './memory-index-pre.js'
...
export const API = {
  ...
  recallStats: '/api/dsh-auto-memory-pre/recall-stats',
  ...
  'rules-list': '/api/dsh-auto-memory-pre/rules',
  'rules-apply': '/api/dsh-auto-memory-pre/rules/apply',
  ...
  'activation-inbox-pre': '/api/dsh-auto-memory-pre/activation-inbox-pre',
  'semantic-status': '/api/dsh-auto-memory-pre/semantic-status',
  'py-setup-detect': '/api/dsh-auto-memory-pre/python-setup/detect',
  'py-setup-venv': '/api/dsh-auto-memory-pre/python-setup/venv',
  'py-setup-deps': '/api/dsh-auto-memory-pre/python-setup/deps',
  'py-setup-model': '/api/dsh-auto-memory-pre/python-setup/model',
  'py-setup-cancel': '/api/dsh-auto-memory-pre/python-setup/cancel',
  'py-setup-status': '/api/dsh-auto-memory-pre/python-setup/status',
  'semantic-deep-detect': '/api/dsh-auto-memory-pre/semantic-deep-detect',
  'handoff-state': '/api/dsh-auto-memory-pre/handoff-state',
  'kanban-board': '/api/dsh-auto-memory-pre/kanban-board',
  // ★v3.1.2：卡片全文按需取（看板载荷不再内联 full ⇒ 展开全文时单取一张）。
  'kanban-card': '/api/dsh-auto-memory-pre/kanban-card',
  'handoff-continue': '/api/dsh-auto-memory-pre/handoff-continue',
  'handoff-permission': '/api/dsh-auto-memory-pre/handoff-permission',
  'auto-continue-state': '/api/dsh-auto-memory-pre/auto-continue-state',
  'auto-continue-decide': '/api/dsh-auto-memory-pre/auto-continue-decide',
  'subagent-gc': '/api/dsh-auto-memory-pre/subagent-gc',
  'semantic-download': '/api/dsh-auto-memory-pre/semantic-download',
  'semantic-emit': '/api/dsh-auto-memory-pre/semantic-emit',
  'shadow-recent': '/api/dsh-auto-memory-pre/shadow-recent',
  'review-feedback': '/api/dsh-auto-memory-pre/review-feedback',
  'memory-hub': '/api/dsh-auto-memory-pre/memory-hub',
  'storage-manage': '/api/dsh-auto-memory-pre/storage-manage',
  models: '/api/dsh-auto-memory-pre/models',
  'migrate-export': '/api/dsh-auto-memory-pre/migrate-export',
  'migrate-inspect': '/api/dsh-auto-memory-pre/migrate-inspect',
  'migrate-import': '/api/dsh-auto-memory-pre/migrate-import',
}
...
const DEFAULT_CONFIG = {
   ...
   *  全文永远靠 `memory_recall_pre` / `memory_read_pre` 按需下钻（Tier-0 目录常驻就是为此）。
  ...
  /** 定时做梦式固化(默认开):每天到点自动跑一次 memory_consolidate_pre 等价流程(读最近日志→发散提炼→项目笔记/用户记忆)。 */
  consolidateScheduleEnabled: true,
  /** 做梦式固化触发时间(HH:MM,插件日界内每天一次;命中时刻需宿主在线)。 */
  consolidateScheduleTime: '09:30',
  /** 做梦式固化回看天数(读最近 N 天日志)。 */
  consolidateScheduleDays: 7,
  /** 定时 30 天蒸馏(默认开):每天到点自动跑一次 memory_maintain_pre 等价流程;无超过 30 天的旧日志时零成本跳过(不调 AI)。 */
  ...
  handoffEnabled: true,
  /** 白板 PLAN.md 注入预算(字符,硬截断;全文可经 memory_read_pre/文件读取)。 */
   ...
   *  精简版的目标不是"看到全文"，而是"看到它长什么样、以及它已经旧了"；全文走文件/`memory_read_pre`。 */
  slimPlanChars: 400,
  slimLedgerChars: 300,
   ...
   *  `lib/memory-mutation-pre.js:validateMutationBoundaryPre` 里无条件生效
   ...
   *  默认 40=几个字/关键词级(省 token,agent 需要细节时用 memory_read_pre 取全文);
   ...
   *  都与"晋升"无关 —— context-host-pre.js:537（技能是否**注入上下文**）、
   *  activation-host-pre.js:330（**主动唤起**这条臂开不开）；真正的晋升由 store 门限 +
   *  路由动作决定。旧键默认 false ⇒ 新用户装完"技能永不生效"且无任何提示。
   *  ⚠️ 解析口径统一走 `lib/procedure-switch-pre.js` 的 `resolveProcedureInjectEnabledPre`，
...
const DEFAULT_PROMPT_LAYERS = Object.freeze({
  ...
  snapshotTier0Title: '[记忆索引 — Tier-0 常驻目录(指引层:每条 1 行=标题·结论·层·状态·日期;要原文用 memory_read_pre / 要检索用 memory_recall_pre)]',
  ...
  snapshotPlanTitle: '[白板 PLAN.md — 项目全貌快照(模型维护,用户在面板可见);**还是上面那种自动骨架、或内容已过时、或理解有实质变化时,用 memory_note_pre(kind=plan) 重写**(重写=整体覆盖,旧版自动归档)]',
  snapshotHandoffTitle: '[最近交接 — 四段式账本(任务状态/目标/已试方案与失败原因/进度与下一步)]',
  snapshotWaterTitle: '[上下文水位提示 — 接近窗口上限]',
  snapshotWaterBody: '上下文水位约 {pct}%(官方公式估算:4 字符≈1 token)。窗口将满,按序执行:①memory_note_pre(kind=handoff) 写交接账本——四段各 ≤5 行:任务状态(一句话+当前产物路径)、目标(可验证的完成标准)、已试方案与失败原因(写成「方案→失败原因」,保留关键报错词)、进度与下一步(可直接执行的第一步,带文件或命令);②memory_note_pre(kind=plan) 刷新白板全貌;③在回复正文明确建议用户开新窗口续接。可分解的子任务优先派子代理隔离上下文。不要跳过账本直接继续对话。',
  ...
  snapshotSlimNote: '[精简注入 · 完整记忆快照每 {n} 轮一次] 本轮为精简版：只给规则、记忆索引目录与日程。上面目录里看得到、但正文未展开的内容，用 memory_recall_pre / memory_read_pre 按需取全文。',
 ...
 *  与 `l0-extract-pre.js` 的 L0 口径一致(单行、可 grep、不整段读)。
...
function diagLogFile() {
  return path.join(dshHome(), 'dsh-auto-memory-pre-diagnose.log')
}
...
class MemoryEngine {
  ...
  async persistConfigPre() {
    try {
      ...
      if (!r.ok) { console.error('[dsh-auto-memory-pre] persistConfigPre failed', r.error); return false }
      return true
    } catch (e) {
      console.error('[dsh-auto-memory-pre] persistConfigPre failed', e)
      return false
    }
  }
  ...
  persistConfigSyncPre() {
    try {
      ...
      if (!r.ok) { console.error('[dsh-auto-memory-pre] persistConfigSyncPre failed', r.error); return false }
      return true
    } catch (e) {
      console.error('[dsh-auto-memory-pre] persistConfigSyncPre failed', e)
      return false
    }
  }
  ...
  skeletonPlanTextPre() {
    ...
    return [
      ...
      '模型在理解本项目全貌后，请用 memory_note_pre(kind=plan) 整体重写本页，写入真实的全貌。',
      '',
      '## 进度与下一步',
      'type:progress',
   ...
   *
   * **为什么把两个门放一个函数**：三条写入路径（工具 `memory_note_pre` / 水位骨架直调 /
  ...
  missingCardsExcerptPre(beforeText, res) {
    ...
    // 卡片边界：**与解析器同口径**（wb-contract-pre.js 的 isCardTitle = /^#{3,}\s+\S/，即 ### 起步；
    ...
    try {
      ...
      if (this.handoffChainEnabledPre() && this.config.waterLevelAutoHandoff !== false && (!rt.waterLevelAutoHandoffDone || compacted || overflowed)) {
        ...
        const notesLines = goalLines.length
          ? goalLines
          : ['(本项目笔记里没有可识别的「目标」小节 · 需原文请用 memory_read_pre(kind=notes) 或 memory_recall_pre)']
        const pct = Math.round(ratio * 100)
        const skeleton = dedupe([
          '## 任务状态', '(系统自动快照 · 估算 ' + estTokens + ' token ≈ 水位 ' + pct + '%' + (compacted ? ' · 检测到 compaction' : '') + ';模型应尽快用 memory_note_pre(kind=plan) 重写白板全貌)', '',
          '## 目标', ...notesLines, '',
        ...
        if (r && r.ok) {
          ...
          const warned = '> [降级] 本骨架由水位触发自动写入，未通过判据门（素材不足）。'
            + '请下一个窗口优先用 memory_note_pre(kind=plan) 重写白板全貌，并补一篇合格账本。\n\n'
            + skeleton
  ...
  refreshRitualPrompt() {
    return [
      ...
      '1. 调用 memory_note_pre(kind=plan, content=<项目白板的最新完整全貌:项目全貌/当前目标/下一步,旧版会自动归档>) 重写 PLAN.md;',
      '2. 调用 memory_note_pre(kind=handoff, content=<四段式交接账本:## 任务状态 / ## 目标 / ## 已试方案与失败原因 / ## 进度与下一步,每段≤5行,下一步必须是可以直接执行的第一步,带文件路径或命令>) 写一篇新账本。',
  ...
  async buildPrevSessionPack(preferSid) {
    try {
      ...
      if (slim.trimmed) body.push('', '(更早内容已截断——完整历史可试 memory_recall_pre(scope=\'sessions\', query=\'关键词\') 检索;该检索需宿主侧开启会话内容检索,条件不满足时会明确报错,此时请改用 scope=\'handoff\' 或直接 read 本文件)')
      try {
  ...
  async buildContinueCarry(preferSid) {
    try {
      ...
      if (ledger) {
        ...
        try {
          const { weightedTrimHandoffLedgerPre } = await import('./handoff-anchor-pre.js')
      ...
      if (pack) {
        ...
        if (pack.transcriptPath) {
          ...
          guide.push('- 完整转写已归档,**不必在接续前通读**:仅在第0-2层不足以推进时再 read;也可用 memory_recall_pre(scope=\'sessions\', query=\'关键词\') 定位片段(前提:宿主侧已开启会话内容检索;该调用返回不可用时改用 scope=\'handoff\' 或直接 read 本转写,不要反复重试)。')
        } else {
          guide.push('- 旧会话 ID: ' + pack.sessionId + '(转写文件生成失败,可用 memory_recall_pre 检索)')
        }
        guide.push('- 历史会话全文检索: memory_recall_pre(scope=\'sessions\', query=\'关键词\') 可跨全部历史会话查证细节 —— **前提:宿主 DSH 侧已开启会话内容检索(出厂默认关闭)且历史日志格式与当前编解码器兼容**;不满足时该调用会明确报错(未部署/格式不兼容),此时改用 scope=\'handoff\' 或 read 转写文件,不要重复重试。')
      ...
      //  等价于自动回退现有行为。
      try {
        const { buildL0IndexPre } = await import('./l0-extract-pre.js')
      ...
      if ((String((this.config || {}).boardMode || '').trim().toLowerCase() === 'graph')) {
        try {
          ...
          if (tagMap) bulkParts.push('【白板 tag 地图(结构化导航)】' + tagMap + NL + '  · 用 memory_expand_pre(tag) 按 tag 展开条目;用 memory_trace_pre(id) 回溯某条的来源与版本链。')
        } catch (eTagMap) {}
      }
      ...
      if ((String((this.config || {}).boardMode || '').trim().toLowerCase() === 'graph')) {
        bulkParts.push('【白板结构化检索(优先于通读第3层)】' + NL +
          ...
          '- 仅当索引查不到所需内容时,再回落 read 第3层转写或 memory_recall_pre(scope=\'handoff\')。')
      }
   ...
   * （`m4-corpus-pre.js:150-166`）。注入装配是每轮热路径，不能顺手触发一次全量重建。
   *
   ...
   * 纯文本处理(零额外 IO、零 LLM 调用 —— 契约 S9;目录生成器 = lib/tier0-catalog-pre.js,
   * 装配 = lib/tier-layer-inject-pre.js),产物挂 `state.tier0LayerText` 供 renderMemoryDynamic 读取。
   *
   ...
   *     放在精简版里收益最高（模型知道**有什么**可读，需要时用 `memory_recall_pre` / `memory_read_pre` 取全文）；
  ...
  renderSlimSnapshotPre(wsHint) {
    try {
      ...
      // 要全文仍走 `memory_read_pre(kind=notes)` / 直接读文件（标题行已提示）。
  ...
  renderMemoryDynamic(context) {
    ...
    if (s.workspaceMap && s.workspaceMap.length) {
      pushPart('otherDynamic', 'workspace-map', '其他工作区记忆(开发/排查时可调用 memory_recall_pre 检索其日志/笔记): ' + s.workspaceMap.join('、'))
    }
    ...
    if (this.external.cache && this.external.cache.length) {
      ...
      if (sess.length) {
        pushPart('otherDynamic', 'external-sessions', '· 历史会话索引: ' + sess.map((x) => x.name + ' ' + x.files.length + ' 个').join(', ') + ' —— 需要时用 memory_recall_pre 检索。')
      }
    }
  ...
  renderMemoryStatic() {
    ...
    lines.push('\n[记忆写入纪律 — 必须遵守]')
    lines.push('- 会话开始:若任务与历史工作/历史决策相关,先回顾以上记忆;**遇到不熟悉的代码、领域或项目时,主动调用 memory_recall_pre 检索本机所有 AI 工具的历史记忆(WorkBuddy/CodeBuddy/Claude Code/Codex/ZCode/Kimi Code/TRAE 会话),或直接读取外部记忆标注的绝对路径文件,不要凭空猜测**。')
    ...
    lines.push('- **交接与白板(长任务续命)**:阶段产出或方向变化时,调用 memory_note_pre(kind=handoff) 写四段式交接账本——任务状态/目标/已试方案与失败原因/进度与下一步,给下一个上下文窗口续命;白板 PLAN.md(人能读的项目规划图,用户在面板实时可见)：**新工作区的骨架由宿主自动落(标题带「自动建立 · 待模型重写」),那只是占位——你理解项目全貌后要用 memory_note_pre(kind=plan) 把它重写成真内容**;此后对项目全貌的理解发生实质变化时同样重写(整体覆盖,旧版自动归档)。账本质量纪律:每段 ≤5 行;失败项写成「方案→失败原因」并保留关键报错词;下一步必须是可直接执行的第一步(带文件路径或命令);不写临时信息。两者会注入到你的动态快照首位,是跨窗口交接的凭据。')
    lines.push('- 只记录有跨会话长期价值的;不记临时信息(搜索结果、临时路径、工具报错)。')
    // ★T6（2026-09-20 用户拍板）：**看板分列机制** —— 模型此前完全不知道有 tag 这回事。
    //   根因：wb-sidecar-pre.js:594-597 的 WB_KANBAN_LANES_PRE_V1 靠 matchTags 匹配
    ...
    lines.push('- **看板分列(白板与账本的可视化归类)**:面板看板按 **5 条泳道**分列 —— `目标` / `进行中` / `失败与弯路` / `进度与下一步` / `版本归档`。'
      ...
      + '注意:这是**白板/账本**的分列规则,与 `memory_note_pre` 的 `kind=plan/handoff/note` 是两回事 —— 后者决定"写到哪个文件",前者决定"面板上排到哪列"。')
    ...
    lines.push('- **★技能库(procedure memory)直写**：你刚跑通一个**多步骤、可重复、下次遇到类似场景能照做**的流程（或踩坑后总结出正确做法）时，调 `memory_procedure_pre` 把它写进去——**这是技能库的唯一模型入口，不写就没有**。'
  ...
  renderReflectionRequest() {
    ...
    return neutralizePromptTemplateVars([
      ...
      '要求:' + styleText,
      '生成后**必须**调用 memory_reflect_pre(date="' + pending.date + '", text=完整反思内容)保存落盘 —— 只呈现不落盘会导致该提示在后续轮次重复出现。',
    ].join('\n'))
  }
  ...
  renderPlanUpdateRequest() {
    try {
      ...
      return neutralizePromptTemplateVars([
        ...
        '① `memory_note_pre(kind=plan, content=…)` —— 用**完整新全貌**重写白板（不是增量补丁），至少覆盖：当前目标 / 已完成 / 进行中 / 下一步第一步（带文件路径或命令）。',
        '② `memory_note_pre(kind=handoff, content=…)` —— 追加一篇四段式账本（任务状态 / 目标 / 已试方案与失败原因 / 进度与下一步），每段 ≤5 行，失败项写成「方案→失败原因」并保留关键报错词。',
  ...
  get docStore() {
    ...
    if (!this._docStore) {
      this._docStore = new MemoryDocumentStore({
        sidecarDir: path.join(dshHome(), 'memory', 'index-pre', 'files'),
      })
    }
  ...
  async applyNoteStatusPre(notesPath, plan, fallbackText) {
    try {
      const { applyStatusToRecordPre, readRecordStatusPre } = await import('./note-status-apply-pre.js')
      let text = ''
      ...
      //   memory_note_pre 只有 `supersedes` 一个通道且硬编码映射到 superseded ⇒ **retracted 模型写不了**。
  ...
  async recall(query, limit = 8, agent, scope = 'all', opts = null) {
    ...
    const { readRecordStatusPre: statusOfNotePre } = await import('./note-status-apply-pre.js')
    ...
    const localWindowNote = limitedSources.length
      ? '\n[本地检索范围受限] ' + limitedSources.join('；') + '。窗口外文件未检索；未命中不代表从未记录，可按日期使用 memory_read_pre 或检索磁盘原文。'
      : ''
    ...
    if (l0Mode) {
      const { buildL0IndexPre, isCurrentPre } = await import('./l0-extract-pre.js')
      ...
      const { groupL0ByLayerPre: groupL0 } = await import('./l0-extract-pre.js')
      ...
      const { isRetrievablePre: retrievableL0, supersededMarkPre: markL0 } = await import('./l0-extract-pre.js')
      const l0Corpus = []
      ...
      // 写侧 `evidence-store-pre.js:126` 的 mkdirSync 只在**首次成功 append** 时建目录，
      // 且 persistEvidence 的两个调用点都带内容前置条件（context-host-pre.js:653/:699）
      ...
      try {
        const evDir = path.join(dshHome(), 'memory', 'evidence-pre', 'events')
        if (!existsSync(evDir)) {
          // 目录不存在 = 从未产生过证据事件 = 合法状态 ⇒ 保持 impMap 空（全体中性），直接跳过。
          // 不抛异常、不写 diag：这不是故障，是「这条臂暂无输入」。
        } else {
          const { scanEvidenceEventsPre, aggregateEvidenceEventsPre } = await import('./evidence-agg-pre.js')
          const { computeImportancePre } = await import('./memory-importance-pre.js')
      ...
      let dateMsOf = null
      try {
        const tp = await import('./temporal-parse-pre.js')
      ...
      if ((opts && opts.fusion) !== 'legacy') {
        try {
          const { rankFusionRRFPre } = await import('./recall-fusion-pre.js')
    ...
    if (scope === 'all' && !l0Mode && typeof this._jsSemanticRank === 'function') {
      try {
        ...
        const { isRetrievablePre: retrievableSem, supersededMarkPre: markSem } = await import('./l0-extract-pre.js')
        // R4-B：分层呈现用的分组函数（单独 import，**不动上面两行的字面形态** —— 既有套件
        // smoke-test-p2 以源码字面量锁定它们，改形态会造成假红）。
        const { groupL0ByLayerPre: groupSem } = await import('./l0-extract-pre.js')
        const { createHash } = await import('node:crypto')
   ...
   *  + recordDigest,memory-anchor-pre.js:82),每条原文=锚点标记之后到下一条标记之前的切片,天然不串条。
   *  搜索面与 L0 语料同源(当前项目日志/反思/项目笔记/用户级记忆);找不到如实回报,不猜测不串条。 */
  async expandMemoryRecordPre(id, agent) {
    const mid = String(id || '').trim()
    if (!/^mem_[0-9a-f]{32}$/.test(mid)) return 'memory_recall_pre: expand 需要合法记忆 id(mem_ + 32 个十六进制字符),收到: ' + mid.slice(0, 64)
    const p = await this.resolvePaths(agent)
    const sources = []
  ...
  // ★IO 全在本节，纯逻辑全在 lib/migrate-pack-pre.js（可单测、可守卫）。
...
_wsSummaryFile() { return path.join(dshHome(), 'memory', 'workspaces-summary.json') }
   ...
   *
   * **为什么不能只用 stats**：`getStats()`（fact-store-pre.js:824）返回的是累计计数，
  ...
  async consolidateTurn(turn, agent) {
    ...
    // 逻辑已抽到 lib/intent-clean-pre.js(纯函数,可回归锁定),此处只做等价调用。
      ...
      try {
        ...
        if (notePts.length) {
          ...
          if (r.ok) {
            ...
            console.log('[dsh-auto-memory-pre] auto-consolidate: NOTE ' + this.budgetRefusalTextPre(r, 'note') + ',已跳过')
          }
        }
        if (userPts.length) {
          ...
          if (r.ok) {
            ...
            console.log('[dsh-auto-memory-pre] auto-consolidate: USER ' + this.budgetRefusalTextPre(r, 'user') + ',已跳过')
          }
        }
        if (written) {
          // 自动沉淀统计(跨天重置)
  ...
  async consolidateMemory(agent, days = 7) {
    ...
    this.state.loadedAt = Date.now()
    if (!written.length) {
      if (skipped.length) return 'memory_consolidate_pre: AI 已提炼,但自动整理未能腾出空间,以下层被跳过: ' + skipped.join('、') + '。'
  ...
  async maintain(days = 30, agent) {
    ...
    for (const log of oldLogs) {
      ...
      } catch (e) { console.error('[dsh-auto-memory-pre] maintain archive failed for ' + log.name + ':', e && e.message ? e.message : e) }
    }
  ...
  return {
    ...
    async execute(args, exec) {
      try {
        ...
        if (name === 'memory_log_pre' || name === 'memory_note_pre' || name === 'memory_user_pre') throw e
...
export function apply(ctx, config) {
  ...
  try {
    if (!process.__damFatalDiagGuard) {
      ...
      process.on('uncaughtException', (err) => { damSafeDiag(process.stderr, '[dsh-auto-memory-pre] uncaughtException: ' + damDiagLine(err)) })
      process.on('exit', (code) => { damSafeDiag(process.stdout, '[dsh-auto-memory-pre] process exit code=' + code) })
    }
  } catch (e) {}
  ...
  try {
    if (!process._dshAutoMemoryRejectionGuard) {
      ...
      process.on('unhandledRejection', (reason) => {
        ...
        damSafeDiag(process.stderr, '[dsh-auto-memory-pre] unhandledRejection guard #' + rejStat.count + ': ' + line)
      })
    }
  } catch (e) {}
...
engine._recallStats = createRecallStatsPre({
  file: () => path.join(dshHome(), 'memory', 'recall-stats-pre.json'),
})
  ...
  // 机制只记录不阻断；留痕自身 fail-soft（见 degrade-pre.js 的元规则）。
...
//   file 传**函数**而非字符串：DSH_HOME 可能在运行期变化，每次落盘重新解析路径。
  ...
  engine._indexSyncHost = createIndexSyncHostPre({
    ...
    switchPersistPath: path.join(dshHome(), 'memory', 'semantic-pre', 'engine-switch-state.json'),
  })
  ...
  {
    const hubDir = path.join(dshHome(), 'memory', 'hub-pre')
    // ★#110（2026-09-22）：hub 持久化 IO 失败**可见化**（实现已抽到 `hub-io-pre.js`，本处只做接线）。
  ...
  engine._storageManager = createStorageManagerPre({
    ...
    io: { sidecarDir: path.join(dshHome(), 'memory', 'index-pre', 'files'), readFileSync },
  ...
  // 治理式写入 notesPath/userFile(autoConsolidate 同款 appendText 原子事务)→ 进 M7 语料。
  {
    ...
    const hubFlushState = { date: '', count: 0, flushed: {} } // flushed[factId]=true;持久化 hub-pre/flush-state.json
    const hubFlushFile = () => path.join(dshHome(), 'memory', 'hub-pre', 'flush-state.json')
    const hubFlushLoad = () => {
    ...
    const hubCorpusLookup = async (memoryId) => {
      try {
        ...
        if (hubCorpusCache.key !== key || !hubCorpusCache.snap) {
          ...
          if (!engine._hubCorpusRegistry) engine._hubCorpusRegistry = new CorpusRegistry({ sidecarDir: path.join(dshHome(), 'memory', 'index-pre', 'files') })
    ...
    const hubFeedTick = () => {
      try {
        ...
        const f = path.join(dshHome(), 'memory', 'semantic-pre', 'judgement-shadow.jsonl')
        let st
  ...
  // 只做检索排序,激活决策仍属两车道策略。下载器落位=发行包布局 lib/models。
  {
    ...
    // 背景：`lib/l0-index-pre.js`（L0 自己的向量索引，增量）此前**全仓零引用**——L0 层没有自己的索引，
    ...
    //     （按层各一份，原因见 lib/l0-index-sync-pre.js 文件头「为什么按层各一份文件」）；
    ...
    engine.l0IndexDir = () => path.join(dshHome(), 'memory', 'semantic-pre', 'l0')
    ...
    engine._pySemanticRank = async (corpusSnap, queryText) => {
      try {
        ...
        if (pp && pp.ws) {
          ...
          if (!engine._pyRankCorpusRegistry) engine._pyRankCorpusRegistry = new CorpusRegistry({ sidecarDir: path.join(dshHome(), 'memory', 'index-pre', 'files') })
    ...
    engine.jsEmitMode = () => {
      ...
      try {
        const raw = JSON.parse(readFileSync(path.join(dshHome(), 'memory', 'semantic-pre', 'embedding-config.json'), 'utf8'))
  ...
  const disposeContext = ctx.systemPrompt.context({
    ...
    text: (context) => {
      try {
        ...
        //     · 记忆写入（memory_log_pre / 自动沉淀）与检索（memory_recall_pre）—— 关注入 ≠ 关记忆
  ...
  // ---------- 工具 ----------
  const tools = [
    defineTool('memory_log_pre', '向当前工作区的 .dsh-memory/ 今日日志追加一条工作记录(append-only,自动建目录/文件)。完成实质性工作(改代码/修 bug/写文档/重构/技术选型/用户偏好约定)后必须调用;有跨会话长期价值的内容在同一轮内一并写入记忆(memory_note_pre 项目/ memory_user_pre 跨项目),progress 与 memory 一起写;不要记录临时信息。**★顺手维护白板(G4/M3,2026-09-17)**:若本次工作让**白板或账本所述与现状不符**(方向变了/阶段完成/旧结论被推翻),请在同一次回复里顺带调用 `memory_note_pre`:项目稳定事实变了用 `kind=plan` 重写白板(旧版自动归档),动态状态变了用 `kind=handoff` 新开一篇账本(append-only,不追改旧账本)。**这是条件触发,不是每回都做**;白板功能关闭时跳过即可,不要因此报错。**调用后必须在本轮回复正文(摘要可见的正文,不是工具调用区)中向用户转述一句:如"已把 X 记入今日日志"**。', {
    ...
    defineTool('memory_note_pre', '更新当前项目长期笔记 .dsh-memory/MEMORY.md(本项目专属的约定、决策、架构要点),或写交接白板。kind=note(默认):action=append 追加一段(自动带日期标题)/action=replace 整体替换(需先基于注入内容或 memory_recall_pre 结果给出完整新内容),本项目笔记有**容量上限**(字符,设置项 noteCapacityChars,默认 24000);**超出时的行为**(你不需要自己控制长度,写就是):①先把较早内容交给 AI 折叠成要点(同一层 10 分钟内只整理一次);②整理失败则**整条原文**归档到 `.dsh-memory/archive/`(信息不丢);③整理后仍超才拒绝(正常不会发生)。kind=handoff:写一篇四段式交接账本(任务状态/目标/已试方案与失败原因/进度与下一步)到 handoff/,阶段产出或方向变化时用,给下一个上下文窗口续命;**账本/白板内容要落进面板看板泳道,须在标题或正文写 tag**:type:goal / type:state / type:dead-end / type:progress。kind=plan:整体写入/重写白板 PLAN.md(人能读的项目全貌规划图)。**首建亦可**——工作区若还没有白板,宿主已自动落了一版骨架,你理解全貌后直接整体重写成真内容即可;对项目全貌的理解发生实质变化时同样重写。旧版自动归档。**别把骨架留在那里当白板**。**结论失效时的两个通道(不要混用)**:supersedes=被更新结论取代(有后继);retract=**当时就做错了、直接撤回**(无后继,本身即教训)。**调用后必须在本轮回复正文中向用户转述:更新了什么**。', {
      ...
      if (args.kind === 'handoff' || args.kind === 'plan') {
        ...
        if (!gateH.ok) return 'memory_note_pre: 写入被记忆卫生闸门拦截(' + WRITE_GATE_REASON[gateH.reason] + '),未写入。请改写为客观陈述后重试。'
        ...
        if (!r || !r.ok) {
          ...
          if (r && r.gate === 'mutation') return 'memory_note_pre: ' + detail
          if (r && r.gate === 'criteria') return 'memory_note_pre: ' + detail
          return 'memory_note_pre: 白板写入失败(' + detail + ')。'
        }
      ...
      if (!acct.ok) return 'memory_note_pre: ' + engine.budgetRefusalTextPre(acct, 'note') + ' 本次未写入(原内容未改动)。'
      let body
      if (replace) {
        body = write
    ...
    defineTool('memory_user_pre', '更新用户级记忆 ~/.dsh/memory/MEMORY.md(跨所有项目的长期规则/偏好,用户明确要求记住时用)。action=append 追加;action=replace 整体替换。有**容量上限**(字符,设置项 userCapacityChars,默认 24000);**超出时的行为**(你不需要自己控制长度,写就是):①先把较早内容交给 AI 折叠成要点(同一层 10 分钟内只整理一次);②整理失败则**整条原文**归档到 `.dsh-memory/archive/`(信息不丢);③整理后仍超才拒绝(正常不会发生)。**调用后必须在本轮回复正文中向用户转述:已记住该规则/偏好**。', {
      ...
      if (!acct.ok) return 'memory_user_pre: ' + engine.budgetRefusalTextPre(acct, 'user') + ' 本次未写入(原内容未改动)。'
      let body
      if (replace) {
        body = write
    ...
    defineTool('memory_status_pre', '查看自动记忆的当前状态:存储位置、各记忆文件大小、今日日志条数、待反思、上次刷新时间。★**什么时候用**:①用户问「记忆系统正常吗/我的记忆存在哪/有多少条」;②怀疑某轮没写进记忆时自查;③新工作区开工前确认路径对不对。★**不该用于**:每轮例行检查(它是只读诊断,不是流程环节);想看记忆**内容**用 memory_read_pre / memory_recall_pre。', {}, async (_args, exec) => {
    ...
    defineTool('memory_reflect_pre', '保存每日反思。★**触发条件严格**:仅在收到框架的「昨日反思待生成」提示、且**已在回复正文中向用户呈现了反思内容之后**才调用——不是你想反思就反思。落盘到 .dsh-memory/reflections/YYYY-MM-DD.md 并标记该日完成(标记后当天不再提示)。date 传**被反思那天的日志日期**(通常是昨天),不是今天。', {
    ...
    defineTool('memory_rules_pre', '用户级硬性约束(每轮无条件注入的 [规则] 段,真源 ~/.dsh/memory/MEMORY.md)的**条目级增删改**。op=list 列出全部条目(0 基序号/来源日期段/文本);op=add 追加;op=update 改写一条;op=remove **真删**一条。★**删/改必须带 expect**(=该条**当前**文本,从 list 原样回填),不符即拒 —— 防索引漂移删错行。★这段**每轮注入且不参与裁剪**,所以它同时是 prompt 成本的主要来源:清理过时条目的首选手段就是本工具的 remove(而不是把整篇用户级记忆用 memory_user_pre replace 重写)。★status 被标 superseded/retracted 的条目自 P9 起**不再进规则段**,但仍留在 list 里(要彻底消失用 remove;要软失效用 memory_note_pre 的 retract/supersedes)。**调用后必须在本轮回复正文中向用户转述改了什么**。', {
      ...
      if (op === 'list') {
        ...
        if (!r0.ok) return 'memory_rules_pre: ' + r0.error
      ...
      if (op === 'add') {
        ...
        if (!gate.ok) return 'memory_rules_pre: 写入被记忆卫生闸门拦截(' + WRITE_GATE_REASON[gate.reason] + '),未写入。请改写为客观陈述后重试。'
        const ra = await applyRuleEditPre(engine, 'add', { text: gate.clean || String(args.content || ''), dateSection: args.dateSection })
        if (!ra.ok) return 'memory_rules_pre: ' + ra.error
      ...
      if (!r.ok) return 'memory_rules_pre: ' + r.error
      return (op === 'remove' ? '已删除' : '已改写') + '第 ' + args.index + ' 条,现共 ' + (r.items || []).length + ' 条。\\n' + r.preview
    }),

    defineTool('memory_consolidate_pre', 'AI 主动维护长期记忆(做梦式固化):读最近 days 天的工作日志,由 AI 发散提炼出有跨会话长期价值的决策/架构/用户偏好,自动写入项目笔记 MEMORY.md(带日期标题)与用户级 MEMORY.md(跨项目规则),并在正文向用户转述固化结果。★**与「自动沉淀」的边界**:每轮对话结束框架会**自动**评估并写日志/升格要点,**你不需要为日常轮次做这件事**;本工具是**你主动发起的加料**——隔一段时间(或一个阶段收尾时)用来做一次更彻底的提炼,读的是**多日日志**、输出进项目笔记与用户级记忆。⇒ 用它做「阶段性固化」,不要用它替代每轮的 memory_log_pre。', {
    ...
    // **工具数 17→18**（无条件注册；P9 新增 memory_rules_pre）。★硬锁联动**六处**（原注释只写四处，
  ...
  //   另有发版侧两表必然漏登记（memory_rules_pre，见 tools/release.mjs 的 T7-e 注与 smoke-test-t7e）。
    // 背景：此前三条记忆线（episodic / semantic / procedural）**全部只有机械生成**，
    //   模型没有任何写入通路。procedural 线的唯一来源是 `memory-hub-pre.js` 的 crossFeed：
    ...
    // 护栏（授权也不放行，见 procedure-store-pre.js promote 注释）：observationOnly 短路 /
    //   必须有 successCriteria / correction 记录阻止。行为全程 fail-soft + diag 留痕。
    defineTool('memory_procedure_pre', '把一个**值得复用的流程**写进 procedure memory（技能库）——这是模型直写通路，取代此前的机械生成。什么时候用：你刚跑通了一个多步骤流程、踩坑后总结出了正确做法、或发现某个操作值得下次照做时。**判据**：有明确步骤、可重复、下次遇到类似场景能直接照做。action=write 进审批列表（保守，推荐先这样）；action=activate 一步到位激活并可被自动召回（仅当你确信它稳定可复用）。写出的条目须含 title + steps，建议一并给 successCriteria（**没有 successCriteria 的条目永远无法晋升**）。', {
      ...
      try {
        const hub = engine._memoryHub
        if (!hub || !hub.stores || !hub.stores.procedures) return 'memory_procedure_pre: 记忆中枢未启用（hub 或 procedure store 不可用），未写入。'
        const procs = hub.stores.procedures
        ...
        if (!title) return 'memory_procedure_pre: title 必填。'
        if (!steps.length) return 'memory_procedure_pre: steps 必填（至少一步）。'
        const cand = {
        ...
        if (!r || !r.ok) return 'memory_procedure_pre: 写入失败（' + String((r && r.reason) || 'unknown') + '）。'
        ...
        if (!pr || !pr.ok || pr.decision !== 'promote') {
          ...
          const EXPLAIN = {
            ...
            'no-success-criteria': '缺少 successCriteria ⇒ 结构上无法晋升。请用 memory_procedure_pre 补写「怎么算跑通」再调 action=activate。',
        ...
        return 'memory_procedure_pre 失败: ' + ((e && e.message) || String(e))
      }
    }),
  ]
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          try {
            const emRaw = JSON.parse(fsMod.readFileSync(path.join(dshHome(), 'memory', 'semantic-pre', 'embedding-config.json'), 'utf8'))
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          const cfgPath = pathMod.join(dshHome(), 'memory', 'semantic-pre', 'embedding-config.json')
          let cfg = {}
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const fsMod = await import('node:fs')
          const pathMod = await import('node:path')
          const file = pathMod.join(dshHome(), 'memory', 'semantic-pre', 'activation-shadow-v2.jsonl')
          ...
          try {
            ...
            const evDir2 = pathMod.join(dshHome(), 'memory', 'evidence-pre', 'events')
            const seenClusters = []
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const fsMod = await import('node:fs')
          const pathMod = await import('node:path')
          const dir = pathMod.join(dshHome(), 'memory', 'semantic-pre')
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          if ((req.method || 'GET') === 'POST') {
            ...
            //   （procedure-store-pre.js:235）且全仓无任何代码能置真，promote 只会返回 decision:'ask'
  ...
  },
  {
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          // ★P9：与模型工具 memory_rules_pre 共用同一写盘口（requireExpect 保持 false ——
