...
class MemoryEngine {
  ...
  // ---------- 检索 ----------
  async recall(query, limit = 8, agent, scope = 'all') {
    ...
    if (scope === 'all' && typeof this._jsSemanticRank === 'function') {
      try {
        const { buildL0IndexPre } = await import('./l0-extract-pre.js')
...
export function apply(ctx, config) {
  ...
  const tools = [
    ...
    defineTool('memory_recall_pre', '检索记忆:本地记忆文件(当前工作区 + 其他所有工作区的每日日志、项目笔记、用户级记忆、反思、交接白板)关键词匹配 + 历史 DSH 会话全文检索(如部署启用)。开发/排查中不懂的、用户提到过去的做法/讨论/决定而当前上下文没有时调用——跨工作区的记忆也能检索到(结果标注来源工作区)。查询必须自包含。scope=handoff 只搜交接白板语料(白板 PLAN+交接账本——接续长任务时先查这里);scope=sessions 只搜历史会话。**检索后必须在本轮回复正文中向用户转述:检索了什么、找到什么(或没找到)**。', {
    ...
    }, async (args, exec) => engine.recall(args.query, args.limit, exec.agent, args.scope || 'all')),
