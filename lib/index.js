...
this._readError = undefined
   ...
   *  ★2026-09-23(issue #134):必须读「旧会话所属工作区」的目录 —— 跨工作区接续时全局
   *  this.state.handoffDir 是**另一个工作区**的目录(原 TODO(separate)),基线与比对全都比错地方。
   *  sid 缺失/解析不出时才退回旧全局口径(保底降级,与 resolvePathsForSession 的 wsBound 语义一致)。 */
  async handoffMaterialStamp(sessionId) {
    try {
      const sid = String(sessionId || '')
      const p = sid ? await this.resolvePathsForSession(sid) : null
      const dir = (p && p.wsBound !== false && p.handoffDir) || this.state.handoffDir || (await this.resolvePaths(undefined)).handoffDir
      const planPath = (p && p.wsBound !== false && p.planPath) || this.state.planPath || path.join(dir, 'PLAN.md')
  ...
  async hostRefreshRitual(oldSid) {
    ...
    st.ritualAt = now
    try {
      const before = await this.handoffMaterialStamp(sid)
      ...
      while (Date.now() - t0 < timeoutMs) {
        ...
        if (canInspect) {
          ...
          const cur = await this.handoffMaterialStamp(sid)
  ...
  async buildPrevSessionPack(preferSid) {
    try {
      ...
      const p = await this.resolvePathsForSession(sid)
      // ★2026-09-19 修复（issue #90，2026-09-23 重做于 issue #134）：转写包落盘目录与序号记账按
      // **包内会话**解析 —— 旧写法 resolvePaths(undefined) 裸调落「GUI 最后轮询的工作区」，多工作区下
      // 包会写进错误工作区的 handoff/。活会话走 resolvePaths(agent)（含 C 类回退与无人值守按会话锁），
      // 冷会话走 registry/持久化头反查；解析不出时内部回落全局口径（与旧行为一致，由 buildContinueCarry
      // 的 wsFallback 留可见痕迹）。
      const NL = String.fromCharCode(10)
  ...
  async buildContinueCarry(preferSid) {
    try {
      ...
      // ★2026-09-19 修复（issue #90，2026-09-23 重做于 issue #134）：白板/账本按**被接续会话**
      // （preferSid=旧会话）解析，不再取全局 default state.ws —— 多窗口并行时旧实现把「GUI 最后轮询
      // 工作区」的 PLAN/账本当接续材料交给新窗口（张冠李戴，终端用户实证接续到错误工作区）。
      // preferSid 为空时 resolvePathsForSession('') 内部回落全局口径（兼容）；解析不出时 wsBound=false
      // 并同样回落全局 —— 此时下方 wsFallback 会留可见痕迹（diag + 面板提示），不再静默。
      const p = await this.resolvePathsForSession(preferSid)
      const NL = String.fromCharCode(10)
