import { continuationHost as makeContinuationHost, armContinuation, continuationState } from './continuation-host.js'
/**
...
import { MemoryDocumentStore, atomicReplace } from './memory-writer.js'
...
class MemoryEngine {
  ...
  async writeHandoffLedger(projectDir, content) {
    ...
    try {
      ...
      const final = '# 交接账本 · ' + this.memToday() + ' ' + nowHm() + '\n\n' + clean
      await this.writeFullRaw(p, final)
      return { ok: true, path: p, clean, final }
    } catch (e) {
      return { ok: false, error: String((e && e.message) || e) }
    }
  }
  ...
  armAutoContinue(agent, wl, opts = null) {
    return armContinuation(this, agent, wl, opts, shouldArmAutoContinuePre, diag)
  }


  async tickAutoContinue() {
    return this.continuationHost().tick()
  }


  continuationHost() {
    return makeContinuationHost(this, {
      directory: path.join(dshHome(), 'memory', 'continuations-v1'),
      atomicReplace,
      diag,
    })
  }
  ...
  async hostAutoContinue() {
    const sid = this._autoContState?.armed?.sessionId
    if (!sid) return { ok: false, error: 'source-session-required' }
    return this.continuationHost().request(sid)
  }


  autoContinueState(selfSid) {
    return continuationState(this, selfSid)
  }


  async decideAutoContinue(action, edgeAt, fromSessionId) {
    return this.continuationHost().decide(action, edgeAt, fromSessionId)
  }
  ...
  async buildContinueCarry(preferSid, material = null) {
    try {
      if (this.config.handoffEnabled === false) return { ok: false, error: 'handoff disabled' }
      const p = await this.resolvePaths(undefined)
      const NL = String.fromCharCode(10)
      const plan = material ? material.planText : await this.readTextSafe(p.planPath)
      let ledger = material ? material.handoffText : await this.readLatestHandoff(p.handoffDir)
      let ledgerFrom = material ? 'ritual' : 'workspace'
      if (!material && !ledger) {
      ...
      const planMt = material ? 0 : await stat(p.planPath).then((s) => Number(s.mtimeMs)).catch(() => 0)
      let ledgerName = material ? path.basename(material.handoffPath) : '', ledgerMt = 0
      if (!material) try {
      ...
      return {
        ...
        planPath: material ? material.planPath : p.planPath,
...
export function apply(ctx, config) {
  ...
  // Fence source steps before other plugin work. Pending input is preserved, never cancelled.
  engine._continuationGuardInstalled = false
  ctx.on('agent/pre-step', (payload, next) => engine.continuationHost().guardSource(payload, next))
  engine._continuationGuardInstalled = true
  ...
  const tools = [
    ...
    defineTool('memory_note', '更新当前项目长期笔记 .dsh-memory/MEMORY.md(本项目专属的约定、决策、架构要点),或写交接白板。kind=note(默认):action=append 追加一段(自动带日期标题)/action=replace 整体替换(需先基于注入内容或 memory_recall 结果给出完整新内容),本项目笔记有**容量上限**(字符,设置项 noteCapacityChars,默认 12000);超出时自动整理——先把较早内容交给 AI 折叠成要点、失败则退回整条归档(原文都进 archive,不丢),整理后仍超才拒绝(正常不会发生)。kind=handoff:写一篇四段式交接账本(任务状态/目标/已试方案与失败原因/进度与下一步)到 handoff/,阶段产出或方向变化时用,给下一个上下文窗口续命。kind=plan:整体重写白板 PLAN.md(人能读的项目全貌规划图),对项目全貌的理解发生实质变化时用,旧版自动归档。**调用后必须在本轮回复正文中向用户转述:更新了什么**。', {
      ...
      if (args.kind === 'handoff' || args.kind === 'plan') {
        ...
        try { await engine.continuationHost().recordWrite(exec, args.kind, r) } catch (eProof) { diag('ritual receipt: ' + String(eProof && eProof.message || eProof)) }
  ...
  const routes = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          const body = await readJsonBody(req).catch(() => ({}))
          const r = await engine.decideAutoContinue(body && body.action, body && body.edgeAt, body && body.fromSessionId)
