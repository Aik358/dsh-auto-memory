...
import { MemoryDocumentStore } from './memory-writer-pre.js'
import { parseAnchors } from './memory-anchor-pre.js'
...
import { scanPluginSubagentSessions, recycleSessions, PLUGIN_LABEL_PREFIX, decodeZstdFrames } from './subagent-gc-pre.js'
import { parseModelWindowsPre, pickWindowPre, findOfficialContextWindowPre, findSessionModelPre, scanPressureSignalsPre } from './water-window-pre.js'
import { buildSourceCatalog, CorpusRegistry, canonicalize } from './m4-corpus-pre.js'
...
const DEFAULT_CONFIG = {
  ...
  /** M-CM1 交接白板:PLAN.md(项目全貌快照,模型重写+旧版归档)+四段式交接账本;注入动态快照首位。false=完全不读写。实验特性:出厂默认关闭,设置 → 自动化 → 交接白板 开启。 */
  ...
  // 2026-09-10 改为默认关。原因:现有触发口径把「预留输出」从分母里扣掉(effectiveWin = win − reserve),
  // 实测在本机路由下「上下文刚过半」就触发,与官方「约 80% 才压缩」相差约 45%,对公开用户是纯浪费。
  // 待口径改为与官方压缩同坐标系后再评估是否翻回(见 docs/internal/NEXT-VERSION-TODO.md)。
...
class MemoryEngine {
  ...
  async writePlanSnapshot(projectDir, content) {
    const dir = path.join(projectDir, 'handoff')
    const planPath = path.join(dir, 'PLAN.md')
    try {
      let archived = ''
      const existing = await this.readTextSafe(planPath)
      if (existing && existing.trim() && existing.trim() !== String(content || '').trim()) {
      ...
      return { ok: true, path: planPath, archived, movedHistory: movedHistory || undefined, historyPath: historyPath || undefined, final: finalContent }
    } catch (e) {
  ...
  async writeHandoffLedger(projectDir, content) {
    const dir = path.join(projectDir, 'handoff')
    ...
    try {
      ...
      await this.writeFullRaw(p, '# 交接账本 · ' + this.memToday() + ' ' + nowHm() + '\n\n' + clean)
      return { ok: true, path: p, clean }
    } catch (e) {
      return { ok: false, error: String((e && e.message) || e) }
    }
  }
  ...
  async waterWindowForSession(sid) {
    ...
    try {
      ...
      if (dir) {
        ...
        const events = raw ? decodeZstdFrames(raw).split('\n').map((l) => { try { return JSON.parse(l) } catch (e) { return null } }).filter(Boolean) : []
  ...
  async checkWaterLevel(agent) {
    try {
      if (this.config.handoffEnabled === false) return
      ...
      try {
        ...
        if (cached && sid && cached.sid === sid && Date.now() - cached.at < 300000 && cached.info) {
          sessModel = cached.info
        } else {
          sessModel = findSessionModelPre(sessionEventsOf(agent && agent.session))
          this._officialWindowCache = { sid, at: Date.now(), info: sessModel }
        }
      } catch (eWin) {}
      ...
      if (win > 0 && !/^manual/.test(winSource) && sessModel.contextWindow > 0) {
        // 官方容量:request/context 的 contextWindow(路由权威值,last-wins)
        win = sessModel.contextWindow
        winSource = 'official-context'
      }
      if (win <= 0) return
      ...
      const effectiveWin = (reserve > 0 && reserve < win * 0.9) ? (win - reserve) : win
      let estTokens = 0
      ...
      const ratio = effectiveWin > 0 && Number.isFinite(estTokens) ? Math.min(estTokens / effectiveWin, 99) : 0
      const threshold = Math.max(Number(this.config.waterLevelThreshold) || 0.75, 0.1)
      // 硬触发:官方已经压缩过/已撞过窗口墙 —— 这一刻必须接续,不依赖比例是否算得准。
      const hard = compacted || overflowed
      ...
      // 双口径(2026-09-10):官方小圈的分母是**声明窗口**(win),我们的分母是**可用额度**(effectiveWin),
      // 两者在本机差 384,000(预留输出)。用户看到「才 50% 就接续」正是因为只看到小圈那个数 ——
      // 把 ring 一并上报,界面就能同时显示「占可用额度 x% · 官方小圈 y%」,不再各说各话。
      this.state.waterLevelRing = (win > 0 && Number.isFinite(estTokens)) ? (estTokens / win) : 0
      ...
      this.state.waterLevelHardTrigger = hard ? (compacted ? 'compaction' : 'overflow') : ''
      ...
      this.rememberWaterRecord(agent && agent.session && agent.session.id ? String(agent.session.id) : '', {
        ratio: armRatio, measuredRatio: ratio, tokens: estTokens, measuredTokens: estTokens, window: effectiveWin,
      ...
      if (!rt.waterLevelAdvised || compacted || overflowed) {
        ...
        diag('water level advisory: ratio=' + ratio.toFixed(2) + (hard ? '→' + armRatio.toFixed(2) + '(hard:' + this.state.waterLevelHardTrigger + ')' : '') +
          ' tokens=' + estTokens + '/' + effectiveWin + '(win=' + win + ' reserve=' + reserve + ')' +
          ' src=' + (winSource || '?') + ' meter=' + meterLayer + (compacted ? ' compaction-detected' : ''))
      }
      if (this.config.waterLevelAutoHandoff !== false && (!rt.waterLevelAutoHandoffDone || compacted || overflowed)) {
        ...
        const notesLines = (await this.readTextSafe(p.notesPath)).split(NL).filter((l) => l.trim()).slice(0, 6)
  ...
  checkWaterLevelAtStep(agent, minGapMs = 0) {
    try {
      ...
      void this.checkWaterLevel(agent).then(() => {
        try {
          const rt2 = this.runtimeFor(agent)
          this.armAutoContinue(agent, { ratio: rt2.waterLevel, tokens: rt2.waterLevelTokens, window: rt2.waterLevelWindow, source: rt2.waterLevelSource }, { awaitIdle: true })
        } catch (eArm) {}
      }).catch(() => {})
  ...
  armAutoContinue(agent, wl, opts = null) {
    try {
      ...
      if (!wl || !(wl.ratio >= (Number(this.config.autoContinueThreshold) || 0.75))) return
  ...
  async hostAutoContinue() {
    ...
    try {
      ...
      const ritualNote = (ritual && ritual.ok) ? 'updated' : ((ritual && ritual.reason) || 'skipped')
      // 落闩:旧会话已接续过,此后不再被 arm(即使 30 分钟冷却已过)
      try { this.markContinuedSession(oldSid, newId) } catch (eL) {}
      st.lastRunAt = Date.now()
      st.lastOk = { sessionId: newId, model: d.model || '', reasoningEffort: d.reasoningEffort || '', workspaceId: d.workspaceId || '', permissionPreset: (perm && perm.ok && perm.preset) ? perm.preset : '', refreshRitual: ritualNote }
      diag('auto-continue host-executed: new session ' + newId + ' model=' + (d.model || 'default') + ' perm=' + ((perm && perm.ok && perm.preset) || ('(' + ((perm && perm.reason) || 'n/a') + ')')) + ' ritual=' + ritualNote)
      return { ok: true, sessionId: newId, model: d.model || '', reasoningEffort: d.reasoningEffort || '', permissionPreset: (perm && perm.ok && perm.preset) ? perm.preset : '', refreshRitual: ritualNote }
    } catch (e) {
  ...
  autoContinueState(selfSid) {
    ...
    return {
      enabled: this.config.autoContinueEnabled !== false && this.config.handoffEnabled !== false,
      ...
      lastOk: st.lastOk ? { sessionId: st.lastOk.sessionId, model: st.lastOk.model, reasoningEffort: st.lastOk.reasoningEffort, workspaceId: st.lastOk.workspaceId, permissionPreset: st.lastOk.permissionPreset, refreshRitual: st.lastOk.refreshRitual, at: st.lastRunAt || 0 } : null,
      error: st.error || '',
    }
  }
  ...
  async hostRefreshRitual(oldSid) {
    ...
    try {
      ...
      while (Date.now() - t0 < timeoutMs) {
        ...
        if (cur && cur !== before) { diag('host refresh ritual: material updated after ' + Math.round((Date.now() - t0) / 1000) + 's'); return { ok: true, waited: 'updated' } }
      }
  ...
  async buildPrevSessionPack(preferSid) {
    try {
      ...
      for (const m of msgs) {
        ...
        if (total + chunk.length > 60000) { body.push('(更早内容已截断——完整历史可用 memory_recall_pre(scope=\'sessions\') 关键词检索)'); break }
        body.push(chunk)
        total += chunk.length
      }
      await mkdir(p.handoffDir, { recursive: true })
      await writeFile(outPath, body.join(NL), 'utf8')
      const tail = msgs.slice(-20).map((m) => m.role + ': ' + m.text.slice(0, 700)).join(NL + '---' + NL)
      diag('prev-session pack built: sid=' + sid + ' msgs=' + msgs.length + ' model=' + (model || '-') + '@' + (provider || '-') + ' effort=' + (reasoningEffort || '-') + ' -> ' + outPath)
      return { sessionId: sid, transcriptPath: outPath, provider, model, reasoningEffort, agentPreset: preset, cwd: cwd || p.ws, tailText: tail, msgCount: msgs.length }
  ...
  async buildContinueCarry(preferSid) {
    try {
      if (this.config.handoffEnabled === false) return { ok: false, error: 'handoff disabled' }
      const p = await this.resolvePaths(undefined)
      const NL = String.fromCharCode(10)
      const plan = await this.readTextSafe(p.planPath)
      let ledger = await this.readLatestHandoff(p.handoffDir)
      ...
      if (pack) {
        ...
        if (pack.transcriptPath) {
          ...
          guide.push('- 完整转写已归档,**不必在接续前通读**:仅在第0-2层不足以推进时再 read;优先用 memory_recall_pre(scope=\'sessions\', query=\'关键词\') 定位片段,避免整篇读入。')
        } else {
          guide.push('- 旧会话 ID: ' + pack.sessionId + '(转写文件生成失败,可用 memory_recall_pre 检索)')
        }
        guide.push('- 历史会话全文检索: memory_recall_pre(scope=\'sessions\', query=\'关键词\') 可跨全部历史会话查证细节。')
      ...
      // 接续序号:统计 handoff 目录里已有的 prev-session 包数量 +1(每次一键/自动接续产出一个)
      let contSeq = 1
  ...
  async handoffPanelData(fileQ, sessionId) {
    ...
    const handoffDir = this.state.handoffDir || (await this.resolvePaths(undefined)).handoffDir
    const planPath = this.state.planPath || path.join(handoffDir, 'PLAN.md')
    if (fileQ) {
      const m = String(fileQ).match(/^(?:PLAN\.md|handoff-\d{8}-\d{6}(?:-[a-z])?\.md|archive\/PLAN-\d{8}-\d{6}\.md)$/)
  ...
  async ensureBudget(agent, layer, text) {
    const p = await this.resolvePaths(agent)
    let acct = this.capacityCheck(layer, await this.layerCharCount(layer, p), text)
    if (acct.ok) return { ok: true, acct }
    let last = null
    let compactedOnce = false
    for (let round = 0; round < 3; round++) {
      const cur = await this.layerCharCount(layer, p)
      const need = cur + String(text == null ? '' : String(text)).length + this.appendOverheadChars()
      last = await this.compactLayer(agent, layer, { needChars: need, round })
      if (!last || !last.ok) break
      compactedOnce = true
      acct = this.capacityCheck(layer, await this.layerCharCount(layer, p), text)
  ...
  async compactLegacyLayer(agent, layer, p, keepBudget, allowFold) {
    ...
    const segs = []
    let curSeg = { title: '(文件头)', body: [] }
    for (const ln of cur.split('\n')) {
      const m = ln.match(/^##\s+(.+)$/)
      if (m) { segs.push(curSeg); curSeg = { title: m[1].trim(), body: [] } } else { curSeg.body.push(ln) }
    }
    segs.push(curSeg)
    const spanOf = (s) => ('## ' + s.title + '\n' + s.body.join('\n')).length
    ...
    const keptText = segs.filter((_, i) => keep[i]).map((s) => '## ' + s.title + '\n' + s.body.join('\n')).join('\n').trim()
  ...
  async recall(query, limit = 8, agent, scope = 'all', opts = null) {
    ...
    // 词法口径与词法臂一致(小写包含);语义分 fail-soft 叠加(_jsSemanticRank,与 P2 语义臂同源)。语料上限 256 对齐 P2。
    if (l0Mode) {
      const { buildL0IndexPre } = await import('./l0-extract-pre.js')
      const l0Corpus = []
      const pushL0 = (label, text) => { if (text) for (const it of buildL0IndexPre(text)) l0Corpus.push({ id: it.id, l0: it.l0, label }) }
      for (const log of logs) pushL0(log.name, await this.readTextSafe(path.join(p.projectDir, log.name)))
      for (const rf of reflections) pushL0('reflections/' + rf.name, await this.readTextSafe(path.join(p.reflectDir, rf.name)))
      pushL0(p.projectDir + '/MEMORY.md', await this.readTextSafe(p.notesPath))
      pushL0('~' + p.userFile.slice(homedir().length), await this.readTextSafe(p.userFile))
      if (l0Corpus.length > 256) l0Corpus.length = 256
      ...
      if ((opts && opts.fusion) !== 'legacy') {
        try {
          ...
          l0Top = rankFusionRRFPre(l0Hits.map((c) => ({ memoryId: c.id, dense: typeof c.sem === 'number' ? c.sem * (0.5 + 0.5 * (impMap.get(c.id) != null ? impMap.get(c.id) : 0.5)) : null, lex: c.lex, temp: tempFieldOf(c) })))
            .map((f) => byId.get(f.memoryId))
            .filter(Boolean)
      ...
      if (l0Top.length) {
        ...
        for (const c of l0Top) {
          ...
          out.push('· [' + c.id + '] ×' + sc + ' ' + (c.reason || '语义') + ' ' + c.label + ' — ' + c.l0)
        }
      }
    } else {
      for (const log of logs) {
      ...
      try {
        ...
        const semanticArm = async (env) => {
          ...
          for (const src of env.sources) {
            ...
            if (!text) continue
            const items = env.buildL0IndexPre(text)
            for (const it of items) corpus.push({ memoryId: it.id, text: it.l0, label: src.label })
            if (corpus.length >= env.maxRecords) break
          }
          if (!corpus.length) return []
          corpus.length = Math.min(corpus.length, env.maxRecords)
        ...
        const semHits = await semanticArm({ sources: semSources, buildL0IndexPre, createHash, query, limit, maxRecords: 256, minScore: 0.5 })
        if (semHits.length) {
      ...
      if (hasAgent) {
        ...
        try { void engine.checkWaterLevel(agent).then(function () {
          ...
          try {
            ...
            engine.armAutoContinue(agent, { ratio: rt2.waterLevel, tokens: rt2.waterLevelTokens, window: rt2.waterLevelWindow, source: rt2.waterLevelSource })
          } catch (eArm) {}
        }) } catch (e) {}
  ...
  const tools = [
    ...
    defineTool('memory_note_pre', '更新当前项目长期笔记 .dsh-memory/MEMORY.md(本项目专属的约定、决策、架构要点),或写交接白板。kind=note(默认):action=append 追加一段(自动带日期标题)/action=replace 整体替换(需先基于注入内容或 memory_recall_pre 结果给出完整新内容),本项目笔记有**容量上限**(字符,设置项 noteCapacityChars,默认 12000);超出时自动整理——先把较早内容交给 AI 折叠成要点、失败则退回整条归档(原文都进 archive,不丢),整理后仍超才拒绝(正常不会发生)。kind=handoff:写一篇四段式交接账本(任务状态/目标/已试方案与失败原因/进度与下一步)到 handoff/,阶段产出或方向变化时用,给下一个上下文窗口续命。kind=plan:整体重写白板 PLAN.md(人能读的项目全貌规划图),对项目全貌的理解发生实质变化时用,旧版自动归档。**调用后必须在本轮回复正文中向用户转述:更新了什么**。', {
      ...
      if (args.kind === 'handoff' || args.kind === 'plan') {
        ...
        if (!r || !r.ok) return 'memory_note_pre: 白板写入失败(' + ((r && r.error) || '未知') + ')。'
    ...
    defineTool('memory_user_pre', '更新用户级记忆 ~/.dsh/memory/MEMORY.md(跨所有项目的长期规则/偏好,用户明确要求记住时用)。action=append 追加;action=replace 整体替换。有**容量上限**(字符,设置项 userCapacityChars,默认 12000);超出时自动整理——先把较早内容交给 AI 折叠成要点、失败则退回整条归档(原文都进 archive,不丢),整理后仍超才拒绝(正常不会发生)。**调用后必须在本轮回复正文中向用户转述:已记住该规则/偏好**。', {
    ...
    defineTool('memory_external_pre', '查看/接入其他 AI 工具(AI 助手/CodeBuddy/Claude Code/Codex/ZCode/Kimi Code/TRAE/项目约定文件)的记忆。action=list 列出全部检测到的外部记忆源(路径/大小/预览/会话数);action=import 以纯链接模式接入(source 为源 id,target=project 接进项目笔记 / user 接进用户级记忆,只记录源文件路径指针、不写入内容,需要时按需读取;防止外部脏内容混入本地记忆)。首次在新工作区工作、或用户提到其他软件里做过的事时调用。', {
