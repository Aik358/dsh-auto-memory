...
class MemoryEngine {
  ...
  armAutoContinue(agent, wl, opts = null) {
    try {
      ...
      deferCount: 0,
      // 2026-09-14 (issue#35):agree 若在回合活跃时点击,deferredAt 记录最近一次顺延
      // (仅诊断展示用;defer 计数与 tick 路径共用 armed.deferCount)
      deferredAt: 0,
    }
  ...
  async hostAutoContinue() {
    ...
    st.error = ''
    try {
      // 2026-09-14 (issue#35 Defect B 透明化):官方 sessionController 没有停止回合的 API
      // (prompt 仅 queue/steer 两种模式且都不打断,见 docs/internal 研究实录;M4 契约亦
      // 明确不取消当前模型请求),插件无法「先停旧回合再建新会话」。 agree 侧已补 awaitIdle
      // defer 闸(与 tick 一致)把执行推到空闲边界,但 defer 满 5 次仍会执行(可用性优先),
      // 届时若旧回合仍活跃就存在两会话并行 —— 无法根除,至少如实记录,让并行可被事后审计。
      const busyAtExec = (() => { try { const la = Number(this._globalLastActiveAt) || 0; return la > 0 && Date.now() - la < 30000 } catch (_) { return false } })()
      if (busyAtExec) diag('auto-continue executing while turn still active (parallel risk, no stop API): sid=' + (armed && armed.sessionId || ''))
      ...
      st.lastOk = { sessionId: newId, model: d.model || '', reasoningEffort: d.reasoningEffort || '', workspaceId: d.workspaceId || '', permissionPreset: (perm && perm.ok && perm.preset) ? perm.preset : '', refreshRitual: ritualNote, busyAtExec }
      diag('auto-continue host-executed: new session ' + newId + ' model=' + (d.model || 'default') + ' perm=' + ((perm && perm.ok && perm.preset) || ('(' + ((perm && perm.reason) || 'n/a') + ')')) + ' ritual=' + ritualNote + (busyAtExec ? ' busy=turn-active' : ''))
  ...
  autoContinueState(selfSid) {
    ...
    const armedRaw = st.armed ? {
      ...
      // 2026-09-14 (issue#35):确认卡据此显示「回合进行中,已顺延 N 次」
      awaitIdle: !!st.armed.awaitIdle,
      deferCount: st.armed.deferCount || 0,
      deferredAt: st.armed.deferredAt || 0,
    } : null
    ...
    return {
      ...
      lastOk: st.lastOk ? { sessionId: st.lastOk.sessionId, model: st.lastOk.model, reasoningEffort: st.lastOk.reasoningEffort, workspaceId: st.lastOk.workspaceId, permissionPreset: st.lastOk.permissionPreset, refreshRitual: st.lastOk.refreshRitual, busyAtExec: !!st.lastOk.busyAtExec, at: st.lastRunAt || 0 } : null,
      error: st.error || '',
    }
  }
  ...
  async decideAutoContinue(action, edgeAt) {
    ...
    if (action === 'agree') {
      ...
      // 2026-09-14 (issue#35 Defect A):agree 路径此前完全没有回合活跃检查 —— pre-step 侧
      // armed(awaitIdle=true)的确认卡在回合仍活跃时点「同意」立即执行接续,awaitIdle 标记
      // 被完全忽略;旧回合继续跑原任务、新会话同时按交接材料跑同一批任务,两会话并行。
      // 现补齐与 tickAutoContinue 完全相同的 defer 闸(30s 内有活动 → 顺延 20s 再评估,
      // 最多 5 次,超过仍执行 —— 与 tick 的升级语义一致,避免长任务永不接续):
      // 返回 deferred=true,客户端保留确认卡并提示「回合进行中,将在空闲后接续」;
      // armed 不清除,倒计时顺延后由心跳 tick 在空闲时自然接管执行。
      if (st.armed.awaitIdle) {
        const lastActiveAgree = Number(this._globalLastActiveAt) || 0
        const busyAgree = lastActiveAgree > 0 && Date.now() - lastActiveAgree < 30000
        if (busyAgree && (st.armed.deferCount || 0) < 5) {
          st.armed.deferCount = (st.armed.deferCount || 0) + 1
          st.armed.deferredAt = Date.now()
          st.armed.expiresAt = Date.now() + 20000
          diag('auto-continue agree deferred (turn still active): count=' + st.armed.deferCount)
          return { ok: true, deferred: true, deferCount: st.armed.deferCount }
        }
      }
  ...
  /** 旧会话 assistant 消息计数(2026-09-14 issue#35 Defect C,刷新仪式完成第二判据)。
   *  数 session 事件流里的 assistant/message 事件条数;取不到 agent/session/事件 → null
   *  (调用方据此退回 stamp-only 判据)。刻意不引用模块级 helper(extractSessionMessages
   *  等):hostRefreshRitual 的测试用 new Function 从源码抽方法体拼假 engine,
   *  方法体自包含才能被单独抽取验证。 */
  sessionAssistantCount(sid) {
    try {
      const ag = this.agentForSessionId(sid)
      const s = ag && ag.session
      if (!s) return null
      let events = []
      if (Array.isArray(s.events)) events = s.events
      else if (typeof s.snapshotEvents === 'function') { const snap = s.snapshotEvents(); if (Array.isArray(snap)) events = snap }
      let n = 0
      for (let i = 0; i < events.length; i++) { const ev = events[i]; if (ev && ev.type === 'assistant/message') n++ }
      return n
    } catch (e) { return null }
  }
  ...
  async hostRefreshRitual(oldSid) {
    ...
    try {
      ...
      // 2026-09-14 (issue#35 Defect C):材料戳变化 ≠ 仪式被模型执行 —— 旧回合仍在运行时,
      // 水位检查自身(waterLevelAutoHandoff)也会写账本/PLAN,stamp 随之变化即被误判为
      // 「仪式完成」(实录:旧回合仍在等 API 响应,10s 后误报 updated)。补第二判据:
      // 仪式 prompt 之后旧会话必须新增了 assistant 消息(模型真的处理了仪式)。
      // agent 取不到(宿主重启/冷会话/会话中途消失)→ 计数基线为 null → 退回 stamp-only
      // 判据(fail-soft,行为与旧版一致)。sessionAssistantCount 自包含实现,便于测试抽取。
      let asstBase = null
      try { asstBase = this.sessionAssistantCount(sid) } catch (_) { asstBase = null }
      const reqId = (typeof randomUUID === 'function')
        ? randomUUID()
      ...
      // _ritualTimeoutMs:测试覆盖用(与 _ritualPollMs 同款注入点);生产走配置/默认 90s
      const timeoutMs = Number(this._ritualTimeoutMs) > 0 ? Number(this._ritualTimeoutMs) : (sec >= 15 && sec <= 600 ? sec : 90) * 1000
      const t0 = Date.now()
      ...
      while (Date.now() - t0 < timeoutMs) {
        ...
        if (cur && cur !== before) {
          if (asstBase != null) {
            let asstNow = null
            try { asstNow = this.sessionAssistantCount(sid) } catch (_) {}
            if (asstNow != null && asstNow <= asstBase) continue // 材料变了但没有新 assistant 消息 → 非仪式产出(水位检查自写),继续等
          }
          diag('host refresh ritual: material updated after ' + Math.round((Date.now() - t0) / 1000) + 's')
