...
import { loadAndVerifyPolicy, decideActivationV2, lexicalContainment, checkPolicyPairConsistencyPre } from './semantic-decide.js'
  ...
  async debugInfo() {
    ...
    return {
      ...
      associativeMemory: {
        ...
        // issue #109：冻结参数双份（lib/policies ↔ python/policies）摘要不一致时的可见事实。
        // null = 一致或无从对照；非空 = 两侧对同一输入可能给出不同激活决策。
        policyPairDrift: this._policyPairDrift || null,
  ...
  // 只做检索排序,激活决策仍属两车道策略。下载器落位=发行包布局 lib/models。
  {
    ...
    engine.ensureJsDecide = () => {
      try {
        ...
        // issue #109：冻结参数在 lib/policies 与 python/policies 各存一份，JS 优先读前者、Python
        // 只读自己那份，而 loadAndVerifyPolicy 只验**单份内部**自洽 ⇒ 重新标定只改一份时两侧对
        // 同一输入给出不同决策，运行时零告警。这里做逐文件摘要对照，把漂移变成可见事实
        // （不改优先级、不拒绝加载 —— 是否硬失败属参数治理决策，留给维护者按 #117 之后的口径定）。
        try {
          const pair = checkPolicyPairConsistencyPre(
            ['activation_policy_v2.json', 'recall_intent_lr_v1.json'],
            [jsPolicyDir, pyPolicyDir],
          )
          engine._policyPairDrift = pair.ok ? null : pair.drift
          if (!pair.ok) {
            diag('[降级] policy 双份不一致（JS 档与 Python 档可能对同一输入给出不同激活决策）: '
              + pair.drift.map((d) => d.name + ' ' + d.digestA + '≠' + d.digestB).join('; '))
          }
        } catch (ePair) { engine._policyPairDrift = null }
