...
const DEFAULT_CONFIG = {
  ...
  /** M-CM4 水位感知:会话消息按官方 token-meter 公式(4 字符≈1 token+每消息 4)估算 token;0=关闭。默认 65536(128K 窗口的保守半量)。 */
  waterLevelWindowTokens: 65536,
...
class MemoryEngine {
  ...
  async checkWaterLevel(agent) {
    try {
      const win = Number(this.config.waterLevelWindowTokens) || 0
      if (win <= 0 || this.config.handoffEnabled === false) return
      ...
      if (!rt.waterLevelAdvised || compacted) {
        ...
        diag('water level advisory: ratio=' + ratio.toFixed(2) + ' tokens=' + estTokens + '/' + win + (compacted ? ' compaction-detected' : ''))
      }
  ...
  async handoffPanelData(fileQ) {
    ...
    return { enabled: true, planPath, plan: await this.readTextSafe(planPath), planVersions: versions.slice(0, 30), ledgers: ledgers.slice(0, 60) }
  }
