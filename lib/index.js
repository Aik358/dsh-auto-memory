...
import { readFile, writeFile, mkdir, readdir, stat, rm, copyFile, appendFile, rename, unlink } from 'node:fs/promises'
...
import { createDegradeSinkPre, persistDegradeLedgerPre, createQuotaProbePre, deriveQuotaVerdictPre } from './degrade.js'
...
import { scanPluginSubagentSessions, recycleSessions, PLUGIN_LABEL_PREFIX, decodeZstdFramesHead } from './subagent-gc.js'
import { parseModelWindowsPre, pickWindowPre, findSessionModelPre, scanPressureSignalsPre, reusableWindowCachePre, shouldArmAutoContinuePre } from './water-window.js'
...
import { parseWhiteboardPre, extractProtectedRegionsPre, checkHandoffCriteriaPre, checkPlanCriteriaPre, criteriaRefusalTextPre } from './wb-contract.js'
...
import { buildSidecarEntryPre, rebuildSidecarIndexPre, expandByTagPre, traceByIdPre, applyAnchorsPre, wbRefPre, buildKanbanPre, buildSectionCardsPre, buildKanbanMatrixPre } from './wb-sidecar.js'
// P6A 规则层（规则类与参考类分开措辞；规则真源 = 既有用户级记忆，不新增 RULES.md）
import { extractRulesLayerPre, renderRulesSectionPre, RULES_SECTION_TITLE_V1, RULES_SECTION_GUIDE_V1, LOG_KINDS_V1 } from './rules-layer.js'
...
import { buildIndex as buildMemoryIndexFile, INDEX_MAX_FILE_BYTES } from './memory-index.js'
...
_wsSummaryFile() { return path.join(dshHome(), 'memory', 'workspaces-summary.json') }
  ...
  async syncDirProbe(dir) {
    ...
    if (exists) {
      const probe = path.join(expanded, '.dam-sync-probe-' + randomUUID())
      try { await writeFile(probe, 'ok', { flag: 'wx' }); await unlink(probe); writable = true } catch (_) { writable = false }
    }
...
//   file 传**函数**而非字符串：DSH_HOME 可能在运行期变化，每次落盘重新解析路径。
  ...
  // ---------- 注册与清理 ----------
  const disposers = []
  disposers.push(disposeContext, disposeSection, disposeTailSurface, () => {
