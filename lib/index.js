...
import { createJsSemanticEnginePre, createSemanticDownloaderPre, fuseD6Pre, E5_SMALL_Q8_MANIFEST_PRE_V1 } from './semantic-js-pre.js'
...
class MemoryEngine {
  constructor() {
    ...
    this._ownSubagents = new Set() // 本插件 spawn 的子代理(防套娃:其生命周期事件零处理)
  ...
  async runSubagent(text, label, agent, timeoutMs) {
    ...
    try {
      ...
      // 配置性错误熔断(问题③):UNKNOWN_MODEL / 无配置模型 / provider 缺失——重试无意义,30 分钟内不再 spawn
      if (/UNKNOWN_MODEL|has no configured model|provider|apiproxy|not configured/i.test(em)) {
