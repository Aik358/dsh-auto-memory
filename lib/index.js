...
const DEFAULT_PROMPT_LAYERS = Object.freeze({
  ...
  snapshotPlanTitle: '[白板 PLAN.md — 项目全貌快照(模型维护,用户在面板可见);内容过时/理解有实质变化时,用 memory_note_pre(kind=plan) 重写]',
...
class MemoryEngine {
  ...
  renderMemoryDynamic(context) {
    ...
    // 开关 `rulesLayeringMode`（**默认 'off'**）= 新旧并存 + 一键回退：关掉即完全回到旧行为。
  ...
  renderMemoryStatic() {
    ...
    lines.push('- **交接与白板(长任务续命)**:阶段产出或方向变化时,调用 memory_note_pre(kind=handoff) 写四段式交接账本——任务状态/目标/已试方案与失败原因/进度与下一步,给下一个上下文窗口续命;对项目全貌的理解发生实质变化时,用 memory_note_pre(kind=plan) 重写白板 PLAN.md(人能读的项目规划图,旧版自动归档,用户在面板实时可见)。账本质量纪律:每段 ≤5 行;失败项写成「方案→失败原因」并保留关键报错词;下一步必须是可直接执行的第一步(带文件路径或命令);不写临时信息。两者会注入到你的动态快照首位,是跨窗口交接的凭据。')
