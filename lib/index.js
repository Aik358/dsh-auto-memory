...
class MemoryEngine {
  ...
  /** M-CM4·精修(对齐官方 token-meter,公式=dsh-context 忠实移植版):会话消息估算 token / waterLevelWindowTokens。越阈值或检测到 compaction 事件→advisory+自动写抽取性骨架账本。 */
  async checkWaterLevel(agent) {
    try {
      if (this.config.handoffEnabled === false) return
      const winInfo = await this.resolveWaterWindow()
      const win = Number(winInfo && winInfo.window) || 0
      if (win <= 0) return
      const messages = extractSessionMessages(agent) || []
      const estTokens = this.estimateSessionTokens(messages)
      ...
      this.state.waterLevelSource = winInfo.source
      ...
      if (!rt.waterLevelAdvised || compacted) {
        ...
        diag('water level advisory: ratio=' + ratio.toFixed(2) + ' tokens=' + estTokens + '/' + win + ' src=' + (winInfo.source || '?') + (compacted ? ' compaction-detected' : ''))
      }
