...
class MemoryEngine {
  ...
  async consolidateTurn(turn, agent) {
    ...
    for (const m of messages) {
      // 2026-08-30 intent 提纯:tool/result 在协议里也是 user 角色——只认真人消息事件,
      // 否则「最后一条 user」常是工具回包(JSON/行号文本),污染沉淀与 episode intent。
      // 另:skip harness 合成的上下文注入消息(M5 snapshot,以 "Current runtime context."
      // 开头/含 <memory_system>),真人问题在它之前的 user/message 事件里。
      if (m.role === 'user' && m.eventType === 'user/message') {
        if (/^current runtime context\./i.test(m.text) || m.text.includes('<memory_system>')) continue
        userText = m.text
      } else if (m.role === 'assistant' && m.text) assistantText = m.text
    }
    // 剥离注入的记忆快照块:真问题在最后一个 </memory_system> 之后(注入拼进同一消息时)
    if (userText) {
      const idx = userText.lastIndexOf('</memory_system>')
      if (idx >= 0) userText = userText.slice(idx + '</memory_system>'.length).trim()
    }
...
export function apply(ctx, config) {
  ...
  // 治理式写入 notesPath/userFile(autoConsolidate 同款 appendText 原子事务)→ 进 M7 语料。
  {
    ...
    setTimeout(() => { hubFeedTick(); void hubFlushTick() }, 90 * 1000)
  ...
  const routes = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        // G-02 前置(append-only 审批队列;loopback POST):用户 A/P/S/H/E 判定落盘,不直接改任何策略
        if (!isLoopbackRequest(req)) return writeJson(res, 403, { error: 'forbidden: loopback-only' })
        if ((req.method || 'POST') !== 'POST') return writeJson(res, 405, { error: 'method not allowed' })
        const body = await readJsonBody(req).catch(() => ({}))
        try {
