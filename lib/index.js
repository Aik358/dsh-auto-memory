...
import { workspaceSlugPre as migrateSlugPre, buildPackPre, validatePackPre, planImportPre, mergeSummaryRecordPre, MIGRATE_PACK_FORMAT_PRE } from './migrate-pack-pre.js'
...
class MemoryEngine {
  ...
  async kanbanBoardData(sessionId, opts = {}) {
    ...
    try {
      ...
      const docs = await this._collectWhiteboardDocsPre(projectDir)
   ...
   */
  async _collectWhiteboardDocsPre(projectDir) {
    const sideDir = path.join(projectDir, 'handoff')
    const docs = []
    ...
    const archNames = await readdir(path.join(sideDir, 'archive')).catch(() => [])
...
_wsSummaryFile() { return path.join(dshHome(), 'memory', 'workspaces-summary.json') }
  ...
  /** 导入：执行。★顺序 = 重算计划 → 备份 → 落盘 → 合并 summary（任一步失败即中止并回报）。 */
  ...
  const routes = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        const body = await readJsonBody(req).catch(() => ({}))
        try {
          const ws = String((body && body.ws) || (engine._lastAgent && engine._lastAgent.cwd) || '')
