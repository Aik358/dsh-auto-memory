...
import { parseModelWindowsPre, pickWindowPre, findOfficialContextWindowPre, findSessionModelPre, scanPressureSignalsPre, shouldArmAutoContinuePre } from './water-window.js'
...
class MemoryEngine {
  ...
  async checkWaterLevel(agent) {
    try {
      ...
      // arm 侧据此判断能否按比例触发(见 shouldArmAutoContinuePre)。与 state 成对维护。
      ...
      this.state.waterLevelModelKnown = rt.waterLevelModelKnown
      this.state.waterLevelHard = hard
