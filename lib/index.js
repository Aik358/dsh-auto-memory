...
import { parseModelWindowsPre, pickWindowPre, findOfficialContextWindowPre, findSessionModelPre } from './water-window-pre.js'
...
class MemoryEngine {
  ...
  async waterWindowForSession(sid) {
    ...
    try {
      ...
      if (dir) {
        const raw = await readFile(path.join(dir, 'session.jsonl.zstd')).catch(() => null)
  ...
  async checkWaterLevel(agent) {
    try {
      ...
      try {
        ...
        for (let i = Math.max(0, events.length - 64); i < events.length; i++) {
          ...
          if (t === 'compaction/summary' || t === 'compaction/prune') {
            const seq = Number(events[i].seq) || i + 1
            if (seq > (Number(rt.lastCompactionSeen) || 0)) { compacted = true; rt.lastCompactionSeen = seq }
          }
        }
      } catch (e) {}
      // 旧实现硬截断到 1.5 → 任何超额水位都显示成 150%,掩盖真实占用(用户实测 761692/131072 被显示为 150%)。
      // 现在如实上报,仅做防溢出的 99 倍上限。
      const ratio = win > 0 && Number.isFinite(estTokens) ? Math.min(estTokens / win, 99) : 0
      rt.waterLevel = ratio
      rt.waterLevelTokens = estTokens
      this.state.waterLevelRatio = ratio
      this.state.waterLevelTokens = estTokens
      this.state.waterLevelWindow = win
      ...
      this.rememberWaterRecord(agent && agent.session && agent.session.id ? String(agent.session.id) : '', {
        ratio, tokens: estTokens, window: win, source: winSource, meter: meterLayer,
        model: this.state.waterLevelModel || '', threshold, at: Date.now(), live: true,
      })
      const over = ratio >= threshold || (ratio >= 0.5 && compacted)
      if (!over) return
      if (!rt.waterLevelAdvised || compacted) {
        rt.waterLevelAdvised = true
        diag('water level advisory: ratio=' + ratio.toFixed(2) + ' tokens=' + estTokens + '/' + win + ' src=' + (winSource || '?') + ' meter=' + meterLayer + (compacted ? ' compaction-detected' : ''))
      }
      if (this.config.waterLevelAutoHandoff !== false && (!rt.waterLevelAutoHandoffDone || compacted)) {
   ...
   *  节流:同一 agent 至少间隔 minGapMs(默认 5s),避免每个 step 都扫会话事件/读 tokenMeter。 */
  checkWaterLevelAtStep(agent, minGapMs = 5000) {
    try {
      if (this.config.handoffEnabled === false) return
      ...
      void this.checkWaterLevel(agent)
  ...
  armAutoContinue(agent, wl) {
    try {
      ...
      diag('auto-continue armed: sid=' + sid + ' ratio=' + st.armed.ratio.toFixed(2) + ' expiresAt=' + st.armed.expiresAt)
  ...
  async hostAutoContinue() {
    ...
    try {
      ...
      const d = await this.buildContinueCarry()
      ...
      if (typeof sc.prompt === 'function') {
        ...
        await sc.prompt({ sessionId: newId, content: [{ type: 'text', text: d.carryText }], mode: 'queue' }, signal)
      }
      st.lastRunAt = Date.now()
      st.lastOk = { sessionId: newId, model: d.model || '', reasoningEffort: d.reasoningEffort || '', workspaceId: d.workspaceId || '' }
      diag('auto-continue host-executed: new session ' + newId + ' model=' + (d.model || 'default'))
      return { ok: true, sessionId: newId, model: d.model || '', reasoningEffort: d.reasoningEffort || '' }
    } catch (e) {
  ...
  async buildPrevSessionPack() {
    try {
      ...
      let file = null
      for (const w of await readdir(sessionsDir).catch(() => [])) {
        for (const cand of [path.join(sessionsDir, w, sid, 'session.jsonl.zstd'), path.join(sessionsDir, w, sid, 'session.jsonl')]) {
          try { const st = await stat(cand); if (st.isFile()) { file = cand; break } } catch (e) {}
        }
        if (file) break
      }
  ...
  async buildContinueCarry() {
    try {
      ...
      const pack = await this.buildPrevSessionPack()
  ...
  async recall(query, limit = 8, agent, scope = 'all', opts = null) {
    ...
    if (l0Mode) {
      ...
      if (typeof this._jsSemanticRank === 'function') {
        try {
          ...
          const rank = await this._jsSemanticRank({ memoryIndexVersion: miv, records: l0Corpus.map((c) => ({ memoryId: c.id, text: c.l0 })) }, query)
          const semScores = rank && rank.scores ? rank.scores : null
  ...
  async runSubagent(text, label, agent, timeoutMs) {
    ...
    try {
      ...
      if (run && run.agent) { try { await this.recycleSubagentSession(run.agent) } catch (e3) {} }
    }
  }
  ...
  async subagentGcSweep(force) {
    try {
      ...
      const now = Date.now()
      // 无论保留期多长,最近 10 分钟的一律不动(可能与正在运行的子代理竞争)
      const floorMs = Math.max(keepDays * 24 * 3600 * 1000, 10 * 60 * 1000)
      const eligible = scan.candidates.filter((c) => now - c.mtimeMs >= floorMs)
  ...
  const routes = [
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        if ((req.method || 'POST') !== 'POST') return writeJson(res, 405, { error: 'method not allowed' })
        try {
          const r = await engine.buildContinueCarry()
