...
class MemoryEngine {
      ...
      if (text) {
        ...
        if (ai >= 0) {
          for (let i = ai + 1; i < Math.min(lines.length, ai + 6); i++) {
            ...
            if (!m) break
            if (m[1] === 'provider') provider = m[2].trim()
            if (m[1] === 'model') { model = m[2].trim(); break }
          }
        }
        if (model) {
          const windowsByRoute = {}
          let currentProvider = ''
          let currentId = null
          for (let i = 0; i < lines.length; i++) {
            const pm = lines[i].match(/^ {4}([^ #][^:]*): *$/)
            if (pm) { currentProvider = pm[1]; currentId = null; continue }
            const im = lines[i].match(/^ *- id: *(.+?) *$/)
            if (im) { currentId = im[1]; continue }
            const cm = lines[i].match(/^ +contextWindow: *([0-9]+) *$/)
            if (cm && currentId) { const route = currentProvider + '/' + currentId; if (windowsByRoute[route] === undefined) windowsByRoute[route] = Number(cm[1]) }
          }
          if (windowsByRoute[(provider || '') + '/' + model] !== undefined) value = windowsByRoute[provider + '/' + model]
          else { for (const route of Object.keys(windowsByRoute)) if (route.endsWith('/' + model)) { value = windowsByRoute[route]; break } }
        }
      }
      const result = value !== null
        ? { window: value, source: 'auto' + (provider ? ':' + provider : '') + (model ? '/' + model : ''), model: model || '' }
        : { window: 131072, source: 'fallback', model: model || '' }
      this._waterWindowCache = { at: now, value: result }
      return result
  ...
  async checkWaterLevel(agent) {
    try {
      ...
      const winInfo = await this.resolveWaterWindow()
      let win = Number(winInfo && winInfo.window) || 0
      let winSource = (winInfo && winInfo.source) || ''
      if (win > 0 && !/^manual/.test(winSource)) {
        // 官方容量:最新一条 request/context 事件的 contextWindow(路由容量,last-wins)
        try {
          const evts = sessionEventsOf(agent && agent.session)
          for (let i = evts.length - 1; i >= Math.max(0, evts.length - 256); i--) {
            const ev = evts[i]
            if (ev && ev.type === 'request/context') {
              const cw = Number(ev && ev.data && ev.data.contextWindow)
              if (Number.isFinite(cw) && cw > 0) { win = cw; winSource = 'official-context' }
              break
            }
          }
        } catch (eWin) {}
      ...
      const ratio = Math.min(estTokens / win, 1.5)
      rt.waterLevel = ratio
      ...
      this.state.waterLevelModel = winInfo.model || ''
      this.state.waterLevelAt = Date.now()
      const threshold = Math.max(Number(this.config.waterLevelThreshold) || 0.8, 0.1)
   ...
   *  zstd 解压抽取 header(cwd/agentPreset)、最新 request/context(provider/model)、user+assistant 消息全量转写;
   *  转写落盘 handoff/prev-session-*.md → 新会话 AI 用 read 工具即可随时读取旧会话全部内容(原始 .zstd AI 读不了,这是关键桥)。
   *  返回 {sessionId, transcriptPath, provider, model, agentPreset, cwd, tailText} 或部分为空(任何一步失败都 fail-soft)。 */
  async buildPrevSessionPack() {
    try {
      ...
      let preset = '', cwd = ''
      try { const h = JSON.parse(lines[0]); if (h && typeof h === 'object') { preset = String(h.agentPreset || ''); cwd = String(h.cwd || '') } } catch (e) {}
      ...
      for (const l of lines) {
        ...
        if (t === 'request/context') {
          const d = (ev && ev.data) || {}
          if (d.model) { provider = String(d.provider || provider); model = String(d.model) }
          if (d.reasoningEffort || d.reasoning || d.reasoningEffortLevel) { reasoningEffort = String(d.reasoningEffort || d.reasoning || d.reasoningEffortLevel || reasoningEffort) }
          ...
          if (m && m.role && Array.isArray(m.content)) {
            ...
            if (txt) msgs.push({ role: m.role, text: txt })
          }
        }
      }
      ...
      const tail = msgs.slice(-8).map((m) => m.role + ': ' + m.text.slice(0, 300)).join(NL + '---' + NL)
      diag('prev-session pack built: sid=' + sid + ' msgs=' + msgs.length + ' -> ' + outPath)
      return { sessionId: sid, transcriptPath: outPath, provider, model, reasoningEffort, agentPreset: preset, cwd: cwd || p.ws, tailText: tail }
  ...
  async buildContinueCarry() {
    try {
      ...
      if (plan) parts.push('### 白板 PLAN.md(节选)' + NL + plan.slice(0, 3000))
      if (ledger) parts.push('### 交接账本(' + ledgerFrom + ')' + NL + ledger.slice(0, 8000))
      if (pack) {
        const guide = ['### 旧会话上下文包(主动读取指引)']
        ...
        if (pack.tailText) guide.push('#### 最近对话摘录(最后 8 条)' + NL + pack.tailText)
        if (pack.model) guide.push('- 状态沿用: 模型 ' + pack.model + (pack.provider ? ' @ ' + pack.provider : '') + ' 已在新会话自动选回(失败则回退路由默认);工作区 cwd 与权限绑定沿用 ' + (pack.cwd || p.ws) + ';插件配置(无人值守/水位/交接等)为全局配置,自动继承。')
        if (pack.agentPreset) guide.push('- 旧会话 agentPreset: ' + pack.agentPreset + '(preset 不随创建传递,如需同款预设请手动选择;注意 code→ptc 改名的历史会话需用户级兼容预设)。')
  ...
  async handoffPanelData(fileQ) {
    ...
    return {
      ...
      waterLevel: {
        ...
        threshold: Number(this.config.waterLevelThreshold) || 0.8,
  ...
  renderMemoryDynamic(context) {
    ...
    if (cfg.waterLevelAdvisory !== false && !this.isUnattendedNow() && (s.waterLevelRatio || 0) >= (Number(cfg.waterLevelThreshold) || 0.8) && Date.now() - (s.waterLevelAt || 0) < 600000) {
  ...
  ctx.on('agent/turn-stopping', (payload) => {
    ...
    try {
      ...
      if (hasAgent) {
        ...
        try { void engine.checkWaterLevel(agent) } catch (e) {}
    ...
    },
    {
      ...
      handler: async (req, res) => {
        ...
        try {
          ...
          return writeJson(res, 200, await engine.handoffPanelData(url.searchParams.get('file')))
        } catch (e) { return writeJson(res, 500, { error: String(e && e.message ? e.message : e) }) }
      },
    },
    {
