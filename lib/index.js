...
class MemoryEngine {
  ...
  async buildPrevSessionPack(preferSid) {
    try {
      ...
      for (const m of msgs) {
        ...
        if (total + chunk.length > 60000) { body.push('(更早内容已截断——完整历史可试 memory_recall_pre(scope=\'sessions\', query=\'关键词\') 检索;该检索需宿主侧开启会话内容检索,条件不满足时会明确报错,此时请改用 scope=\'handoff\' 或直接 read 本文件)'); break }
        body.push(chunk)
        total += chunk.length
      }
      try {
  ...
  async recall(query, limit = 8, agent, scope = 'all', opts = null) {
    ...
    if (scope === 'all' && !l0Mode && typeof this._jsSemanticRank === 'function') {
      try {
        const { buildL0IndexPre } = await import('./l0-extract-pre.js')
        const { isCurrentPre: isCurrentPreArms } = await import('./l0-extract-pre.js')
        const { createHash } = await import('node:crypto')
...
export function apply(ctx, config) {
  ...
  try {
    if (!process._dshAutoMemoryRejectionGuard) {
      ...
      process.on('unhandledRejection', (reason) => { damSafeDiag(process.stderr, '[dsh-auto-memory-pre] unhandledRejection guard: ' + damDiagLine(reason)) })
    }
  } catch (e) {}
