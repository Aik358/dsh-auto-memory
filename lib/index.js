...
this._readError = undefined
  ...
  /** 取该会话的实测记录。10 分钟内算新鲜;过期的**照样返回**并打 stale ——
   *  旧实现在此返回 null,于是面板把「刚测过但闲置了一会儿」当成「从没测过」,整卡退回
   *  「0 / 尚未测量」(2026-09-23 两台 Windows 实证症状)。过期不是无数据,是旧数据。 */
  waterRecordFor(sid) {
    if (!sid) return null
    ...
    if (age < 600000) return rec
    // 但也不能无限期当真:闲置期间官方压缩可能已把上下文压下去而插件没测到 ⇒ 陈旧值只会**偏高**
    // (用户误以为快满了,比偏低更糟)。超过 6 小时按无记录处理,让界面如实显示未测量。
    if (age > 6 * 60 * 60 * 1000) return null
    return Object.assign({}, rec, { stale: true })
  }
  ...
  async waterWindowForSession(sid) {
    ...
    try {
      ...
      if (dir) {
        ...
        // ⚠️ 未压缩日志必须走明文分支:旧实现无条件喂给 zstd 解码器 ⇒ 扫不到任何帧 ⇒ 事件恒空 ⇒
        //   窗口静默降级成 131072 回退值(终端用户实证:卡片分母与真机 1,000,000 官方容量对不上)。
        //   明文分支同样**必须限长**:本函数在面板轮询路径上同步跑,整文件 toString + 逐行 JSON.parse
        //   对未压缩的巨型会话日志会卡住宿主事件循环(同 :9789「同步遍历阻塞主线程」那条教训)。
        //   取前 8MB 与 zstd 分支的解码上限同量级 —— request/header 与 request/context 都在会话开头。
        const text = raw ? (String(sessionFile).endsWith('.zstd') ? decodeZstdFramesHead(raw, 32, 8 * 1024 * 1024) : raw.subarray(0, 8 * 1024 * 1024).toString('utf8')) : ''
  ...
  /** 面板只读取数(2026-09-23):该会话在宿主 agents 注册表里仍活着、但本进程从没测过时,
   *  按 checkWaterLevel 的**同一口径**(官方 tokenMeter → 启发式降级)现场算一次分子。
   *  与 checkWaterLevel 的关键区别:**纯读、零副作用** —— 面板是轮询的,任何写 state / 置
   *  planUpdatePending / arm 接续的动作都不许在这里发生。
   *  无活 agent(冷会话)时返回 null:真机取证 DSH 会话日志里**没有 usage 事件**,而头帧解码
   *  只覆盖会话开头(实测 60/1088 行)⇒ 从磁盘推分子只会给出伪装成实测的错数,宁缺毋伪。 */
  waterTokensForLiveSession(sid) {
    const agent = this.agentForSessionId(sid)
    if (!agent || !agent.session) return null
    try {
      const meter = agent.ctx && typeof agent.ctx.get === 'function' ? agent.ctx.get('tokenMeter') : null
      if (meter && typeof meter.measure === 'function') {
        ...
        if (Number.isFinite(t) && t >= 0 && m && m.baseline && m.baseline.kind !== 'none') {
          return { tokens: t, meter: 'official:' + (m.baseline.kind || 'estimated') }
        }
      }
    } catch (eMeter) {}
    try {
      return { tokens: this.estimateSessionTokens(extractSessionMessages(agent) || []), meter: 'heuristic' }
    } catch (eEst) { return null }
  }
  ...
  async handoffPanelData(fileQ, sessionId) {
    ...
    if (sessionId) {
      ...
      if (rec) {
        ...
        // 本进程没测过 ≠ 测不了:会话在宿主注册表里还活着时现场量一次(纯读,见 waterTokensForLiveSession)。
        // 冷会话(既无记录又无活 agent)留 tokens:0/at:0 —— 客户端据此如实显示「尚未测量」,不得编数。
        const liveTok = this.waterTokensForLiveSession(sessionId)
        if (liveTok && Number(liveTok.tokens) > 0) {
          water.tokens = Number(liveTok.tokens)
          water.meter = liveTok.meter || ''
          // 分母用**声明窗口**而非 checkWaterLevel 的 triggerWin(= min(声明, provider 自报硬限)):
          // hardWin 只有撞过 400 才从事件里扫得到,本路径拿不到 —— 与卡片上展示的那个分母保持自洽即可。
          water.ratio = water.window > 0 ? water.tokens / water.window : 0
          water.at = Date.now()
          water.live = true
        }
      }
