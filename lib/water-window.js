...
export function parseModelWindowsPre(text) {
  ...
  for (const line of lines) {
    ...
    // 字符类须含 `/`:provider 前缀式 id(`deepseek/deepseek-v4.1-flash`、`z-ai/glm-5.3-flash`、
    // `meituan/LongCat-2.0:free`)是常见形态,旧字符类 `[A-Za-z0-9._:-]` 不含斜杠 →
    // 整条 id 行匹配失败,其 contextWindow 被静默丢弃,窗口退化为 fallback 131072。
    // 实测:真实窗口 1,000,000 的会话被按 131,072 当分母,水位放大 7.63 倍(12.8% 显示成 98%),
    // 未达 0.75 阈值即误弹「接续到新会话」确认卡。
 ...
 *
 * `wl.modelKnown === false` 采用显式判等:未传该字段的旧调用点视为「已知」,行为不变。
 *
...
export function shouldArmAutoContinuePre(wl) {
  if (!wl) return false
  if (wl.modelKnown === false && !wl.hard) return false
  return true
}
