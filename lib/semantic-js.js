...
function defaultModelsDirCandidates(pluginDir) {
  ...
  const dshHome = (process.env.DSH_HOME && process.env.DSH_HOME.trim()) || path.join(homedir(), '.dsh')
  return [
...
export function createSemanticDownloaderPre(opts = {}) {
  ...
  async function fetchToFile(fileRec, base) {
    ...
    // 每次尝试必须从零开始:镜像重试共用同一 dst,上一来源断流的半截残留会经 appendChunk
    // 的 flag:'a' 拼进本来源,而 sha256 只对网络流累积(hash.update 在下方),拼接文件照样
    // 通过校验落位 → models 目录里是"半截源A+完整源B"的坏模型。
