 ...
 *
 * 可观察性(最小投影,不泄内容):lastSegmentRuntimeKey、
 * lastSegmentSessionRef、lastDrop{reason,contextVersion,runtimeKey}、readyState。
 * （issue#107 移除 capturedPathKeys：它指向一个从不写入的死集合，恒为 []。）
 * UTF-8 无 BOM。
 */
...
export function createIndexSyncHostPre(opts = {}) {
  ...
  // issue#107：这里曾有 `const enabledKeys = new Set()`，注释称「同一 miv 内不再重试」，但全仓
  // 没有任何 `.add()` ⇒ 纯死码（只有 debugView 读它、dispose 清它）。失败退避改由 sidecar 层的
  // 熔断 + 连续超时看门狗统一负责，不在这一层再造第二套并行的抑制机制。
