...
/**
 * 保留语法开标记(issue #54):写入侧**必须**复用本常量做前置校验。
 * 导出而非私有 —— 过去只被本文件的读取路径(:177/:182 判定)引用,写入路径零引用,
 * 于是"文件已有内容干净"≠"本次要写的内容干净",一次误写即让该文件此后永久拒写。
 */
export const MARKER_OPEN = '<!-- memory:'
...
/**
 * issue #54：检测**待写入正文**是否含保留 marker 语法。
 *
 * 背景：`appendAnchoredRecord()` 等写入原语只对「文件已有内容」跑 `parseAnchors()`，
 * 从不校验本次要写入的 `text`。含 `MARKER_OPEN` 的正文落盘后，从**下一次**写入起
 * `parseAnchors` 判 `orphan-content` fail-closed ⇒ 该文件全部写入被永久拒绝。
 *
 * 本函数与 `parseAnchors` 用**同一个** `MARKER_OPEN` 常量判据，保证读写两侧口径永不漂移。
 * 不做转义/静默改写（转义救不了：`parseAnchors` 无转义机制，`.includes()` 照样命中）。
 *
 * @param {string} text 待写入正文
 * @returns {{ok:true}|{ok:false,reason:'reserved-syntax-in-content',line:number,detail:string}}
 */
export function checkReservedSyntaxInContent(text) {
  const s = String(text == null ? '' : text)
  if (!s.includes(MARKER_OPEN)) return { ok: true }
  const lines = s.split(/\r?\n/)
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes(MARKER_OPEN)) {
      return {
        ok: false,
        reason: 'reserved-syntax-in-content',
        // 行号按**待写入正文内**的 1-based 相对行号；调用方负责再映射为文件绝对行号。
        line: i + 1,
        detail: '待写入内容含保留 marker 语法 ' + MARKER_OPEN + '（第 ' + (i + 1) + ' 行）: ' + lines[i].slice(0, 60),
      }
    }
  }
  // 理论上不可达（上面已判 includes），保留兜底以便未来改判据时不静默漏过。
  return { ok: false, reason: 'reserved-syntax-in-content', line: 1, detail: '待写入内容含保留 marker 语法' }
}
...
/**
 * issue #54 配套：把**跨文档引用**的正文里的结构 marker 行剥掉。
 *
 * 背景：把 A 文档的原文（含 A 的 `<!-- memory:mem_xxx -->` 行）当正文追加进 B 文档时，
 * 那行 marker 会变成 B 的结构锚点 —— 产生一条**幻影记录**（身份属于 A、却挂在 B 上），
 * 且此后 `parseAnchors` 会把它当真实锚点参与 sidecar/索引，属静默结构损坏。
 *
 * 口径：只剥「整行恰好是合法 marker」的行（`MARKER_RE`），保留其余一切字节；
 * 行内出现的 marker 片段不在此处理（那属 #54 写入守卫的职责）。
 * @param {string} text 待内联的正文
 * @returns {string} 剥掉 marker 行后的文本
 */
export function stripAnchorLines(text) {
  const s = String(text == null ? '' : text)
  if (!s.includes(MARKER_OPEN)) return s
  return s.split(/\r?\n/).filter((ln) => !MARKER_RE.test(ln)).join('\n')
}
