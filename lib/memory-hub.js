  ...
  function ingestJudgement(row) {
    ...
    try {
      ...
      if (layer === 'episodic' && stores.episodic) {
        // episodic_candidate 行不保证满足 episode 必填集;restore 是"快照整体替换"语义,
        // 逐行走 restore 会先 episodes=[] 再校验,坏行把已巩固 episode 与 current 全部清空
        // (并在下次 consolidate/flush 落盘)。改走增量导入:逐条校验,合法才追加,绝不清空。
        const r = stores.episodic.importEpisodes([row])
        stats.consumedEpisodic++
        return { consumed: 'episodic', outcome: r.restored > 0 ? 'restored' : ('rejected:' + r.rejected) }
      }
    } catch (e) {
