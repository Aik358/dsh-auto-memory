...
export function createActivationHost(opts = {}) {
  ...
  function recordTierGateHits(req) {
    try {
      ...
      // ★2026-09-19 修复（issue #94⑤）：先 delete 再 set 刷新插入序 —— Map.set 对既有键不刷新序，
      // keys().next() 淘汰的是「最老创建」而非「最久未用」，最活跃会话的投影反被逐出（该轮不下探 Tier-1）。
  ...
  /**
   * 释放一个 runtime 的全部 per-runtime 激活态(会话 dispose 时由 SessionRuntimeStore 调用)。
   * 入参优先给 **runtime 对象**:stepsByRuntime 的键是 `sessionId|ws:<workspaceKey>`、inbox 注册表
   * 的键是 `session:<sessionId>|ws:<workspaceKey>`,都与 runtime.key 不同形(runtime.key 还可能是
   * `agent:<id>` —— identity 在创建时才补 session),只有 runtime.sessionId 能可靠对上;
   * 兼容旧签名传 key 字符串(此时按 `session:` 前缀反推 sessionId)。
   * mivCacheByWs 是**工作区**级共享缓存,不随单个会话释放清理(否则每轮都要重算语料快照)。
   * @returns {number} 清掉的条目数(0 = 该 runtime 本就没有激活态);重复调用幂等。
   */
  function disposeRuntime(runtimeOrKey) {
    const rt = runtimeOrKey && typeof runtimeOrKey === 'object' ? runtimeOrKey : null
    const key = String((rt ? rt.key : runtimeOrKey) || '')
    if (!key) return 0
    let cleared = 0
    if (runtimeState.delete(key)) cleared++
    if (pathsByKey.delete(key)) cleared++
    const sessionId = String((rt && rt.sessionId) || (key.startsWith('session:') ? key.slice('session:'.length) : ''))
    if (!sessionId) return cleared
    for (const k of stepsByRuntime.keys()) {
      if (k.startsWith(sessionId + '|ws:')) { stepsByRuntime.delete(k); cleared++ }
    }
    cleared += registry.disposeBySession(sessionId)
    return cleared
  }
  function disposeAll(reason) {
    runtimeState.clear()
    stepsByRuntime.clear()
    pathsByKey.clear() // 与 disposeRuntime 同集合:整体卸载也要放开 paths 快照
