/**
 * 记忆写入保护门（memory_mutation_pre_v1）—— 3.0 主体拥有「共同提交与保护入口」。
 *
 ...
 *   - **格式由适配器提供**：白板走 `lib/wb-contract-pre.js:parseWhiteboardPre` → `toMutationProjectionPre`；
...
export const MEMORY_MUTATION_VERSION = 'memory_mutation_pre_v1'

/** 拒绝/提示的原因码 → 可读中文。 */
export const MUTATION_REASONS_PRE_V1 = Object.freeze({
...
export function describeMutationReasonPre(code) {
  ...
  return MUTATION_REASONS_PRE_V1[k] || k || '未知原因'
}
...
export function mutationRefusalTextPre(res) {
  ...
  return '写入被记忆保护门拦截（' + failed.map((h) => h.id).join('/') + '），原文件未改动：\n' + lines.join('\n')
    // ★v3.1.4 批 I：删掉一条**对模型不可达的出路**（原先承诺"显式归档即可合法消失"）——
    //   经三层取证该通道从未接通：底层 API 认 archivedIds，但中间层从不被喂、调用层只传 4 个字段、
    //   工具参数表也没有该键。给不存在的出路会让模型反复试同一条路（用户报障"每次都重试"）。
    //   现改为**唯一真出路**：把消失卡片原样带回（文案上方会附其原文块）。
    //   ⚠️ 计数断言的坑（本轮踩到）：替换文本自身若提到被删的那句，cnt() 就不为 0 ⇒ 注释里不再复述原句。
    + '\n请修正后重试：把上述卡片**原样带回**（标题 + 锚点行 + 正文，含其用户备注区，逐字节不变）——'
    + '这是唯一可通过的写法。白板重写前请先 read 磁盘 handoff/PLAN.md 原文抄全锚点（注入快照会被截断）。'
}
