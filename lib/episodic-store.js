...
export function createEpisodicStorePre(opts = {}) {
  ...
  function restore(data) {
    ...
    current = validCurrentState(data.current)
    return { ok: true, restored: episodes.length }
  }
  /** current 形状校验(与 snapshot 产出的形状对齐):损坏快照置 null,
   *  丢当前未巩固段优于 consolidate 每次 TypeError 卡死巩固链。 */
  function validCurrentState(c) {
    if (!c || typeof c !== 'object' || Array.isArray(c)) return null
    if (typeof c.sessionRef !== 'string' || !c.sessionRef) return null
    if (typeof c.startedAt !== 'number' || !Number.isFinite(c.startedAt)) return null
    if (!Array.isArray(c.segments) || !Array.isArray(c.userTexts) || !Array.isArray(c.assistantTexts)) return null
    return c
  }
  /** 增量导入(与 restore 的"快照整体替换"相对):逐条校验,合法才追加;
   *  绝不清空既有 episodes/current,按 episodeId 幂等去重。
   *  供 hub 消费 episodic_candidate 行 —— restore 是整体替换语义,逐行喂会把
   *  候选行校验失败变成"清空全部已巩固 episode"(issue: episodic_candidate 逐行 restore 覆写)。 */
  function importEpisodes(rows) {
    if (!Array.isArray(rows)) return { ok: false, reason: 'bad-episodes' }
    const seen = new Set(episodes.map((x) => x.episodeId))
    let restored = 0, rejected = 0
    for (const e of rows) {
      const v = validateEpisodePre(e)
      if (!v.ok) { rejected++; continue }
      if (seen.has(v.episode.episodeId)) continue
