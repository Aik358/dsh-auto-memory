...
/**
 * 未巩固段(current)形状校验:与 append() 产出的形状同规则(sessionRef/startedAt/三类数组)。
 */
export function validateEpisodeCurrentPre(cur) {
  if (!cur || typeof cur !== 'object' || Array.isArray(cur)) return { ok: false, reason: 'not-object' }
  const p = []
  if (typeof cur.sessionRef !== 'string' || !cur.sessionRef) p.push('sessionRef')
  if (typeof cur.startedAt !== 'number' || !Number.isFinite(cur.startedAt)) p.push('startedAt')
  if (!Array.isArray(cur.segments)) p.push('segments')
  if (!Array.isArray(cur.userTexts)) p.push('userTexts')
  if (!Array.isArray(cur.assistantTexts)) p.push('assistantTexts')
  if (p.length) return { ok: false, reason: 'invalid:' + p.join(',') }
...
export function createEpisodicStorePre(opts = {}) {
  ...
  let disposed = false
  const stats = { segmentsAppended: 0, consolidated: 0, droppedTooShort: 0, retained: 0, droppedCurrent: 0 }

  function defaultEpisodeId(sessionRef, startedAt) {
    const h = createHash('sha256').update(['episode-v1', String(sessionRef), String(startedAt)].join('\u0000')).digest('hex')
  ...
  function restore(data) {
    ...
    // issue#57:current 也要逐字段把关。接线侧 load() 只保证 JSON.parse 成功、不校验 schema,
    // 而损坏的 current(如 {})一旦进来,之后每次 consolidate() 读 current.segments.length 都抛
    // TypeError,且抛错点在 current 置空之前 ⇒ 整条巩固链路静默停摆到重启。
    // 丢弃一个未巩固段,优于卡死整条链路;丢弃计入 stats,不留完全静默的数据丢失。
    if (data.current == null) {
      current = null
    } else {
      const vc = validateEpisodeCurrentPre(data.current)
      if (vc.ok) current = vc.current
      else { current = null; stats.droppedCurrent++ }
    }
