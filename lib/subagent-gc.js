...
export async function scanPluginSubagentSessions(opts = {}) {
  ...
  for (const ws of wsDirs) {
    ...
    for (const ent of entries) {
      ...
      let file = ''
      let st = null
      // ★v3.1.3：不再固定候选名 —— 扫目录取任意 session.*\.jsonl[.zstd]（兼容未来 vN 命名）。
  let names = []
  try { names = readdirSync(dir) } catch (_) { names = [] }
  const hits = names.filter((n) => /^session[.-].*\.jsonl(\.zstd)?$/i.test(n) && !/backup|corrupt/i.test(n))
  for (const name of (hits.length ? hits : ['session.v3.jsonl.zstd', 'session.jsonl.zstd'])) {
...
export async function recycleSessions(opts = {}) {
  ...
  for (const c of opts.candidates || []) {
    ...
    try {
      ...
      if (projcacheRoot) {
        // 2026-09-24 修复:真实存储是 <sid>.json(storages/session_projcache/sessions/),
        // 旧写法拼 <sid> 无扩展名 ⇒ existsSync 恒 false,投影缓存回收从未生效。
        for (const cacheName of [c.sid + '.json', c.sid]) {
          const cacheFile = path.join(projcacheRoot, cacheName)
          if (!(await exists(cacheFile))) continue
          const cacheDest = path.join(backupRoot, '_projcache', cacheName)
...
/**
 * 从活存储的父会话文件里剔除「已不存在的子代理」目录条目。
 *
 * 背景(2026-09-24 实测):前端「子代理不可用」的唯一数据源是父会话投影
 * `.record.rows.subagentCatalog.val.head.values[].childId`。会话目录被搬走后,
 * 该数组仍保留 childId ⇒ dsh-subagent 观测失败后返回
 * `{kind:'diagnostic', reason:'unavailable'}`,前端即显示为不可用条目。
 * 搬移会话目录时必须同步剔除这里,否则每次回收都在制造幽灵。
 *
 * 边界:
 *   - 只改 subagentCatalog 数组,其余键原样带回(整对象 JSON 读改写);
 *   - 只剔 `removedIds` 里明确给出的 id,且该 id 必须「已不在磁盘」;
 *   - 写前备份到 backupRoot/_catalog-purge/<ts>/,失败只记录不抛错;
 *   - 幂等:无可剔条目则完全不写盘。
 *
 * @param {{liveDir:string,removedIds:Iterable<string>,onDiskIds?:Iterable<string>,backupRoot?:string,apply?:boolean,now?:Date}} opts - 参数。
 * @returns {Promise<{scanned:number,files:number,purged:number,written:number,backupDir:string,failed:object[]}>}
 */
export async function purgeSubagentCatalog(opts = {}) {
  const out = { scanned: 0, files: 0, purged: 0, written: 0, backupDir: '', failed: [] }
  const liveDir = opts.liveDir
  if (!liveDir) { out.failed.push({ error: 'liveDir required' }); return out }
  const apply = opts.apply === true
  const removed = new Set()
  for (const id of opts.removedIds || []) if (id) removed.add(String(id))
  if (!removed.size) return out
  const onDisk = new Set()
  for (const id of opts.onDiskIds || []) if (id) onDisk.add(String(id))
  let names = []
  try { names = await readdir(liveDir) } catch (e) { out.failed.push({ error: 'readdir: ' + e.message }); return out }
  const targets = []
  for (const name of names) {
    ...
    out.scanned++
    let txt = ''
    try { txt = await readFile(file, 'utf8') } catch (e) { continue }
    let any = false
    for (const id of removed) { if (txt.includes(id)) { any = true; break } }
    if (!any) continue
    let doc = null
    try { doc = JSON.parse(txt) } catch (e) { out.failed.push({ file: name, error: 'json' }); continue }
    const cat = doc && doc.record && doc.record.rows && doc.record.rows.subagentCatalog
    const arr = cat && cat.val && cat.val.head && Array.isArray(cat.val.head.values) ? cat.val.head.values : null
    if (!arr) continue
    const kept = arr.filter((row) => {
      const cid = row && row.childId ? String(row.childId) : ''
      if (!cid || !removed.has(cid)) return true
      return onDisk.has(cid) // 磁盘上还在的保守保留
    })
    if (kept.length === arr.length) continue
    out.files++
    out.purged += arr.length - kept.length
    targets.push({ file, name, doc, arr, kept, removedCount: arr.length - kept.length })
  }
  if (!targets.length) return out
  if (!apply) { out.written = 0; return out }
  // 备份后写
  const ts = (opts.now instanceof Date ? opts.now : new Date()).toISOString().replace(/[-:T]/g, '').slice(0, 14)
  const backupDir = path.join(opts.backupRoot || path.join(liveDir, '_catalog-purge'), ts)
  try { await mkdir(backupDir, { recursive: true }) } catch (e) { }
  out.backupDir = backupDir
  for (const t of targets) {
    try {
      const bak = path.join(backupDir, t.name)
      try { await writeFile(bak, await readFile(t.file)) } catch (e) { }
      t.arr.length = 0
      for (const row of t.kept) t.arr.push(row)
      const outTxt = JSON.stringify(t.doc)
      const tmp = t.file + '.purge-tmp'
      await writeFile(tmp, outTxt, 'utf8')
      await rename(tmp, t.file)
      out.written++
    } catch (e) {
      out.failed.push({ file: t.name, error: e && e.message ? e.message : String(e) })
    }
  }
  return out
}
