...
export class EvidenceEventStore {
  ...
  /** §9 幂等:同 evidenceId 进程内只**成功**落盘一次(seed replay 与 live 双喂安全网;磁盘侧靠 rebuild 去重兜底)。
   *  写失败不算落盘,登记随失败回滚 ⇒ 同 id 可重试(issue#56)。 */
  async append(evidence, opts = {}) {
    ...
    // issue#56:登记要在**写失败**时回滚。原先只登记、从不清理(唯一清理点是 dispose),于是一次
    // 瞬时写失败(EDQUOT/EBUSY/杀软占用)就让该 evidenceId 在进程存活期内恒被判 duplicate-evidence,
    // 证据静默丢失。登记时机仍留在入链前 —— 这样并发/重放窗口的 in-flight 去重不受影响。
    this._chain = this._chain.then(async () => {
      const written = await this._writeLine(proj.line)
