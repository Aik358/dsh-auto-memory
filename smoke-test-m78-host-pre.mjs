...
console.log('[P3] 编排器单元:首同步/同 miv 幂等/epoch 重同步/miv 替换')
{
  ...
  const host = SYNC_HOST.createIndexSyncHostPre({ engine: {
    ...
    _pythonSidecar: await (async () => { const CL = await import('./lib/python-sidecar-client-pre.js'); const c = CL.createPythonSidecarClientPre({ command: 'python', scriptPath: () => path.join(process.cwd(), 'python', 'worker_semantic_pre_v1.py'), dshHome: h.home, requestTimeoutMs: 4000 }); globalThis.__p3client = c; return c })(),
  } })
...
console.log('[P4] sync 失败 → 不 push context;后续可重试')
{
  ...
  const badClient = CL.createPythonSidecarClientPre({ command: 'python', scriptPath: () => path.join(process.cwd(), 'python', 'no-such-worker.py'), dshHome: h.home, requestTimeoutMs: 1000 })
...
console.log('[P5] A/B workspace 同 miv 零串线(编排器层面)')
{
  ...
  const SYNC_HOST = await import('./lib/m7-index-sync-host-pre.js')
  const CL = await import('./lib/python-sidecar-client-pre.js')
  const c = CL.createPythonSidecarClientPre({ command: 'python', scriptPath: () => path.join(process.cwd(), 'python', 'worker_semantic_pre_v1.py'), dshHome: h.home, requestTimeoutMs: 4000 })
...
console.log('[P7] activation 回流:index ready 后 context_push 到达 → fake worker 激活过 M6 validator;门关闭零 worker')
{
  ...
  await h.cfgPost({ associativeMemoryEnabled: true, contextBridgeEnabled: true, pythonBackendEnabled: true, contextSinkMode: 'python', pythonBackendWorkerPath: path.join(process.cwd(), 'python', 'worker_semantic_pre_v1.py'), pythonBackendExecutable: process.env.M78_PY || 'python' })
