...
import {
  SKILL_USAGE_NOTICE_V1, SKILL_NOTICE_ANCHORS_V1,
  renderSkillMarkdownPre, validateSkillMarkdownPre, skillDirNamePre,
} from '../../lib/skill-export.js'
import {
  SKILL_EXPORT_STAMP_V1, resolveSkillsRootPre, projectNameFromPre,
  collectProgramsPre, exportSkillForPre, listExportedSkillsPre,
} from '../../lib/skill-export-host.js'
...
const PROC = {
  procedureId: 'proc_0123456789abcdef0123456789abcdef',
  ...
  sourceEpisodes: ['epi_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'],
  evidence: { seen: 3, success: 2, correction: 0 },
}

console.log('\n[1] ⑪-1 约束条款：五个锚点必须原样出现')
for (const a of SKILL_NOTICE_ANCHORS_V1) {
  ok(SKILL_USAGE_NOTICE_V1.includes(a), '条款含锚点: ' + a)
}
...
ok(skillDirNamePre({ procedureId: 'proc_ffffffffffffffffffffffffffffffff', title: '别的流程' }) !== d1, '不同 procedure ⇒ 不同目录名')
...
const cn1 = skillDirNamePre({ procedureId: 'proc_x', title: '中文标题' })
const cn2 = skillDirNamePre({ procedureId: 'proc_x', title: '中文标题' })
...
ok(cn1 !== skillDirNamePre({ procedureId: 'proc_x', title: '另一个中文标题' }), '★ 不同中文标题 ⇒ 不同名字（不再堆成一堆 untitled）')
...
try {
  ...
  ok(onDisk.includes(SKILL_EXPORT_STAMP_V1), '含自检标记（可识别为本插件产物）')
...
console.log('\n[11] 源码接线锁（防"只声明未使用"）')
const host = codeOnly(src('skill-export-host.js'))
...
ok(host.includes('SKILL_EXPORT_STAMP_V1'), '写入自检标记')
const pre = codeOnly(src('skill-export.js'))
