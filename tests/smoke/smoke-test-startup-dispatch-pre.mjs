...
ok(/function dispatchStartupDialog\(d, cfg\) \{/.test(SRC), '具名分发函数存在(可状态机驱动,含宿主配置入参)')
...
const fnSrc = extractFn('function dispatchStartupDialog(d, cfg) {')
const allowPreSrc = extractFn('function welcomeAutoAllowedPre(cfg) {')

function run(current, storageMap, cfg) {
  const factory = new Function('openDialog', 'cmpVersion', 'allowPreSrc', 'cfgJson', `
    ...
    ${allowPreSrc}
    ${fnSrc}
    try { dispatchStartupDialog({ current: '${current}' }, JSON.parse(cfgJson)) } catch (e) { opened.push('THROW:' + (e && e.message)) }
    return { opened: opened }
  `)
  return factory(function () {}, function () {}, allowPreSrc, JSON.stringify(cfg === undefined ? null : cfg))
}
...
{
  const r = run('0.1.37', [['dsh-auto-memory-pre.seenVersion', '0.1.37'], S('dsh-auto-memory-pre.semWizardDone'), S('dsh-auto-memory-pre.majorTourV130')], CFG_ON)
  ok(r.opened.length === 0, '看完向导后静默(seen===current)')
}

console.log('[startup-dispatch] G3 老用户 seen=0.1.29 升级且向导未看过 → welcomeTour')
{
  const r = run('0.1.37', [S('dsh-auto-memory-pre.seenVersion') && ['dsh-auto-memory-pre.seenVersion', '0.1.29']], CFG_ON)
...
{
  const r = run('0.1.37', [['dsh-auto-memory-pre.seenVersion', '0.1.29'], S('dsh-auto-memory-pre.semWizardDone')], CFG_ON)
  ok(r.opened.length === 1 && r.opened[0] === 'update', '看过向导 → 弹 changelog')
}

console.log('[startup-dispatch] G5 老用户 seen=0.1.36(有更新,向导看过)→ update')
{
  const r = run('0.1.37', [['dsh-auto-memory-pre.seenVersion', '0.1.36'], S('dsh-auto-memory-pre.semWizardDone'), S('dsh-auto-memory-pre.majorTourV130')], CFG_ON)
...
{
  const r = run('2.1.0', [['dsh-auto-memory-pre.seenVersion', '0.1.39'], S('dsh-auto-memory-pre.semWizardDone'), S('dsh-auto-memory-pre.majorTourV130')], CFG_ON)
...
console.log('[startup-dispatch] G8 issue#40: welcomeTourEnabled=false → 自动弹出全关')
{
  ...
  const rOld = run('0.1.37', [S('dsh-auto-memory-pre.seenVersion') && ['dsh-auto-memory-pre.seenVersion', '0.1.29']], { welcomeTourEnabled: false })
...
{
  const r = run('0.1.37', [['dsh-auto-memory-pre.tourDismissed', '1']], CFG_ON)
