...
const RULES_SRC = readFileSync(new URL('../../lib/rules-layer-pre.js', import.meta.url), 'utf8')
...
console.log('[P6A-6] ★T7-1（在能测的范围内）：规则在**每一轮**的注入函数返回值里都在')
{
  ...
  const rulesCode = readFileSync(new URL('../../lib/rules-layer-pre.js', import.meta.url), 'utf8').replace(/^export /gm, '')
...
console.log('[P6A-7] ★T7-7：不能只靠"引导语不同"判"遵守问题已解决" —— 要有可机械判断的执行结果')
{
  ...
  const budget = readFileSync(new URL('../../lib/memory-envelope-pre.js', import.meta.url), 'utf8')
...
console.log('[P6A-8] 边界与卫生：开关默认关 / 模式非法 fail-soft / 无 BOM / S9')
{
  ...
  // 卫生
  for (const p of ['../../lib/rules-layer-pre.js', '../../lib/index.js']) {
