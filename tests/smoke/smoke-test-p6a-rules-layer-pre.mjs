...
import {
  ...
  firstLine,
  RULES_SECTION_TITLE_V1,
  RULES_SECTION_GUIDE_V1,
  REFERENCE_SECTION_GUIDE_V1,
  RULES_LAYER_VERSION,
  RULE_MARKERS_V1,
  RULE_CONSTRAINT_WORDS_V1,
  RULE_DESCRIPTION_MARKERS_V1,
...
console.log('[P6A-1] 规则 vs 参考：分层抽取（真源 = 既有用户级记忆，不新增 RULES.md）')
{
  ...
  ok(!RULE_CONSTRAINT_WORDS_V1.includes('不要'), '约束语汇表刻意不含宽泛的「不要」（避免把偏好描述误升为规则）')
}
...
console.log('[P6A-2] ★T7-4 修订（v2 纠正我方原设计）：描述性"必须"不得自动升级为规则')
{
  ...
  ok(RULE_DESCRIPTION_MARKERS_V1.length >= 5, '引用/历史语境标记表存在（' + RULE_DESCRIPTION_MARKERS_V1.length + ' 条）')
  ok(RULE_MARKERS_V1.includes('【用户硬性规则'), '结构化前缀表含本机真实形态')
  ok(RULE_CONSTRAINT_WORDS_V1.includes('严禁'), '约束语汇表含「严禁」')
}

// ═══════════════════════════════════════════════════════════════════
console.log('[P6A-3] ★T7-3 措辞分层：规则类用约束语，参考类保留"参考"语义，两者引导语**必须不同**')
{
  eq(RULES_SECTION_GUIDE_V1 === REFERENCE_SECTION_GUIDE_V1, false, '★两段引导语不相同（防止改回统一措辞）')
  ok(/必须|约束|不是可选/.test(RULES_SECTION_GUIDE_V1), '规则段引导语含约束语（"必须"/"约束"）')
  ok(/参考/.test(REFERENCE_SECTION_GUIDE_V1), '参考段引导语保留"参考"语义')
  ok(!/参考/.test(RULES_SECTION_GUIDE_V1), '★规则段引导语里**没有**"参考"二字（这正是要修的病）')
  ok(/\u5fc5\u987b\u9075\u5b88/.test(RULES_SECTION_TITLE_V1), '规则段标题含"必须遵守"（醒目）')

  const layer = extractRulesLayerPre({ userText: USER_MEM, rulesLayeringMode: 'self' })
  const sec = renderRulesSectionPre(layer)
  ok(sec.text.includes(RULES_SECTION_TITLE_V1), '渲染出的规则段带标题')
  ok(sec.text.includes(RULES_SECTION_GUIDE_V1), '渲染出的规则段带约束语引导语')
  ...
  ok(RULES_SECTION_GUIDE_V1 !== head, '规则段引导语 ≠ 开场白（分层落地）')
}
...
console.log('[P6A-6] ★T7-1（在能测的范围内）：规则在**每一轮**的注入函数返回值里都在')
{
  ...
  ok(out1.includes(RULES_SECTION_GUIDE_V1), '规则段带约束语引导语（不是"只是参考"）')
...
console.log('[P6A-8] 边界与卫生：开关默认关 / 模式非法 fail-soft / 无 BOM / S9')
{
  ...
  eq(RULES_LAYER_VERSION, 'rules_layer_v1', '模块版本标识')
  // 切条器：无锚点文件不丢内容
