 ...
 *
 * 背景（实测不一致，2026-09-18）：
 *   Tier-0 **注入**侧有层次优先级（tier-layer-inject.js:58 TIER_LAYER_ORDER_V1
 ...
 *   ③ **常量一致性**：本模块的 FUSION_LAYER_ORDER_V1 ≡ 注入侧 TIER_LAYER_ORDER_V1
 *      （本地复刻 + 测试锁定，防双源漂移）
 *   ④ 与时间臂**互不依赖**：可各自独立生效、可同时生效
 *   ⑤ rank-space 纪律：层次只贡献秩，**不乘分**（Hindsight #3956）
 */
import {
  rankFusionRRFPre, FUSION_RRF_K_V1,
  FUSION_LAYER_ORDER_V1, FUSION_RRF_DIVISOR_V1,
...
{
  eq(FUSION_LAYER_ORDER_V1.length, TIER_LAYER_ORDER_V1.length, '长度一致')
  let same = true
  for (let i = 0; i < TIER_LAYER_ORDER_V1.length; i++) {
    if (FUSION_LAYER_ORDER_V1[i] !== TIER_LAYER_ORDER_V1[i]) { same = false; break }
  }
  ok(same, '★ 逐元素相同：' + JSON.stringify([...FUSION_LAYER_ORDER_V1]))
  eq(String(FUSION_LAYER_ORDER_V1[0]), 'project', '首项 project（最高优先）')
  eq(String(FUSION_LAYER_ORDER_V1[FUSION_LAYER_ORDER_V1.length - 1]), 'log', '末项 log（最低优先）')
  ok(Object.isFrozen(FUSION_LAYER_ORDER_V1), '常量被冻结')
}
...
console.log('\n[5] rank-space 纪律（Hindsight #3956 禁止 score-space 加权）')
{
  ...
  const expect = 1 / (FUSION_RRF_K_V1 + 1 / FUSION_RRF_DIVISOR_V1)
