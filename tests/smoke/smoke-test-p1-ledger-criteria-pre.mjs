 ...
 * 勘误(施工时实测推翻方案假设): P0 窗口的 wb-contract.js 已实现 H1-H4/S1-S4 判据并接线
...
import {
  ...
  HANDOFF_SECTIONS_V1, LEDGER_MAX_CHARS_V1, LEDGER_CRITERIA_VERSION,
} from '../../lib/ledger-criteria.js'
...
t('C8 权威常量对账: 四段标题与 handoff-anchor-pre 权重表同源; 上限 8000', async () => {
  assert.deepEqual([...HANDOFF_SECTIONS_V1], ['任务状态', '目标', '已试方案与失败原因', '进度与下一步'])
  assert.equal(LEDGER_MAX_CHARS_V1, 8000)
  const anchorSrc = await readFile('lib/handoff-anchor.js', 'utf8')
  for (const t of HANDOFF_SECTIONS_V1) assert.ok(anchorSrc.includes(t), '同源标题: ' + t)
})
...
t('C10 接线现状(勘误后守卫): P0 已建 wb-contract 判据 + checkMutationPre 两咽喉已接线; 本窗口交付=对账验证', async () => {
  ...
  assert.ok(src.includes("from './wb-contract.js'"), 'index.js import wb-contract(判据真源)')
  ...
  const lc = await readFile('lib/ledger-criteria.js', 'utf8')
  assert.ok(lc.includes("from './wb-contract.js'"), 'ledger-criteria 与契约同源(单一真源)')
})
