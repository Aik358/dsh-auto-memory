...
import {
  ...
  DEGRADE_SCHEMA_V1, DEGRADE_CAP_V1, DEGRADE_REASON_MAX_V1,
  ARM_STATES_V1, DEGRADE_KINDS_V1,
} from '../../lib/degrade.js'
...
console.log('[1] 记录与聚合')
{
  ...
  eq(snap.schemaVersion, DEGRADE_SCHEMA_V1, '快照带 schemaVersion')
...
console.log('\n[2] ★ 元规则：留痕自身 fail-soft（不得影响主流程）')
{
  ...
  ok(snapVal && snapVal.schemaVersion === DEGRADE_SCHEMA_V1, '快照仍返回结构性合法对象（非 undefined）')
  ...
  eq(longReason.reason.length, DEGRADE_REASON_MAX_V1, '超长 reason 被截断（防撑爆状态文件）')
...
console.log('\n[3] 有界性')
{
  ...
  // 默认 cap
  eq(createDegradeSinkPre()._capForTest, DEGRADE_CAP_V1, '默认 cap = ' + DEGRADE_CAP_V1)
  // 非法 cap 回退默认
  eq(createDegradeSinkPre({ cap: -1 })._capForTest, DEGRADE_CAP_V1, '非法 cap(-1) 回退默认')
  eq(createDegradeSinkPre({ cap: 0 })._capForTest, DEGRADE_CAP_V1, '非法 cap(0) 回退默认')
}
...
console.log('\n[6] 常量完整性')
{
  eq(DEGRADE_SCHEMA_V1, 'degrade_v1', 'schema 常量正确')
  eq(DEGRADE_CAP_V1, 200, 'cap 常量 = 200')
  eq(DEGRADE_REASON_MAX_V1, 200, 'reason 上限 = 200')
  ok(ARM_STATES_V1.includes('no-input'), '★ 臂状态含 no-input（合法状态，区别于 degraded）')
  ok(ARM_STATES_V1.includes('degraded'), '臂状态含 degraded')
  ok(ARM_STATES_V1.includes('unknown'), '★ 臂状态含 unknown（fail-closed 出口）')
  ok(Object.isFrozen(ARM_STATES_V1), 'ARM_STATES 被冻结（防运行时篡改）')
  ok(Object.isFrozen(DEGRADE_KINDS_V1), 'DEGRADE_KINDS 被冻结')
  eq(DEGRADE_KINDS_V1.length, 3, '已知 kind 3 个（仅文档用途，不限制新 kind）')
...
console.log('\n[7] R3-② 落盘：持久化台账到可查询状态文件')
{
  ...
  ok(fs.existsSync(file), '★ 文件真实存在（含父目录自动创建）')
  {
    ...
    eq(o.schemaVersion, DEGRADE_SCHEMA_V1, '落盘内容含 schemaVersion')
