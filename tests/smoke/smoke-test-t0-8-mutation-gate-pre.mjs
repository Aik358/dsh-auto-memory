 ...
 *   3.0 主体拥有 `lib/memory-mutation-pre.js:validateMutationBoundaryPre`（**只收规范化投影**，
 *   不解释图格式）；白板线拥有 `lib/wb-contract-pre.js:parseWhiteboardPre`（解释白板格式）。
 *
...
import {
  ...
  MUTATION_REASONS_PRE_V1,
} from '../../lib/memory-mutation-pre.js'
import {
  parseWhiteboardPre,
  ...
  WB_MARKERS_PRE_V1,
  WB_ANCHOR_RE_PRE_V1,
  HANDOFF_REQUIRED_SECTIONS_PRE_V1,
...
console.log('[WB-1] 白板格式适配器：解析卡片 / 锚点 / 人机分区')
{
  ...
  eq(p.version, 'wb_contract_pre_v1', '适配器版本标识')
  ...
  // 锚点正则与 L0 抽取一致
  ok(WB_ANCHOR_RE_PRE_V1.test('<!-- memory:' + ID('d') + ' -->'), '锚点正则命中合法形态')
  ok(!WB_ANCHOR_RE_PRE_V1.test('<!-- memory:mem_XYZ -->'), '锚点正则拒绝非法形态')
}
...
console.log('[判据] 账本用 H1–H4（不能套给 PLAN）；PLAN 用 P-H1/P-H2/P-S1（刻意最弱）')
{
  ...
  // 四段标题必须逐字
  eq(HANDOFF_REQUIRED_SECTIONS_PRE_V1.length, 4, '四段标题共 4 条（与 handoff-anchor-pre 权重表同源）')
...
console.log('[T0-8B] ★三条写入路径都不能绕过共同保护（源码接线守卫）')
{
  ...
  // 三条路径：① 工具层 memory_note_pre（kind=plan/handoff）② 水位骨架直调 ③ 刷新仪式产物
  ...
  ok(/if \(r && r\.gate === 'mutation'\) return 'memory_note_pre: ' \+ detail/.test(code)
...
{
  const mut = readFileSync(new URL('../../lib/memory-mutation-pre.js', import.meta.url), 'utf8')
  ...
  const wb = readFileSync(new URL('../../lib/wb-contract-pre.js', import.meta.url), 'utf8')
...
{
  for (const p of ['../../lib/memory-mutation-pre.js', '../../lib/wb-contract-pre.js', '../../lib/index.js']) {
    const raw = readFileSync(new URL(p, import.meta.url))
    ok(!(raw[0] === 0xEF && raw[1] === 0xBB && raw[2] === 0xBF), '无 BOM：' + p.split('/').pop())
  }
  for (const p of ['../../lib/memory-mutation-pre.js', '../../lib/wb-contract-pre.js']) {
  ...
  eq(MEMORY_MUTATION_VERSION, 'memory_mutation_pre_v1', '保护门版本标识')
  for (const k of Object.keys(MUTATION_REASONS_PRE_V1)) {
    ok(typeof MUTATION_REASONS_PRE_V1[k] === 'string' && MUTATION_REASONS_PRE_V1[k].length > 0, '原因码有可读中文：' + k)
  }
  // 确定性
