 ...
 *   ① `lib/tier-layer-inject-pre.js` 旧总长门只裁"下探段"、`headParts` 从不动，且降级说明在
...
import {
  ...
  ENVELOPE_BUCKETS_PRE_V1,
  ENVELOPE_REASONS_PRE_V1,
  MEMORY_ENVELOPE_VERSION,
} from '../../lib/memory-envelope-pre.js'
...
console.log('[T0-3-2] ★反面：不允许"未计费尾巴"—— 任何字符都必须归属某个分项')
{
  ...
  const src = readFileSync(new URL('../../lib/memory-envelope-pre.js', import.meta.url), 'utf8')
...
console.log('[T0-3-7] 确定性 / 边界 / 版本')
{
  ...
  eq(MEMORY_ENVELOPE_VERSION, 'memory_envelope_pre_v1', '模块版本标识')
  eq(ENVELOPE_BUCKETS_PRE_V1.length, 3, '分项取值域 = 3 个（与总纲 §0.5 的 chars 形状一致）')
  for (const k of Object.keys(ENVELOPE_REASONS_PRE_V1)) {
    ok(typeof ENVELOPE_REASONS_PRE_V1[k] === 'string' && ENVELOPE_REASONS_PRE_V1[k].length > 0,
      '原因码有可读中文：' + k)
  }
...
{
  const raw = readFileSync(new URL('../../lib/memory-envelope-pre.js', import.meta.url))
