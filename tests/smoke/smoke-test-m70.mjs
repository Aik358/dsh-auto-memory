...
console.log('[G3] worker 往返:ack M5 兼容/激活 M6 兼容/幂等/determinism/零目录')
{
  ...
  ok(!existsSync(path.join(home, 'memory', 'semantic')), 'G3 push-only 不产生 semantic-pre 目录')
  await c.dispose('test')
...
console.log('[G7] 默认关闭零进程/零协议 IO/零目录(harness 经 lib/index.js apply)')
{
  ...
  ok(!existsSync(path.join(home, 'memory', 'semantic')), 'G7 默认零 semantic-pre 目录')
  ...
  ok(!existsSync(path.join(home, 'memory', 'semantic')), 'G7 全程零 semantic-pre 目录')
