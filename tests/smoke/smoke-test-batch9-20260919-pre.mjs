 ...
 *   ⑤ `[Retrieved memory refe` —— **本插件自己注入的召回块标记**（activation-inbox.js:58）
 ...
 */
import { stripRuntimeIntentPre, RUNTIME_ENVELOPE_V1 } from '../../lib/intent-clean-safe.js'
...
// 逐字取自 ~/.dsh/memory/hub/procedures.json（title 前 40 字符，与 client.js:2577 渲染一致）
const REAL_DIRTY = [
...
const F3_ONLY = [
  ...
  'If a reference hints at what you need, use memory_recall or memory_read to fetch full details.',
...
const epi = codeOnly(src('episodic-store.js'))
const hub = codeOnly(src('memory-hub.js'))
ok(epi.includes('stripRuntimeIntentPre'), 'episodic-store-pre 仍在洗')
ok(hub.includes('stripRuntimeIntentPre'), 'memory-hub 仍在洗')
ok(codeOnly(src('intent-clean-safe.js')).includes('PLUGIN_TAIL_MARKER_RE.test'), 'F3 已接入 isEnvelopeLinePre')

console.log('\n[10] 反证：若 F3 未接，⑤ 必然漏网')
const clean = codeOnly(src('intent-clean-safe.js'))
