...
function mkReq(cands) {
  return {
    ...
    memoryIndexVersion: 'idx_pre_' + 'ab'.repeat(16),
...
ok(text.split('\n').filter((l) => l === A.TAIL_MARKER_LINE_PRE_V1).length === 3, '标记行仍每条一个(3 条)')
ok(text.endsWith(A.TAIL_VERIFY_LINE_PRE_V1), 'Verify 收尾行仍在末尾')
...
const jsHost = readFileSync(new URL('../../lib/context-host-pre.js', import.meta.url), 'utf8')
ok(/js-decide lane=/.test(jsHost), 'context-host-pre.js 仍构造 js-decide reason')
