...
import { scanEvidenceEventsPre, aggregateEvidenceEventsPre, EVIDENCE_AGG_DEFAULTS_PRE_V1 } from '../../lib/evidence-agg-pre.js'
import { computeImportancePre, IMPORTANCE_NEUTRAL_PRE_V1 } from '../../lib/memory-importance-pre.js'

const HERE = path.dirname(fileURLToPath(import.meta.url))
const SRC = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'evidence-agg-pre.js'), 'utf8')
let pass = 0, fail = 0
...
console.log('[evidence-agg] G2 聚合形状 = importance 输入契约')
{
  ...
  ok(r1.importance > IMPORTANCE_NEUTRAL_PRE_V1, '聚合直喂 importance:a 高于中性(' + r1.importance.toFixed(2) + ')')
}
console.log('[evidence-agg] G3 correction 负向(口径复用)')
{
  ...
  assert.ok(rb.correctionRate > 0 && rb.importance < IMPORTANCE_NEUTRAL_PRE_V1)
...
ok(EVIDENCE_AGG_DEFAULTS_PRE_V1.maxAgeDays === 7 && EVIDENCE_AGG_DEFAULTS_PRE_V1.maxLinesPerFile === 400, '默认有界参数 7 天/400 行(冻结)')
{
