...
// ---------- ① 家目录口径唯一(从代码取常量,不硬编码) ----------
ok(DSH_HOME_ENV_V1 === 'DSH_HOME', `环境变量名由 dsh-home.js 导出(${DSH_HOME_ENV_V1})`)
const ORIGINAL = process.env[DSH_HOME_ENV_V1]
const PROBE = path.join(ROOT, '.vision-tmp') // 任意非空路径即可,只验证"认这个变量"
process.env[DSH_HOME_ENV_V1] = PROBE
...
if (ORIGINAL === undefined) delete process.env[DSH_HOME_ENV_V1]
else process.env[DSH_HOME_ENV_V1] = ORIGINAL
...
for (const rel of SUITES) {
  ...
  const assignRe = new RegExp(`process\\.env\\.${DSH_HOME_ENV_V1}\\s*=|process\\.env\\[['"]${DSH_HOME_ENV_V1}['"]\\]\\s*=`)
  ok(assignRe.test(code), `${name}: 确实把 ${DSH_HOME_ENV_V1} **赋值**为一次性临时目录(而非仅出现名字)`)
  const firstAt = codeLines.findIndex((l) => assignRe.test(l))
  ok(firstAt >= 0 && firstAt < 60, `${name}: ${DSH_HOME_ENV_V1} 的赋值在文件前部(<60 行,先于任何探测)`, `实际第 ${firstAt + 1} 行`)
