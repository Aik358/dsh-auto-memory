...
import {
  ...
  RERANK_WINDOW_MS_V1, RERANK_KILL_GRACE_MS_V1, RERANK_HOST_VERSION,
} from '../../lib/rerank-host.js'
...
t('T5-3R 精排未完成时前台已返回(offer 即返, 模式 accepted)', async () => {
  ...
  const r = host.offerRerankPre({ query: 'q', orderedIds: ['a', 'b'], memoryIndexVersion: 'idx_0', requestKey: 'rk1' })
...
t('T5-3R 59 秒内完成 + inputKey 完全一致 ⇒ 下一请求复用; 到 60 秒不可复用', async () => {
  ...
  const ids = ['a', 'b']
  host.offerRerankPre({ query: 'q', orderedIds: ids, memoryIndexVersion: 'idx_0', requestKey: 'rk1' })
  ...
  clock.t = RERANK_WINDOW_MS_V1 - 1000 // 59 秒: 窗口内
  const r1 = host.offerRerankPre({ query: 'q', orderedIds: ids, memoryIndexVersion: 'idx_0', requestKey: 'rk2' })
  assert.equal(r1.mode, 'reuse', '59 秒内复用(新 requestKey, inputKey 相同)')
  clock.t = RERANK_WINDOW_MS_V1 + 1 // 60 秒: 到期不续命
  const r2 = host.offerRerankPre({ query: 'q', orderedIds: ids, memoryIndexVersion: 'idx_0', requestKey: 'rk3' })
...
t('T5-3R 改变任一身份字段(query/ordered/miv) ⇒ inputKey 不同 ⇒ 不复用', async () => {
  const k1 = computeRerankInputKeyPre({ query: 'q', orderedIds: ['a', 'b'], memoryIndexVersion: 'idx_0' })
  assert.notEqual(k1, computeRerankInputKeyPre({ query: 'q2', orderedIds: ['a', 'b'], memoryIndexVersion: 'idx_0' }), '查询变 → 不复用')
  assert.notEqual(k1, computeRerankInputKeyPre({ query: 'q', orderedIds: ['b', 'a'], memoryIndexVersion: 'idx_0' }), '顺序变 → 不复用')
  assert.notEqual(k1, computeRerankInputKeyPre({ query: 'q', orderedIds: ['a', 'b'], memoryIndexVersion: 'idx_1' }), 'miv 变 → 不复用')
})

// ---------- T5-4R: 撤回候选后下一轮不得输出; 不能复用旧 fv2 放行结论 ----------
t('T5-4R 缓存只存 scores 不推送 — 主机无 envelope/emit 面(结构性守卫)', async () => {
  const src = await readFile('lib/rerank-host.js', 'utf8')
...
t('T5-5R 超窗任务在宽限内可 kill(killFn 调用+占用释放); busy 门不阻塞稠密路径', async () => {
  ...
  host.offerRerankPre({ query: 'q', orderedIds: ['a'], memoryIndexVersion: 'idx_0', requestKey: 'rk' })
  clock.t = RERANK_WINDOW_MS_V1 + RERANK_KILL_GRACE_MS_V1 + 1
  ...
  const r2 = host.offerRerankPre({ query: 'q2', orderedIds: ['b'], memoryIndexVersion: 'idx_0', requestKey: 'rk2' })
...
// ---------- 版本守卫 ----------
t('V1 版本常量 = rerank_host_v1; 窗口 60s; 宽限 5s(与卡内口径一致)', () => {
  assert.equal(RERANK_HOST_VERSION, 'rerank_host_v1')
  assert.equal(RERANK_WINDOW_MS_V1, 60000)
  assert.equal(RERANK_KILL_GRACE_MS_V1, 5000)
})
