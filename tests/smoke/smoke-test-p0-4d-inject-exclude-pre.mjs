...
import {
  filterExcludedSourcesPre, composeTieredInjectionPre, TIER_BUDGET_V1,
...
test('C1 注入上限写死在冻结常量里(可测,不靠约定)', () => {
  assert.ok(Object.isFrozen(TIER_BUDGET_V1), '预算常量必须冻结(改不动=写死)')
  for (const k of ['B0', 'L1', 'K', 'B2', 'maxTier2Blocks']) {
    assert.equal(typeof TIER_BUDGET_V1[k], 'number', '预算项必须可测: ' + k)
  }
})
