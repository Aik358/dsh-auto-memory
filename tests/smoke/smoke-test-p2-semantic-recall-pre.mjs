...
ok(SRC.includes("defineTool('memory_recall'"), 'memory_recall 工具定义仍在(schema 零改动)')
...
await ta('语义命中:词法不重合(L0 无查询词)但高 cosine 的记录被召回', async () => {
  ...
  assert.match(seenSnap.snap.memoryIndexVersion, /^idx_[0-9a-f]{32}$/, 'miv 格式合法(rank() 校验通过)')
