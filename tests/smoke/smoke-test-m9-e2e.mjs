...
//
// ★#82（2026-09-20）：`loadConfigSync` / `persistConfigSyncPre` 现在调用 `config-io.js`
...
//   （dsh-auto-memory-pre.json → dsh-auto-memory.json）。它用到两个模块作用域符号 ——
//   切片注入的模块里没有 ⇒ 与 #82 同型：ReferenceError 被函数自身的 fail-soft catch 吞掉，
//   表现为「迁移没发生」而非报错。按本文件既有做法**注入真实实现 + 最小替身**：
//     · dshHome()：指向本用例的临时目录（真实实现里就是 ~/.dsh），由用例设置 DAM_TEST_HOME
//     · diag()：仅收集诊断串，供断言「迁移确实发生过 / 幂等时不再写盘」
//     · readJsonQuarantinePreSync / writeTextAtomicPreSync：已从 config-io.js 注入真实实现
const DIAG = []
const dshHome = () => process.env.DAM_TEST_HOME
const diag = (m) => { DIAG.push(String(m)) }
export { DIAG, dshHome, diag }
export class Engine {
...
const { Engine, DIAG } = await import(pathToFileURL(modFile).href)

/** 造临时配置文件 + 引擎实例 */
function fresh(initialConfig) {
  const dir = mkdtempSync(path.join(tmpdir(), 'm9e2e-'))
  // ★去 pre：把 dshHome() 指向本用例的临时目录 ⇒ 旧配置名只在临时目录里找，
  //   既不误读真实 profile 的 ~/.dsh/dsh-auto-memory-pre.json，也让「旧名不存在」成为确定前提。
  process.env.DAM_TEST_HOME = dir
  ...
  return { eng, file, read: () => JSON.parse(readFileSync(file, 'utf8')), clean: () => rmSync(dir, { recursive: true, force: true }), dir }
}
