...
import {
  buildSidecarEntryPre, sidecarEntryToCatalogItemPre, WB_ANCHOR_LINE_RE_V1,
} from '../../lib/wb-sidecar.js'
...
console.log('\n[4] 桥接 sidecarEntryToCatalogItemPre（只映射，不发明）')
{
  const entry = buildSidecarEntryPre({
    workspaceKey: 'wk_m1', relPath: 'handoff/PLAN.md',
    text: '## 卡片标题\n<!-- memory:' + ID + ' -->\n这是正文的一句话。',
...
console.log('\n[6] 常量一致性')
{
  ...
  const lineRe = WB_ANCHOR_LINE_RE_V1
...
console.log('\n[7] 设计纪律：不新建独立文件（S10.4）')
{
  ...
  const CAT_RAW = fs.readFileSync('lib/tier0-catalog.js', 'utf8')
  const SIDE_RAW = fs.readFileSync('lib/wb-sidecar.js', 'utf8')
