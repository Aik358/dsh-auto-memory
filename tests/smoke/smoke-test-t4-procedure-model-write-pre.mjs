...
//   procedural 线唯一来源是 memory-hub.js 的 crossFeed —— 它把每个"成功 episode"机械切成候选,
...
//
// 本批新增 memory_procedure 模型直写通路 + promote() 的 opts.authorizedBy 授权跳门。
// 本文件锁死:
...
//   T4-5 工具存在性:index.js 真注册了 memory_procedure(定义 + 收进 tools 数组两步)
...
console.log('[T4-1] 向后兼容:不传 opts ⇒ 两条统计门逐字节照拦(授权特性不得改变既有行为)')
{
  ...
  ok(String((pr.reasonCodes || []).join(',')) === 'diversity-below-' + PROCEDURE_DEFAULT_GATES_V1.minSessionDiversity,
    '原因码恰为 diversity-below-' + PROCEDURE_DEFAULT_GATES_V1.minSessionDiversity + '(未被 model-authorized 污染)')
  ok(pr.procedure === undefined || pr.procedure.authorizedBy === undefined, '未被授权 ⇒ 不写 authorizedBy')
  // 传空对象 opts 也必须与不传等价
  const pr2 = st.promote(pid, {}, {})
  ok(pr2.decision === 'keep' && String((pr2.reasonCodes || []).join(',')) === 'diversity-below-' + PROCEDURE_DEFAULT_GATES_V1.minSessionDiversity,
    'opts={} 与不传等价(空授权不放行)')
}
...
console.log('\n[T4-3] 授权**不能**突破的结构门(护栏必须仍然有效)')
{
  ...
  const { st } = mk()
  const r = st.observe({ title: 'Current runtime context. This snapshot s', steps: ['观察任务：x'], observationOnly: true, sourceEpisodes: ['epi_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'], sourceMemoryIds: [] })
...
{
  ok(INDEX_SRC.includes("defineTool('memory_procedure'"), '① index.js 里有 defineTool(\'memory_procedure\')')
  ...
  ok(body.includes("defineTool('memory_procedure'"),
    '② 定义**位于 tools 数组体内**(不是数组外——BUG-15 同款缺陷)')
  // 关键能力必须在描述里写明(模型靠描述决定何时调用)
  const descStart = body.indexOf("defineTool('memory_procedure'")
...
{
  ok(INDEX_SRC.includes('★技能库(procedure memory)直写') || INDEX_SRC.includes('memory_procedure') ,
  ...
  ok(seg.includes('memory_procedure'), '★ 该文案位于 renderMemoryStatic() 方法体内(会真的注入)')
...
{
  const PS = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'procedure-store.js'), 'utf8')
