 ...
 * ② 接线断言为辅：index.js 必须 import 并委托 hub-io-pre.js、debugInfo 必须暴露 hubIo 投影、
...
import {
  ...
  HUB_IO_ERRNO_MESSAGES_PRE_V1,
} from '../../lib/hub-io-pre.js'
...
const dir = path.join(tmp, 'hub-pre')
...
try {
  ...
  console.log('== ③ errno → 人话（用户口径：暴露给前端的原因必须人能看懂） ==')
  {
    ...
    ok(Object.isFrozen(HUB_IO_ERRNO_MESSAGES_PRE_V1), 'errno 词典是冻结常量（防运行时被改写）')
    ok(Object.keys(HUB_IO_ERRNO_MESSAGES_PRE_V1).length >= 8, 'errno 词典覆盖常见失败码')
  }
  ...
  {
    ok(/import \{[^}]*createHubIoPre[^}]*\} from '\.\/hub-io-pre\.js'/.test(INDEX), 'index.js 从 ./hub-io-pre.js 导入适配器')
