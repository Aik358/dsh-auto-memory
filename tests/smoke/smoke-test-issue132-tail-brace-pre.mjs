...
console.log('[issue132] G2b 出口兜底:身份行与技能段字段不得穿透(二次修复回归)')
{
  ...
  ok(String(r2.text).trim().endsWith(A.TAIL_VERIFY_LINE_V1), 'Verify 收尾行仍在')
}
...
if (!renderContextSections) {
  ...
  for (const [name, reference] of cases) {
    ...
    let err = null
    try {
      renderContextSections({ sections: [], tools: [], variables: [], contexts: [{ name: 'dsh:m6-reference-tail', text: r.text }] })
    } catch (e) { err = e }
...
console.log('[issue132] G4 packet 构建:digest 往返一致(渲染即投递不掉单)')
{
  ...
  if (built.ok) {
    ...
    const re = A.renderReferenceTail(p.references, { reason: p.triggerReason, budgetBytes: A.REFERENCE_TAIL_BUDGET_V1.maxPacketBytes, skill: p.skill })
