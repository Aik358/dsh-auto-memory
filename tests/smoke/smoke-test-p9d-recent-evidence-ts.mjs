...
//   「孪生体未被手改」这个对象**已被有意移除**。
//   按纪律 35（守的语义未变、对象被有意移除 ⇒ 反转断言，并补保留面）：
//   语义「源里必须有新写法、不得退回旧写法」保持不变，只是检查对象从「产物」变成「源本身」。
ok(SRC.includes('Number(e.event && e.event.ts)'), '源 context-host.js 含修复后写法（去 pre 后它就是唯一源）')
