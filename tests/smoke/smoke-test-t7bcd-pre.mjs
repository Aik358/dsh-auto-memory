...
import { skillDirNamePre } from '../../lib/skill-export-pre.js'
...
t('T7b-1 ★ 纯中文标题不得回落 untitled', () => {
  const d = skillDirNamePre({ procedureId: 'proc_pre_x', title: '中文标题' })
...
t('T7b-3 ★★ 产出的名字必须能过 DSH SKILL_NAME 硬校验', () => {
  ...
  const bad = cases.map((title) => skillDirNamePre({ procedureId: 'proc_pre_x', title })).filter((d) => !SKILL_NAME_RE.test(d))
  assert(bad.length === 0, '★ 以下名字会被宿主拒绝加载: ' + bad.join(' | '))
})
t('T7b-4 哈希回落确定（同 title 同名字）+ 不同 title 不撞名', () => {
  const a = skillDirNamePre({ procedureId: 'proc_pre_x', title: '中文标题' })
  const b = skillDirNamePre({ procedureId: 'proc_pre_x', title: '中文标题' })
  const c = skillDirNamePre({ procedureId: 'proc_pre_x', title: '另一个中文标题' })
  assert(a === b, '同 title 不稳定')
  assert(a !== c, '不同 title 撞名')
})
t('T7b-5 ★ description 取 title 而非 steps[0]（技能用途优先于首个动作）', () => {
  assert(!/const s = step \|\| t/.test(
    readFileSync(path.join(ROOT, 'lib', 'skill-export-pre.js'), 'utf8')
...
t('T7d-1 ★★ 铭文② 必须是可判定硬映射：改过 lib/ ⇒ 必写 kind=handoff', () => {
  ...
  assert(/memory_note_pre\(kind=handoff\)\*\* 四段式账本/.test(IDX), '缺少"必写 handoff 账本"的强制措辞')
})
...
t('T7d-5 铭文② 仍保留 kind=note 这条正常通路（不得只剩强制性）', () => {
  // 最终文案形态：'才用 **memory_note_pre(kind=note, action=append)**。'
  assert(/memory_note_pre\(kind=note, action=append\)/.test(IDX), 'kind=note 通路被误删')
})
