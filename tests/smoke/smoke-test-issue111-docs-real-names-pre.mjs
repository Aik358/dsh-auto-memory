...
const REL_HUB = relName('hub-pre')
const REL_EVIDENCE = relName('evidence-pre')
...
for (const [f, text] of docs) {
  ...
  // ★必须连**目录分隔符**一起匹配:`memory/hub` 是 `memory/hub-pre` 的前缀,
  //   只比裸名会被 pre 写法"蹭"过去(变异实测:把 hub/ 全换成 hub-pre/ 时裸名断言仍绿)。
