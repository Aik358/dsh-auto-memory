...
console.log('[G2] framing:partial/multiple/bad JSON/oversize/epoch 门/type 混用')
{
  ...
  // before the manually injected partial frame is completed, making the test racy on
  // faster CI runners. Use a stdin-draining silent worker for all synthetic responses,
  // then switch back to the real worker only for the crash-recovery assertion.
