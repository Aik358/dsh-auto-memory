...
// ---------- 源码级事实（剥离注释后断言，防注释误伤） ----------
const SRC_STORAGE = stripComments(readSrc('storage-manage.js'))
const SRC_EVIDENCE = stripComments(readSrc('evidence-store.js'))
const SRC_EPISODIC = stripComments(readSrc('episodic-store.js'))
const SRC_ACT = stripComments(readSrc('activation-host.js'))
const SRC_BRIDGE = stripComments(readSrc('context-bridge.js'))
const SRC_INDEX = stripComments(readSrc('index.js'))

console.log('=== issue #55 storage-manage.js：readSidecarPrev 的 docStore ===')
...
await t('#55-3 readSidecarPrev 经真实调用：注入 docStore 后能取到 prev（修复前恒 null）', async () => {
  const { createStorageManagerPre } = await import(new URL('storage-manage.js', LIB))
...
console.log('=== issue #56 evidence-store.js：写失败必须释放幂等登记 ===')
...
await t('#56-3 行为：首次写失败 → 同 id 重试必须成功（修复前恒 duplicate-evidence）', async () => {
  const { EvidenceEventStore } = await import(new URL('evidence-store.js', LIB))
  ...
  const { validateAccessEvidencePre } = await import(new URL('context-bridge.js', LIB))
...
console.log('=== issue #57 episodic-store.js：restore 的 current 形状校验 ===')
...
await t('#57-4 行为：伪造 current={} 恢复后不崩，consolidate 不抛 TypeError', async () => {
  const { createEpisodicStorePre } = await import(new URL('episodic-store.js', LIB))
  const io = { save: () => {}, clear: () => {} }
...
console.log('=== issue #58 activation-host.js：dispose 接线 + 键格式统一 ===')
...
await t('#58-5 行为：disposeRuntime 真的清掉步进计数器（键对齐实证）', async () => {
  const { createActivationHost } = await import(new URL('activation-host.js', LIB))
