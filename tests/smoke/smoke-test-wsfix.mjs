...
function makeCaller(routes) {
  return async (p) => {
    let body = null
    const route = routes.find((r) => r.path === '/api/dsh-auto-memory/handoff-state')
...
const rA = await call1('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_A)
...
const rC = await call1('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_C)
ok(rC && rC.ws === WS_R && rC.wsBound === true, 'W4 registry 反查命中:workspaceId 创建的会话解析到绑定工作区')
const rD = await call1('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_D)
ok(rD && rD.ws === WS_D && rD.wsBound === true, 'W5 持久化会话头回退:磁盘首行 cwd 生效')
const rE = await call1('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_E)
...
const rG = await call1('/api/dsh-auto-memory/handoff-state')
...
const rF = await call1('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_A + '&file=PLAN.md')
...
const r2A = await call2('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_A)
ok(r2A && r2A.ws === WS_C2 && r2A.wsBound === true, 'W9 默认配置:会话工作区跟随当前 cwd(无锁)')
agents[SID_A].session.header.cwd = WS_B
const r2A2 = await call2('/api/dsh-auto-memory/handoff-state?sessionId=' + SID_A)
