...
console.log('[G5] release 构建已退化为纯复制（去 pre 后无改名表）')
// ★★去 pre 后的语义反转（2026-09-23，用户拍板「两张改名表整段删除，构建退化为纯复制」）：
//   本节原有断言守的是 `tools/release.mjs` 的 transforms / residual 两张表里
//   「memory_rules_pre → memory_rules」这条登记 —— **该对象已被有意移除**。
//   按纪律 35（守的语义未变、对象被移除 ⇒ 反转断言并补保留面）：
//   语义「发布物不含任何带后缀的名字」保持不变，只是它现在由**源码本身**保证
//   （无需转换表），故改为断言「改名表不存在」+「源码工具名本身即裸名」双面。
ok(!/const transforms\s*=\s*\[/.test(SRC_REL), '★ transforms 改名表已整段删除（构建退化为纯复制）')
ok(!/const residual\s*=\s*\[/.test(SRC_REL), '★ residual 残留闸门表已整段删除')
ok(SRC_REL.includes('note-status'), 'note-status 仍是模块基名（无 -pre 后缀）')
// 保留面：源码里的工具名不得再带 _pre（去 pre 的核心承诺，由源码直接保证）
const preToolNames = (SRC_IX.match(/defineTool\('([A-Za-z_]+)'/g) || []).filter((s) => /_pre'/.test(s))
eq(preToolNames.length, 0, '★ 源码中不存在任何带 _pre 后缀的工具名（去 pre 承诺）')
