...
import {
  createL0IndexPre,
} from '../../lib/l0-index-pre.js'
import {
  computeEngineIdentityPre, isEngineIdentityPre, engineIdentityMatchesPre,
  aliasKeyPre, vectorKeyPre, JS_E5_IDENTITY_DESC_PRE_V1, pyBgeM3IdentityDescPre,
  canonicalEngineDescPre, ENGINE_IDENTITY_FIELDS,
} from '../../lib/engine-identity-pre.js'
import {
  createEngineSwitchPre, ENGINE_SWITCH_VERSION,
} from '../../lib/engine-switch-pre.js'
import { createIndexSyncHostPre } from '../../lib/m7-index-sync-host-pre.js'
import { createL0IndexSyncPre } from '../../lib/l0-index-sync-pre.js'
...
// ---------- 注入件（与 smoke-test-l0-index-pre 同口径） ----------
...
const ID_E5 = computeEngineIdentityPre(JS_E5_IDENTITY_DESC_PRE_V1)
...
t('宽身份：tokenizer 变化 → 身份必变（单个 PROVIDER_ID 不足以标识）', () => {
  const a = computeEngineIdentityPre({ ...JS_E5_IDENTITY_DESC_PRE_V1 })
  const b = computeEngineIdentityPre({ ...JS_E5_IDENTITY_DESC_PRE_V1, tokenizer: 'e5-tokenizer-v2' })
  const c = computeEngineIdentityPre({ ...JS_E5_IDENTITY_DESC_PRE_V1, dim: 768 })
  const d = computeEngineIdentityPre({ ...JS_E5_IDENTITY_DESC_PRE_V1, weightsDigest: 'other-weights' })
  assert.notEqual(a, b, 'tokenizer 进身份')
  assert.notEqual(a, c, '维度进身份')
  assert.notEqual(a, d, '权重摘要进身份')
})
t('宽身份：同一描述两次计算确定性；字段顺序无关', () => {
  const d = { ...JS_E5_IDENTITY_DESC_PRE_V1 }
  const flipped = {}
...
const mkSnapshot = () => ({
  memoryIndexVersion: 'idx_pre_' + '1'.repeat(32),
...
t('接线守卫：L0 索引注入真实宽身份（computeEngineIdentityPre(JS_E5_…)）', () => {
  assert.ok(/engineIdentity: computeEngineIdentityPre\(JS_E5_IDENTITY_DESC_PRE_V1\)/.test(SRC_INDEX), 'L0 用真实身份')
