...
import { INDEX_MAX_FILE_BYTES } from '../../lib/memory-index.js'

const T0 = await import('../../lib/tier0-catalog.js')
const L0 = await import('../../lib/l0-extract.js')
const IDX = await import('../../lib/l0-index.js')
const SYNC = await import('../../lib/l0-index-sync.js')
const A = await import('../../lib/activation-inbox.js')

const HERE = path.dirname(fileURLToPath(import.meta.url))
const ROOT = path.resolve(HERE, '..', '..')
const SRC_INDEX = readFileSync(path.join(ROOT, 'lib', 'index.js'), 'utf8')
const SRC_SYNC = readFileSync(path.join(ROOT, 'lib', 'l0-index-sync.js'), 'utf8')
const SRC_SEMJS = readFileSync(path.join(ROOT, 'lib', 'semantic-js.js'), 'utf8')
...
console.log('\n[G4] 命中后按 id 下探：expand 取回原文（契约 §7.4 回归钉子）')
{
  ...
  ok('G4-非法 id → 结构化提示（不抛异常）', bad.startsWith('memory_recall: expand 需要合法记忆 id'))
...
console.log('\n[G6] C3 接线：开关默认开（设 false 才不写索引、零 IO）；开启 + 假 embedder → 写出索引且条目带 layer/status')
{
  ...
  ok('G6-宿主 import 了新接线模块', /import \{ createL0IndexSyncPre \} from '\.\/l0-index-sync\.js'/.test(SRC_INDEX))
  ...
  ok('G6-零引用的 l0-index 现在被接线模块引用', /from '\.\/l0-index\.js'/.test(SRC_SYNC) && /idx\.update\(/.test(SRC_SYNC))
  ...
  ok('G6-索引可被 l0-index 读回（load.ok 且条目 ≥1）', layerOk && loadedCount >= 5)
...
console.log('\n[G7] C7 回归：注入块含 `Score: 0.xx (rank n/m)` 且块序严格降序')
{
  ...
  const mkReq = (cands) => ({
    schemaVersion: 1, namespace: A.NAMESPACE, kind: 'activation_request',
    activationId: A.ACTIVATION_ID_PREFIX + 'ab'.repeat(16), observationId: 'obs_' + 'cd'.repeat(16),
    workerEpoch: 'worker-e1', sessionId: 'sess-9', agentId: 'agent-9', workspaceKey: 'd:/ws',
    scope: 'Workspace', contextVersion: 7, memoryIndexVersion: 'idx_' + 'ab'.repeat(16),
    threshold: { policyVersion: 'thr_v1', score: 0.91, threshold: 0.8, reason: 'fv2 lane=explicit emit intent=0.91 dense=0.88 margin=0.21 explicit_lane' },
