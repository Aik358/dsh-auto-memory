...
import {
  ...
  TIER_HITS_REUSE_REASONS_PRE_V1,
...
const SID = 'sess-A'
const WS = 'D:\\ws-a'
const OBS = 'obs_pre_' + 'b'.repeat(32)
...
console.log('[T0-2-3] fail closed：凡不能证明同版同源，一律不复用（对比旧实现的 fail open）')
{
  ...
  // ⑤ 观测身份不匹配
  eq(call({ observationId: 'obs_pre_' + 'e'.repeat(32) }).reason, 'observation-mismatch',
...
{
  for (const code of Object.keys(TIER_HITS_REUSE_REASONS_PRE_V1)) {
...
{
  const src = readFileSync(new URL('../../lib/activation-inbox-pre.js', import.meta.url), 'utf8')
  ...
  const mod = src
  ok(!/\uFEFF/.test(mod), 'activation-inbox-pre.js 无 BOM')
  for (const p of ['../../lib/index.js', '../../lib/tier-layer-inject-pre.js', '../../lib/activation-host-pre.js', '../../lib/activation-inbox-pre.js']) {
