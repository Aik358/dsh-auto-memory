...
import { rankFusionRRFPre, FUSION_RRF_K_PRE_V1, FUSION_RRF_DIVISOR_PRE_V1, RECALL_FUSION_VERSION } from '../../lib/recall-fusion-pre.js'
import { fuseD6Pre, D6_FUSION_WEIGHTS_PRE_V1 } from '../../lib/semantic-js-pre.js'

let pass = 0, fail = 0
const t = (name, fn) => { try { fn(); pass++; console.log('  ok -', name) } catch (e) { fail++; console.error('FAIL', name + ':', e.message) } }
const ok = (c, n) => { if (c) { pass++; console.log('  ok -', n) } else { fail++; console.error('FAIL', n) } }
const HERE = path.dirname(fileURLToPath(import.meta.url))
const SJ = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'semantic-js-pre.js'), 'utf8')

// ---------- 常量 ----------
t('常量', () => {
  assert.equal(FUSION_RRF_K_PRE_V1, 60)
  assert.equal(FUSION_RRF_DIVISOR_PRE_V1, 60)
  assert.equal(RECALL_FUSION_VERSION, 'rrf_fusion_pre_v1')
})
...
t('fuseD6Pre 存量行为零改动:minmax 语义仍在(源码+行为双守卫)', () => {
  assert.ok(SJ.includes('const normArm = (key) => {'), 'minmax normArm 仍在 semantic-js-pre.js')
  assert.ok(SJ.includes('D6_FUSION_WEIGHTS_PRE_V1.dense * denseN + D6_FUSION_WEIGHTS_PRE_V1.lexical * lexN'), '加权公式仍在')
  ...
  assert.equal(D6_FUSION_WEIGHTS_PRE_V1.dense, 0.7)
