 ...
 *
 * 背景:`smoke-test-peer-probe.mjs` / `smoke-test-m81-c2-wiring.mjs` /
 * `smoke-test-m81-fact-metadata.mjs` 曾读**真实 `~/.dsh`**(其中 fact-metadata 还手拼
 * `process.env.USERPROFILE || process.env.HOME`),后果是「本机绿、别机红」——
 * 「回归全绿」不可判定,因为绿灯描述的是**这台机器恰好有什么**,不是代码是否正确。
 *
 * 本守卫钉住四件事:
 *   ① 家目录口径唯一:`lib/dsh-home.js` 的 `resolveDshHomePre()` 必须认 `DSH_HOME`
...
import { resolveDshHomePre, DSH_HOME_ENV_PRE_V1 } from '../../lib/dsh-home.js'
...
// ---------- ① 家目录口径唯一(从代码取常量,不硬编码) ----------
ok(DSH_HOME_ENV_PRE_V1 === 'DSH_HOME', `环境变量名由 dsh-home.js 导出(${DSH_HOME_ENV_PRE_V1})`)
...
// ---------- ②③④ 三个套件的源码文本断言(去注释后) ----------
const SUITES = [
  'tests/smoke/smoke-test-peer-probe.mjs',
  'tests/smoke/smoke-test-m81-c2-wiring.mjs',
  'tests/smoke/smoke-test-m81-fact-metadata.mjs',
]
...
const NEEDS_SKIP = ['tests/smoke/smoke-test-peer-probe.mjs', 'tests/smoke/smoke-test-m81-fact-metadata.mjs']
...
const fm = stripComments(read('tests/smoke/smoke-test-m81-fact-metadata.mjs'))
