...
writeFileSync(OKW, [
  ...
  '      protocolVersion: \'m7_wire_v1\', frameId: \'f-\' + req.requestId, requestId: req.requestId,',
...
function feedAckFor(c, phase) {
  ...
  const line = JSON.stringify({
    protocolVersion: 'm7_wire_v1', frameId: 'f-' + f.requestId, requestId: f.requestId,
...
console.log('\n[G1] 契约与影响面(源码级反向锁,不起进程)')
{
  ...
  ok(/const TERMINAL_SUCCESS_TYPES_V1 = new Set\(\['health', 'context_push', 'index_sync_commit'\]\)/.test(src),
    'G1 终态成功集合 = health/context_push/index_sync_commit')
  ok(!/TERMINAL_SUCCESS_TYPES_V1 = new Set\(\[[^\]]*index_sync_begin/.test(src),
    'G1 段内进度帧 index_sync_begin 不在终态集合(否则熔断永不可达)')
  ok(!/TERMINAL_SUCCESS_TYPES_V1 = new Set\(\[[^\]]*index_sync_page/.test(src),
  ...
  ok(!/semantic-js-pre|memory-hub|fact-store-pre|procedure-store-pre|context-host-pre/.test(src),
...
console.log('\n[G2] 熔断口径:段内进度帧成功不清零 / 终态成功才清零(#107)')
{
  ...
  const p = c.request('index_sync_begin', { syncId: 'syn_' + '0'.repeat(32) })
  feedAckFor(c, 'begin')
  const rb = await p
  ...
  const bad = c2.request('index_sync_begin', { syncId: 'syn_' + '0'.repeat(32) }, { timeoutMs: 120 })
...
console.log('\n[G6] 模型友好:Python 档未启用时零进程零 IO,不作为 JS 语义生效的前提')
{
  ...
  ok(!/semantic-js-pre|semantic-json|memory-hub/.test(readFileSync(SRC, 'utf8')), 'G6 3 本模块不触碰 JS 语义实现')
