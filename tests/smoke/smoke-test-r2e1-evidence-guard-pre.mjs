 ...
 *   写侧 evidence-store-pre.js:126 的 mkdirSync 是**懒建**（只在首次成功 append 时创建），
...
const SRC_STORE = fs.readFileSync(path.join(ROOT, 'lib', 'evidence-store-pre.js'), 'utf8')
...
{
  const { EvidenceEventStore } = await import(pathToFileUrl(path.join(ROOT, 'lib', 'evidence-store-pre.js')))
  const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'dam-r2e1-'))
  const root = path.join(tmpRoot, 'evidence-pre')
