...
function makeHub(opts = {}) {
  return createMemoryHubPre({
    ...
    now,
    // ★A-1（2026-09-23）：机械 procedure 切片**整条通路已删除**（用户裁定「机械通路已被我弃用」）。
    //   旧实现在此显式开启该开关以锁「旧行为」。现在开关与分支都不存在 ⇒ 选项本身已无意义，
    //   传了也是无害的未知键（createMemoryHubPre 按 opts 白名单读取）。
    //   issue #30 真正要守的语义（观察行与富候选互不污染、观察行不可自证晋升）改为
    //   用 `stores.procedures.observe()` 直接构造候选来锁 —— 见下方 [P1']/[P5b]。
    ...opts,
  })
}
...
//   一个锁「显式开启时的旧行为」，一个锁「缺省时的新行为」。
{
  ...
  ok(cfDefault.ok, 'T10-1 crossFeed 仍正常返回(不报错)')
  ok(!cfDefault.fed.some((f) => f.to === 'procedure'), 'T10-2 ★★ **不**产出机械 procedure 观察（无模型介入的截断行不再入店）')
  ok(hubDefault.stores.procedures.query().length === 0, 'T10-3 ★★ procedures 店保持空（procedural 线只留模型直写/人工）')
}

console.log('[P1] 观察行（无 memoryId）不可自证晋升 + promote 首闸')
{
  // ★A-1（2026-09-23）：crossFeed 不再产出 procedure 观察（机械切片通路已删）。
  //   但本块要守的语义**没变**且更重要：**纯观察行不能自证晋升**。
  //   ⇒ 改为直接经 stores.procedures.observe() 构造观察行来锁该语义
  //   （这条通路仍是活代码：episode 侧若有其它来源、或 judgement 行缺 memoryId，
  //     产出的就是这种 observationOnly 行）。
  const hub = makeHub()
  ...
  ok(cf.ok, 'crossFeed 仍正常返回(不报错)')
  ok(!cf.fed.some((f) => f.to === 'procedure'), '★A-1 crossFeed 不再产出 procedure 观察（机械切片整段删除）')
  // 直接构造观察行 —— 这是 issue #30 真正要守的对象
  const o = procs.observe({
    title: '观察任务：部署这个服务', riskLevel: 'low',
    steps: ['观察任务：帮我部署这个服务到服务器'],
    observationOnly: true, sourceEpisodes: ['epi_x'], sourceMemoryIds: [],
  })
  ok(o.ok, '观察行仍可入店(observe 通路未被 A-1 破坏)')
...
console.log('[P5b] 根因回归:同名 观察行 与 富候选 必须互不污染(issue#30 真正病灶)')
{
  // 实测根因:episode 自动观察出的"仅观察行"(无 successCriteria)通常**先**入店;
  // 之后带真实步骤/成功判据的富候选若与它**同名**,旧实现的按 title 去重会把富候选
  // 合并进观察行,而合并只拷贝证据源字段 ⇒ 富候选的 steps/successCriteria **整体丢失**,
  // 表现为 promote() 永远返回 no-success-criteria,技能永远无法晋升。
  // (PR 自带用例只验了 observationOnly 标记本身,**未覆盖这条同名碰撞链**,故在此补齐。)
  const io = memIO()
  const st = createProcedureStorePre({ io: io.io, now })
  const obs = st.observe({ title: '发布流程', riskLevel: 'low', steps: ['观察任务：发布'], observationOnly: true, sourceEpisodes: ['epi_1'], sourceMemoryIds: [] })
  ok(obs.ok && obs.merged === false, '先入店的仅观察行独立成行')
  const rich = st.observe({ title: '发布流程', riskLevel: 'low', steps: ['步骤1'], successCriteria: ['部署成功'], sourceMemoryIds: ['mem_1'], sourceEpisodes: ['epi_2'] })
  ok(rich.ok && rich.merged === false, '同名富候选**不得**被合并进观察行(旧实现此处 merged=true)')
  ok(rich.procedure.successCriteria.length === 1, '富候选的 successCriteria 必须完整保留(旧实现被吞成 [])')
  ok(st.query().length === 2, '观察行与富候选共存为 2 行(标题不是身份)')
  // 富候选必须能真正晋升(这是 issue#30 的用户可见症状:永远晋升不了)
  const pr = st.promote(rich.procedure.procedureId, { distinctSessions: 3, successCount: 2 })
  ok(pr.ok && pr.decision === 'promote', '富候选在证据达标后可正常晋升(实得 ' + pr.decision + ' / ' + JSON.stringify(pr.reasonCodes) + ')')
  // 观察行必须被显式告知"结构上不晋升",而不是报"缺判据"
  const prObs = st.promote(obs.procedure.procedureId, { distinctSessions: 3, successCount: 2 })
  ok(prObs.decision === 'keep' && prObs.reasonCodes[0] === 'observation-only', '观察行晋升被短接为 observation-only(不是误导性的 no-success-criteria)')
}
...
console.log('[P6] overview 审批队列表透出 observationOnly')
{
  const hub = makeHub()
  const procs = hub.stores.procedures
  // ★A-1（2026-09-23）：crossFeed 不再产出 procedure 观察 ⇒ 直接构造观察行，
  //   本块要守的是「overview 的行带 observationOnly（前端据此打观察标）」，与来源无关。
  procs.observe({
    title: '观察任务：整理并归档报告文件', riskLevel: 'low',
    steps: ['观察任务：整理并归档报告文件'],
    observationOnly: true, sourceEpisodes: ['epi_z'], sourceMemoryIds: [],
  })
