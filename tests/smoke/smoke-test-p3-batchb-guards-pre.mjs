...
const CH = rd('lib/context-host-pre.js')
const CI = rd('lib/config-io-pre.js')
const SJ = rd('lib/semantic-js-pre.js')
const SH = rd('lib/shadow-host-pre.js')
const EV = rd('lib/evidence-store-pre.js')
const M7 = rd('lib/m7-index-sync-host-pre.js')
...
t('P3B-2a ★★ 零运行时依赖不变量 + 只动异步版（同步 renameSync 必须保留且恰好 1 处）', () => {
  ...
  // 故 P3-12 的退避 rename 必须**内联**，不得 import 兄弟模块（fs-retry-pre.js）。
...
await ta('P3B-2b ★★ 真行为：retryRename 对瞬时 EBUSY 必须退避重试（改回裸 rename 必红）', async () => {
  const M = await load('fs-retry-pre.js')
  const calls = []
...
await ta('P3B-2d ★★★ 真行为：并发同路径写必须**全成功**、不产半截、不留 tmp（tmp 唯一性回归绊索）', async () => {
  const M = await load('config-io-pre.js')
...
await ta('P3B-3c ★★ 真行为：dev 树闸门=存在性（无目录→不回落；有目录→无环境变量也命中）', async () => {
  const M = await load('semantic-js-pre.js')
...
await ta('P3B-4e ★★ 真行为：连续 2 次写失败只透出 1 条诊断（首条立即、次条被节流）', async () => {
  const M = await load('evidence-store-pre.js')
...
await ta('P3B-5d ★★ 真行为：debugView 开门后不得再投影该字段（且确实走到了真投影分支）', async () => {
  const M = await load('m7-index-sync-host-pre.js')
  const engine = {
