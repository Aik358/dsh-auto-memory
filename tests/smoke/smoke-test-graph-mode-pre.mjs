...
import { buildKanbanMatrixPre as buildMatrixForTest } from '../../lib/wb-sidecar-pre.js'
...
t('A2 两遍历工具契约完整(parameters.id/tag + execute 是函数)', () => {
  ...
  const tr = bg.tools.find((x) => x.name === 'memory_trace_pre')
...
// ─────────────────────────────────────────────────────────────
const sb = await import('../../lib/wb-sidecar-pre.js')
...
t('D1 legacy 档工具数 = 16(与 graph 档 18 形成对照, 证明闸门真的在起作用)', () => {
  ...
  try {
    ...
    assert(bl.tools.length === 16, 'legacy 档必须仍为 16 工具(14 + T4 memory_procedure_pre + P9 memory_rules_pre), 实为 ' + bl.tools.length)
...
t('E2 P3-2 第3层唤醒句在场(且指明两工具名)', () => {
  ...
  assert(IDX_SRC.includes('memory_expand_pre(tag=') && IDX_SRC.includes('memory_trace_pre(id)'),
    'P3-2 唤醒句必须同时点名 memory_expand_pre 与 memory_trace_pre')
})
...
t('F3 宿主侧 kanbanBoardData 挂载 + REST 路由 + 两侧路径表同步', () => {
  ...
  assert(IDX_SRC.includes("'kanban-board': '/api/dsh-auto-memory-pre/kanban-board'"), '宿主 API 表缺 kanban-board')
...
t('F6 宿主侧 kanbanBoardData 挂载 + **被调符号必须已 import** + 路由两侧同步', () => {
  ...
  assert(imp, '未找到 wb-sidecar-pre.js 的 import 语句')
...
ta('F8 看板路由**端到端真调用**: graph 档返回 enabled=true 且 cardSource=section(绝不是 error)', async () => {
  ...
  try {
    ...
    await route.handler({
      url: '/api/dsh-auto-memory-pre/kanban-board', method: 'GET',
...
t('G3 矩阵投影: 数据层 export + **被调符号已 import** + host 载荷回传', () => {
  const WBS = readFileSync(new URL('../../lib/wb-sidecar-pre.js', import.meta.url), 'utf8')
  ...
  const imp = /import\s*\{([\s\S]*?)\}\s*from\s*'\.\/wb-sidecar-pre\.js'/.exec(IDX_SRC)
  assert(imp, '未找到 wb-sidecar-pre.js import')
  const imported = imp[1].split(',').map((s) => s.trim()).filter(Boolean)
