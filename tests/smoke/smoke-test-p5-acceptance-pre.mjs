...
import { createAcceptanceLedgerPre, ACCEPTANCE_ITEMS_V1, ACCEPTANCE_VERSION } from '../../lib/acceptance.js'
...
t('A1 清单完整性: 7 必需项(T6-1/2/3/5/6/7 + compat-U1), 版本常量正确', () => {
  assert.equal(ACCEPTANCE_VERSION, 'acceptance_v1')
  assert.equal(ACCEPTANCE_ITEMS_V1.length, 7)
  assert.ok(ACCEPTANCE_ITEMS_V1.every((x) => x.required === true), '全部为必需项')
  const ids = ACCEPTANCE_ITEMS_V1.map((x) => x.id)
...
t('A4 U1 兼容门: 带实测数据(device+load)可标 pass; 全部 pass ⇒ releaseReady=true', () => {
  const led = createAcceptanceLedgerPre()
  for (const it of ACCEPTANCE_ITEMS_V1) {
...
t('A5 任何一项 fail/not-run ⇒ not-ready(逐项可审计)', () => {
  const led = createAcceptanceLedgerPre()
  for (const it of ACCEPTANCE_ITEMS_V1) led.recordAcceptancePre(it.id, 'pass', { evidence: it.compatGate ? JSON.stringify({ device: 'd', load: 'l' }) : 'e' })
