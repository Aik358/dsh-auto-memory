...
import { parseModelWindowsPre, pickWindowPre, findOfficialContextWindowPre, findSessionModelPre, scanPressureSignalsPre } from '../../lib/water-window-pre.js'
...
// 都必须显式提供，否则测的就不是真代码。
const envelopeCode = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'memory-envelope-pre.js'), 'utf8')
  .replace(/^export /gm, '')
...
ok(SRC.includes("async recall(query, limit = 8, agent, scope = 'all', opts = null)"), 'recall 带 scope 参数(默认 all;P4 可选 opts 扩展)')
ok(SRC.includes("if (scope === 'handoff') {") && SRC.includes("if (scope === 'sessions') {"), 'handoff/sessions 早返分支存在')
ok(SRC.includes("scope: { type: 'string', enum: ['all', 'handoff', 'sessions']") && SRC.includes("engine.recall(args.query, args.limit, exec.agent, args.scope || 'all', { format: args.format || 'l0', expand: args.expand })"), 'memory_recall_pre 工具声明 scope 枚举并透传(P4 可选 format/expand 随 opts 透传)')
