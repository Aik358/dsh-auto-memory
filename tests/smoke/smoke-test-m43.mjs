...
// ---------- H1 默认关闭零留存 ----------
{
  ...
  try {
    ...
    if (existsSync(path.join(h.home, 'memory', 'retrieval'))) throw new Error('retrieval-pre must not exist when disabled')
...
// ---------- H3 三开 + retrieve 链路 + audit 卫生 ----------
{
  ...
  const auditDir = path.join(h.home, 'memory', 'retrieval', 'audit')
