 ...
 * P6:lib/handoff-anchor.js 纯函数(parse/weightedTrim)fixture 锁定 + buildContinueCarry 接线守卫。
 */
import assert from 'node:assert/strict'
import { readFileSync } from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { buildL0IndexPre } from '../../lib/l0-extract.js'
...
console.log('[handoff-anchor] G0 源码守卫')
{
  ...
  const posImport = SRC.indexOf("const { buildL0IndexPre } = await import('./l0-extract.js')")
...
const W = await import('../../lib/handoff-anchor.js')
const WSRC = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'handoff-anchor.js'), 'utf8')
{
  ...
  ok(SRC.includes("await import('./handoff-anchor.js')"), 'buildContinueCarry 动态 import 权重截断模块')
