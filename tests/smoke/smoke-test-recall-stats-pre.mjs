...
import { createRecallStatsPre, RECALL_STATS_VERSION_PRE } from '../../lib/recall-stats-pre.js'
...
console.log('\n[S3] 宿主埋点：位置正确 + 只读')
ok(cnt(IX, "from './recall-stats-pre.js'") === 1, 'S3a 统计模块已 import')
...
ok(cnt(IX, "recallStats: '/api/dsh-auto-memory-pre/recall-stats'") === 1, 'S3i API 路径键存在（A1/A4 路径锁要求）')
...
const SH = fs.readFileSync(path.join(ROOT, 'lib/shadow-host-pre.js'), 'utf8')
ok(cnt(SH, "channel: 'shadow'") === 1, 'S3b8 shadow 通道埋点在 shadow-host-pre.js（命中 1）')
...
// 只读姿势：统计页不得出现写配置/写召回的调用
{
  ...
  ok(!/saveConfigPatch|setCfg|memory_note_pre|POST[^']*recall'/.test(body), 'S4k 统计页无写配置/写召回调用（唯一写操作是统计清零）')
