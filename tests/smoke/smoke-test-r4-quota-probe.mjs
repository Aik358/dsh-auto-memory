...
import {
  ...
  QUOTA_PROBE_SCHEMA_PRE_V1, QUOTA_PROBE_CAP_PRE_V1, QUOTA_VERDICTS_PRE_V1, QUOTA_THRESHOLDS_PRE_V1,
...
{
  eq(QUOTA_PROBE_SCHEMA_PRE_V1, 'quota_probe_pre_v1', 'schema 常量')
  eq(QUOTA_PROBE_CAP_PRE_V1, 200, '采样环上限 200（与降级台账同口径）')
  ok(Object.isFrozen(QUOTA_VERDICTS_PRE_V1), '判定枚举被冻结')
  eq(QUOTA_VERDICTS_PRE_V1.length, 4, '判定 4 档')
  for (const v of ['under-quota', 'over-quota', 'balanced', 'insufficient-data']) {
    ok(QUOTA_VERDICTS_PRE_V1.includes(v), '枚举含 ' + v)
  }
  ok(Object.isFrozen(QUOTA_THRESHOLDS_PRE_V1), '阈值常量被冻结')
  ok(QUOTA_THRESHOLDS_PRE_V1.minSamples >= 8, '★ 最小样本量 ≥8（"长期观察"的机械下限）')
}
...
console.log('\n[3] fail-soft：观测/快照永不抛，且返回结构合法对象')
{
  ...
  eq(snap.schemaVersion, QUOTA_PROBE_SCHEMA_PRE_V1, '快照带 schemaVersion')
...
console.log('\n[4] ★ 判定：样本不足一律 insufficient-data（"不能拍脑子"的机械保证）')
{
  ...
  for (let i = 0; i < QUOTA_THRESHOLDS_PRE_V1.minSamples - 1; i++) {
...
{
  const p = createQuotaProbePre({ now: () => 0 })
  const N = QUOTA_THRESHOLDS_PRE_V1.minSamples
  for (let i = 0; i < N; i++) {
  ...
  ok(v.usage > QUOTA_THRESHOLDS_PRE_V1.usageForOver, '★ 优先级：占用高也不能掩盖"被丢"（under 判据先于 over）')
}
...
console.log('\n[6] ★ 判定：各层都不丢且 token 远未用满 ⇒ over-quota（配额偏大、白花 token）')
{
  ...
  ok(v.usage < QUOTA_THRESHOLDS_PRE_V1.usageForOver, 'usage 低于阈值（' + v.usage + '）')
}
...
console.log('\n[7] 判定：有丢但未持续 + 占用合理 ⇒ balanced')
{
  ...
  ok(v.perLayer.log.dropRate < QUOTA_THRESHOLDS_PRE_V1.dropRateForUnder, 'dropRate 低于阈值')
}
...
console.log('\n[8] ★ 判据并列不混：配额观测绝不进入降级台账')
{
  ...
  const N = QUOTA_THRESHOLDS_PRE_V1.minSamples + 2
  ...
  ok(SRC.includes("path.join(memoryDir('degrade'), 'latest.json')"),
...
console.log('\n[9] ★ 端到端：真实方法体执行 + 真实落盘（避免"源码接线正确但功能不存在"）')
{
  ...
  const file = path.join(tmp, 'degrade', 'latest.json')
  ...
  ok(existsSync(file), '★ 文件真实存在（含父目录自动创建）')
  {
    ...
    eq(o.schemaVersion, 'degrade_pre_v1', '★ degrade 原有键保持不变（向后兼容既有读者）')
    ok(o.quota && o.quota.schemaVersion === QUOTA_PROBE_SCHEMA_PRE_V1, '★ 同一文件内带 quota 键')
