...
const ACT = readFileSync(path.join(ROOT, 'lib', 'activation-host-pre.js'), 'utf8').replace(/\r\n/g, '\n')
const CTX = readFileSync(path.join(ROOT, 'lib', 'context-host-pre.js'), 'utf8').replace(/\r\n/g, '\n')
...
test('S6 判定口径不得被削弱:五道门仍在(T0-2 语义一字未改)', () => {
  const tier = readFileSync(path.join(ROOT, 'lib', 'tier-layer-inject-pre.js'), 'utf8').replace(/\r\n/g, '\n')
...
/** 从 activation-host-pre.js 抽取 recordTierGateHits 函数体(花括号配平),在受控 engine 上跑。 */
