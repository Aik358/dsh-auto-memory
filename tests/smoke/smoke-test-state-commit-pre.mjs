// P1 · state-commit-pre.js 纯函数契约测试（设计稿 §4 的 T1-5*/T1-6/T1-8）。
//
...
console.log('[state-commit] T1-5a 业务内容改变 ⇒ miv 变')
{
  ...
  ok(a !== b, '★内容变 ⇒ miv 必变')
  ok(a.startsWith(MIV_PREFIX_PRE), '前缀与既有口径一致(idx_pre_)')
...
console.log('[state-commit] 回执与状态隔离')
{
  ...
  ok(STATE_COMMIT_VERSION_PRE === 'state_commit_pre_v1', '契约版本号')
  // 三种状态不得混（命名隔离）
