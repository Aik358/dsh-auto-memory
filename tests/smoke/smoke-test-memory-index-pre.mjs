...
// ---------- B1 确定性重建 + 版本 + 文件级 stale ----------
{
  ...
  // ★issue #92:prev 键名与宿主缓存一致(sourceVersion) —— 旧实现读 version 而宿主只写
  // sourceVersion,版本门生产路径恒读 undefined ⇒ 版本封顶 2、未变重读回退 1
  const i2 = buildIndex('a.md', buf, { fileDigest: i1.fileDigest, sourceVersion: i1.sourceVersion })
  ...
  const i3 = buildIndex('a.md', edited, { fileDigest: i1.fileDigest, sourceVersion: i1.sourceVersion })
  if (i3.sourceVersion !== 2) throw new Error('edited file must bump sourceVersion to 2')
  // ★issue #92 回归:二次编辑继续递增、未变重读保持(旧实现的 1↔2 振荡+回退)
  const edited2 = Buffer.from(text.replace('架构重构', '架构重构 v2'), 'utf8')
  const i4 = buildIndex('a.md', edited2, { fileDigest: i3.fileDigest, sourceVersion: i3.sourceVersion })
  if (i4.sourceVersion !== 3) throw new Error('second edit must bump sourceVersion to 3')
  const i5 = buildIndex('a.md', edited2, { fileDigest: i4.fileDigest, sourceVersion: i4.sourceVersion })
  if (i5.sourceVersion !== 3) throw new Error('unchanged reread must keep sourceVersion 3 (must not fall back to 1)')
...
// ---------- B4.5 模块级 5MB 上限(含合法标题的超限文件) ----------
{
  ...
  const idx = buildIndex('big.md', big, { fileDigest: 'x', sourceVersion: 1 })
...
// ---------- B5 关闭零行为变化 + engine 版本递增 + stat 守卫 + 并发归属 ----------
{
  ...
  // ★issue #92 引擎级回归:未变重读必须保持版本(旧实现 prev 键名错位 ⇒ 2 回退 1),二次编辑继续递增
  await new Promise((r2) => setTimeout(r2, 60))
  await dbgRoute.handler({ socket: { remoteAddress: '127.0.0.1' }, headers: { host: '127.0.0.1:3080' }, method: 'GET', url: '/debug' }, res)
  const logBEntryKeep = body.associativeMemory.memoryIndex.files.find((f) => f.sourceFile === logB)
  if (!logBEntryKeep || logBEntryKeep.sourceVersion !== 2) throw new Error('unchanged reread must keep sourceVersion 2 (must not fall back to 1, issue #92)')
  writeFileSync(logB, '## B-会话日志\n- B 专属条目(修订2)\n', 'utf8')
  await new Promise((r2) => setTimeout(r2, 60))
  await dbgRoute.handler({ socket: { remoteAddress: '127.0.0.1' }, headers: { host: '127.0.0.1:3080' }, method: 'GET', url: '/debug' }, res)
  const logBEntry3 = body.associativeMemory.memoryIndex.files.find((f) => f.sourceFile === logB)
  if (!logBEntry3 || logBEntry3.sourceVersion !== 3) throw new Error('second edit must bump sourceVersion to 3 (issue #92)')
