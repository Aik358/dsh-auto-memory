...
import { computeImportancePre, IMPORTANCE_NEUTRAL_V1, MEMORY_IMPORTANCE_VERSION, IMPORTANCE_WEIGHTS_V1 } from '../../lib/memory-importance.js'

const HERE = path.dirname(fileURLToPath(import.meta.url))
const SRC = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'index.js'), 'utf8')
const MOD = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'memory-importance.js'), 'utf8')
const PSRC = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'procedure-store.js'), 'utf8')
let pass = 0, fail = 0
const ok = (c, n) => { if (c) { pass++; console.log('  ok -', n) } else { fail++; console.error('FAIL', n) } }
const close = (a, b) => Math.abs(a - b) < 1e-9

console.log('[memory-importance] G1 版本与中性缺省(验收 3)')
ok(MEMORY_IMPORTANCE_VERSION === 'memory_importance_v1', '版本标识 memory_importance_v1')
for (const agg of [undefined, null, {}, { seen: 0, read: 0, cite: 0, reuse: 0, success: 0, correction: 0, distinctSessions: 0 }]) {
  ...
  assert.equal(r.importance, IMPORTANCE_NEUTRAL_V1)
...
ok(Array.isArray(Object.freeze(IMPORTANCE_WEIGHTS_V1) && Object.keys(IMPORTANCE_WEIGHTS_V1)) && Object.isFrozen(IMPORTANCE_WEIGHTS_V1), '权重常数冻结')

console.log('[memory-importance] G5 接线守卫(M8-2b 2026-09-09 更新:经 evidence-agg 管道接入 recall L0 融合)')
ok(SRC.includes("await import('./memory-importance.js')"), 'index.js 已接线(M8-2b:dense 臂乘以 0.5+0.5×importance 加权因子;中性 0.5 → 因子 0.75 均匀缩放排序不变)')
ok(SRC.includes("await import('./evidence-agg.js')"), 'evidence 聚合层已接入(有界只读扫描)')
