...
console.log('[H4] hub 编排')
{
  ...
  // ★A-1（2026-09-23）：机械 procedure 切片通路已整段删除（用户裁定「机械通路已被我弃用」）。
  //   旧实现传 `mechanicalProcedureFeedEnabled: true` 锁那条通路 —— 现该键无消费者，故不再传。
  //   本块要守的编排语义（judgement 三类分流 + crossFeed 正常返回 + overview 齐全）不变。
  ...
  // crossFeed: ★A-1 后不再产出 procedure 观察（机械切片已删）；仍须正常返回
  ...
  ok(!cf.fed.some((f) => f.to === 'procedure'), '★A-1 crossFeed 不再产出机械 procedure 观察')
  // renderChecklists
