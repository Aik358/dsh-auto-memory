 ...
 *
 * **#76-5 hub 死委托分支**：`memory-hub-pre.js:81-84` 用
 ...
 *   ⇒ 两适配器各自演化。
 *
 * **#76-6c clear() 漏重置统计**：`fact-store-pre.js` 的 `stats` 有 8 个字段
 *   （upserts/created/merged/superseded/conflictAdded/inferenceBlocked/expiredIgnored/revoked），
 *   而 `clear()` 只归零 3 个 ⇒ 其余 5 个残留，诊断读数失真。
 */
import { createFactStorePre, factCandidateFromJudgementRow } from '../../lib/fact-store-pre.js'
...
console.log('\n[1] #76-5 store 实例必须暴露 judgement-row 消费器（否则 hub 委托恒假）')
{
  ...
  // hub 的委托判据现在为真
  const hubSrc = codeOnly(src('memory-hub-pre.js'))
...
console.log('\n[2] #76-6c clear() 必须把**全部**统计字段归零')
{
  ...
  const code = codeOnly(src('fact-store-pre.js'))
