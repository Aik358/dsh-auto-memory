...
import {
  lintWhiteboardPre, WB_LINT_VERSION, WB_LINT_CODES_V1, WB_LINT_DEFAULTS_V1,
...
console.log('[1] ① 孤立条目（无入站引用）')
{
  ...
  const orphans = byCode(r, WB_LINT_CODES_V1.ORPHAN).map((f) => f.title)
  ...
  eq(byCode(r2, WB_LINT_CODES_V1.ORPHAN).length, 0, '反例：互引 ⇒ 无孤立')
  ...
  const orph3 = byCode(r3, WB_LINT_CODES_V1.ORPHAN).map((f) => f.title)
  ...
  ok(byCode(r4, WB_LINT_CODES_V1.ORPHAN).length >= 1, '★ 单字标题不参与匹配（宁漏不误）')
}
...
console.log('\n[2] ② 陈旧（状态 + 日期）')
{
  ...
  eq(byCode(r, WB_LINT_CODES_V1.INACTIVE).length, 2, 'superseded + retracted 各报一条')

  const r2 = lintWhiteboardPre([
    mk(A, '老卡', { date: '2026-01-01' }),
    mk(B, '新卡', { date: '2026-09-15', body: '引 ' + A }),
  ], { now: NOW, staleDays: 90 })
  const exp = byCode(r2, WB_LINT_CODES_V1.EXPIRED)
  ...
  eq(byCode(r3, WB_LINT_CODES_V1.EXPIRED).length, 0, '反例：阈值内不报')

  // 边界：正好 staleDays 天前不报（严格小于才报）
  const r4 = lintWhiteboardPre([mk(A, '边界', { date: '2026-06-20' })], { now: NOW, staleDays: 90 })
  eq(byCode(r4, WB_LINT_CODES_V1.EXPIRED).length, 0, '边界：恰好 90 天不报（严格 < 才报）')

  // 缺日期不报（不猜）
  const r5 = lintWhiteboardPre([mk(A, '无日期', { date: '' })], { now: NOW })
  eq(byCode(r5, WB_LINT_CODES_V1.EXPIRED).length, 0, '★ 无日期不报（不猜）')
}
...
console.log('\n[3] ③ 被引用却无卡（只判客观可证的悬空 id）')
{
  ...
  const d = byCode(r, WB_LINT_CODES_V1.DANGLING_REF)
  ...
  eq(byCode(r2, WB_LINT_CODES_V1.DANGLING_REF).length, 0, '反例：引用存在的 id 不报')

  // 外部 corpus 里出现的 id 也算"被引用"
  const r3 = lintWhiteboardPre([mk(A, '甲卡', { body: 'x' })], { now: NOW, corpus: '笔记引用了 ' + DEAD })
  eq(byCode(r3, WB_LINT_CODES_V1.DANGLING_REF).length, 1, '外部语料里的悬空引用也报')
}
...
console.log('\n[4] ④ 缺交叉引用（共享 tag 却互不引用）')
{
  ...
  eq(byCode(r, WB_LINT_CODES_V1.NO_CROSS_REF).length, 1, '共享 2 tag 互不引用 ⇒ 报 1 条')
  ...
  eq(byCode(r2, WB_LINT_CODES_V1.NO_CROSS_REF).length, 0, '反例：已互引 ⇒ 不报')
  ...
  eq(byCode(r3, WB_LINT_CODES_V1.NO_CROSS_REF).length, 0, '★ 共享 1 tag（<2）⇒ 不报（宁漏不误）')
  ...
  eq(byCode(r4, WB_LINT_CODES_V1.NO_CROSS_REF).length, 0, '单方引用即算已链（不要求双向）')
}

// ── 5. ★ 只读纪律：跑完后文件 mtime 不变 ──────────────────────────
console.log('\n[5] ★ 只读纪律（契约 §6 尾注）')
{
  const target = 'lib/wb-contract.js'
...
console.log('\n[6] ⑤ 矛盾检测的边界')
{
  ...
  for (const c of expectCodes) {
    ok(Object.values(WB_LINT_CODES_V1).includes(c), '码常量含 ' + c)
  }
  eq(Object.keys(WB_LINT_CODES_V1).length, 5, '★ 恰好 5 个码（多一个少一个都要红）')
}
...
console.log('\n[8] 确定性与常量')
{
  ...
  eq(WB_LINT_VERSION, 'wb_lint_v1', '版本常量')
  eq(WB_LINT_DEFAULTS_V1.staleDays, 90, '默认 staleDays=90')
  eq(WB_LINT_DEFAULTS_V1.minSharedTags, 2, '默认 minSharedTags=2')
