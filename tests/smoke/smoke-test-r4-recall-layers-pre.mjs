...
import {
  groupL0ByLayerPre, L0_LAYER_DISPLAY_ORDER_PRE_V1, L0_LAYER_LABELS_PRE_V1,
  L0_LAYER_UNKNOWN_LABEL_PRE_V1, classifyLayerPre,
} from '../../lib/l0-extract-pre.js'
import { FUSION_LAYER_ORDER_PRE_V1 } from '../../lib/recall-fusion-pre.js'
import { TIER_LAYER_ORDER_PRE_V1 } from '../../lib/tier-layer-inject-pre.js'
import { createDegradeSinkPre } from '../../lib/degrade-pre.js'
...
{
  eq(L0_LAYER_DISPLAY_ORDER_PRE_V1.length, 5, '层序 5 项')
  eq(L0_LAYER_DISPLAY_ORDER_PRE_V1[0], 'project', '首项 project（结论层最高优先）')
  eq(L0_LAYER_DISPLAY_ORDER_PRE_V1[4], 'log', '末项 log（流水层最低优先）')
  ok(Object.isFrozen(L0_LAYER_DISPLAY_ORDER_PRE_V1), '层序常量被冻结')
  ok(Object.isFrozen(L0_LAYER_LABELS_PRE_V1), '标题常量被冻结')

  // ★ 与融合侧 / 注入侧逐元素相同（三处必须同序，否则"分层"口径打架）
  const same = (a, b) => a.length === b.length && a.every((x, i) => x === b[i])
  ok(same([...L0_LAYER_DISPLAY_ORDER_PRE_V1], [...FUSION_LAYER_ORDER_PRE_V1]),
    '★ 与 FUSION_LAYER_ORDER_PRE_V1 逐元素相同')
  ok(same([...L0_LAYER_DISPLAY_ORDER_PRE_V1], [...TIER_LAYER_ORDER_PRE_V1]),
    '★ 与 TIER_LAYER_ORDER_PRE_V1 逐元素相同')

  // 五层标题齐全，且"结论"与"流水"措辞可分辨（这正是痛点的靶心）
  for (const l of L0_LAYER_DISPLAY_ORDER_PRE_V1) ok(!!L0_LAYER_LABELS_PRE_V1[l], `层 ${l} 有标题`)
  ok(L0_LAYER_LABELS_PRE_V1.project.includes('结论层'), 'project 标题含「结论层」')
  ok(L0_LAYER_LABELS_PRE_V1.user.includes('结论层'), 'user 标题含「结论层」')
  ok(L0_LAYER_LABELS_PRE_V1.whiteboard.includes('结论层'), 'whiteboard 标题含「结论层」')
  ok(L0_LAYER_LABELS_PRE_V1.log.includes('流水层'), 'log 标题含「流水层」')
  ok(L0_LAYER_UNKNOWN_LABEL_PRE_V1 === '未分层', '兜底标题 = 未分层（不猜层）')
}
...
console.log('\n[3] 退化安全：单层、空、非法入参、取层函数抛错')
{
  ...
  eq(noFn[0].label, L0_LAYER_UNKNOWN_LABEL_PRE_V1, '★ 标题 = 未分层（不猜层，不给错误层次暗示）')
...
console.log('\n[5] 源码守卫：分层只改呈现，排序调用点必须原样保留')
{
  ...
  ok(SRC.includes("const { buildL0IndexPre } = await import('./l0-extract-pre.js')"),
