...
//      并从 `lib/tier-layer-inject-pre.js` 的 `TIER_BUDGET_PRE_V1` 解析 `B0` / `B2`。
...
const P = {
  code: 'lib/index.js',
  budget: 'lib/tier-layer-inject-pre.js',
...
const budgetBlockM = src.budget.match(/TIER_BUDGET_PRE_V1\s*=\s*Object\.freeze\(\{([\s\S]*?)\n\}/)
ok('代码侧：定位 lib/tier-layer-inject-pre.js 的 TIER_BUDGET_PRE_V1 字面量', !!budgetBlockM)
