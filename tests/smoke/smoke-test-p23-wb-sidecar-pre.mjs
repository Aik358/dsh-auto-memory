...
import { resolveBoardModePre, BOARD_MODES_PRE_V1 } from '../../lib/board-mode-pre.js'
import { buildSidecarEntryPre, rebuildSidecarIndexPre, expandByTagPre, traceByIdPre, wbEntryIdPre, extractTagsPre, WB_SIDECAR_VERSION } from '../../lib/wb-sidecar-pre.js'
...
t('M9 P3 两工具条件注册 + 工具数闸门(legacy 14 / graph 16)', async () => {
  ...
  for (const f of ['tests/smoke/smoke-test.mjs', 'tests/smoke/smoke-test-m3b3-pre.mjs', 'tests/smoke/smoke-test-context-observer.mjs']) {
    ...
    assert.ok(/!==\s*18/.test(s), f + ' 默认档已翻 graph ⇒ 工具数硬锁应为 18(含 T4 memory_procedure_pre + P9 memory_rules_pre)')
  }
