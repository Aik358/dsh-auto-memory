...
import { createMemoryHubPre } from '../../lib/memory-hub-pre.js'
import { createProcedureStorePre } from '../../lib/procedure-store-pre.js'
import { createFactStorePre } from '../../lib/fact-store-pre.js'
import { INDEX_SYNC_PAGE_BUDGET_PRE_V1 } from '../../lib/index-sync-pre.js'
...
{
  const s = src('episodic-store-pre.js')
  ...
  const good = {
    episodeId: 'epi_pre_' + 'a'.repeat(32), sessionRef: 's1', startedAt: 1, outcome: 'success',
  ...
  // 追加合法新条目
  const good2 = { ...good, episodeId: 'epi_pre_' + 'b'.repeat(32) }
...
{
  const s = src('index-sync-pre.js')
  ok(/maxPageBytes: 252 \* 1024/.test(s), '★★ maxPageBytes = 252KiB（不是 256KiB）', '实际: ' + (s.match(/maxPageBytes: [^,}]+/) || ['?'])[0])
  ok(INDEX_SYNC_PAGE_BUDGET_PRE_V1.maxPageBytes === 252 * 1024, '★ 常量取值确认')
  const MAX_LINE = 256 * 1024
  ok(INDEX_SYNC_PAGE_BUDGET_PRE_V1.maxPageBytes < MAX_LINE, '★ 预算严格小于线帧上限（余量存在）')
  const headroom = MAX_LINE - INDEX_SYNC_PAGE_BUDGET_PRE_V1.maxPageBytes
...
console.log('\n[⑤] 汇总：四处均不得回退（源码级反向锁）')
{
  const es = src('episodic-store-pre.js'), is = src('index-sync-pre.js')
  const ss = src('semantic-js-pre.js'), fs = src('fact-store-pre.js')
