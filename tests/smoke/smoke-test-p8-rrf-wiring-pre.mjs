 ...
 * legacy vs rrf 三查询 top-5 diff / k=60 复用 FUSION_RRF_K_PRE_V1 不硬编码 /
...
const FUSION_SRC = readFileSync(path.resolve(HERE, '..', '..', 'lib', 'recall-fusion-pre.js'), 'utf8')
let pass = 0, fail = 0
...
ok(SRC.includes("await import('./recall-fusion-pre.js')"), 'recall() 动态 import rankFusionRRFPre 模块(证据 A 复现:此前零引用)')
...
ok(/FUSION_RRF_K_PRE_V1 = 60/.test(FUSION_SRC) && !/rankFusionRRFPre\([^)]*\{\s*k:/.test(SRC), 'k=60 来自 FUSION_RRF_K_PRE_V1 默认值,调用处无硬编码')
...
function bindRecall(fake) {
  ...
  const arrow = ('async ' + fnSrc.slice(fnSrc.indexOf('('), fnSrc.indexOf(') {') + 1) + ' => ' + fnSrc.slice(fnSrc.indexOf(') {') + 2)).replaceAll("'./l0-extract-pre.js'", l0Url).replaceAll("'./recall-fusion-pre.js'", rrfUrl).replaceAll("'./note-status-apply-pre.js'", JSON.stringify(new URL('../../lib/note-status-apply-pre.js', import.meta.url).href))
