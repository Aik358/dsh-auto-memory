...
const CIO_SRC = readFileSync(path.join(ROOT, 'lib', 'config-io.js'), 'utf8')
const M = await import(pathToFileURL(path.join(ROOT, 'lib', 'config-io.js')).href)
...
t('T8a-7 ★★★ **原子性本身**：覆盖写期间读方绝不可能看到半截内容（旧值或新值，二者必居其一）', () => {
  ...
  const body = CIO_SRC
  assert(/const tmp = file \+ ATOMIC_TMP_SUFFIX_V1/.test(body),
    '★ 未使用同目录 tmp 路径')
  ...
  const tmpPath = f + M.ATOMIC_TMP_SUFFIX_V1
