 ...
 *
 * 本套件守卫的三件事：
 *   S1 kind 持久化（points[1]）：memory_log_pre 写入行带 [kind:x]；非法值 fail-soft 回落
...
// ---- 动态导入被测模块 ----
const RL = await import('file://' + path.join(ROOT, 'lib', 'rules-layer-pre.js').replace(/\\/g, '/'))

section('S1 kind 持久化（V2-P6B points[1]）')
{
  ok(Array.isArray(RL.LOG_KINDS_PRE_V1) && RL.LOG_KINDS_PRE_V1.join(',') === 'rule,preference,fact,todo',
    'LOG_KINDS_PRE_V1 枚举 = rule/preference/fact/todo')
  ok(RL.LOG_KIND_TAG_RE_PRE_V1 instanceof RegExp && RL.LOG_KIND_TAG_RE_PRE_V1.test(' [kind:rule] x'),
    'LOG_KIND_TAG_RE_PRE_V1 能匹配行内标记')

  // 工具参数面：kind 是可选 enum 参数（不传不影响旧调用方）
  const toolSrc = SRC.slice(SRC.indexOf("defineTool('memory_log_pre'"), SRC.indexOf('defineTool(\'memory_note_pre\''))
  ok(toolSrc.length > 200, 'memory_log_pre 工具定义切片非空（防定位漂移）')
  ...
  ok(/LOG_KINDS_PRE_V1\.includes\(kindRaw\)\s*\?\s*kindRaw\s*:\s*'']/.test(toolSrc.replace(/\s+/g, ' ').replace("'']\s*const", "''] const")) ||
     /LOG_KINDS_PRE_V1\.includes\(kindRaw\)/.test(toolSrc),
...
section('S2 T7-5：打标零额外 LLM 调用（crit[1]）')
{
  ...
  ok(hits.length === 0, 'memory_log_pre 工具体内零 LLM 入口（命中：' + (hits.join(',') || '无') + '）')
  ...
  const rlSrc = readFileSync(path.join(ROOT, 'lib', 'rules-layer-pre.js'), 'utf8')
  ok(!/^import /m.test(rlSrc), 'rules-layer-pre.js 仍零 import（打标未引入新依赖）')
}
