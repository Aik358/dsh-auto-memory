...
console.log('[switch-decouple] D3 行为:buildContinueCarry 白板关 → 载体退化到转写包(不再 handoff disabled)')
{
  ...
  // ★issue #90/#134 回归夹具:全局口径(resolvePaths=GUI 最后轮询工作区)故意指向**另一个**工作区,
  ...
  const mkCarry = (handoffEnabled) => {
    const fake = Object.assign(makeFakeEngine(), {
      ...
      // ★issue #90/#134:buildContinueCarry 按被接续会话解析路径 —— 回落到 D3 原会话工作区 fixture
  ...
  // ★issue #90/#134 回归:材料按被接续会话的工作区(resolvePathsForSession)解析,不落全局口径 ——
  ...
  ok(textOn.includes('PLAN_BODY_MARKER') && !textOn.includes('GLOBAL_PLAN_MARKER'),
    '★issue #90/#134 回归:接续材料来自被接续会话的工作区,全局口径不串味')
}
