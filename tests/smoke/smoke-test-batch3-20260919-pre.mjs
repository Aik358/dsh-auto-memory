 ...
 *   读侧 `m4-corpus.js` 旧实现直接 `buf.subarray(...)` 用在含 BOM 的原 buffer 上
...
import { parseAnchors, buildSidecar } from '../../lib/memory-anchor.js'
...
console.log('\n[5] 全仓 BOM 对齐假设与坐标系自洽')
{
  // `storage-manage.js` 的 `+bomLen` 对齐**是正确的**（坐标系确实相对剥 BOM 内容）——不得反向断言，
  //   而应断言它**仍然存在且与坐标系一致**（防止有人误删）。
  const sm = src('storage-manage.js')
