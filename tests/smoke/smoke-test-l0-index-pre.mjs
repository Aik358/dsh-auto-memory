/**
 * smoke-test-l0-index-pre —— L0 向量索引（l0_index_pre_v1）回归锁定。
...
const PATH = '/mem/index-pre/files/l0-index.json'
...
// ---------- 版本 ----------
t('version constants', () => {
  assert.equal(L0_INDEX_VERSION, 'l0_index_pre_v1')
...
await ta('load fail-soft: 缺文件/损坏 JSON/schema 不符/条目非法 → ok:false + 空数组,不抛', async () => {
  ...
  ioSchema.writeJson(PATH, { schemaVersion: 99, l0IndexVersion: 'l0idx_pre_' + '0'.repeat(32), updatedAt: T0, entries: [] })
  assert.equal(mkEngine(ioSchema).load({ path: PATH }).reason, 'schema')
  // entries 非数组
  const ioArr = makeIO()
  ioArr.writeJson(PATH, { schemaVersion: 1, l0IndexVersion: 'l0idx_pre_' + '0'.repeat(32), updatedAt: T0, entries: 'nope' })
  assert.equal(mkEngine(ioArr).load({ path: PATH }).reason, 'entries')
  // 条目向量含非数值(JSON 往返会把 NaN 变 null,故用字符串构造非法值)
  const ioVec = makeIO()
  ioVec.writeJson(PATH, {
    schemaVersion: 1, l0IndexVersion: 'l0idx_pre_' + '0'.repeat(32), updatedAt: T0,
...
await ta('status: count/version/updatedAt;损坏索引 → ok:false', async () => {
  ...
  assert.match(s.l0IndexVersion, /^l0idx_pre_[0-9a-f]{32}$/)
