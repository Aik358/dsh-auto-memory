 ...
 *   G1 内联等价：rules-layer-pre.js 的 statusOfEntryPre/stripStatusLinesPre ≡ note-status-pre.js 的同名能力
 *   G2 零 import 不变式：rules-layer-pre.js 顶层 import 必须为 0（p6a/p6b 两条守卫锁的设计属性）
...
import * as RL from '../../lib/rules-layer-pre.js'
import * as NS from '../../lib/note-status-pre.js'
import { listRuleItemsPre } from '../../lib/rules-edit-pre.js'
...
const SRC_RL = fs.readFileSync(path.join(ROOT, 'lib/rules-layer-pre.js'), 'utf8')
...
console.log('[G1] 内联解析 ≡ note-status-pre.js（等价性断言，防静默漂移）')
...
eq((SRC_RL.match(/^import /gm) || []).length, 0, 'rules-layer-pre.js 顶层 import = 0')
...
ok(/defineTool\('memory_rules_pre'/.test(SRC_IX), '模型工具 memory_rules_pre 已注册')
...
const apply = new Function('listRuleItemsPre', 'appendRuleItemPre', 'updateRuleItemPre', 'removeRuleItemPre',
  ...
  (await import('../../lib/rules-edit-pre.js')).appendRuleItemPre,
  (await import('../../lib/rules-edit-pre.js')).updateRuleItemPre,
  (await import('../../lib/rules-edit-pre.js')).removeRuleItemPre)
