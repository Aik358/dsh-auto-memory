...
await t('#1-2 superseded 带 by', () => {
  ...
  ok(l.startsWith(M.NOTE_STATUS_OPEN_V1), '以状态开标记起始')
  ok(l.endsWith('-->'), '以 --> 收尾')
})
...
await t('#1-6 reason 超长被截断（元数据不是正文）', () => {
  ...
  const back = M.parseStatusLinePre(l)
  ok(back !== null, '仍可解析')
  ok(back.reason.length <= M.NOTE_STATUS_REASON_MAX_V1, '长度 ≤ 上限', String(back.reason.length))
})
...
await t('#5-2 ★ 幂等：同参数重复调用不叠加', () => {
  ...
  b = M.withStatusLinePre(b, 'superseded', { supersededBy: A })
  b = M.withStatusLinePre(b, 'superseded', { supersededBy: A })
  const n = b.split(M.NOTE_STATUS_OPEN_V1).length - 1
  eq('★ 只保留 1 个状态行', n, 1)
})
...
await t('#5-3 状态变更：旧行被替换而非叠加', () => {
  ...
  eq('★ 只剩 1 行', b.split(M.NOTE_STATUS_OPEN_V1).length - 1, 1)
...
await t('#5-4 current ⇒ 只剥不写（撤销通道的形态基础）', () => {
  ...
  eq('★ 状态行被移除', b.split(M.NOTE_STATUS_OPEN_V1).length - 1, 0)
...
await t('#7-1 ★ 不得复用锚点开标记（复用会撞锚点契约 ⇒ 整份文件拒写）', () => {
  ok(!CODE.includes("NOTE_STATUS_OPEN_V1 = '<!-- memory:'"),
...
await t('#7-2 ★ 不得从别的模块 import 锚点常量（避免与锚点契约耦合）', () => {
  ...
  // ★反向断言要写当前真实模块名：旧写法 negates `./memory-anchor-pre.js`，发布线是裸名 ⇒
  //   永不失败（假绿），它声称守的"不与锚点契约耦合"实际无人守。
