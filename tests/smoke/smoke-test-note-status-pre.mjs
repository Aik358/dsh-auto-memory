/**
 * G3 磁盘形态 · 纯函数核心套件（note-status-pre.js）
 *
...
const SRC = readFileSync(new URL('note-status-pre.js', LIB), 'utf8')
...
const M = await import(new URL('note-status-pre.js', LIB))
...
await t('#4-4 ★★ 端到端：剥了状态行后 L0 不被污染（真实调用 l0-extract-pre）', async () => {
  const L0 = await import(new URL('l0-extract-pre.js', LIB))
...
await t('#7-5 ★ 当前无调用方（未接线状态必须被断言锁住，防"以为已生效"）', async () => {
  ...
  ok(!IDX.includes('note-status-pre.js'),
