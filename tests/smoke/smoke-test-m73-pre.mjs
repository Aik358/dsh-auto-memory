...
// worker_semantic_v1.py 复用 worker_v1 协议层——
...
// 全程 hash-v1 确定性 provider(纯标准库)——不下载模型、不联网、不依赖 torch。
...
function mkEmbConfig(home, dim = 64) {
  ...
  writeFileSync(p, JSON.stringify({ provider: 'hash-v1', dimension: dim }), 'utf8')
  return p
}
...
function mkRecords(tag, n, wsrSeed, opt = {}) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    out.push({
      ...
      chunkId: 'chk_' + hex32(tag + 'c' + i), chunkOrdinal: 0, chunkCount: 1,
    })
  }
  return out
}
...
const semDir = (home) => path.join(home, 'memory', 'semantic')
...
console.log('[N1] semantic worker 启动 + health embedding 视图(协议层零改动)')
{
  ...
  ok(h.frame.payload.embedding.policyVersion === 'm7_chunk_v1', 'chunk policyVersion=m7_chunk_v1')
...
console.log('[N2] index_sync E2E → versioned vectors + identity block 落盘')
{
  ...
  const planN2 = mkPlan(recs, 'idx_' + hex32('n2miv'), 'D:/tmp/wsA')
  ...
  ok(vecFiles.length === 1, 'semantic 下恰一个 vectors 文件')
  const v = JSON.parse(readFileSync(path.join(semDir(home), vecFiles[0]), 'utf8'))
  eq(v.identity.chunkPolicyVersion, 'm7_chunk_v1', 'identity.chunkPolicyVersion')
  eq(v.identity.provider, 'hash-v1', 'identity.provider')
  ...
  eq(v.memoryIndexVersion, 'idx_' + hex32('n2miv'), 'vectors 绑 memoryIndexVersion')
  ...
  ok(multi.every((x) => x.chunkId.startsWith('chk_')) && new Set(multi.map((x) => x.chunkId)).size === multi.length, '多 chunk 各 ordinal chunkId 互异(序数入 hash)')
  const single = v.chunks.find((x) => x.memoryId === recs[0].memoryId)
  ok(single.chunkOrdinal === 0 && single.chunkCount === 1 && single.chunkId.startsWith('chk_'), '单 chunk 记录 chunkOrdinal=0/chunkCount=1')
...
console.log('[N3] context_push → 影子候选(本地日志,零新帧) + gold top1')
{
  ...
  const miv = 'idx_' + hex32('n3miv')
  await sendAll(c, mkPlan(recs, miv, 'D:/tmp/wsA'))
  let activation = null
  c.onActivation((evt) => { activation = evt })
  const push = await c.request('context_push', pushPayload('obs_' + hex32('n3obs'), miv, 'breaker 连续失败三次之后会熔断吗'))
  ...
  const row = rows[0]
  eq(row.observationId, 'obs_' + hex32('n3obs'), '影子行绑 observationId')
  ...
  ok(row.candidates[0].memoryId.startsWith('mem_') && row.candidates[0].chunkId.startsWith('chk_'), '候选带 chunkId/memoryId')
  ...
  const dupPush = await c.request('context_push', pushPayload('obs_' + hex32('n3obs'), miv, 'breaker 连续失败三次之后会熔断吗'))
  ok(dupPush.ok && dupPush.frame.payload.accepted === false && dupPush.frame.payload.reason === 'busy', '同 obs 幂等 busy(worker_v1 语义保持)')
...
console.log('[N4] miv 隔离:跨 workspaceRef/版本 不串')
{
  ...
  const mivA = 'idx_' + hex32('n4mivA')
  const mivB = 'idx_' + hex32('n4mivB')
  ...
  await c.request('context_push', pushPayload('obs_' + hex32('n4o1'), mivA, 'alpha 工作区 breaker 语义是什么'))
  await sleep(400)
  ...
  await c.request('context_push', pushPayload('obs_' + hex32('n4o2'), mivB, 'beta 工作区 workerEpoch 语义是什么', 1, 'D:/tmp/wsB'))
  await sleep(400)
  rows = shadowRows(home)
  ok(rows[rows.length - 1].candidates.every((x) => x.workspaceRef === planB.begin.workspaceRef), 'mivB 查询只命中 B 条目(零串线)')
  const ghostMiv = 'idx_' + hex32('ghost')
  const rowsBeforeGhost = rows.length
  await c.request('context_push', pushPayload('obs_' + hex32('n4o3'), ghostMiv, '任意内容'))
  await sleep(300)
  rows = shadowRows(home)
...
console.log('[N5] stale:identity 不匹配拒用,commit 后重建')
{
  ...
  { // second run: same corpus already on disk, config now dim 128 -> stale, refuses
    ...
    await c.request('context_push', pushPayload('obs_' + hex32('n5o'), miv, 'breaker 语义'))
    await sleep(300)
    ...
    await c.request('context_push', pushPayload('obs_' + hex32('n5o2'), miv2, 'breaker 语义 v2'))
    await sleep(300)
    const rows = shadowRows(home)
...
console.log('[N9] 同 miv 跨 workspace:三重过滤显式隔离(审计 P1 泄漏用例)')
{
  ...
  const c = mkClient(home, mkEmbConfig(home))
  const mivSame = 'idx_' + hex32('n9same')
  ...
  await c.request('context_push', pushPayload('obs_' + hex32('n9o1'), mivSame, '共享快照内容 breaker 是什么', 1, 'D:/tmp/wsA'))
  await sleep(400)
  let rows = shadowRows(home)
  ...
  await c.request('context_push', pushPayload('obs_' + hex32('n9o2'), mivSame, '共享快照内容 breaker 是什么', 2, 'D:/tmp/wsB'))
  await sleep(400)
  rows = shadowRows(home)
...
console.log('[N6] 协议零回退:rejection 矩阵在 semantic worker 上保持')
{
  ...
  const planN6 = mkPlan(recs, 'idx_' + hex32('n6miv'), 'D:/tmp/wsA')
...
console.log('[N7] 无 embedding 配置 → 降级纯协议,永不崩')
{
  ...
  const rs = await sendAll(c, mkPlan(recs, 'idx_' + hex32('n7miv'), 'D:/tmp/wsA'))
  ...
  const push = await c.request('context_push', pushPayload('obs_' + hex32('n7o'), 'idx_' + hex32('n7miv'), '任意'))
...
console.log('[N8] 真实 provider 门:bge-m3-v1 配置校验(不加载模型)')
{
  const home = mkdtempSync(path.join(tmpdir(), 'm73-n8-'))
  const cfgPath = path.join(home, 'emb-real.json')
  writeFileSync(cfgPath, JSON.stringify({ provider: 'bge-m3-v1', modelDir: path.join(home, 'no-such-model'), dimension: 1024 }), 'utf8')
  ...
  const rs = await sendAll(c, mkPlan(recs, 'idx_' + hex32('n8miv'), 'D:/tmp/wsA'))
