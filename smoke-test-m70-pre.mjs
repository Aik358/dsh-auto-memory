...
function buildEnvelope(sess, cv, refs) {
  ...
  const built = BRIDGE.buildContextPushEnvelopePre({
    ...
    index: { memoryIndexVersion: 'idx_pre_' + 'ab'.repeat(16), sourceEpochs: ['ep-1'] },
...
{
  ok(Object.isFrozen(WIRE.M7_TRANSPORT_BUDGET_PRE_V1), 'G1 传输预算冻结')
  eq(WIRE.M7_WIRE_PROTOCOL_VERSION_PRE_V1, 'm7_wire_pre_v1', 'G1 协议版本字符串')
  const js = new Set(WIRE.JS_FRAME_TYPES_PRE_V1), py = new Set(WIRE.PY_FRAME_TYPES_PRE_V1)
  ...
  for (const req of ['health', 'context_push', 'index_sync_begin', 'index_sync_page', 'index_sync_commit']) {
    ok(WIRE.RESPONSE_TYPE_FOR_PRE_V1[req], 'G1 响应映射含 ' + req)
  }
  ok(!WIRE.RESPONSE_TYPE_FOR_PRE_V1.cancel && !WIRE.RESPONSE_TYPE_FOR_PRE_V1.close_session, 'G1 cancel/close_session 无响应帧')
  ...
  const v = WIRE.validateTransportFramePre({ protocolVersion: 'm7_wire_pre_v1', frameId: 'f1', requestId: 'r1', workerEpoch: 'wk', type: 'health_result', payload: {}, sentAt: 1 })
  ...
  const wireSrc = await readFileP(new URL('./lib/m7-wire-pre.js', import.meta.url), 'utf8')
...
console.log('[G2] framing:partial/multiple/bad JSON/oversize/epoch 门/type 混用')
{
  ...
  const wrongEpochFrame = { protocolVersion: WIRE.M7_WIRE_PROTOCOL_VERSION_PRE_V1, frameId: 'f', requestId: 'zz', workerEpoch: 'wk_other_epoch', type: 'health_result', payload: {}, sentAt: 1 }
  c._feedForTest(JSON.stringify(wrongEpochFrame) + '\n')
  eq(c._statsForTest.dropped.staleEpoch, 1, 'G2 错误 epoch 丢弃(fail closed)')
  const unknownReq = { protocolVersion: WIRE.M7_WIRE_PROTOCOL_VERSION_PRE_V1, frameId: 'f2', requestId: 'req_pre_none', workerEpoch: epoch, type: 'health_result', payload: {}, sentAt: 1 }
  ...
  const mkResp = (type, payload) => ({ protocolVersion: WIRE.M7_WIRE_PROTOCOL_VERSION_PRE_V1, frameId: 'resp-' + Math.random().toString(36).slice(2), requestId: sent.requestId, workerEpoch: epoch, type, payload, sentAt: 5 })
  c._feedForTest(JSON.stringify(mkResp('context_ack', { observationId: 'obs_pre_x', accepted: true })) + '\n')
...
console.log('[G3] worker 往返:ack M5 兼容/激活 M6 兼容/幂等/determinism/零目录')
{
  ...
  ok(h.ok && h.frame.payload.protocol === 'm7_wire_pre_v1', 'G3 health_result 回显协议版本')
  ...
  async function rawRound() {
    const lines = [
      JSON.stringify({ protocolVersion: 'm7_wire_pre_v1', frameId: 'fix-f1', requestId: 'fix-r-health', workerEpoch: 'wk_fix', type: 'health', payload: {}, sentAt: 123 }),
      JSON.stringify({ protocolVersion: 'm7_wire_pre_v1', frameId: 'fix-f2', requestId: 'fix-r-push', workerEpoch: 'wk_fix', type: 'context_push', payload: ENV_A, sentAt: 124 }),
    ]
  ...
  ok(!existsSync(path.join(home, 'memory', 'semantic-pre')), 'G3 push-only 不产生 semantic-pre 目录')
  await c.dispose('test')
...
console.log('[G4] workerEpoch 重启/旧响应丢弃/崩溃恢复')
{
  ...
  c._feedForTest(JSON.stringify({ protocolVersion: WIRE.M7_WIRE_PROTOCOL_VERSION_PRE_V1, frameId: 'old', requestId: 'old-req', workerEpoch: e0, type: 'health_result', payload: {}, sentAt: 1 }) + '\n')
...
console.log('[G5] 四种身份不混用(requestId/observationId/activationId/syncId)')
{
  ...
  const goodAck = { protocolVersion: WIRE.M7_WIRE_PROTOCOL_VERSION_PRE_V1, frameId: 'x2', requestId: sentCtx.requestId, workerEpoch: epoch, type: 'context_ack', payload: { schemaVersion: 1, observationId: ENV_A.observationId, accepted: true, workerEpoch: epoch, reason: 'ok' }, sentAt: 2 }
  c._feedForTest(JSON.stringify(goodAck) + '\n')
  const rctx = await pctx
  ok(rctx.ok, 'G5 正确 context_ack 完成关联(四种身份各归各位)')
  ok(sentCtx.requestId.startsWith('req_pre_'), 'G5 transport 身份=requestId(req_pre_*)')
  ok(ENV_A.observationId.startsWith('obs_pre_'), 'G5 M5 身份=observationId(obs_pre_*)')
  await c.dispose('test')
}

// ---------- G8 ----------
console.log('[G8] Python 不可用结构化失败 + lexical_pre_v2 回退不受破坏')
{
  ...
  const snap = {
    memoryIndexVersion: 'idx_pre_' + 'cd'.repeat(16),
    sources: [],
    records: [{
  ...
  eq(lexAfter, lexBefore, 'G8 Python 故障前后 lexical_pre_v2 结果逐项一致(回退语义不受破坏)')
...
console.log('[G7] 默认关闭零进程/零协议 IO/零目录(harness 经 lib/index.js apply)')
{
  ...
  apply(ctx, {})
  const dbgRoute = routes.find((r) => r.path === '/api/dsh-auto-memory-pre/debug')
  const cfgRoute = routes.find((r) => r.path === '/api/dsh-auto-memory-pre/config')
  const actRoute = routes.find((r) => r.path === '/api/dsh-auto-memory-pre/activation-inbox-pre')
  ...
  ok(!existsSync(path.join(home, 'memory', 'semantic-pre')), 'G7 默认零 semantic-pre 目录')
  ...
  ok(!existsSync(path.join(home, 'memory', 'semantic-pre')), 'G7 全程零 semantic-pre 目录')
