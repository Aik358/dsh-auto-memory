...
| lib/python-sidecar-client-pre.js | SidecarClient:no-shell spawn(lazy start,仅显式启用路径触达);每次启动新 workerEpoch(wk_pre_+32hex),入站 epoch 不符即丢;request() 永不 reject——结构化 {ok:false,code}(timeout/aborted/crashed/unavailable/circuit-open/backpressure/protocol/worker-error/disposed);JSONL partial 重组/multi-lines 单 chunk 多请求各自关联/单行超限 fatal(清残留 buffer 防污染重生流)+crash 自动重生;error 帧=终局答复(先于类型匹配关联);AbortSignal→aborted+向 worker 发 cancel 帧;circuit breaker 连续失败≥3 打开 30s,冷却后半开探测成功归零(★issue#107 口径澄清:**闭合态**需连续 2 次成功才清零,因 index_sync 是分段序列、每段 begin 帧必先成功一帧,旧「任一成功即清零」使失败计数峰值恒为 1、阈值永不触发;**半开探测**仍是一次成功即闭合,与 smoke-test-m70 G6 一致。另有超时看门狗:连续 3 次 timeout 判定 worker wedged ⇒ 调 restart() 杀进程重生,不再每帧白等);四种身份(requestId req_pre_/observationId obs_pre_/activationId act_pre_/syncId syn_pre_)不混用 |
