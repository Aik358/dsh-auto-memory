...
- All data stays on your machine: `~/.dsh/memory/` (memory files), `~/.dsh/dsh-auto-memory.json` (config).
...
| Diagnostics log | `~/.dsh/dsh-auto-memory-diagnose.log` (subagent circuit-breaking, consolidation skips, GC, recall degradation — all in here) |
...
| Feedback / grab logs | `~/.dsh/dsh-auto-memory-diagnose.log`; QQ group in README |
...
| Content | Path |
|---|---|
| Plugin config | `~/.dsh/dsh-auto-memory.json` |
...
| Diagnostics log | `~/.dsh/dsh-auto-memory-diagnose.log` |
