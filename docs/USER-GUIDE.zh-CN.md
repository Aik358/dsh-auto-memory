...
| 会话自动归档（autoArchiveEnabled / autoArchiveDays） | 开 / 2 天 | **本插件自持**，不依赖外部插件：子代理会话结束后保留 → 超过 2 天自动归档。见 §8.5 |
| 归档后自动删除（autoDeleteEnabled / autoDeleteDays） | 开 / 7 天 | 归档满 7 天后自动删除。**归档时刻未知的会话永不自动删除**（安全边界） |
| 归档检查间隔（autoArchiveCheckMin） | 60 分钟 | 扫描节流；宿主需在线 |
| 会话归档总开关（sessionArchiveEnabled） | 开 | 关掉则归档与删除都不跑 |
...
### 8.5 子代理会话的生命周期：工作台轮换 → 归档 → 删除

**先说要解决的问题**：子代理会话是**持久化**的，删不掉。旧设计里每个子代理都新建一个会话，于是持久会话无限增长。现在的做法是：

**① 记忆工作台（唯一挂载点）**
全部子代理挂到**同 1 个工作台会话**（`记忆中枢 #<期号>`）之下，不再各自新建。工作台**每 2 天轮换一次**：旧期让位、新期接管。

- 轮换的相位：`active → draining（排水）→ sealing（封存）→ active（新期）`；
- **主判据是「子代理全部静止」**（无在飞任务 + 子代理目录静默 2 分钟），30 分钟超时只是防御，**绝不强切**；
- **旧工作台会话保留、不删除**——删掉会让 catalog 指向不存在的目录，反而造出「会话暂时无法读取」的幽灵条目；
- **轮换周期不是独立设置**：它**跟随「归档阈值（天）」**，设置页里那一项是**只读**的。
  原先两者各有一个数字（都默认 2），但它们本质是同一段生命周期的两个阶段，各配一个数必然失配：
  轮换比归档慢 ⇒ 旧工作台**永不静默**、归档链断裂；轮换比归档快 ⇒ 白建会话。
  ⇒ 现在只留**一个旋钮**（归档阈值），轮换恒等于它（相等即最优）。**这是正确用法，不是缺功能。**

**② 自动归档（2 天）→ ③ 自动删除（归档后 7 天）**
由本插件自持（`autoArchiveDays=2` / `autoDeleteDays=7` / `autoArchiveCheckMin=60`），**客户机不需要装 `dsh-session-archive`**，配置入口就在本插件设置页 → 自动化。

三条**安全边界**（不建议放宽）：

- **归档时刻未知的会话永不自动删除**（拿不到 `archivedAt` 就不进删除名单）；
