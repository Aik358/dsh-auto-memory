...
## 10. 延伸评估：白板升级为任务看板（2026-09-13，用户提出，仅预研不立项）

用户意向：PLAN 白板 → 任务看板式安排，让白板从接续专用变为用户可读的人机交互面（契合 README 项目定位）。影响与待解点：

1. **接续收益**：恢复点显式化（卡片状态+下一步），不再靠语义拼现状；判据更可计算（每卡必有状态与下一步）。
2. **丢卡风险（最严重）**：白板是整篇重写式更新，自由文本丢句无感、看板丢卡=任务蒸发。解法=sidecar 重写前后卡片集合对比（只能移动/完成，不能消失）——看板化使 P2 从可选变刚需。
3. **旧卡撑爆截断**：carryText 第0层 slice(0,3000)（2712）会被已完成卡挤占。解法=列顺序约定+完成卡归档；**P7 老化（1633-1656，按标题关键词）对看板列名失配，需升级为列感知**。
4. **卡片腐烂**：状态漂移误导接续（重做已完成/跳过已解决）。解法=卡片带接续链 contSeq 时间轴 + 刷新仪式逐卡对状态 + 僵尸卡注入时打过期标记。
5. **人机冲突（核心新问题）**：用户改卡 vs 模型整篇重写互相覆盖。三选一：(a)只读 (b)卡 id+接口合并（依赖 P2-6 锚点，工程最大）(c)卡片分模型维护区/用户备注区（推荐起步）。
6. **双状态源**：账本四段与看板并存。解法=看板=现在时（唯一事实）、账本=过去时（历史），账本「任务状态」段改为指路；写进判据。
7. **多任务混板**：卡带接续链标记（Cue/contSeq），新窗口只展开自己链——与 §3 Cue 设计闭环。

节奏建议：P0 级零成本验证——先出看板格式约定文档，真实工作区试两周（模型守格式？接续取任务顺不顺？）再定 P1/P2 与交互方案 (a/b/c)。界面渲染属 client.js（冻结外），可与大排期合流；引擎侧等拍板。
