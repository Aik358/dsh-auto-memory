...
| **`archiveAnswerPre` 不以图遍历为必要前提**：「必须等 `memory_expand` 才有意义」的因果不成立；保存、幂等和普通 recall 回流可先验收 | **采纳**。总纲 §5 该行作废 |
...
- **Phase 6 的 `kind` 参数即使不增工具数也改 schema** ⇒ 必须保留旧调用兼容测试（`memory_log({note,date})` 仍有效）。
