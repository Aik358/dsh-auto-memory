...
- **S10.5 答案归档回流**（补上"复利"回路）：一次检索/分析的结论要能**一键存成白板页面或记忆条目**（现有 `memory_note` / handoff 即为通道）；否则知识只在对话里蒸发 —— 这是 Karpathy 方案最核心的收益点，也是我们目前**半闭环**的地方。
