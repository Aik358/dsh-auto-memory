...
- 本次未覆盖：宿主 17 个**模型工具**（`memory_*` / `calendar_*`）本身不是界面能力，故不列入「未接线」；但其中 `memory_expand` / `memory_trace`（P3 白板遍历，`lib/index.js:10541`/`:10545`）的**结果**在面板上是否可视化，未在本轮范围内（推断：白板看板已承接，未单独取证）。
