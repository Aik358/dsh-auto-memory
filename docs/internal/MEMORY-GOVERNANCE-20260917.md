...
| 层 | 现有机制 | 为什么不够 |
|---|---|---|
| 日志 | `memory_log` append-only | 追加是**正确**的（日志本就该流水） |
| 项目笔记 | `memory_note` append | ⚠️ **笔记不是流水，是结论**，但用了流水的语义 |
| 用户级 | `memory_user` append | 同上 |
| 白板 | `memory_note(kind=plan)` **replace** | ✅ **唯一有 replace 语义的层** |
...
**成本**：改 `memory_note` 写入模板 + `pushL0`/`semSources` 读取时过滤。**零新增文件、零 LLM 调用。**
...
| **改（覆盖式）** | ⚠️ **仅白板** | `memory_note(kind=plan)` = replace |
...
3. ~~**G3 的 `supersedes` 自动还是手写**~~ → **已拍板：只在结论层（`memory_note`）自动比对**，见 §8.2。
...
**用户原话**：「b➕c 应该才是正路，每次 memory-log 的时候都应该语义臂检索更新纠正记忆」+「(b) 只在 `memory_note`（结论层）时比对」+「标错成 superseded 后，要不要有个撤销通道？**落到文档里，和 M2 一起做**」。
...
**只挂 `memory_note`，不挂 `memory_log`。**

理由（与 §2 根因表同源）：
- **日志 = 流水**，append 是**正确**语义，流水里不存在"这条日志作废了另一条日志"的概念；
- **笔记 = 结论**，append 是**错误**语义，才是矛盾的滋生地。

> ⚠️ 用户原话提到"每次 memory-log 的时候" —— 但随后自己修正为「只在 `memory_note`」。
> **以 `memory_note` 为准**，这是更小、更准的落点；日志路径保持零改动（也符合"最热路径不加成本"）。
...
| 方案 | 形式 | 评价 |
|---|---|---|
| **U1** | `memory_note` 加参数 `restore: [mem_id...]` ⇒ 把指定 id 的状态改回 `current` | 明确、可审计；但用户得先知道 id |
...
| **G3 状态写入** | `memory_note` 内的判定 + 写 `status` | ✅ 写（经正常写入通道） | 写入侧 |
...
3. **`memory_note` 在热路径上**：新增语义检索有成本。需实测单次开销，必要时加**同主题缓存**或只在结论内容显著时触发。
...
- [ ] `memory_log` 路径**零改动**（断言：日志写入不触发任何 status 写入）。
